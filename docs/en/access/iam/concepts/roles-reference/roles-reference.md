...
Basic roles allow users to perform actions across all Yandex Cloud services. These roles, except for the `mcs_owner` role, are used for [configuring access rights](/en/access/iam/instructions/manage-roles) in combination with [permissions](/en/access/iam/concepts/permissions-reference).
