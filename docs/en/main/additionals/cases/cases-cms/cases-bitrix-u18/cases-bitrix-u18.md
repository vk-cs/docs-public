...
1. Create a configuration file `bitrix.conf` by running the command:
...
3. Create a new database for 1C-Bitrix: Site Management using the command:
...
4. Create a user with full access rights to the created database and assign a password to it using the command:
    ...
    For example: CREATE USER mxuser@localhost IDENTIFIED BY 'mypassword';
    ```
