This article provides examples of creating and configuring the VK Object Storage buckets and objects using Terraform.
...
1. Set up the provider:
    ...
    1. Create the `aws_provider.tf` file and add the following content to it:
        ...
        - `<REGION>` — the region where the VK Object Storage service is hosted. The setting must match the [region](/en/tools-for-using-services/account/concepts/regions) of the account:
...
- `acl` — [ACL settings](/ru/storage/s3/concepts/s3-acl#fiksirovannyy_acl "change-lang") for the object. Available values: `private`, `public-read`, `auth-read`.
...
The example below will add an [automatic deletion rule](/ru/storage/s3/instructions/manage-lifecycle "change-lang") (lifecycle) for objects with the `tmp` prefix from the bucket after one day.
...
It is not possible to specify an ACL for a single VK Object Storage [account](/ru/storage/s3/instructions/access-management/access-keys "change-lang") from another project.
...
- `id = "1f417590-xxxx-xxxx-xxxx-edacf23b1f96"` — the canonical identifier of the user (Canonical User ID) that is granted access rights to the bucket. [How to find out the canonical identifier of the user](/ru/storage/s3/concepts/s3-acl#user-id "change-lang").
...
1. [Connect](/ru/storage/s3/connect/s3-cli "change-lang") to VK Object Storage via AWS CLI.
1. Verify that the bucket and objects have been created. Run the command:
    ...
    Here:
      ...
      - `endpoint-url` is the VK Object Storage service domain, must match the [region](/en/tools-for-using-services/account/concepts/regions) of the account:
