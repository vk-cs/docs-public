...
- Resources:
  ...
  - `vkcs_lb_loadbalancer`: manages the load balancer resource in VK Cloud. The following arguments are supported:
    ...
    - `availability_zone`: [availability_zone](/en/intro/start/concepts/architecture#architecture-az), where the balancer will be placed. To optimize and speed up the balancer connection to VMs, place them in the same zone. You can get the list of availability zones in your VK Cloud management console or via [OpenStack CLI](/en/tools-for-using-services/cli/openstack-cli#examples_of_openstack_cli_commands).
