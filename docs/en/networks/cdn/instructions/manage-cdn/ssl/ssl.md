...
1. Expand the **Encryption settings** box and select how to apply SSL certificate in the **SSL certificate** parameter:
...
- To use a free [Let's Encrypt](https://letsencrypt.org) certificate, specify the `"sslEnabled": true` parameter and the parameters to update the certificate:
  ...
  To create or configure a Let's Encrypt certificate, use the [methods](/en/tools-for-using-services/api/api-spec/api-cdn) of the **Let's Encrypt certificates** section.

- To use your SSL certificate, specify the `"sslEnabled": true`, and also specify the certificate identifier in the `sslData` parameter. To create or configure your certificate, use the [methods](/en/tools-for-using-services/api/api-spec/api-cdn) of the **SSL certificates** section.
