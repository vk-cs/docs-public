...
By using the Hystax Acura Migration service, you agree to the license agreements of [Marketplace](/ru/start/legal/offer/private-special-conditions/policy-marketplace "change-lang") and [Hystax Acura Migration](https://хст.рф/terms-of-use).
...
1. Adjust the plan parameters according to your VM migration requirements.
   ...
   Plan parameters:
    ...
    - `availability_zone` — a name of an [availability zone](/en/intro/start/concepts/architecture#architecture-az) where the VM will be deployed.
    - `security_groups` — a list of [security group](/en/networks/vnet/instructions/secgroups) names or IDs for `Ubuntu-MR`.
    ...
    - `custom_image_metadata` — custom metadata for the VM.
      ...
      - `os_distro` — the name of the guest OS distribution. Find the name by following the instructions in the [Filling os_distro and os_version tags](/en/computing/iaas/instructions/images/image-metadata#find_os_distro_and_os_version) section.
      - `os_version` — the guest OS version. Find the version by following the instructions in the [Filling os_distro and os_version tags](/en/computing/iaas/instructions/images/image-metadata#find_os_distro_and_os_version) section.
      - `os_admin_user` — the name of the guest OS user with administrator rights. The password can be set via [your management console](/en/computing/iaas/instructions/vm/vm-manage#password).
...
Find the added VM in VK Cloud (`<project PID>_cloud_agent`), [perform](/en/computing/iaas/instructions/vm/vm-manage) arbitrary operations on it.
...
- [Delete](/en/networks/vnet/instructions/ip/floating-ip#vnet-floating-ip-delete) floating IP address if it was created during migration.
