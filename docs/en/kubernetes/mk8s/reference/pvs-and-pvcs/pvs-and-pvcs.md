...
- Storage classes must be configured in the Kubernetes cluster. Kubernetes clusters in the Managed Containers service already contain [pre-configured storage classes](/en/kubernetes/mk8s/concepts/storage#pre_configured_storage_classes).
...
In VK Cloud Kubernetes clusters, the default storage class [is not configured](/en/kubernetes/mk8s/concepts/storage#pre_configured_storage_classes). If you do not plan to explicitly set the storage class in the PVC, then [manually set the default storage class](https://kubernetes.io/docs/tasks/administer-cluster/change-default-storage-class/) before creating the PVC.
...
- Delete Volume (`Delete`).
  ...
  In VK Cloud, due to [integration with Cinder CSI](/en/kubernetes/mk8s/concepts/storage#working_with_container_storage_interface_csi), deleting a PV will also delete the VK Cloud disk associated with it.

  {/note}

In VK Cloud Kubernetes clusters, the storage type selected affects [available reclaim policies](/en/kubernetes/mk8s/concepts/storage#available_reclaim_policies_for_persistent_volumes).

## See also

- [How storage is organized in Managed Containers](/en/kubernetes/mk8s/concepts/storage).
- [List of pre-configured storage classes](/en/kubernetes/mk8s/concepts/storage#pre_configured_storage_classes).
- [Use-case](/en/kubernetes/mk8s/how-to-guides/storage) that demonstrates using of various PVCs.
