Labels are key/value pairs that are attached to Kubernetes objects, such as [pods](/en/kubernetes/mk8s/reference/pods). They are designed to specify and use identifying attributes for objects relevant to the user. Taints can be used to organize and select subsets of objects.
