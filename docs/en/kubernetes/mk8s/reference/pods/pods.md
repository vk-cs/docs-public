...
- A rarer case: a pod can consist of multiple containers. These containers can communicate with each other and share resources assigned to the pod (e.g., storage).
  ...
  - It is recommended to [limit compute resource consumption](/en/kubernetes/mk8s/reference/resource-limiting) for the pod's container for stable cluster operation.
