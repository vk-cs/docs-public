Gatekeeper is a controller embedded between the Kubernetes API and the Open Policy Agent (OPA) policy engine to check that Kubernetes resources created, modified and deleted are compliant with policies. For more information about Gatekeeper, see [Kubernetes reference](/en/kubernetes/mk8s/reference/gatekeeper) and [official Gatekeeper documentation](https://open-policy-agent.github.io/gatekeeper/website/docs/).

{note:warn}

Use these instructions if your cluster is version 1.20 or lower. Starting with Kubernetes 1.21, Gatekeeper is [already installed](/en/kubernetes/mk8s/concepts/architecture) in the cluster along with [preconfigured templates and restrictions](/en/kubernetes/mk8s/concepts/addons-and-settings/settings).

{/note}

## Installation

1. [Install Helm](/en/kubernetes/mk8s/install-tools/helm) if the utility is not already installed.
