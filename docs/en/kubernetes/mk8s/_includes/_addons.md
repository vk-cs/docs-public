...
- Add-ons are installed on the worker nodes of the cluster and consume their computing resources.
  ...
  Some add-ons can be installed on all cluster nodes (including master nodes). For more details, see [Configuring and installing add-ons](/en/kubernetes/mk8s/instructions/addons/advanced-installation).
...
- There are three options for installing add-ons:
  ...
  The installation process is described in the section [Configuring and installing add-ons](/en/kubernetes/mk8s/instructions/addons/advanced-installation).
...
S3-CSI can create buckets in any S3-compatible storage that you specify when [installing](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-s3-csi) the add-on.
...
See [Connecting to the Docker registry](/en/kubernetes/mk8s/connect/docker-registry) for details.
...
For the details on how to use the addon with the VK Cloud Secrets Manager, refer to [Using External Secrets Operator](/en/kubernetes/mk8s/how-to-guides/external-secrets-operator).
...
The sources of the logs are [kubelet services](https://kubernetes.io/docs/concepts/overview/components/#kubelet) and [pods](/en/kubernetes/mk8s/reference/pods) located on cluster nodes.

{/tab}

{tab(System requirements)}

The add-on does not have its own system requirements. The add-on subs use [limit settings](/en/kubernetes/mk8s/concepts/addons-and-settings/settings#limits_settings_for_pods) by default.
...
<!--Fluent Bit for Cloud Logging (logaas-integration) for first-generation clusters -->

{tabs}

{tab(Description)}

Fluent Bit in combination with [special filters](https://docs.fluentbit.io/manual/pipeline/filters/lua), written in Lua, allows you to organize the delivery of logs from a Kubernetes cluster in the Managed Containers service to the [Cloud Logging](/en/monitoring-services/logging) service for further analysis of these logs.

The sources of the logs are [kubelet services](https://kubernetes.io/docs/concepts/overview/components/#kubelet) and [pods](/en/kubernetes/k8s/reference/pods) located on cluster nodes. For more information about how the add-on works, see [Installing the add-on](/en/kubernetes/k8s/instructions/addons/advanced-installation/install-advanced-logaas-integration-magnum).

{/tab}

{tab(System requirements)}

The add-on does not have its own system requirements. The add-on subs use [limit settings](/en/kubernetes/k8s/concepts/addons-and-settings/settings#limits_settings_for_pods) by default.
...
<!--Fluent Bit for Cloud Logging (logaas-integration) for second-generation clusters -->
...
The sources of the logs are [kubelet services](https://kubernetes.io/docs/concepts/overview/components/#kubelet) and [pods](/en/kubernetes/mk8s/reference/pods) located on cluster nodes. For more information about how the add-on works, see [Installing the add-on](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-logaas-integration-mk8s).
...
GPU Operator allows you to manage [GPUs on cluster nodes](/en/kubernetes/mk8s/concepts/flavors) to perform machine learning or big data processing.

The following options for using GPUs in a cluster are available:

- One pod uses one or more GPUs.
- The add-on distributes one GPU between several pods using the [MIG](/en/kubernetes/mk8s/concepts/flavors#gpu-sharing) strategy.
- The add-on distributes one GPU between several pods using the [MPS](/en/kubernetes/mk8s/concepts/flavors#gpu-sharing) strategy.
...
The pre-installed Ingress controller integrates tightly with the VK Cloud platform. For more information, see [Network in cluster](/en/kubernetes/mk8s/concepts/network).
...
- **RAM**: 238Mi — 660Mi
- **Standard load balancer**: one piece
- **Floating IP**: one piece (when installed with [default settings](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-ingress#installing_addon))
...
- The number of worker nodes must be at least the selected number of Elasticsearch replicas.
  ...
  For more information about choosing the number of replicas, see [section about installing Jaeger](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-jaeger).
...
For more details on working with the add-on, refer to the [Using Kgateway](/en/kubernetes/mk8s/how-to-guides/kgateway) section.
...
See [Cluster Monitoring](/en/kubernetes/mk8s/monitoring#using_grafana) for details.
