...
1. Solve the issue using one of the following approaches:
   ...
   - [Enable](/en/kubernetes/mk8s/instructions/scale) automatic scaling of node groups or manually [increase](/en/kubernetes/mk8s/instructions/manage-node-group) the number of nodes in the group if there are not enough existing nodes to accommodate the pods.
...
1. Solve the issue using one of the following approaches:
   ...
   - In the pod manifest, adjust the values in the `resources.requests` and `resources.limits` blocks to match the set quotas and limits.
      ...
      For more details on resources and limits, refer to the [Resource limiting](/en/kubernetes/mk8s/reference/resource-limiting) section and the [official Kubernetes documentation](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/). 
...
{include(/en/_includes/_pod_events_diagnostics.md)}
   ...
   If it does, the pod has run into issues with [PVs or PVCs](/en/kubernetes/mk8s/reference/pvs-and-pvcs). Examples of such issues:
   ...
   - A [storage class](/en/kubernetes/mk8s/concepts/storage#pre_configured_storage_classes) is not configured, or it does not meet the requirements of the PVC.

1. Solve the issue using one of the following approaches:

   - Create a PV manually or adjust parameters for its [dynamic provisioning](/en/kubernetes/mk8s/reference/pvs-and-pvcs#dynamic-provisioning). For more details on working with PVs, refer to the respective [how-to guide](/en/kubernetes/mk8s/how-to-guides/storage) and the [official Kubernetes documentation](https://kubernetes.io/docs/concepts/storage/persistent-volumes/). 
...
1. Solve the issue using one of the following approaches:
   ...
   - [Increase](/en/kubernetes/mk8s/instructions/manage-node-group) the number of nodes (add them to an existing group or create a new one).
...
1. Solve the issue using one of the following approaches:
   ...
   In the manifest, [change or specify](/en/kubernetes/mk8s/connect/docker-registry#using_docker_registry_in_kubernetes_cluster) the secret for accessing the registry. 
...
1. View information about the system pods of the cluster located in the `kube-system` namespace using the command:
   ...
   If any of these pods are stuck in the `Pending` state, this might be caused by issues in integration with the [container network interface (CNI) or the container storage interface (CSI)](/en/kubernetes/mk8s/concepts/architecture#integration_with_vk_cloud_platform).
...
1. View the errors in the `Conditions` section and fix them:
   ...
   | `NetworkUnavailable`
   | The network is unavailable as the [CNI](/en/kubernetes/mk8s/concepts/network#supported-cni) has failed
   | 
