When trying to connect to OpenStack or the Kubernetes API, the [Cluster Autoscaler](/en/kubernetes/mk8s/concepts/cluster-autoscaler) pod has the `CrashLoopBackOff` status, causing it to fail authentication.
