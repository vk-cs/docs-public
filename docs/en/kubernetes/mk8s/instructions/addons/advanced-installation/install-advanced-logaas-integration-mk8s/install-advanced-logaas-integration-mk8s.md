## {heading(Before you begin)[id=preparation]}
...
For this add-on, only the [standard installation option](/en/kubernetes/mk8s/concepts/addons-and-settings/addons#features_of_installing_addons) via the VK Cloud management console is available.
...
1. Adds additional metadata to the logs, which makes it easier to work with cluster logs (for example, searching for the necessary logs in Cloud Logging). Special Fluent Bit filters are used for this, [written in Lua](https://docs.fluentbit.io/manual/pipeline/filters/lua). This metadata contains, among other things, the severity level of logged events.
   ...
   For each log entry, identifiers are set that allow you to determine the source of logs when using Cloud Logging:
    ...
    - The resource ID (`stream-id`) in one of the formats:
        ...
        - For [pod](/en/kubernetes/mk8s/reference/pods): `<namespace>.<name>`.
