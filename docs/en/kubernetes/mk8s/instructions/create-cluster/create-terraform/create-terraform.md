Description of how to create a cluster using Terraform is provided below. You can also create a cluster [via the VK Cloud management console](/en/kubernetes/mk8s/instructions/create-cluster/create-webui-gen-2).
...
1. Read about [Terraform features](/en/kubernetes/mk8s/instructions/helpers/terraform-howto) in the container service.
...
1. Add data sources to the configuration file:
   ...
   1. [Virtual machine template](/en/kubernetes/mk8s/concepts/flavors#configuration_templates) for master nodes. Example:
...
To install add-ons in the cluster via Terraform, [get the list of availible add-ons](/en/kubernetes/mk8s/instructions/addons/manage-addons#348-tabpanel-1) and [install that you need](/en/kubernetes/mk8s/instructions/addons/advanced-installation).
...
This operation is described in detail in [Worker node group management](/en/kubernetes/mk8s/instructions/manage-node-group).
