The following describes how to create a Kubernetes cluster from your management console. For these clusters, integration with the [VK Cloud monitoring service](/en/monitoring-services/monitoring) is enabled, and you cannot disable it.
...
1. In the **Master nodes** block, click the **Set up** button and specify the basic cluster settings:
   ...
   - **Number of Master nodes**: select `1`, `3`, or `5` depending on how highly available you want the cluster to be. We recommend creating clusters with 3 or 5 master nodes, as one master node does not provide cluster high availability at the master node level. The number of master nodes for fault-tolerant clusters is automatically set to `3`, but you can change it to `5`.
      ...
      Master nodes are managed by the VK Cloud platform, so the rest of the master node settings are configured automatically. For more information about cluster topologies and possible configurations, see [Service architecture](/en/kubernetes/mk8s/concepts/architecture#cluster_topologies).

   - **Availability zone**: select the [availability zone(s)](/en/intro/start/concepts/architecture#architecture-az) for nodes. All three availability zones are automatically selected for a regional cluster and cannot be changed.
...
1. In the **Network and settings** block, click the **Set up** button and specify the network settings:
   ...
   - **CNI**: select a [Container Network Interface (CNI)](/en/kubernetes/mk8s/concepts/network#supported-cni).
...
1. In the **Node group** block, click the **Set up** button and specify the [settings](/en/kubernetes/mk8s/instructions/helpers/node-group-settings) for the worker node group.

    1. (Optionally) Add one or more worker-node groups by clicking the **Add node group** button, and [configure](/en/kubernetes/mk8s/instructions/helpers/node-group-settings) them.
...
1. Click the **Create** button.
   ...
   Once the cluster is created, its [configuration file](https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/) (kubeconfig) will be automatically downloaded to your machine with the name in the following format: `<CLUSTER NAME>_kubeconfig.yaml`. You will need this file to [connect](/en/kubernetes/mk8s/connect) to the cluster.

## What's next?

- [Set up the environment](/en/kubernetes/mk8s/connect) on the host from which you plan to connect to the cluster.
- [Familiarize yourself with the usage scenarios](/en/kubernetes/mk8s/how-to-guides) of the cluster.
- [Familiarize yourself with the concepts](/en/kubernetes/mk8s/concepts) of the container service.
