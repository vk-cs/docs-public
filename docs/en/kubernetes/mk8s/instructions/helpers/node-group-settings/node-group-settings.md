These settings are set when [creating a cluster](/en/kubernetes/mk8s/instructions/create-cluster/create-webui-gen-2) or when [adding a worker node group](/en/kubernetes/mk8s/instructions/manage-node-group#add_group) to an existing cluster:
...
- Worker node settings:
  ...
  - **Node type:** [virtual machine template](/en/kubernetes/mk8s/concepts/flavors#configuration_templates) for worker-nodes.
  ...
  - Storage settings:
    ...
    - **Disk type**: [storage type](/en/kubernetes/mk8s/concepts/storage#storage_types) to be used by nodes.
  ...
  - **Enable autoscaling**: enable this option to allow [auto-scaling](/en/kubernetes/mk8s/concepts/architecture#cluster_scaling_options) the number of nodes in the group. Then set the minimum and maximum number of nodes. Within these limits the scaling will be performed.

  - **Percentage of unavailable nodes when updating the cluster version**: the percentage of nodes that can be removed from the node group when [updating](/en/kubernetes/mk8s/instructions/update) the cluster.
  
    You can set the value of this setting both when [creating](/en/kubernetes/mk8s/instructions/manage-node-group#add_group) the node group and before starting the cluster [update](/en/kubernetes/mk8s/instructions/manage-node-group#configure_node_update). During the update process, the Managed Containers service redirects the load from the nodes that are being updated to other nodes, so you need to ensure there are [enough spare nodes available](/en/kubernetes/mk8s/concepts/update#unavailable-nodes). Otherwise, applications that run on the nodes that are being updated may not have enough resources.

  - Kubernetes parameters: labels, taints and tolerations.

    [More about labels and taints](/en/kubernetes/mk8s/reference/labels-and-taints).
