The Ingress controller can be deployed [in conjunction with the HTTP load balancer](/en/kubernetes/mk8s/concepts/network) of the VK Cloud platform. As an example, a simple demo application and an Ingress resource will be deployed to test the operation of the controller.
...
1. [Make sure](/en/kubernetes/mk8s/instructions/addons/manage-addons#viewing_addons) that the NGINX Ingress add-on (`ingress-nginx`) **is not installed** in the cluster. For demonstration purposes, the Ingress controller will be installed manually.

1. [Make sure](/en/kubernetes/mk8s/connect/kubectl) that you can connect to the cluster using `kubectl`.

1. [Install](/en/kubernetes/mk8s/install-tools/helm) Helm if the utility is not already installed.
...
1. Delete the resources described in the manifests, the `example-nginx-ingress-http` app, as well as the `example-nginx-ingress-http` namespace and the resources associated with it:
