...
1. If you already have an existing Kubernetes cluster with the Kube Prometheus Stack add-on that needs to be upgraded in the Managed Containers service, skip this step.
   ...
   1. [Create](/en/kubernetes/mk8s/instructions/create-cluster) a Kubernetes cluster version `1.26.5`.

      When you create the cluster, select the **Assign external IP** option. Other cluster parameters can be set at your discretion.

   1. [Install](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-monitoring) the Kube Prometheus Stack add-on version `36.2.0` in the cluster.

      Perform a **quick installation** of the add-on (without editing the add-on configuration code).

1. [Verify](/en/kubernetes/mk8s/connect/kubectl) that you can connect to the cluster via `kubectl`.

   To connect, use the cluster configuration file (kubeconfig) downloaded from the VK Cloud management console.

1. Make sure the add-on is available and working. To do this, [access the Grafana web interface](/en/kubernetes/mk8s/monitoring#using_grafana).
...
1. [Install](/en/kubernetes/mk8s/install-tools/helm) Helm version 3.0.0 or higher if the utility has not been already installed.
...
1. [Go to editing the add-on configuration code](/en/kubernetes/mk8s/instructions/addons/manage-addons#editing_addon_code).
...
1. Get information about [Persistent Volumes (PVs) and Persistent Volume Claims (PVCs)](/en/kubernetes/mk8s/reference/pvs-and-pvcs). They are used to store the collected metrics, as well as other data necessary for the add-on to work.
...
1. [Uninstall the current version of the add-on](/en/kubernetes/mk8s/instructions/addons/manage-addons#removing_addon) using VK Cloud interfaces.
1. [Install](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-monitoring) the Kube Prometheus Stack add-on version `54.2.2` in the cluster.
...
1. Get information about the [Persistent Volume Claims (PVCs) and persistent volumes (PVs)](/en/kubernetes/mk8s/reference/pvs-and-pvcs) used by the add-on:
