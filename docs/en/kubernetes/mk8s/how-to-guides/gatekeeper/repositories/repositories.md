...
1. [Make sure](/en/kubernetes/mk8s/connect/kubectl) that you can connect to the cluster with `kubectl`.
...
1. Delete the created `allowed-pod` pod, the constraint template, and the constraint itself:
