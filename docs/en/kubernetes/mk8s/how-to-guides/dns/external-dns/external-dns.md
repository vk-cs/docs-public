...
1. [Create](/en/kubernetes/mk8s/instructions/create-cluster) a Kubernetes cluster of the latest version that has an external IP address and is accessible from the Internet.
...
1. [Install](/en/kubernetes/mk8s/install-tools/helm) Helm version 3.0.0 or higher if the utility is not already installed.
...
1. [Install](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-ingress) the Ingress NGINX add-on of the latest version to the cluster.
...
1. [Delete the Ingress NGINX add-on](/en/kubernetes/mk8s/instructions/addons/manage-addons#removing_addon).
