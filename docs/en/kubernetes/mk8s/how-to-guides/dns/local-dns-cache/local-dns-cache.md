...
Let some [pod](/en/kubernetes/mk8s/reference/pods) initiate a DNS query.
...
- IP addresses that the server listens to:
  ...
  This configuration is used because `kube-proxy` in Kubernetes clusters in Managed Containers [operates](/en/kubernetes/mk8s/concepts/addons-and-settings/settings#kube_proxy_operation_mode) in `iptables` mode.
...
{include(/en/_includes/_remove-k8s-resources.md)} a local DNS cache configuration, delete them:

1. Delete the created pod and resources described in the `nodelocaldns.yaml` manifest: 
