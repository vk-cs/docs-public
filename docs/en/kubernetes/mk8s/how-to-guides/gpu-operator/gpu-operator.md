Managed Containers allows you to create [GPU clusters](/en/kubernetes/mk8s/concepts/flavors) for machine learning or big data processing.
...
- GPU resources are shared across multiple pods using the [MPS](/en/kubernetes/mk8s/concepts/flavors#gpu-sharing) strategy. Each process gets a dedicated set of GPU resources, and they communicate with each other via inter-process communication (IPC).
- GPU resources are shared across multiple pods using the [MIG](/en/kubernetes/mk8s/concepts/flavors#gpu-sharing) strategy. In this case, each part has its own resources (memory, compute units) and can work in isolation from others.
...
1. [Create](/en/kubernetes/mk8s/instructions/create-cluster) a cluster with GPU if not done so already. Specify the following parameters:
...
1. [Connect](/en/kubernetes/mk8s/connect/kubectl) to the cluster via kubectl.

## {counter(gpu)}. Install GPU Operator add-on

Follow the [installation instructions](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-gpu-operator).
...
1. [Add](/en/kubernetes/mk8s/instructions/addons/manage-addons#editing_addon_code) the [NVIDIA device plugin](https://github.com/NVIDIA/k8s-device-plugin/tree/v0.17.0?tab=readme-ov-file#with-cuda-mps) configuration to GPU Operator add-on code:
...
1. Add a label for the node with the GPU, which will enable the `mps-on` mode added in the add-on configuration:
   ...
   You can [add a label](/en/kubernetes/mk8s/instructions/manage-node-group#labels_taints) for all nodes in a group via your management console.
