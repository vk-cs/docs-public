Managed Containers provides environment for working with Kubernetes clusters on the VK Cloud platform. With its architecture based on [OpenStack](https://www.openstack.org/), Managed Containers offers extensive capabilities, fault tolerance, scalability, and integration with other platform services.

{cut(How Managed Containers interacts with other VK Cloud services)}

Managed Containers ensures the proper functioning of Kubernetes clusters, while:
...
![Service architecture](/en/kubernetes/mk8s/assets/k8s_arch.png){params[noBorder=true; width=50%]}

{/cut}

## Cluster topologies

A Kubernetes cluster in Managed Containers consists of two types of nodes, master nodes and worker nodes:
...
- _Master nodes_ store cluster-wide state information and manage workload distribution across worker nodes. Users cannot manage cluster nodes, as they are managed by the VK Cloud platform.
  ...
  When you [create](/en/kubernetes/mk8s/instructions/create-cluster/create-webui-gen-2) a Kubernetes cluster in Managed Containers, it automatically selects the minimum appropriate [configuration template](/en/kubernetes/mk8s/concepts/flavors#configuration_templates) for its master nodes. By default, it is a VM with an Intel Cascade Lake processor, 2 CPUs, and 6 GB RAM. The default [disk type](/en/kubernetes/mk8s/concepts/storage#storage_types) for master nodes is a 20 GB High-IOPS SSD.

  [Automatic scaling](/en/kubernetes/mk8s/concepts/scale#autoscaling) is enabled for all master nodes by default. When the workload of the cluster changes, the amount of the resources used by master nodes changes respectively.
...
- Cluster with one master node.
  ...
  ![Cluster with one master node](/en/kubernetes/mk8s/assets/cluster_types_1.png){params[noBorder=true; width=80%]}
  {/cut}
...
- Standard cluster with 3 or 5 master nodes.
  ...
  ![Standard cluster with 3 or 5 master nodes](/en/kubernetes/mk8s/assets/cluster_types_2.png){params[noBorder=true; width=80%]}
  {/cut}   
...
- Fault-tolerant cluster with 3 or 5 master nodes.
  ...
  ![Fault-tolerant cluster with 3 or 5 master nodes](/en/kubernetes/mk8s/assets/cluster_types_3.png){params[noBorder=true; width=80%]}
  {/cut}
...
See [Available Kubernetes versions and version support policy](/en/kubernetes/mk8s/concepts/versions) for details.
...
The cluster API endpoint in the Managed Containers service is placed behind [dedicated load balancer](/en/kubernetes/mk8s/concepts/network), so the cluster API can be accessed via the same IP address regardless of the number of master nodes.
...
- [Container Storage Interface](https://kubernetes-csi.github.io/docs/) (CSI): integration with storage services.
  ...
  Integration is achieved using OpenStack Cinder API. See [Storage in cluster](/en/kubernetes/mk8s/concepts/storage) for details.
...
- [Container Network Interface](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/) (CNI): integration with network services.
  ...
  Every Kubernetes cluster that you create in Managed Containers has plugins that support this interface: [Calico](https://projectcalico.docs.tigera.io/about/about-calico) and [Cilium](https://docs.cilium.io/en/stable/index.html).

  These plugins provide:

  - Network connectivity between containers, [pods](/en/kubernetes/mk8s/reference/pods), and cluster nodes
  ...
  Calico and Cilium integrate with the VK Cloud platform using SDN Sprut. See [Network in cluster](/en/kubernetes/mk8s/concepts/network) for details.
...
[Open Policy Agent Gatekeeper](/en/kubernetes/mk8s/reference/gatekeeper) is built into the cluster to increase the cluster's resistance to attacks. It allows you to apply constraints, which help to increase the security of deployed workload.

## Cluster scaling options

Managed Containers has built-in [scaling capabilities for master nodes and worker nodes](/en/kubernetes/mk8s/concepts/scale) that automatically adjusts the number of nodes depending on the workload requirements:

- For master nodes, automatic scaling is enabled by default, and you cannot disable it. 
- For worker nodes, automatic scaling is performed via [Cluster Autoscaler](/en/kubernetes/mk8s/concepts/cluster-autoscaler), and you need to manually enable it for each worker node group when [configuring its settings](/en/kubernetes/mk8s/instructions/helpers/node-group-settings).
