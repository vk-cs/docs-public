...
- _Vertical scaling_: The [virtual machine templates](/en/kubernetes/mk8s/concepts/flavors) that are used by the master and worker nodes of the cluster are being changed. In this way, you can manage the computing resources of the cluster without affecting the number of nodes.

  Scaling of worker nodes is performed within the framework of the [worker node group](/en/kubernetes/mk8s/concepts/architecture#cluster_topologies) via [Cluster Autoscaler](/en/kubernetes/mk8s/concepts/cluster-autoscaler).
...
.2+|If at least one of the thresholds is exceeded, the autoscaling agent requests the [Managed Containers](/en/kubernetes/mk8s) service to change the master node flavor:
...
|The autoscaling agent requests the [Managed Containers](/en/kubernetes/mk8s) service to change the master node flavor:
...
You can change the flavor of the master node VM to one with smaller CPU and RAM only in [manual mode](/en/kubernetes/mk8s/instructions/scale#scale_master_nodes).
...
If at least one of the thresholds is exceeded, a request will be sent to the [Managed Containers](/en/kubernetes/mk8s) service to change the master node VM flavor. In this case, the CPU and RAM values ​​will be doubled. For example, the flavor `STD2-2-6` will be changed to `STD2-4-12`.

{/cut}

### Horizontal autoscaling

You can [configure](/en/kubernetes/mk8s/instructions/scale#autoscale_worker_nodes) _horizontal autoscaling_ for a group of nodes via [Cluster Autoscaler](/en/kubernetes/mk8s/concepts/cluster-autoscaler). The number of worker nodes in the group will be automatically adjusted depending on the needs of the workload. This mechanism allows you to save up to 60% on computing power.
...
- Vertical scaling capabilities are limited by [current quotas](/en/tools-for-using-services/account/concepts/quotasandlimits) and available [virtual machine templates](/en/kubernetes/mk8s/concepts/flavors#configuration_templates).
...
- It is not possible to perform horizontal scaling manually if automatic scaling is configured. To perform manual scaling, [turn off automatic scaling](/en/kubernetes/mk8s/instructions/scale#scaling_groups_of_worker_nodes_c172481b).
...
- If the cluster is not [fault-tolerant](/en/kubernetes/mk8s/concepts/architecture#cluster_topologies) and contains a single master node, then the Kubernetes API will be unavailable until scaling is completed.
...
How to set up overprovisioning resources for cluster scaling, see [Reserving nodes in group](/en/kubernetes/mk8s/how-to-guides/autoscaling-placeholder).
