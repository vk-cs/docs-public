...
- VM disks.
- [Persistent volumes (PVs)](/en/kubernetes/mk8s/reference/pvs-and-pvcs).
...
1. A second-generation cluster you create is installed in this system cluster. Your cluster has network connectivity with your project, so you can [interact](/en/kubernetes/mk8s/instructions/manage-cluster) with it as usual.

System components in second-generation clusters, including their master and worker nodes, are managed by VK Cloud. For more details, refer to [Service architecture](/en/kubernetes/mk8s/concepts/architecture).
