...
To get the same capabilities for older clusters, [upgrade](/en/kubernetes/mk8s/instructions/update) to version 1.23 or higher.

{/note}

## How does the authentication process work

When using [kubectl](/en/kubernetes/mk8s/connect/kubectl), the `keystone-auth` utility is responsible for authentication.
