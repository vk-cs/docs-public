For Kubernetes clusters, a version update operation to one of [supported versions](/en/kubernetes/mk8s/concepts/versions/version-support) is available in VK Cloud. You cannot downgrade versions.

During the cluster update, the following entities also are updated:

- Part of the cluster [components](/en/kubernetes/mk8s/concepts/versions/components):
...
Installed add-ons do not get updated with cluster updates. If necessary, you can [update](/en/kubernetes/mk8s/instructions/addons/manage-addons#updating_addon_version) them separately after the cluster update.
...
1. Make sure that you have enough worker nodes in your cluster so that Managed Containers can transfer the load from the existing nodes to them. If you don't have enough worker nodes, [add](/en/kubernetes/mk8s/instructions/manage-node-group#add_group) them.
