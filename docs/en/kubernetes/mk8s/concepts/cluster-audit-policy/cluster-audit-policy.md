In Managed Containers, an [audit policy](https://kubernetes.io/docs/tasks/debug/debug-cluster/audit/#audit-policy) is configured for Kubernetes clusters you create. This policy determines which [Kubernetes API](/en/kubernetes/mk8s/concepts/architecture#kubernetes-api-integration) events in the cluster and to what extent are logged into the [Cloud Audit](/en/monitoring-services/event-log/concepts/about) service on the VK Cloud platform. This audit policy is configured for all clusters you create in the Managed Containers service, and you cannot change it.
...
Following is the full file with audit policy settings for Kubernetes clusters.
