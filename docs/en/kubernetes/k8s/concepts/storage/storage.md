...
- Container data is inaccessible to other containers, even if all containers are in the same [pod](/en/kubernetes/k8s/reference/pods).
...
- _Persistent volumes_ (PVs) have their own [lifecycle](/en/kubernetes/k8s/reference/pvs-and-pvcs), independent of the pod's lifecycle. Due to the separation of lifecycles, such volumes can be reused later with other pods. Pods and other workloads use Persistent Volume Claim (PVC) to handle persistent volumes.
...
- [Pre-configured storage classes](#pre_configured_storage_classes) that implement different [persistent volume reclaim policies](/en/kubernetes/k8s/reference/pvs-and-pvcs#4_reclaiming_830589dc) are available for block storage in the cluster.
...
- Statically and dynamically [provision](/en/kubernetes/k8s/reference/pvs-and-pvcs#dynamic-provisioning) PV that is based on block storage.
...
A [reclaim policy](/en/kubernetes/k8s/reference/pvs-and-pvcs#4_reclaiming_830589dc) can be set for a PV. The policy will be triggered when the PVC associated with that volume is deleted:
...
When using [dynamic provisioning](/en/kubernetes/k8s/reference/pvs-and-pvcs#dynamic-provisioning) of a persistent volume, a storage class must be specified. The default storage class is not configured in Kubernetes clusters you create in the Managed Containers service. You can either set the default class manually, or explicitly specify the required class when creating a PVC.

Managed Containers provides pre-configured storage classes that use Cinder CSI for block storage. They provide different storage types that you can use when configuring dynamic provisioning of a PV for a specific [region](/en/tools-for-using-services/account/concepts/regions) indicating the required availability zone.
...
- [Network in a cluster](/en/kubernetes/k8s/concepts/network).
