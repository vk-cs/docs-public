...
[Docker Registry](/en/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-registry) stores the default Docker container image, which includes libraries, dependencies, and settings for deploying the Cloud Spark cluster and running user applications on it.
