...
1. On the **Configuration** step, specify the instance parameters:
    ...
    - **Availability zone**: selecting the [data center](/en/intro/start/concepts/architecture#architecture-az) where the instance will be launched.
    ...
    - **Connect S3 bucket as disk**: enable the option to add an S3 bucket to the instance to store data in it. The bucket will be available inside the instance as a directory, and the data stored in it will be available in [VK Object Storage](/ru/storage/s3) even after the instance is deleted.
    - **Bucket**: if the **Connect S3 bucket as disk** option is enabled, in the list, select the bucket that you want to connect to the instance. If there are no suitable buckets, select **Create new bucket** — a new VK Object Storage bucket will be created when creating the instance.
