...
| Category of virtual machine | Flavors of the VMs. For more information, see [Cloud Servers service overview](/en/computing/iaas/concepts/about#flavors). |
...
| Disk size | Specifies the size of the VM disk in GB. Growth or shrinkage of the disk size of a JupyterHub instance is currently not supported. We recommend storing large amounts of data in external storage, for example, in [S3 object storage](/ru/storage/s3).|
| Disk type | Type of instance disk being created, [more](/en/computing/iaas/concepts/volume-sla). |
