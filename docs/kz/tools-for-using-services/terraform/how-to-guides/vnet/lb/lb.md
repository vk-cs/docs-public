# {heading(Жүктеме теңгергіштерін жасау және баптау)[id=terraform-lb]}
...
## {heading(Желіні жасау)[id=terraform-lb-net]}
...
## {heading(Жүктеме теңгергішін жасау)[id=terraform-lb-create]}
...
- Дереккөздер (data source):
  ...
  - `vkcs_compute_flavor`: ВМ флейворы (CPU, RAM, Disk). Оны {var(cloud)} жеке кабинетіндегі ВМ жасау шеберінен көруге болады.
...
- Ресурстар (resource):
  ...
  - `vkcs_compute_instance`: виртуалды машина данасы ресурсын басқарады. Осы ВМ-дарға түсетін трафик жасалатын жүктеме теңгергіші арқылы бөлінеді.
    ...
    - `name`: ВМ атауы.
    - `flavor_id`: оны жасау кезінде пайдаланылатын ВМ флейворы.
    - `security_groups`: осы ВМ-ға тағайындалған қауіпсіздік топтары атауларының тізімі.
    - `image_id`: осы ВМ-ды жасау кезінде пайдаланылатын ОС образы.
    - `network`: ВМ жасау кезінде қосылатын желі.
    - `depends_on`: ВМ көрсетілген ресурстар жасалғаннан кейін іске қосылады.

  - `vkcs_lb_loadbalance`: {var(cloud)} ішіндегі жүктеме теңгергіші ресурсын басқарады. Келесі ресурстарды қамтиды:

    - `name`: жүктеме теңгергіші үшін оқуға ыңғайлы атау. Бірегей болуы міндетті емес.
    - `vip_subnet_id`: жүктеме теңгергіші үшін адрес бөлінетін ішкі желі. Жүктеме теңгергіштерін тек саясатпен рұқсат етілген желілерде ғана жасай аласыз (мысалы, өзіңізге тиесілі желілерде немесе ортақ желілерде). Бұл параметрді өзгерту жаңа жүктеме теңгергішін жасайды.
    - `availability_zone`: теңгергіш орналастырылатын [қолжетімділік аймағы](/kz/start/concepts/architecture#architecture-az). ВМ-дарды теңгергішке қосуды оңтайландыру және жеделдету үшін оларды бір қолжетімділік аймағында орналастырыңыз. Қолжетімділік аймақтарының тізімін жеке кабинеттен немесе [OpenStack CLI](/kz/tools-for-using-services/cli/openstack-cli) арқылы көруге болады.

    - `tags`: жүктеме теңгергішіне тағайындалған қарапайым жолдар тізімі.

  - `vkcs_lb_listener`: {var(cloud)} ішіндегі тыңдаушы ресурсын басқарады. Келесі ресурстарды қамтиды:

    - `name`: тыңдаушы үшін оқуға ыңғайлы атау. Бірегей болуы міндетті емес.
    - `protocol`: хаттама `TCP`, `HTTP`, `HTTPS`, `TERMINATED_HTTPS`, `UDP` болуы мүмкін. Бұл параметрді өзгерту жаңа тыңдаушыны жасайды.
    - `protocol_port`: клиенттік трафик тыңдалатын порт. Бұл параметрді өзгерту жаңа тыңдаушыны жасайды.
    - `loadbalancer_id`: осы тыңдаушы ұсынылатын жүктеме теңгергіші. Бұл параметрді өзгерту жаңа тыңдаушыны жасайды.

  - `vkcs_lb_pool`: {var(cloud)} ішіндегі пул ресурсын басқарады. Келесі ресурстарды қамтиды:

    - `name`: пул үшін оқуға ыңғайлы атау.
    - `protocol`: хаттама `TCP`, `HTTP`, `HTTPS`, `PROXY` немесе `UDP` болуы мүмкін. Бұл параметрді өзгерту жаңа пулды жасайды.
    - `lb_method`: трафикті пул қатысушылары арасында бөлуге арналған жүктемені теңгеру алгоритмі. `ROUND_ROBIN`, `LEAST_CONNECTIONS`, `SOURCE_IP` немесе `SOURCE_IP_PORT` мәндерінің бірі болуы керек.
    - `listener_id`: пул қатысушылары байланыстырылатын тыңдаушы. Бұл параметрді өзгерту жаңа пулды жасайды. Ескерту: идентификаторлардың бірін көрсету қажет: `loadbalancer_id` немесе `listener_id`.

  - `vkcs_lb_member`: {var(cloud)} ішіндегі қатысушы ресурсын басқарады. Келесі ресурстарды қамтиды:

    - `address`: жүктеме теңгергішінен трафик алу үшін қатысушының IP-адресі. Бұл параметрді өзгерту жаңа қатысушыны жасайды.
    ...
    - `weight`: осы қатысушы пулдан алуы тиіс трафиктің салыстырмалы үлесін көрсететін оң бүтін сан. Мысалы, салмағы 10 болатын қатысушы салмағы 2 болатын қатысушыдан бес есе көп трафик алады. Әдепкі бойынша 1 мәні пайдаланылады.
...
## {heading(Өзгерістерді қолдану)[id=terraform-lb-apply]}
