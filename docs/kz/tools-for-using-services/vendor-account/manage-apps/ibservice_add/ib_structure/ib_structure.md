...
1. Сервис директориясында келесі директориялар мен файлдарды жасаңыз:
   ...
   * `images` директориясы — дүкендегі сервис белгішесінің кескіні бар файлды қамтиды.
      ...
      * `png` немесе `svg` кеңейтімі бар, сервис белгішесі ретінде пайдаланылатын файл (толығырақ — {linkto(../../service_description#service_description_icon)[text=%text]} бөлімінде). Файл атауы `service.yaml` файлын толтырған кезде `icon` параметрінде пайдаланылады.
...
{caption(Бірнеше `.tf` манифесі бар `deployment` директориясы мазмұнының мысалы)[align=left;position=above]}
```text
