...
1. Terraform-ды жергілікті түрде орнатыңыз:
   ...
   1. Terraform-ды [VK Cloud ресми айнасынан](https://hashicorp-releases.mcs.mail.ru/terraform) жүктеп алыңыз.
...
1. Жұмыс директориясында `terraform apply` және `deploy.tf` файлдарында сипатталған ресурстарды құруды іске қосу үшін `extended.tf` пәрменін орындаңыз.
