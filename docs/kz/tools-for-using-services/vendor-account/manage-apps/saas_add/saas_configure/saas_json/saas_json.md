...
1. `catalog_<SERVICE_NAME>.json` файлын жасаңыз.
1. JSON-файлда сервис конфигурациясын келесі құрылым бойынша сипаттаңыз:
...
* `<SERVICE_PARAMETERS>` — сервис параметрлері (толығырақ {linkto(../saas_param#saas_param)[text=%text]} бөлімінде).
* `preview` секциясы — тарифтік жоспарлар матрицасына арналған тарифтік опцияларды сипаттайды (толығырақ {linkto(../saas_preview#saas_preview)[text=%text]} бөлімінде).
* `plans` секциясы — тарифтік жоспарлар мен олардың опцияларын сипаттайды (толығырақ [Секция plans](../saas_plan) бөлімінде).

## {heading(SaaS-қосымшаға арналған JSON-файл мысалы)[id=saas_example]}
