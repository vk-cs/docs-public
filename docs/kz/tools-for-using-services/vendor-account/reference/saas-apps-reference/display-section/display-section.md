...
Бұл мысалда бэкаптарды жасау жиілігі бапталады. Егер `backup_method` тарифтік опциясының мәні `high-frequency` болса, тарифтік жоспарды конфигурациялау шеберінде `High-frequency бекап` тарифтік опциясы бар `frequency_per_day` тобы көрсетіледі.
