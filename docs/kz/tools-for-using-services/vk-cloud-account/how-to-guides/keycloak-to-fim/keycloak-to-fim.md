...
## {heading(3. {var(cloud)} жүйесінде топтар мен рөлдер арасындағы байланысты баптаңыз)[id=vk-cloud-account-keycloak-connect]}
...
1. {var(cloud)} пайдаланушысына автоматты түрде тағайындалған рөл {linkto(#vk-cloud-account-keycloak-connect)[text=топты қосу]} кезінде таңдалған рөлге сәйкес келетінін тексеріңіз.
