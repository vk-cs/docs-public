# {heading(Деректермен жұмыс)[id=dlh_process-data]}
...
## {heading(Дайындық қадамдары)[id=dlh_process-data_preparations]}
...
1. {linkto(../../quick-start#dlh_quick-start)[text=Өрістетіңіз]} Data Lakehouse сервисінің базалық архитектурасын.

    Мысал ретінде өрістету барысында {linkto(../../quick-start#dlh_quick-start_create_trino_instance)[text=Cloud Trino-ны баптау]} кезінде мыналар жасалды:

    - {var(s3)} қоймасына `s3db1` қосылымы;
...
## {heading(1. S3 қоймасында деректер схемаларын жасаңыз)[id=dlh_process-data_create_data-scheme]}
...
## {heading(2. Сыртқы деректер қорларынан деректерді S3 қоймасына жүктеңіз)[id=dlh_process-data_load_data_from_db]}
...
1. PostgreSQL сыртқы ДҚ-ның `table2` кестесінен деректерді сұратып, оларды S3 қоймасының `sch1` схемасындағы `table2_from_pg` жаңа кестесіне жүктеңіз:
...
## {heading(3. Виртуалды ДҚ кестелерінен деректерді S3 қоймасына жүктеңіз)[id=dlh_process-data_load_data_from_tables]}
...
## {heading(4. S3 қоймасына аналитикалық сұрау орындаңыз)[id=dlh_process-data_make_request_to_s3]}
...
1. Нәтижелік деректер іріктемесін S3 қоймасының `sch2` схемасындағы `analysis_from_s3` жаңа кестесі түрінде жазады.
...
## {heading(5. Деректерді S3 қоймасынан сыртқы деректер қорларына экспорттаңыз)[id=dlh_process-data_upload]}
...
1. S3 қоймасының `sf10` схемасындағы `store` кестесінен деректерді сұратып, оларды PostgreSQL сыртқы ДҚ-дағы аттас жаңа кесте түрінде сақтаңыз.
...
## {heading(6. Бірнеше деректер көзіне бір уақытта аналитикалық сұрау орындаңыз)[id=dlh_process-data_make_request_to_several_sources]}
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=dlh_process-data_delete_unused_resources]}
