...
1. Қажетті деректер жиынын кластерге байланыстырылған VK Object Storage бакетіне орналастырыңыз:

    1. **Объектілік қойма** → **Бакеттер** бөліміне өтіңіз.
    1. Кластерге байланыстырылған бакетті таңдаңыз. Оның атауы `<КЛАСТЕР_АТАУЫ>-<ДОМЕНДІК_АТАУ>-bucket` маскасына сәйкес келеді.
