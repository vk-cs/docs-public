...
- [Cloud Spark кластері](#cluster_spark);
- [Docker Registry бар виртуалды машина](#vm_docker_registry);
- [VK Object Storage](#object_storage);
- [Cloud ML Platform сервисі](#service_ml_platform).

## {heading(Cloud Spark кластері)[id=cluster_spark]}
...
## {heading(Docker Registry бар ВМ)[id=vm_docker_registry]}
...
[VK Object Storage](/kz/storage/s3) — S3 қолдауы бар VK Cloud платформасының объектілік қоймасы. Cloud Spark сервисін өрістету кезінде сервиспен біріктірілген [бакет](/kz/storage/s3/concepts/about#bucket) автоматты түрде жасалады. Ол әдепкі бойынша Spark History Server үшін логтарды сақтау, тәуелділіктерді, пайдаланушы қолданбаларының коды бар файлдарды және пайдаланушы тапсырмаларын орындауға қажетті басқа артефактілерді жүктеу үшін пайдаланылады.

Cloud Spark сервисіне қосымша бакеттерді де қосуға болады.

## {heading(Cloud ML Platform сервисі)[id=service_ml_platform]}
