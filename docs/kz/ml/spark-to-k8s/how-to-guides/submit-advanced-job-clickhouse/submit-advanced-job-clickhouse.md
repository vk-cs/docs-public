# {heading(Кеңейтілген Spark тапсырмаларымен жұмыс істеу)[id=mlspark-htg-advanced]}

{include(/kz/_includes/_translated_by_ai.md)}

Spark тапсырмаларын кластерге әртүрлі тәсілдермен жіберуге болады:

- Тәуелділіктері жоқ Spark қолданбалары үшін қолданба кодын тапсырма манифесіне беру жеткілікті. Бұл тәсіл {linkto(../submit-basic-job-pi#mlspark-htg-basic)[text=π санын есептеу мысалында]} көрсетілген.
...
## {heading(Дайындық қадамдары)[id=mlspark-htg-advanced-prerequirements]}

1. {linkto(../../ml-platform-library/install#mlspark-library-install)[text=Кітапхананы орнатыңыз]}, егер бұл әлі жасалмаған болса.
1. {linkto(../../ml-platform-library/authz#mlspark-library-authz)[text=Қолжетімділік токенін жасаңыз]} `Администратор` рөлімен, егер бұл әлі жасалмаған болса. Бұл токен кітапханамен жұмыс істеу үшін қажет.
...
1. {linkto(../../instructions/create#mlspark-instructions-create)[text=Cloud Spark кластерін жасаңыз]}.
...
1. Жасалған ClickHouse инстансына тағайындалған IP-мекенжайды біліңіз:
   ...
   1. Инстанс атауын басып, **Информация** қойындысына өтіңіз.
   1. Инстанстың сыртқы IP-мекенжайын жазып алыңыз.

## {heading(1. Spark қолданбасының коды бар файлды жасаңыз)[id=mlspark-htg-advanced-create-spark-file]}
...
Аталған орта айнымалыларының барлығы Spark тапсырмасын кластерге жіберу кезінде [кейінірек орнатылады](#mlspark-htg-advanced-send-to-cluster).

## {heading(2. Қажетті файлдарды бакетке жүктеңіз)[id=mlspark-htg-advanced-upload-to-bucket]}

1. Кластер пайдаланатын {var(s3)} бакетінің атын анықтаңыз:
   ...
   1. Қажетті ақпаратты табыңыз:
      ...
      - {var(s3)} бакетінің атауы (`s3_bucket_name` өрісінде болады).

1. Файлдарды осы бакеттің `spark-files` директориясына [жүктеңіз](/kz/storage/s3/instructions/objects/upload-object#s3-instructions-upload-object-standard):
...
## {heading(3. Spark тапсырмасын кластерге жіберіңіз)[id=mlspark-htg-advanced-send-to-cluster]}
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mlspark-htg-advanced-delete-unused-resources]}
...
1. Осы кластер пайдаланған [бакеттен объектілерді](/kz/storage/s3/instructions/objects/manage-object#udalenie_obektov) және [бакеттің өзін](/kz/storage/s3/instructions/buckets/manage-bucket#s3-instructions-manage-bucket-delete) жойыңыз.
1. [Қолжетімділік токенін жойыңыз](../../ml-platform-library/authz).
1. [ClickHouse инстансын жойыңыз](/kz/dbs/dbaas/instructions/manage-instance/clickhouse#dbaas-clickhouse-delete-instance).
