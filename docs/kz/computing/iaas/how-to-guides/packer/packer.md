...
Packer VK Cloud ішінде алдын ала орнатылған [конфигурация қалыптарының](../../concepts/vm/flavor) кез келгені мен қажетті қонақ ОС-ы бар базалық образ негізінде жаңа ВМ образын құрастыруға мүмкіндік береді. Жаңа образдың параметрлері Packer конфигурациялық файлының көмегімен беріледі. Мысал ретінде мыналар пайдаланылады:
...
1. Packer-дің соңғы нұсқасын [орнатыңыз](https://developer.hashicorp.com/packer/tutorials/docker-get-started/get-started-install-cli).
   ...
   Packer-ді VK Cloud [айнасынан](https://hashicorp-releases.mcs.mail.ru/packer/) жүктеп ала аласыз.
...
## 1. Образды RAW форматына түрлендіріңіз
...
## 2. Базалық образды бұлтқа жүктеңіз

Образды [нұсқаулық бойынша](../../instructions/images/images-manage#obrazdy_importtau) импорттаңыз.

## 3. Packer конфигурациялық файлын жасаңыз
...
1. Жаңа ВМ образы үшін қажетті конфигурация қалыбын таңдаңыз. Мысалда — `STD3-2-6`.
   ...
   Өзекті конфигурация қалыптары VK Cloud жеке кабинетінің **Виртуалды машина түрі** тізіміндегі [ВМ жасау бетінде](https://kz.cloud.vk.com/app/services/infra/servers/add) берілген.
...
## 4. Дайындалған образды бұлтқа жүктеңіз
...
Егер образ енді қажет болмаса, оны [жойыңыз](../../instructions/images/images-manage#obrazdy_zhoyu).
