# {heading(Windows ОС-ы бар ВМ образын жасау)[id=iaas-win-image]}

{include(/kz/_includes/_translated_by_ai.md)}

{note:info}
{var(cloud)} ішіне көші-қон қолдау көрсетілетін Windows ОС тізімі [операциялық жүйелер](../../concepts/oper-system) бөлімінде келтірілген.
{/note}

{var(cloud)} ішіне ВМ образын жасау және жүктеу Windows Server 2016 Standard редакциясы мысалында қарастырылады. Образды дайындау үшін Windows 11 Pro ОС-ы бар компьютер пайдаланылады. Барлық командалар Windows PowerShell консолінде орындалады.
...
## {heading(Дайындық қадамдары)[id=iaas-win-image-preparatory-steps]}

1. {var(cloud)} ішіне көшіру жоспарланған Windows ОС-ының орнату ISO-образын жүктеп алыңыз. Образдың en-US нұсқасын пайдалану ұсынылады.
...
## {heading(1. Құралдар жиынтығын дайындаңыз)[id=iaas-win-image-toolkit-prepare]}

1. Hyper-V виртуализация механизмін қосыңыз.

   1. Компьютердің басқару тақтасына өтіңіз. Ол үшін WIN+R пернелер тіркесімін басыңыз, ашылған **Выполнить** терезесінде `control` енгізіп, **OK** батырмасын басыңыз.
   1. Басқару тақтасында **Программы и компоненты** таңдаңыз.
   1. Сол жақ бүйірлік мәзірден **Включение или отключение компонентов Windows** таңдаңыз.
...
1. Windows Assessment and Deployment Kit (ADK) құралдар жиынтығын орнатыңыз.
   ...
   1. ADK орнатылғанын тексеріңіз. Ол үшін басқару тақтасындағы **Программы и компоненты** бөліміне өтіп, **Windows Assessment and Deployment Kit** бағдарламасын табыңыз.
...
## {heading(2. Hyper-V ішінде сыртқы виртуалды коммутатор жасаңыз)[id=iaas-win-image-switch-create]}
...
1. Hyper-V диспетчерін іске қосыңыз. Ол үшін WIN+R пернелер тіркесімін басыңыз, ашылған **Выполнить** терезесінде `virtmgmt.msc` енгізіп, **OK** батырмасын басыңыз.
1. Сол жақ мәзірден компьютеріңіздің атауын таңдаңыз.
1. Оң жақтағы **Действия** панелінен **Диспетчер виртуальных коммутаторов** опциясын таңдаңыз.
1. **Диспетчер виртуальных коммутаторов** терезесінің сол жақ мәзірінен **Создать виртуальный сетевой коммутатор** опциясын таңдаңыз.
1. Коммутатор түрі үшін `Внешний` мәнін таңдап, **Создать виртуальный коммутатор** батырмасын басыңыз.
1. **Имя** өрісіне коммутатор атауын енгізіңіз — `external`.
1. **Тип подключения** бөлімінде сыртқы желі таңдалғанына көз жеткізіп, тізімнен интернетке қолжетімді желілік адаптерді, мысалы компьютеріңіздің Ethernet немесе Wi‑Fi адаптерін таңдаңыз.
1. (Опционалды) Компьютеріңіздің ОС-ына да осы адаптерді пайдалануға рұқсат беру үшін **Разрешить управляющей операционной системе предоставлять общий доступ к этому сетевому адаптеру** опциясын таңдаңыз.
...
## {heading(3. Windows Imaging Tools конфигурациясын дайындаңыз)[id=iaas-win-image-config-prepare]}
...
1. `ISO` директориясына өтіп, Windows орнату образы бар ISO-файлды тінтуірдің оң жақ батырмасымен басып, контекстік мәзірден **Подключить** тармағын таңдаңыз.

   ISO-файл виртуалды диск ретінде монталанады. Windows Explorer-де жаңа диск пайда болады, мысалы `E:`.

## {heading(4. ОС-тың орнату WIM-файлын дайындаңыз)[id=iaas-win-image-file-prepare]}
...
1. `C:` дискінің түбірлік директориясында `Temp` директориясын жасаңыз.
...
## {heading(5. Образды жергілікті түрде жинаңыз)[id=iaas-win-image-locally-assemble]}
...
1. Жасалған файлды ашып, параметрлерін өңдеңіз:
   ...
   {note:info}
   {var(cloud)} платформасы ОС-ты BIOS арқылы жүктеуге арналған таңбаланған жүктеу дискілерін қолдайды. UEFI арқылы жүктеуге арналған таңбалау қолдау көрсетілмейді.
   {/note}
...
## {heading(6. Образды {var(cloud)} ішіне импорттаңыз)[id=iaas-win-image-import]}

Образды импорттау үшін [OpenStack CLI пайдаланыңыз](../../instructions/images/images-manage#iaas-images-manage-import):
...
openstack image create \
    ...
    <ИМЯ_ОБРАЗА>
```
`<ИМЯ_ОБРАЗА>` мәнін өзекті мәнге ауыстырыңыз. `--property <КЛЮЧ>=<ЗНАЧЕНИЕ>` түріндегі аргументтер образға [метатегтерді](../../instructions/images/image-metadata) тағайындау үшін қолданылады.

Операцияның аяқталуын күтіңіз. Образ жүктелгеннен кейін {var(cloud)} платформасының стандартты құралдарымен [ВМ жасауға](../../instructions/vm/vm-create) мүмкіндік пайда болады.

## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=iaas-win-image-delete]}

Егер импортталған образ енді қажет болмаса, оны [жойыңыз](../../instructions/images/images-manage#iaas-images-manage-delete).
