# {heading(AD топтық саясаттарын баптау)[id=desktops-group-policies]}

{include(/kz/_includes/_translated_by_ai.md)}

Соңғы пайдаланушылар Windows ОС басқаруындағы виртуалды жұмыс үстелдеріне қосыла алуы үшін, Active Directory домен контроллерінің топтық саясаттарын баптаңыз.

{note:info}
Нұсқаулық компанияңыздағы Active Directory қызметінің инфрақұрылымын қызмет көрсететін жүйелік әкімшілерге арналған.
{/note}

## {heading(Дайындық қадамдары)[id=desktops-ad-preparatory-steps]}

Active Directory домен контроллерінде Group Policy Management оснасткасын іске қосыңыз.

## {heading(1. Windows Defender Firewall үшін ереже қосыңыз)[id=desktops-ad-windows-role-add]}
...
## {heading(2. Виртуалды жұмыс үстелдеріне қосылуға арналған RDP сеансының қауіпсіздікті келісу механизмін баптаңыз)[id=desktops-ad-windows-rdp]}
...
## {heading(3. Соңғы пайдаланушыларға виртуалды жұмыс үстелдеріне қосылуға рұқсат беріңіз)[id=desktops-ad-windows-connect]}
...
1. `<ИМЯ_ВАШЕГО_ДОМЕНА>\<НАЗВАНИЕ_ГРУППЫ_ПОЛЬЗОВАТЕЛЕЙ_AD>` тобын `BUILTIN\Remote Desktop Users` стандартты тобының мүшелеріне қосыңыз.
