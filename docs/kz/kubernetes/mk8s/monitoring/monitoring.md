# {heading(Кластер мониторингі)[id=mk8s-monitoring]}

{include(/kz/_includes/_translated_by_ai.md)}

Kubernetes кластерінің күйін бақылау үшін келесі құралдар қолжетімді:

- {linkto(../concepts/addons-and-settings/addons#mk8s-addons-kube-prometheus-stack)[text=Мониторинг аддоны]}.
  ...
  - Кластер ресурстарының тұтынылуын {linkto(#mk8s-monitoring-forecast-consumption)[text=болжауға]}, болжау графиктерін қарауға және ресурстардың жақын арада таусылуы туралы хабарламаларды алдын ала алуға мүмкіндік береді. Болжау тек VK Tech тарапынан доработкалары бар {linkto(../concepts/versions/components#mk8s-components-addons)[text=аддон нұсқаларында]} қолжетімді (нұсқа нөмірінде `vk` бар).

  Мониторинг аддоны кластерде орнатылған-орнатылмағанын білу үшін, {linkto(../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=орнатылған аддондар тізімін қараңыз]}.

## {heading(Кластер метрикаларын көру үшін Grafana-ға қосылу)[id=mk8s-monitoring-connect-grafana]}

1. Кластерде мониторинг аддоны (`kube-prometheus-stack`) {linkto(../instructions/addons/advanced-installation/install-advanced-monitoring#mk8s-install-advanced-monitoring)[text=орнатылғанына]} {linkto(../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=көз жеткізіңіз]}.

1. {linkto(../connect/kubectl#mk8s-kubectl-check-connection)[text=Көз жеткізіңіз]}, кластерге `kubectl` көмегімен қосыла алатыныңызға.
...
1. Grafana-ға кіріңіз. Аддон қандай параметрлермен {linkto(../instructions/addons/advanced-installation/install-advanced-monitoring#mk8s-install-advanced-monitoring)[text=орнатылғанына]} байланысты мыналардың бірін пайдаланыңыз:
...
## {heading(Кластер ресурстарының тұтынылуын болжау)[id=mk8s-monitoring-forecast-consumption]}
...
1. Кластерде VK Tech доработкалары бар (нұсқа нөмірінде `vk` жұрнағы бар) `kube-prometheus-stack` мониторинг аддонының нұсқасы {linkto(../instructions/addons/advanced-installation/install-advanced-monitoring#mk8s-install-advanced-monitoring)[text=орнатылғанына]} {linkto(../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=көз жеткізіңіз]}.
1. `kube-prometheus-stack` аддоны кодының өңдеу бетіне {linkto(../instructions/addons/manage-addons#mk8s-manage-addons-edit-code)[text=өтіңіз]} және кодтағы `grafana.alerts.enabled` параметрі `true` мәніне тең екеніне көз жеткізіңіз. Бұл мән параметрге аддонды орнату кезінде беріледі.
...
1. {linkto(#mk8s-monitoring-connect-grafana)[text=Grafana-ға кіріңіз]}.
...
1. Ресурстар тұтынылуының қолжетімді болжамдарын қараңыз:
   ...
   - **Persistent Volume Leaders**: тұрақты томдағы ({linkto(../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=PV]}) орын тұтыну болжамы.
