# {heading(Қолжетімді есептеу ресурстары)[id=mk8s-flavors]}

{include(/kz/_includes/_translated_by_ai.md)}

## {heading(Конфигурация қалыптары)[id=mk8s-flavors-vm-flavor]}

Kubernetes кластерлерін жасаған кезде кластер түйіндеріне қолжетімді есептеу ресурстарын анықтайтын виртуалды машиналардың әртүрлі {linkto(../../../../computing/iaas/concepts/vm/flavor#iaas-concepts-vm-flavor)[text=конфигурация қалыптарын]} таңдауға болады. Қалыптардың бір бөлігі бірден қолжетімді, ал бір бөлігі [қолдау қызметіне](/kz/contacts) сұрау бойынша беріледі.
...
## {heading(Кластердің қолайлы конфигурациясын есептеу)[id=mk8s-flavors-configuration-calculation]}
...
## {heading(CPU және RAM қолжетімді есептеу ресурстарын есептеу)[id=mk8s-flavors-cpu-ram-calculation]}
...
## {heading(Түйіндегі қолжетімді есептеу ресурстары)[id=mk8s-flavors-available-node-resources]}
...
## {heading(GPU бар конфигурация қалыптары)[id=mk8s-flavors-gpu]}

{linkto(../../../../computing/gpu/concepts/about#gpu-about)[text=GPU-үдетілген серверлер]} машиналық оқыту және үлкен деректерді өңдеу саласында күрделі әрі ресурс сыйымды есептеулерді орындауға мүмкіндік береді. 

Кластер түйіндерінде GPU бар конфигурация қалыптарын пайдалану үшін Cloud GPU сервисін қосуға [өтінім қалдырыңыз](https://cloud.vk.ru/cloud-gpu/).

### {heading(GPU бар кластерді жасаудың ерекшеліктері)[id=mk8s-flavors-gpu-features]}

GPU бар конфигурация қалыбын кластердің тек worker-түйіндері үшін ғана пайдалана аласыз. GPU бар конфигурация қалыбы оны пайдаланатын түйінмен бірдей {linkto(../../../../start/concepts/architecture#architecture-az)[text=қолжетімділік аймағында]} болуы керек.
...
### {heading(GPU-ді ортақ пайдалану технологиялары)[id=mk8s-flavors-gpu-sharing]}
