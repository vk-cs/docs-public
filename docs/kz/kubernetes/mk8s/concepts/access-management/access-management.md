# {heading(Қатынасты басқару)[id=mk8s-access-management]}
...
- Бірыңғай кіру технологиясы (Single Sign-On, SSO) қолданылады.
  ...
  Пайдаланушы Kubernetes кластерінде VK Cloud {linkto(../../../../tools-for-using-services/account#tools-account)[text=жеке кабинетіне]} кіру кезіндегідей сол реквизиттермен {linkto(#mk8s-access-management-auth)[text=аутентификациядан өтеді]}.

  SSO функционалдығын өшіру мүмкін емес.

- Жеке кабинеттегі пайдаланушы рөлдері мыналарға әсер етеді:

  - {linkto(../../../../access/iam/concepts/rolesandpermissions#rolesandpermissions-k8s)[text=Жеке кабинетте кластерлермен орындалатын қолжетімді операцияларға]}.

  - {linkto(#mk8s-access-management-kubernetes-roles)[text=Кластердегі қолжетімді әрекеттерге]}.
...
Осындай мүмкіндіктерді ескі кластерлер үшін де алу үшін, оларды 1.23 немесе одан жоғары нұсқаға дейін {linkto(../../instructions/update#mk8s-update)[text=жаңартыңыз]}.

{/note}

## {heading(Аутентификация процесі қалай ұйымдастырылған)[id=mk8s-access-management-auth]}

{linkto(../../connect/kubectl#mk8s-kubectl)[text=kubectl]} пайдаланылғанда, аутентификацияға `keystone-auth` утилитасы жауап береді.
