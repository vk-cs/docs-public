# {heading(Сұрақтар мен жауаптар)[id=mk8s-faq]}
...
{linkto(../instructions/addons/advanced-installation/install-advanced-ingress#mk8s-install-advanced-ingress)[text=ingress-nginx]} аддонын пайдаланыңыз. Аддонды орнату кезінде бұл баптау әдепкі бойынша қосылады.
...
Limit Range — под жасалған кезде қолданылатын саясат. Оны өшіру мүмкін емес, бірақ сіз {linkto(../concepts/addons-and-settings/settings#mk8s-settings-requests-and-limits)[text=контейнерлердің]} конфигурация файлдарында `requests` және `limits` мәндерін өзіңіз орната аласыз.
{/cut}
