# {heading(LL NVMe дискілерін worker-түйіндерге қосу)[id=mk8s-llnvme-disks]}
...
## {heading(Дайындық қадамдары)[id=mk8s-llnvme-disks-prepare]}
...
1. VK Cloud жеке кабинетінде LL NVMe үшін конфигурацияға негізделген ВМ-мен түйіндер тобын {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-add-group)[text=қосыңыз]}:
...
1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=`kubectl` орнатып, баптаңыз]}, егер бұл әлі жасалмаса.
1. {linkto(../../connect/kubectl#mk8s-kubectl-connect)[text=Кластерге]} `kubectl` көмегімен қосылыңыз.

## {heading(1. LL NVMe дискілері үшін сақтау класын (StorageClass) қосыңыз)[id=mk8s-llnvme-disks-nvme-storageclass]}
...
## {heading(2. Қолданба жасаңыз)[id=mk8s-llnvme-disks-nvme-app]}
...
1. Подтың LL NVMe дискілері бар түйіндерге орналасуын қамтамасыз ету үшін бұрын жасалған түйіндер тобына `disktype: ef-nvme` меткасын {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-labels-taints)[text=қосыңыз]}:
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mk8s-llnvme-disks-delete]}
...
1. {linkto(../../concepts/storage#mk8s-storage-reclaim-policies)[text=Жасаған]} тұрақты томды жойыңыз.
