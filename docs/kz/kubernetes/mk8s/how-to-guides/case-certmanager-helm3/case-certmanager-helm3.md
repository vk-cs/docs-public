# {heading(Helm 3 көмегімен cert-manager-пен жұмыс істеу)[id=mk8s-case-certmanager-helm3]}
...
## {heading(Дайындық қадамдары)[id=mk8s-case-certmanager-helm3-prepare]}

{include(/kz/_includes/_create-test-cluster.md)}

1. Кластер нұсқасын анықтаңыз.

1. Кластерге қосылу жоспарланған хостта, егер бұл әлі жасалмаса, `kubectl` утилитасын {linkto(../../connect/kubectl#mk8s-kubectl)[text=орнатып, баптаңыз]}.

1. `kubectl` көмегімен кластерге {linkto(../../connect/kubectl#mk8s-kubectl-check-connection)[text=қосылыңыз]}.
...
## {heading(1. Репозиторийді қосып, орнату үшін нұсқаны таңдаңыз)[id=mk8s-case-certmanager-helm3-add-repo]}
...
1. Кластерге орнатылатын `cert-manager` нұсқасын таңдаңыз.
   ...
   Төменде қосымша түрде `1.12.3` нұсқасына дейінгі {linkto(#mk8s-case-certmanager-helm3-upd-cm)[text=жаңартуды]} көрсету үшін `1.11.3` нұсқасындағы `cert-manager` орнатылады.

   Сіз өзіңізге қолайлы кез келген нұсқаны таңдай аласыз. Төменде келтірілген пәрмендерді таңдалған нұсқаға сәйкес түзетіңіз.

   {/note}

## {heading(2. cert-manager орнатыңыз)[id=mk8s-case-certmanager-helm3-install-cm]}
...
## {heading(3. cert-manager жұмысқа қабілеттілігін тексеріңіз)[id=mk8s-case-certmanager-helm3-check-cm]}
...
## {heading(4. (Опционалды) cert-manager ресурстарының резервтік көшірмесін жасаңыз)[id=mk8s-case-certmanager-helm3-create-backup]}

{note:info}

`cert-manager` құралын {linkto(#mk8s-case-certmanager-helm3-upd-cm)[text=жаңарту]} алдында қауіпсіздік мақсатында резервтік көшірме жасау [ұсынылады](https://cert-manager.io/docs/tutorials/backup).
...
## {heading(5. cert-manager жаңартыңыз)[id=mk8s-case-certmanager-helm3-upd-cm]}
...
1. Кластерде орнатылған CRDs жаңартыңыз.
   ...
   Бұл CRDs бұрын {linkto(#mk8s-case-certmanager-helm3-install-cm)[text=қолмен орнатылғандықтан]}, оларды `cert-manager` құралының өзін жаңартпас бұрын да қолмен жаңартыңыз.
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mk8s-case-certmanager-helm3-delete]}
