# {heading(Пайдалану External Secrets Operator)[id=mk8s-external-secrets-operator]}

{include(/kz/_includes/_translated_by_ai.md)}

{linkto(../../concepts/addons-and-settings/addons#mk8s-addons-eso)[text=External Secrets Operator]} аддонын қолданып, VK Cloud [құпия менеджері](/ru/security/secret-manager/concepts/about#sm-about "change-lang") сақталған Kubernetes секреттерімен синхронизацияны баптаңыз.

## {heading(Дайындық қадамдары)[id=mk8s-eso-prepare]}

1. {linkto(../../instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=Жоқ болса]} жаңа нұсқасы бар кластер жасаңыз.
1. Кластерге `kubectl` арқылы қосыла алатыныңызға көз {linkto(../../connect/kubectl#mk8s-kubectl)[text=жеткізіңіз]}.
1. {linkto(../../instructions/addons/advanced-installation/install-advanced-eso#mk8s-install-advanced-eso)[text=External Secrets Operator аддонын орнатыңыз]}, егер бұрын жасалмаған болса.
...
## {heading({counter(eso)}. Секрет менеджерінде секрет жасаңыз)[id=mk8s-eso-kms]}
...
## {heading({counter(eso)}. Kubernetes секретін жасаңыз)[id=mk8s-eso-create]}
...
## {heading({counter(eso)}. SecretStore жасаңыз)[id=mk8s-eso-store]}
...
## {heading({counter(eso)}. ExternalSecret жасаңыз)[id=mk8s-eso-external]}
...
1. `externalsecret.yaml` манифестін жасаңыз — `ExternalSecret` типті объект, бұл секретті қай `SecretStore`-дан жүктеу керектігін және оны қалай жаңарту керектігін сипаттайды:
   ...
   Мұнда:
    ...
    - `<ID_СЕКРЕТА>` — VK Cloud секреттер менеджерінде бұрын [жасаған](#mk8s-eso-kms) секретіңіздің идентификаторы. Міндетті түрде қос тырнақшаға алу керек: `""`.
...
## {heading({counter(eso)}. Барлық секрет кілттерін синхронизациялау үшін ExternalSecret жасаңыз)[id=mk8s-eso-secrets-all]}
...
## {heading({counter(eso)}. Аддон жұмысын тексеріңіз)[id=mk8s-eso-check]}
...
## {heading(Қолданылмайтын ресурстарды жойыңыз)[id=mk8s-eso-delete]}
