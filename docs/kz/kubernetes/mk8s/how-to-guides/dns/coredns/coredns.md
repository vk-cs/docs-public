# {heading(Статикалық DNS жазбаларын қосу)[id=mk8s-coredns]}
...
## {heading(Дайындық қадамдары)[id=mk8s-coredns-prepare]}
...
## {heading(1. Хост мекенжайын CoreDNS ресурсының ConfigMap файлына қосыңыз)[id=mk8s-coredns-add-address]}
...
## {heading(2. Конфигурацияның қолданылуын тексеріңіз)[id=mk8s-coredns-config-check]}
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mk8s-coredns-delete]}
