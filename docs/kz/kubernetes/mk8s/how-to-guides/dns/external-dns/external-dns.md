# {heading(ExternalDNS орнату және пайдалану)[id=mk8s-external-dns]}
...
## {heading(Дайындық қадамдары)[id=mk8s-external-dns-prepare]}
...
1. Сыртқы IP мекенжайы бар және интернеттен қолжетімді өзекті нұсқадағы Kubernetes кластерін {linkto(../../../instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=жасаңыз]}.
...
1. Егер утилита әлі орнатылмаса, 3.0.0 немесе одан жоғары нұсқадағы Helm-ді {linkto(../../../install-tools/helm#mk8s-helm)[text=орнатыңыз]}.
...
## {heading(1. ExternalDNS үшін пайдаланушыны дайындаңыз)[id=mk8s-external-dns-prepare-user]}
...
1. Бар пайдаланушыны {linkto(../../../../../access/iam/instructions/access-manage#iam-access-manage-view-user)[text=таңдаңыз]} немесе жобаға жаңа пайдаланушыны {linkto(../../../../../access/iam/instructions/access-manage#iam-access-manage-invite-user)[text=шақырыңыз]}.
   ...
   - ExternalDNS DNS-аймақ аясында ресурстық жазбалармен жұмыс істей алуы үшін келесі рөлдердің біреуі {linkto(../../../../../access/iam/instructions/access-manage#iam-access-manage-user-role-edit)[text=тағайындалған]} болуы тиіс:

     - Желі әкімшісі (ең аз қажетті {linkto(../../../../../access/iam/concepts/rolesandpermissions#iam-concepts-rolesandpermissions)[text=рөл]}).
     - Жоба әкімшісі.
     - Суперадминистратор.
     - Жоба иесі.
...
1. API VK Cloud-қа қол жеткізу үшін қажетті деректемелерді алыңыз:
   ...
   1. ExternalDNS үшін бөлінген пайдаланушының деректемелерін пайдаланып, VK Cloud жеке кабинетіне [өтіңіз](https://kz.cloud.vk.kz/app/).
...
## {heading(2. ExternalDNS орнатыңыз)[id=mk8s-external-dns-install]}
...
1. Осы атаулар кеңістігінде API VK Cloud-қа қол жеткізу деректемелерін қамтитын құпияны жасаңыз, олар {linkto(#mk8s-external-dns-prepare-user)[text=пайдаланушыны дайындау кезінде алынған]}:
...
## {heading(3. External DNS жұмысын тексеріңіз)[id=mk8s-external-dns-check]}
...
### {heading(3.1. LoadBalancer түріндегі сервисті пайдаланып қолданбаны жариялаңыз)[id=mk8s-external-dns-publish-loadbalancer]}
...
1. ExternalDNS қажетті ресурстық жазбаларды жасағанына көз жеткізіңіз:
   ...
   1. Тізімнен ExternalDNS жасаған жазбаларды табыңыз:
      ...
      - `externaldns-tea.example.com` және `externaldns-a-tea.example.com` екі TXT-жазбасын.
        ...
        Егер {linkto(#mk8s-external-dns-install)[text=ExternalDNS орнату кезінде]} сіз префикс үшін басқа мән берсеңіз, онда қызметтік TXT-жазбалардың атаулары өзгеше болады.
...
### {heading(3.2. Ingress пайдаланып қолданбаны жариялаңыз)[id=mk8s-external-dns-publish-ingress]}

1. Кластерге Ingress NGINX аддонының ең өзекті нұсқасын {linkto(../../../instructions/addons/advanced-installation/install-advanced-ingress#mk8s-install-advanced-ingress)[text=орнатыңыз]}.
...
1. Қолданба пайдаланатын Kubernetes сервисіне арналған манифест жасаңыз.
   ...
   Мұнда сервис үшін [ClusterIP](https://kubernetes.io/docs/concepts/services-networking/service/#type-clusterip) түрі берілген, бұл жеткілікті, себебі қолданба Ingress көмегімен жарияланады. Мұндай сервис тек кластер ішінен ғана қолжетімді және бұрын {linkto(#mk8s-external-dns-publish-loadbalancer)[text=жасалған]} сервистен айырмашылығы, оған бөлек жүктеме теңгергіші бөлінбейді.
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mk8s-external-dns-delete]}
...
1. Ingress NGINX аддонын {linkto(../../../instructions/addons/manage-addons#mk8s-manage-addons-delete)[text=жойыңыз]}.
...
1. ExternalDNS жасаған {linkto(../../../../../networks/dns/instructions/publicdns/dns-zone#dns-dns-zone-delete)[text=ресурстық жазбаларды жойыңыз]}.
   ...
   Мұны, егер сіз `external-dns-vkcs-values.yaml` файлын {linkto(#mk8s-external-dns-install)[text=ExternalDNS орнату кезінде]} өзгертпеген болсаңыз, орындау қажет: онда ExternalDNS `upsert-only` саясатын пайдаланады және Kubernetes ресурстарын жойғанда DNS-аймақтан ресурстық жазбаларды жоймайды. Егер сіз бұл файлды өзгертіп, `sync` саясатын таңдасаңыз, онда бұл жазбалар автоматты түрде жойылады.
