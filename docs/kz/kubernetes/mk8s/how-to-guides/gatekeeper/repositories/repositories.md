...
VK Cloud Kubernetes кластерлерінің 1.23 және одан жоғары нұсқаларында қажетті {linkto(../../../concepts/security-policies#mk8s-security-policies-allowed-repos)[text=қауіпсіздік саясатын]} жеке кабинет арқылы {linkto(../../../instructions/manage-security#mk8s-manage-security-add-policy)[text=жасауға]} болады.

Бұл жағдайда Gatekeeper ресурстарымен тікелей жұмыс істеу міндетті емес.

{/note}

## {heading(Дайындық қадамдары)[id=mk8s-repositories-prepare]}

{include(/kz/_includes/_create-test-cluster.md)}

   Кластер параметрлерін өз қалауыңыз бойынша таңдаңыз.

1. {linkto(../../../connect/kubectl#mk8s-kubectl)[text=`kubectl` көмегімен кластерге қосыла алатыныңызға көз жеткізіңіз]}.

## {heading(Репозиторийлерді тексеретін шектеуді жасаңыз)[id=mk8s-repositories-create-constraint]}
...
## {heading(Қолданылмайтын ресурстарды жойыңыз)[id=mk8s-repositories-delete]}
