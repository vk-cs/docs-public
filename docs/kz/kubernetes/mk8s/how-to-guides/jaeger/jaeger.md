# {heading(Jaeger пайдалану)[id=mk8s-jaeger]}
...
## {heading(Дайындық қадамдары)[id=mk8s-jaeger-prepare]}

{include(/kz/_includes/_create-test-cluster.md)}
1. Егер бұл әлі жасалмаған болса, {linkto(../../connect/kubectl#mk8s-kubectl)[text=`kubectl` орнатып, баптаңыз]}.
1. `kubectl` көмегімен кластерге {linkto(../../connect/kubectl#mk8s-kubectl-check-connection)[text=қосылыңыз]}.
1. Егер бұл әлі жасалмаған болса, {linkto(../../instructions/addons/advanced-installation/install-advanced-jaeger#mk8s-install-advanced-jaeger)[text=Jaeger аддонын орнатыңыз]}.

## {heading({counter(jaeger)}. HotROD үшін қосымша жасаңыз)[id=mk8s-jaeger-app]}
...
## {heading({counter(jaeger)}. Жүктемені генерациялаңыз және сұраулар трассировкаларын алыңыз)[id=mk8s-jaeger-check]}
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mk8s-jaeger-delete]}
