# {heading(Ingress-контроллерін TCP-балансировщикпен жайып орналастыру)[id=mk8s-ingress-tcp]}
...
Ingress-контроллерін VK Cloud платформасының {linkto(../../../concepts/network#mk8s-network)[text=TCP жүктеме балансировщигімен бірге]} жайып орналастыруға болады. Төменде мысал ретінде контроллердің жұмысқа қабілеттілігін тексеру үшін қарапайым демо-қосымшалар мен Ingress ресурсы жасалады. Төменде NGINX Ingress Controller жайып орналастырылады деп болжанады. Алайда ұсынылған тәсілдерді басқа Ingress-контроллерлерге, мысалы, Traefik-ке де бейімдеуге болады.

## {heading(1. Дайындық қадамдары)[id=mk8s-ingress-tcp-prepare]}
...
1. {linkto(../../../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=Тексеріңіз]}, кластерде NGINX Ingress (`ingress-nginx`) аддоны **орнатылмаған** болуы керек. Көрсету мақсатында Ingress-контроллері қолмен орнатылады.

1. {linkto(../../../connect/kubectl#mk8s-kubectl)[text=Тексеріңіз]}, `kubectl` көмегімен кластерге қосыла алуыңыз керек.

1. Утилита әлі орнатылмаған болса, {linkto(../../../install-tools/helm#mk8s-helm)[text=Helm орнатыңыз]}.

1. Утилита әлі орнатылмаған болса, [curl](https://curl.se/docs/) орнатыңыз.

## {heading(2. Демо-қосымшаларды жайып орналастырыңыз)[id=mk8s-ingress-tcp-deploy-demo]}
...
## {heading(3. Ingress-контроллерін орнатыңыз)[id=mk8s-ingress-tcp-install-ingress]}
...
## {heading(4. Ingress ресурсін жасаңыз)[id=mk8s-ingress-tcp-create-ingress]}
...
## {heading(5. Қосымшалардың қолжетімділігін тексеріңіз)[id=mk8s-ingress-tcp-check]}
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mk8s-ingress-tcp-delete]}
