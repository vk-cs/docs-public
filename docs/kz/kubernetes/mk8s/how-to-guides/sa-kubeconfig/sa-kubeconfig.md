# {heading(Service account үшін kubeconfig файлын жасау)[id=mk8s-sa-kubeconfig]}

{include(/kz/_includes/_translated_by_ai.md)}

Managed Containers кластеріне {linkto(../../connect/kubectl#mk8s-kubectl)[text=kubectl көмегімен қосылғанда]} кластер конфигурациясының файлы — [kubeconfig](https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/) пайдаланылады. Әдетте кластермен жұмыс істеу үшін бірыңғай кіру {linkto(../../concepts/access-management#mk8s-access-management)[text=технологиясын]} пайдалануға бапталған VK Cloud жеке кабинетінен алынған kubeconfig қолданылады. Сондықтан `kubectl` құралымен жұмыс істегенде пайдаланушының паролін мезгіл-мезгіл енгізу керек болады.
...
## {heading(Дайындық қадамдары)[id=mk8s-sa-kubeconfig-prepare]}
...
1. `kubectl` көмегімен жасалған кластерге қосыла алатыныңызға {linkto(../../connect/kubectl#mk8s-kubectl)[text=көз жеткізіңіз]}.
...
1. Кластерге қосылғаннан кейін қажетті Kubernetes ресурстарын жасауға құқықтарыңыз бар екеніне көз жеткізіңіз:
   ...
   Рөлдік модель және қолжетімді рөлдер туралы толығырақ {linkto(../../concepts/access-management#mk8s-access-management)[text=Қолжетімділікті басқару]} бөлімінен оқыңыз.

## {heading(1. Service account жасаңыз және оны рөлмен байланыстырыңыз)[id=mk8s-sa-kubeconfig-create-sa]}
...
1. Service account-қа тағайындау қажет кластерлік рөлді таңдаңыз.
   ...
   Төменде мысал ретінде `edit` рөлі тағайындалады. Ол жеке кабинеттегі `Оператор Kubernetes` рөліне {linkto(../../concepts/access-management#mk8s-access-management-kubernetes-roles)[text=сәйкес келеді]}.
...
## {heading(2. Service account үшін токен алыңыз)[id=mk8s-sa-kubeconfig-get-token]}
...
1. Токен мәнін алыңыз.
   ...
   Токеннің мәні — құпия ақпарат. Егер ол әшкереленсе, {linkto(#mk8s-sa-kubeconfig-revoke-token)[text=токенді кері қайтарып алыңыз]}.

   {/note}

## {heading(4. Service account үшін kubeconfig жасаңыз)[id=mk8s-sa-kubeconfig-create]}
...
1. (Опционалды) kubeconfig құрылымымен танысыңыз:
   ...
   - `clusters`: кластерлердің және оларға қосылу деректерінің тізімі.
     ...
     Managed Containers кластеріне арналған kubeconfig жалғыз кластер туралы жазбаны қамтиды.

   - `users`: пайдаланушылардың және олардың кластердегі аутентификация деректерінің тізімі.

     Managed Containers кластеріне арналған kubeconfig `keystone-auth` көмегімен аутентификацияланатын жалғыз пайдаланушы туралы жазбаны қамтиды.
   ...
   - `contexts`: `kubectl` жұмыс істейтін контекст. Ең қарапайым жағдайда контекст — бұл кластер атауы мен пайдаланушы атының комбинациясы.
     ...
     Managed Containers кластеріне арналған kubeconfig жалғыз контекст туралы жазбаны қамтиды. Бұл контекст kubeconfig ішінде бұрыннан анықталған кластер мен пайдаланушы жазбаларын пайдаланады.
...
## {heading(5. Жасалған kubeconfig жұмысын тексеріңіз)[id=mk8s-sa-kubeconfig-check]}
...
Егер командаларды орындау кезінде пароль сұралмаса, онда алынған kubeconfig-ті Managed Containers кластеріне қол жеткізу үшін автоматтандырылған құралдармен бірге пайдалануға болады.

{note:err}

Kubeconfig файлын қорғау үшін қажетті шараларды қамтамасыз етіңіз. Онда құпия ақпарат бар: токен мәні ашық түрде сақталады.

Kubeconfig әшкереленген жағдайда {linkto(#mk8s-sa-kubeconfig-revoke-token)[text=токенді кері қайтарып алыңыз]}.

{/note}

## {heading(Әшкереленген токенді кері қайтарып алыңыз)[id=mk8s-sa-kubeconfig-revoke-token]}
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=mk8s-sa-kubeconfig-revoke-delete]}
