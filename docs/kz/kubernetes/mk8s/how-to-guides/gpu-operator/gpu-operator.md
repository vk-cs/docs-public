# {heading(GPU Operator пайдалану)[id=mk8s-gpu-operator]}

{include(/kz/_includes/_translated_by_ai.md)}

Managed Containers машиналық оқытуды орындау немесе үлкен деректерді өңдеу үшін {linkto(/kz/kubernetes/mk8s/concepts/flavors#mk8s-flavors-gpu)[text=GPU (графикалық процессорлары) бар кластерлерді]} жасауға мүмкіндік береді.
...
- GPU ресурстары бірнеше pod арасында {linkto(/kz/kubernetes/mk8s/concepts/flavors#mk8s-flavors-gpu-sharing)[text=MPS]} стратегиясы бойынша бөлінеді. Әрбір процесс GPU ресурстарының бөлінген жиынтығын алады және олар бір-бірімен процестер арасындағы өзара әрекеттесу (IPC) арқылы әрекеттеседі.
- GPU ресурстары бірнеше pod арасында {linkto(/kz/kubernetes/mk8s/concepts/flavors#mk8s-flavors-gpu-sharing)[text=MIG]} стратегиясы бойынша бөлінеді. Бұл ретте әрбір бөлік өз ресурстарына (жадына, есептеу блоктарына) ие болады және басқаларынан оқшау жұмыс істей алады.
...
## {heading(Дайындық қадамдары)[id=mk8s-gpu-operator-prepare]}

1. {linkto(/kz/computing/gpu/connect#gpu-connect)[text=Cloud GPU сервисін қосыңыз]}, егер ол әлі қосылмаған болса.
1. {linkto(/kz/kubernetes/mk8s/instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=GPU бар кластер жасаңыз]}, егер ол әлі жасалмаған болса. Келесі параметрлерді көрсетіңіз:
   ...
   **Node-түйіндерінің саны** және **Түйіндердің ең көп саны** параметрлерінде сізге қосылған GPU санынан үлкен мәнді көрсетпеңіз. Бұл кластердің жұмысқа жарамсыз болуына әкелуі мүмкін.

   {/note}

   Бұдан әрі мысал ретінде GPU бар шаблондағы `my-kubernetes-cluster-gpu-group-0` түйінімен `my-kubernetes-cluster` кластері жасалады. Басқа атауларды пайдалансаңыз, командаларда тиісті ауыстыруларды жасаңыз.

1. {linkto(/kz/kubernetes/mk8s/connect/kubectl#mk8s-kubectl)[text=Кластерге kubectl арқылы қосылыңыз]}.

## {heading(1. GPU-operator аддонын орнатыңыз)[id=mk8s-gpu-operator-install]}

{linkto(/kz/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-gpu-operator#mk8s-install-advanced-gpu-operator)[text=Орнату жөніндегі нұсқаулықты]} пайдаланыңыз.

## {heading(2. Кластердің жұмысын тексеріңіз)[id=mk8s-gpu-operator-check]}
      ...
      - name: cuda-vectoradd
        ...
        resources:
          limits:
            nvidia.com/gpu: 1
   ```
   Мұнда `nvidia.com/gpu: 1` параметрі тиісті вендордан (бұл жағдайда — `nvidia.com`) сұралатын GPU ресурстарының санын көрсетеді. Ресурстар мен лимиттер туралы толығырақ [Kubernetes ресми құжаттамасында](https://kubernetes.io/docs/tasks/manage-gpus/scheduling-gpus/).
...
## {heading(3. GPU-ді MPS технологиясы бойынша бөліңіз)[id=mk8s-gpu-operator-mps]}

1. [NVIDIA device plugin](https://github.com/NVIDIA/k8s-device-plugin/tree/v0.17.0?tab=readme-ov-file#with-cuda-mps) конфигурациясын GPU Operator аддонының кодының ішіне {linkto(/kz/kubernetes/mk8s/instructions/addons/manage-addons#mk8s-manage-addons-edit-code)[text=қосыңыз]}:
...
1. Аддон конфигурациясына қосылған `mps-on` режимін қосатын GPU бар түйін үшін метканы (label) қосыңыз:
   ...
   Топтағы барлық түйіндер үшін метканы жеке кабинет арқылы {linkto(/kz/kubernetes/mk8s/instructions/manage-node-group#mk8s-manage-node-group-labels-taints)[text=қоса аласыз]}.
...
## {heading(4. GPU-ді MIG технологиясы бойынша бөліңіз)[id=mk8s-gpu-operator-mig]}
...
1. GPU бөліктерін бөлу нұсқалары сипатталған [MIG технологиясы](https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/24.9.0/gpu-operator-mig.html) бойынша ConfigMap алыңыз:
   ...
   GPU-ді бөліктерге бөлу нұсқалары туралы толығырақ [NVIDIA ресми құжаттамасында](https://docs.nvidia.com/datacenter/tesla/mig-user-guide/index.html#partitioning).
...
## {heading(Қолданылмайтын ресурстарды жойыңыз)[id=mk8s-gpu-operator-delete]}
