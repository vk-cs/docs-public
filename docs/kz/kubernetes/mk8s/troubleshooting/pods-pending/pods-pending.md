# {heading(Кластер подтары Pending күйінде тым ұзақ болады)[id=mk8s-pods-pending]}
...
1. Мәселені келесі тәсілдердің бірімен шешіңіз:
   ...
   - {linkto(../../instructions/scale#mk8s-instructions-scale-vertical-worker-nodes)[text=Іске қосыңыз]} торап топтарын автоматты масштабтауды немесе қолмен {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=көбейтіңіз]} топтағы тораптар санын, егер бар тораптар подтарды орналастыру үшін жеткіліксіз болса.
...
1. Мәселені келесі тәсілдердің бірімен шешіңіз:
   ...
   - Под манифесіндегі `resources.requests` және `resources.limits` блоктарындағы мәндерді орнатылған квоталар мен шектеулерге сәйкес түзетіңіз. 
      ...
      Ресурстар мен шектеулер туралы толығырақ {linkto(../../reference/resource-limiting#mk8s-resource-limiting)[text=Ресурстарды пайдалануды шектеу]} бөлімінде және [Kubernetes ресми құжаттамасында](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/). 
...
1. Мәселені келесі тәсілдердің бірімен шешіңіз: 
   ...
   - {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-labels-taints)[text=Келістіріңіз]} под ережелерін көрсетілген ережелер мен талаптармен.
   - {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=Қосыңыз]} көрсетілген талаптарға сәйкес келетін жаңа тораптар тобын.
...
{include(/kz/_includes/_pod_events_diagnostics.md)}
   ...
   Егер солай болса, подта {linkto(../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=PV немесе PVC-пен]} мәселелер бар. Осындай мәселелердің мысалдары:
   ...
   - {linkto(../../concepts/storage#mk8s-storage-storage-classes)[text=сақтау класы]} (StorageClass) бапталмаған немесе ол PVC талаптарына сәйкес келмейді. 

1. Мәселені келесі тәсілдердің бірімен шешіңіз:

   - PV-ді қолмен жасаңыз немесе оны {linkto(../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs-prepare)[text=динамикалық дайындау]} үшін параметрлерді түзетіңіз. Тұрақты томдармен жұмыс істеу туралы толығырақ {linkto(../../how-to-guides/storage#mk8s-storage)[text=практикалық нұсқаулықта]} және [Kubernetes ресми құжаттамасында](https://kubernetes.io/docs/concepts/storage/persistent-volumes/). 
...
1. Мәселені келесі тәсілдердің бірімен шешіңіз:
   ...
   - {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=Көбейтіңіз]} тораптар санын (оларды бар топқа қосыңыз немесе жаңасын жасаңыз).
...
1. Мәселені келесі тәсілдердің бірімен шешіңіз:
   ...
   {linkto(../../connect/docker-registry#mk8s-docker-registry-using)[text=Түзетіңіз немесе көрсетіңіз]} манифесте реестрге қол жеткізуге арналған құпиясөзді. 
...
1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=Кластерге қосылыңыз]} `kubectl` көмегімен.
...
1. Мына пәрмен арқылы `kube-system` атау кеңістігіндегі кластердің жүйелік подтары туралы ақпаратты шығарыңыз:
   ...
   Егер қандай да бір жүйелік подтар `Pending` күйінде тым ұзақ болса, бұл {linkto(../../concepts/architecture#mk8s-architecture-platform-integration)[text=желілік сервистермен (CNI) және деректерді сақтау сервистерімен (CSI) интеграция мәселелеріне]} байланысты болуы мүмкін.
...
1. `Conditions` блогындағы қателерді қарап, оларды түзетіңіз:
   ...
   | `NetworkUnavailable`
   | Торап желісі қолжетімсіз, себебі {linkto(../../concepts/network#mk8s-network-cni)[text=CNI]} жұмысында ақау болған
   | 
