# {heading(Автоматты масштабтау кезінде жаңа тораптар қосылмайды)[id=mk8s-cluster-does-not-scale-up]}

{include(/kz/_includes/_translated_by_ai.md)}

{linkto(../../concepts/cluster-autoscaler#mk8s-cluster-autoscaler)[text=Автоматты масштабтау агенті (Cluster Autoscaler)]} жұмыс істеп тұрғанына қарамастан, жаңа тораптар қосылмайды, ал подтар `Pending` күйінде болады.
...
1. `kubectl` көмегімен {linkto(../../connect/kubectl#mk8s-kubectl)[text=кластерге қосылыңыз]}.
...
Ресурстар туралы толығырақ {linkto(../../reference/resource-limiting#mk8s-resource-limiting)[text=Ресурстарды пайдалануды шектеу]} бөлімінде және [Kubernetes ресми құжаттамасында](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/).
...
1. Тораптар тобындағы тораптар санын қараңыз.
   ...
   {tab(kubectl)}
   1. `kubectl` көмегімен {linkto(../../connect/kubectl#mk8s-kubectl)[text=кластерге қосылыңыз]}.
   1. {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=автоматты масштабтау агентінің логтарында]} `max size reached` бар ескертулердің бар-жоғын тексеріңіз. Егер бар болса, топта тораптардың ең көп санына жеткен.
   {/tab}
   {/tabs}

1. {linkto(../../instructions/scale#mk8s-instructions-scale-horizontal-autoscaling-worker-nodes)[text=Тораптардың]} ең көп санын қажетті мәнге дейін ұлғайтыңыз.

Толығырақ {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=Worker-тораптар тобын басқару]} бөлімінде.
...
1. vCPU, RAM және виртуалды машиналарға арналған квоталарды қараңыз.
   ...
   1. `kubectl` көмегімен {linkto(../../connect/kubectl#mk8s-kubectl)[text=кластерге қосылыңыз]}.
   1. {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=автоматты масштабтау агентінің логтарында]} `quota exceeded` бар ескертулердің бар-жоғын тексеріңіз. Егер бар болса, жобада тиісті ресурстарға арналған квоталар таусылған.
...
1. `kubectl` көмегімен {linkto(../../connect/kubectl#mk8s-kubectl)[text=кластерге қосылыңыз]}.
1. {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=автоматты масштабтау агентінің логтарында]} `predicate failed` бар ескертулердің бар-жоғын тексеріңіз. Егер бар болса, мәселе жоспарлау ережелерінде.
1. Мәселе байқалатын подтың манифестін келесі пәрменмен шығарыңыз:
...
1. Манифест шығаруында жоспарлау ережелерін қараңыз. Олар тораптар селекторы (`nodeSelector`), тораптарды орналастыру ережелері (`affinity`) немесе ерекшеліктер (`tolerations`) арқылы берілуі мүмкін. Егер бар немесе ықтимал жаңа тораптар қолданыстағы жоспарлау ережелерінің ешқайсысына сәйкес келмесе, нұсқалардың бірін таңдаңыз:
   ...
   - {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-labels-taints)[text=Под ережелерін]} көрсетілген ережелермен және талаптармен сәйкестендіріңіз.
   - {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=Көрсетілген талаптарға]} сәйкес келетін жаңа тораптар тобын қосыңыз.

Белгілер, шектеулер және ерекшеліктер туралы толығырақ {linkto(../../reference/labels-and-taints#mk8s-labels-and-taints)[text=Белгілер мен шектеулер]} бөлімінде.
...
Томдар туралы толығырақ {linkto(../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=Тұрақты томдар және PVC]} бөлімінде.
...
1. `resources.requests` блогында көрсетілген ресурстарды тиісті {linkto(/kz/kubernetes/k8s/instructions/helpers/node-group-settings#mk8s-node-group-settings)[text=тораптар топтарындағы]} виртуалды машиналардың сипаттамаларымен салыстырыңыз.

1. Егер ресурстар жеткіліксіз болса, ресурстар саны бойынша қолайлы {linkto(../../concepts/flavors#mk8s-flavors-vm-flavor)[text=виртуалды машина үлгісі]} бар жаңа тораптар тобын {linkto(/kz/kubernetes/mk8s/instructions/manage-node-group#mk8s-manage-node-group-add-group)[text=жасаңыз]}.
