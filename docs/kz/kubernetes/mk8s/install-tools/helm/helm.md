# {heading(Helm орнату)[id=mk8s-helm]}

{include(/kz/_includes/_translated_by_ai.md)}

[Helm](https://helm.sh/docs/) — Kubernetes үшін танымал пакет менеджері, оны қолданбаларды жылдам орнату және жаңарту үшін `kubectl` үстіндегі қондырма ретінде пайдалануға болады.

## {heading(Орнату)[id=mk8s-helm-install]}

1. {linkto(../../connect/kubectl#mk8s-kubectl-check-connection)[text=Көз жеткізіңіз]}, Helm клиенті орнатылатын хосттан кластерге `kubectl` көмегімен қосыла алатыныңызға.
1. [Helm ресми құжаттамасында](https://helm.sh/docs/intro/install/) сипатталған тәсілдердің кез келгенімен хостқа Helm клиентін орнатыңыз.
...
## {heading(Жұмысқа қабілеттілігін тексеру)[id=mk8s-helm-check]}
...
1. Келесі команданы орындап, NGINX подының жұмысқа қабілеттілігін тексеріңіз:

   ```console
   kubectl get pods
   ```

   Команда шығысында `demo-helm-nginx-...` поды `Running` күйінде болуы керек.
...
## {heading(Жою)[id=mk8s-helm-delete]}
...
1. Қажет болса, Helm жасаған директориялар мен файлдарды жойыңыз:
   ...
   Әртүрлі операциялық жүйелер үшін осы айнымалыларға сәйкес келетін директориялар [Helm ресми құжаттамасында](https://helm.sh/docs/faq/uninstalling/) берілген.
