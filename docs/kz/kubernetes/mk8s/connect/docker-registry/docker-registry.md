# {heading(Docker реестріне қосылу)[id=mk8s-docker-registry]}

{include(/kz/_includes/_translated_by_ai.md)}

Кластерде сәйкес {linkto(../../concepts/addons-and-settings/addons#mk8s-addons)[text=аддон]} орнатылған болса, Docker реестріне қосыла аласыз.

Аддонды {linkto(../../instructions/addons/advanced-installation/install-advanced-registry#mk8s-install-advanced-registry)[text=орнату кезінде]} ол үшін Floating IP-мекенжайы бар стандартты жүктеме теңгергіші жасалады. Сондықтан Docker реестріне интернетке қолжетімділігі бар кез келген хосттан қосылуға болады.

## {heading(Дайындық қадамдары)[id=mk8s-docker-registry-before-work]}

1. Кластерде Docker реестрінің (`docker-registry`) аддоны орнатылғанына {linkto(../../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=көз жеткізіңіз]}.
1. `kubectl` көмегімен кластерге қосыла алатыныңызға {linkto(../kubectl#mk8s-kubectl-check-connection)[text=көз жеткізіңіз]}.
1. Docker реестріне қол жеткізу үшін {linkto(../../instructions/addons/advanced-installation/install-advanced-registry#mk8s-install-advanced-registry-connect)[text=деректерді алыңыз]}.

## {heading(Docker реестріне қосылу)[id=mk8s-docker-registry-connect]}
...
## {heading(Kubernetes кластерінде Docker реестрін пайдалану)[id=mk8s-docker-registry-using]}
