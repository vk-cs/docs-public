# {heading(External Secrets Operator)[id=mk8s-install-advanced-eso]}

{include(/kz/_includes/_translated_by_ai.md)}

## {heading(Дайындық қадамдары)[id=mk8s-install-advanced-eso-prepare]}

{include(/kz/_includes/_addon-prep.md)}

## {heading(Аддонды орнату)[id=mk8s-install-advanced-eso-install]}

{linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-eso)[text=External Secrets Operator]} қосымшасы үшін бірнеше орнату {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-install-features)[text=опциялары]} бар.
...
1. Аддонды орнатыңыз:
   ...
   {tab(Жеке кабинет)}
    ...
    1. `external-secrets-operator` аддонының карточкасындағы **Орнату** түймесін, содан кейін **Аддонды орнату** түймесін басыңыз.
    1. Қажет болса, келесілерді өңдеңіз:

        - таңдалған нұсқаны;
        - қолданба атауын;
        - аддон орнатылатын атаулар кеңістігінің атауын;
        - {linkto(#mk8s-install-advanced-eso-edit-code)[text=аддонды баптау кодын]}.
...
1. (Қосымша) {linkto(../../../../how-to-guides/external-secrets-operator#mk8s-external-secrets-operator)[text=External Secrets Operator жұмысының практикалық нұсқаулығымен танысыңыз]}.
...
1. Егер әлі жасалмаған болса, аддонды орнату үшін бөлінген worker-түйіндер тобын дайындаңыз:
   ...
   {tab(Жеке кабинет)}
    ...
    1. Кластерде аддондар орналастырылатын бөлінген worker-түйіндер тобы бар екеніне көз жеткізіңіз.
       ...
       Егер мұндай топ болмаса — оны {linkto(../../../manage-node-group#mk8s-manage-node-group-add-group)[text=қосыңыз]}.

    1. Егер әлі жасалмаған болса, осы түйіндер тобы үшін {linkto(../../../manage-node-group#mk8s-manage-node-group-labels-taints)[text=орнатыңыз]}:
...
## {heading(Орнату кезінде аддонды баптау кодын өңдеу)[id=mk8s-install-advanced-eso-edit-code]}
