...
## {heading(Дайындық қадамдары)[id=mk8s-install-advanced-kgateway-prepare]}

{include(/kz/_includes/_addon-prep.md)}

## {heading(Аддонды орнату)[id=mk8s-install-advanced-kgateway-install]}
...
1. Аддонды орнатыңыз:
   ...
   {tab(Жеке кабинет)}
    ...
    1. `kgateway` аддонының карточкасындағы **Орнату** түймесін, содан кейін **Аддонды орнату** түймесін басыңыз.
    1. Қажет болса, келесілерді өңдеңіз:

        - таңдалған нұсқаны;
        - қолданба атауын;
        - аддон орнатылатын атаулар кеңістігінің атауын;
        - {linkto(#mk8s-install-advanced-kgateway-edit-code)[text=аддонды баптау кодын]}.
...
1. (Қосымша) {linkto(/kz/kubernetes/mk8s/how-to-guides/kgateway#mk8s-kgateway)[text=Kgateway-пен жұмыс істеу бойынша практикалық нұсқаулықпен танысыңыз]}.
...
## {heading(Орнату кезінде аддонды баптау кодын өңдеу)[id=mk8s-install-advanced-kgateway-edit-code]}
