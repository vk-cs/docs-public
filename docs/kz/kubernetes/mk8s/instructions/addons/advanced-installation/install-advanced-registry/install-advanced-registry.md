...
## {heading(Дайындық қадамдары)[id=mk8s-install-advanced-registry-prepare]}
...
## {heading(Аддонды орнату)[id=mk8s-install-advanced-registry-install]}
...
1. Аддонды орнатыңыз:
   ...
   1. `docker-registry` аддонының карточкасындағы **Орнату** батырмасын, содан кейін **Аддонды орнату** батырмасын басыңыз.
   1. Қажет болса, мыналарды өңдеңіз:

      - қосымша атауын;
      - аддон орнатылатын аттар кеңістігінің атауын.

   1. {linkto(#mk8s-install-advanced-registry-edit-code)[text=аддонды баптау кодын]} өңдеңіз.
...
1. {linkto(#mk8s-install-advanced-registry-connect)[text=Тізілімге қол жеткізу деректерін алыңыз]}.
...
## {heading(Орнату кезінде аддонды баптау кодын өңдеу)[id=mk8s-install-advanced-registry-edit-code]}

{note:info}

- Аддонды баптау кодын өңдеу кезінде {linkto(#mk8s-install-advanced-registry-prepare)[text=бұрын алынған]} мәліметтерді пайдаланыңыз.
...
Кодты өңдегеннен кейін {linkto(#mk8s-install-advanced-registry-install)[text=аддонды орнатуды жалғастырыңыз]}.

## {heading(Тізілімге қосылу)[id=mk8s-install-advanced-registry-connect]}
...
1. {linkto(../../../../connect/docker-registry#mk8s-docker-registry)[text=Docker тізіліміне қосылыңыз]}.
