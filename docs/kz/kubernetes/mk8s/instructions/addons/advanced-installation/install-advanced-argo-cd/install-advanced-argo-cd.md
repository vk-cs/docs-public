# {heading(Argo CD)[id=mk8s-install-advanced-argo-cd]}

{include(/kz/_includes/_translated_by_ai.md)}

## {heading(Дайындық қадамдары)[id=mk8s-install-advanced-argo-cd-prep]}

{include(/kz/_includes/_addon-prep.md)}

## {heading(Аддонды орнату)[id=mk8s-install-advanced-argo-cd-install]}
...
1. Аддонды орнатыңыз:
   ...
   {tab(Жеке кабинет)}
    ...
    1. `argocd` аддонының карточкасындағы **Орнату** батырмасын, содан кейін **Аддонды орнату** батырмасын басыңыз.
    1. Қажет болса, келесілерді өңдеңіз:

        - таңдалған нұсқаны;
        - қолданба атауын;
        - аддон орнатылатын атаулар кеңістігінің атауын;
        - {linkto(#mk8s-install-advanced-argo-cd-edit-code)[text=аддонды баптау кодын]}.
...
## {heading(Орнату кезінде аддонды баптау кодын өңдеу)[id=mk8s-install-advanced-argo-cd-edit-code]}
