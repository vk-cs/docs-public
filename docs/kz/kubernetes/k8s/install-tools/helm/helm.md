# {heading(Helm орнату)[id=k8s-helm]}
...
## {heading(Орнату)[id=k8s-helm-install]}

1. {linkto(../../connect/kubectl#k8s-kubectl-check-connection)[text=Көз жеткізіңіз]}, Helm клиенті орнатылатын хосттан кластерге `kubectl` көмегімен қосыла алатыныңызға.
...
## {heading(Жұмысқа қабілеттілігін тексеру)[id=k8s-helm-check]}
...
## {heading(Жою)[id=k8s-helm-delete]}
