...
[Автоматты масштабтау агенті (Cluster Autoscaler)](/kz/kubernetes/k8s/concepts/cluster-autoscaler) жұмыс істеп тұрғанына қарамастан, жаңал тораптар қосылмайды, ал подтар `Pending` күйінде болады.
...
1. vCPU, RAM және виртуалды машиналарғал арналған квоталарды қараңыз.
   ...
   1. `kubectl` көмегімен [кластерге қосылыңыз](../../connect/kubectl).
   1. [автоматты масштабтау агентінің логтарында](/kz/kubernetes/k8s/how-to-guides/autoscaler-logs) `quota exceeded` бар ескертулердің бар-жоғын тексеріңіз. Егер бар болса, жобада тиісті ресурстарғал арналған квоталар таусылған.
   
   {/tab}
   {/tabs}

1. Егер квоталар таусылып, жобағал қосымша квоталар қажет болса, нұсқалардың бірін таңдаңыз:
...
1. Манифест шығаруында жоспарлау ережелерін қараңыз. Олар тораптар селекторы (`nodeSelector`), тораптарды орналастыру ережелері (`affinity`) немесе ерекшеліктер (`tolerations`) арқылы берілуі мүмкін. Егер бар немесе ықтимал жаңал тораптар қолданыстағы жоспарлау ережелерінің ешқайсысына сәйкес келмесе, нұсқалардың бірін таңдаңыз:
   ...
   - [Көрсетілген талаптарғал](/kz/kubernetes/k8s/instructions/manage-node-group) сәйкес келетін жаңал тораптар тобын қосыңыз.
...
1. Манифест шығаруында `volume node affinity conflict` деп басталатын ескертулерді іздеңіз. Мұндай ескертулер подтың басқал қолжетімділік аймағындағы томғал байланғанын және бұл том қолжетімді торапта жоспарлана алмайтынын көрсетеді.

   Егер мұндай ескертулер болса:
   1. Кластеріңіздің сақтау класы (StorageClass) манифестіндегі `volumeBindingMode` мәнін `WaitForFirstConsumer` етіп өзгертіңіз. Осы параметрмен Kubernetes алдымен бірінші под жоспарланғанша күтеді, содан кейін томды нақты торапқал байлайды.
...
1. Егер ресурстар жеткіліксіз болса, ресурстар саны бойынша қолайлы [виртуалды машина үлгісі](/kz/kubernetes/k8s/concepts/flavors#konfiguraciya_kalyptary) бар жаңал тораптар тобын [жасаңыз](/kz/kubernetes/k8s/instructions/manage-node-group#add_group).
