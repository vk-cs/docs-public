...
- Kubernetes Dashboard ішіне ендірілген құралдар.
  ...
  Олар Kubernetes ресурстары туралы алқпаратты көруге мүмкіндік беретін базалық мониторинг мүмкіндіктерін қамтамасыз етеді.

- [Мониторинг аддондар](../concepts/addons-and-settings/addons#kube_prometheus_stack_822533c9).

  Аддон кластер мониторингінің мүмкіндіктерін кеңейтеді:

  - Kubernetes-тің көптеген ресурстары, жеке подтарғал дейін, бойынша метрикаларды көруге мүмкіндік береді.
  - Метрикалар үшін берілген шектер асқан кезде хабарламалар жіберетін alert-терді қолдайды. Мысалы, кластер ресурстарының қолжетімсіздігін немесе worker-түйіндерде есептеу қуатының жетіспеуін бақылауғал болады.
  - Кластер ресурстарының тұтынылуын [болжауғал](#forecast_consumption), болжау графиктерін қарауғал және ресурстардың жақын арада таусылуы туралы хабарламаларды алдын ала алуғал мүмкіндік береді. Болжау тек VK Tech тарапынан доработкалары бар [аддон нұсқаларында](../concepts/versions/components#addondar) қолжетімді (нұсқал нөмірінде `vk` бар).

  Мониторинг аддондар кластерде орнатылған-орнатылмағанын білу үшін, [орнатылған аддондар тізімін қараңыз](../instructions/addons/manage-addons#addondardy_karau).
...
1. Кластерде мониторинг аддондар (`kube-prometheus-stack`) [орнатылғанына](../instructions/addons/advanced-installation/install-advanced-monitoring) [көз жеткізіңіз](../instructions/addons/manage-addons#addondardy_karau).
...
1. Grafana веб-интерфейсіне қол жеткізіңіз:
   ...
   1. Команда шығысы бойынша Grafana-ғал қол жеткізу үшін `kubectl` ашқан портты анықтаңыз.
...
1. Grafana-ғал кіріңіз. Аддон қандай параметрлермен [орнатылғанына](../instructions/addons/advanced-installation/install-advanced-monitoring) байланысты мыналардың бірін пайдаланыңыз:
...
Енді сіз Grafana-мен жұмыс істей аласыз. Мысалы, дашбордтар жасап, оларды қарай аласыз. Kubernetes-тің әртүрлі ресурстары туралы алқпараты бар алдын ала бапталған дашбордтар бүйірлік мәзірдегі **Dashboards → Browse** бөлімінде қолжетімді.
...
1. Кластерде VK Tech доработкалары бар (нұсқал нөмірінде `vk` жұрнағы бар) `kube-prometheus-stack` мониторинг аддонының нұсқасы [орнатылғанына](../instructions/addons/advanced-installation/install-advanced-monitoring) [көз жеткізіңіз](../instructions/addons/manage-addons#addondardy_karau).
1. `kube-prometheus-stack` аддондар кодының өңдеу бетіне [өтіңіз](../instructions/addons/manage-addons#edit) және кодтағы `grafana.alerts.enabled` параметрі `true` мәніне тең екеніне көз жеткізіңіз. Бұл мән параметрге аддонды орнату кезінде беріледі.
1. Егер бұл параметр `false` болып өзгертілсе, оны `true` етіп түзетіп, жаңартылған баптауларды сақтаңыз.

   {note:warn}
   `grafana.alerts.enabled` параметрі өшірілген болса немесе әдепкі баптаулармен үйлеспейтін басқал өзгерістер енгізілсе, ресурстардың таусылуы туралы хабарламалар жіберілуіне кепілдік берілмейді.
   {/note}

1. [Grafana-ғал кіріңіз](#connect_grafana).
...
1. (Опционалды) Болжамдар есептелетін параметрлерді өзгертіңіз:
   ...
   - **Analysis period**: болжам функциясын құру үшін пайдаланылатын, алғымдағы сәтке дейінгі уақыт аралығы.
   - **Forecast horizon**: болжам есептелетін болашақ сәт пен алғымдағы сәт арасындағы аралық.
...
1. **Grafana** блогында **VK Cloud > K8s Resources Prediction** бумасын ашып, ресурстардың таусылуына арналған ендірілген alert-терді қараңыз:
   ...
   Ендірілген alert-тердің параметрлерін өзгертуге болмайды, бірақ ендірілген alert негізінде өз alert-іңізді жасауғал болады.
...
1. (Опционалды) Ендірілген alert негізінде өз alert-іңізді жасаңыз:
   ...
   1. Мәндерді қажеттілеріне өзгертіңіз. Болжам тереңдігі тек секундпен беріледі, ал талдау тереңдігі мен таңдау аралығы үшін [басқал өлшем бірліктері](https://prometheus.io/docs/prometheus/latest/querying/basics/#float-literals-and-time-durations) де қолжетімді.
   1. Alert-тің басқал параметрлерін [Grafana ресми құжаттамасында](https://grafana.com/docs/grafana/latest/alerting/alerting-rules/) сипатталғандай баптаңыз.
...
- хабарлама алған кезде ықтимал мәселелерді алдын ала шешу үшін басқал объектілер бойынша да болжамдарды қарай аласыз;
- алғымдағы болжамдарды қарап, оларды Grafana-ның стандартты дашбордтарында көрсетілген тұтыну динамикасымен салыстыра аласыз;
