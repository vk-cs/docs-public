# {heading(Қолжетімді есептеу ресурстары)[id=k8s-flavors]}

{include(/kz/_includes/_translated_by_ai.md)}

## {heading(Конфигурация қалыптары)[id=k8s-flavors-vm-flavor]}
...
## {heading(Кластердің қолайлы конфигурациясын есептеу)[id=k8s-flavors-configuration-calculation]}
...
## {heading(CPU және RAM қолжетімді есептеу ресурстарын есептеу)[id=k8s-flavors-cpu-ram-calculation]}
...
## {heading(Түйіндегі қолжетімді есептеу ресурстары)[id=k8s-flavors-available-node-resources]}
...
## {heading(GPU бар конфигурация қалыптары)[id=k8s-flavors-gpu]}
...
### {heading(GPU бар кластерді жасаудың ерекшеліктері)[id=k8s-flavors-gpu-features]}
...
### {heading(GPU-ді ортақ пайдалану технологиялары)[id=k8s-flavors-gpu-sharing]}
