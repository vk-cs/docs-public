...
- Аддондар кластердің worker-тораптарына орнатылады және олардың есептеу ресурстарын тұтынады.
  ...
  Кейбір аддондар кластердің барлық тораптарына (соның ішінде master-тораптарға) орнатылуы мүмкін. Толығырақ [Аддондарды орнату және баптау](../../../instructions/addons/advanced-installation) бөлімінен оқыңыз.
...
Lua тілінде жазылған [арнайы сүзгілермен](https://docs.fluentbit.io/manual/pipeline/filters/lua) бірге Fluent Bit Cloud Containers кластерінен логтарды [Cloud Logging](/kz/monitoring-services/logging) сервисіне жеткізуді ұйымдастыруға мүмкіндік береді, бұл осы логтарды әрі қарай талдау үшін қажет.
...
GPU Operator [кластер тораптарындағы GPU-ді](../../flavors#gpu) машиналық оқыту немесе үлкен деректерді өңдеу міндеттерін орындау үшін басқаруға мүмкіндік береді.
...
[NGINX](https://kubernetes.io/docs/concepts/services-networking/ingress/) негізіндегі [Ingress-контроллер](https://docs.nginx.com/nginx-ingress-controller/intro/overview/) кері прокси (reverse proxy) ретінде жұмыс істейді және HTTP немесе HTTPS бойынша жұмыс істейтін кластер сервистері үшін бірыңғай кіру нүктесін ұйымдастыруға мүмкіндік береді.
...
Jaeger микросервистерден алатын деректер негізінде сұрауларды трассировкалайды. Сондықтан сұраулар туралы деректерді жіберу үшін микросервистерге [OpenTelemetry](https://www.jaegertracing.io/docs/latest/architecture/#tracing-sdks) құралдар стегін [интеграциялау қажет](https://opentelemetry.io). OpenTelemetry-ді микросервистік қолданбаға интеграциялау мысалымен [Hot R.O.D](https://github.com/jaegertracing/jaeger/tree/main/examples/hotrod) арқылы танысуға болады.
...
[Kiali](https://kiali.io/) — [Istio](#istio)-мен жұмыс істеуге арналған веб-интерфейс. Ол service mesh-ті басқаруға, оның күйін бақылауға және оны визуализациялауға мүмкіндік береді.
...
Kubernetes-тің 1.27.x–1.32.x нұсқаларындағы кластерлер үшін VK Cloud платформасы [Kube Prometheus Stack аддонының](../../versions/components#addondar) екі нұсқасын ұсынады: базалық және VK Tech компаниясы жетілдірген нұсқа (`vk` жұрнағы нұсқа нөмірінде көрсетіледі). Жетілдірілген аддонда қосымша кіріктірілген функциялар, атап айтқанда [кластер ресурстарының тұтынылуын болжау](../../../monitoring#forecast_consumption) бар.
...
1. Жаңа және бұрыннан іске қосылған подтардың [ресурс сұрауларына](/kz/kubernetes/k8s/reference/resource-limiting) арналған ұсынымдарды ұсыну және, егер бапталған болса, оларды автоматты түрде қолдану.
...
- Updater: ұсынымдарды алады, VPA объектілерін жаңартады және ұсынылған ресурстардың жаңа және бұрыннан бар подтар үшін қолжетімді болуын қадағалайды. Жұмыс режиміне байланысты Updater компоненті Admission Controller компонентіне бар подты жасауға немесе жаңартуға сұрау жібереді. Updater компонентінің жұмыс режимдері:
  ...
  - `Recreate` — толық көлемде жұмыс істейді. `Recreate` режимінде Updater компоненті, егер осы под сұрайтын ресурстар Recommender есептеген мәндерге сәйкес келмесе, подты жояды. Содан кейін ол Admission Controller компонентіне ұсынылған мәндермен осы подты қайта жасауға сұрау жібереді. Жаңа подтар бірден ұсынылған мәндермен жасалады. `Recreate` режиміндегі жұмыс туралы толығырақ [Vertical Pod Autoscaler аддонын пайдалану](/kz/kubernetes/k8s/how-to-guides/vertical-pod-autoscaler) бөлімінен қараңыз.
