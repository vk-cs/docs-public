...
Логтардың көздері ретінде кластер тораптарында орналасқан [kubelet қызметтері](https://kubernetes.io/docs/concepts/overview/components/#kubelet) мен {linkto(../../../reference/pods#k8s-pods)[text=подтар]} (pods) қолданылады. Аддонның қалай жұмыс істейтіні туралы толығырақ оның {linkto(../../../instructions/addons/advanced-installation/install-advanced-logaas-integration#k8s-install-advanced-logaas-integration)[text=бірінші буын]} және {linkto(/kz/kubernetes/k8s/instructions/addons/advanced-installation/install-advanced-logaas-integration-gen-2#k8s-install-advanced-logaas-integration-gen-2)[text=екінші буын]} кластерлеріне арналған орнату бөлімдерінен оқыңыз.
...
[Istio](https://istio.io/latest/) — сервисаралық тор (service mesh) тұжырымдамасын іске асыратын фреймворк, мұнда қолданба сервистерінің өзара әрекеттесуі үшін бөлек қабат бөлінеді. Istio пайдалану сервистердің кодын өзгертпей-ақ трафикті басқаруды қамтамасыз етеді (sidecar-контейнерлер пайдаланылады). Istio артықшылықтары:

- Трафикті қауіпсіз жеткізу мүмкіндіктері кеңейеді:

  - трафик үшін саясаттарды баптауға болады;
  - сервистер арасындағы коммуникация үшін TLS пайдалануға болады;
