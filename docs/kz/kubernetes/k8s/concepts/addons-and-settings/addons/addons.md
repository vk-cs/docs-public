...
### {heading(Container Storage Interface (CSI) S3 үшін)[id=k8s-addons-s3]}
...
[Container Storage Interface (CSI) S3 үшін](https://github.com/yandex-cloud/k8s-csi-s3) — S3 үйлесімді объектілік сақтау жүйелерін ({linkto(/kz/storage/s3/concepts/about#s3-concepts-about)[text=VK Object Storage]}, AWS S3, MinIO және басқаларын) {linkto(/kz/kubernetes/k8s/reference/pvs-and-pvcs#k8s-pvs-and-pvcs)[text=тұрақты томдар]} (Persistent Volumes, PV) ретінде ілуге мүмкіндік беретін Kubernetes-ке арналған Container Storage Interface драйвері.
...
- Бакеттерді динамикалық жасау (dynamic provisioning). Көрсетілген {linkto(/kz/kubernetes/k8s/concepts/storage#k8s-storage-storage-classes)[text=сақтау класы]} (Storage Class) бар PVC (Persistent Volume Claim) жасалғанда, аддон мақсатты S3 үйлесімді сақтау жүйесінде жаңа бакетті автоматты түрде жасайды. PVC жасалғаннан кейін бұл бакет PV ретінде пайдалануға дайын болады.
...
S3-CSI сіз {linkto(/kz/kubernetes/k8s/instructions/addons/advanced-installation/install-advanced-s3-csi#k8s-install-advanced-s3-csi-install)[text=орнатқанда]} көрсеткен кез келген S3 үйлесімді сақтау жүйесінде бакеттер жасай алады.
