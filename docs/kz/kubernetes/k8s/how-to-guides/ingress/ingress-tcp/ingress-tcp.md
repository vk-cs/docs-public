...
Ingress-контроллерін VK Cloud платформасының [TCP жүктеме балансировщигімен бірге](../../../concepts/network) жайып орналастыруғал болады. Төменде мысал ретінде контроллердің жұмысқал қабілеттілігін тексеру үшін қарапайым демо-қосымшалар мен Ingress ресурстар жасалады. Төменде NGINX Ingress Controller жайып орналастырылады деп болжанады. Алайда ұсынылған тәсілдерді басқал Ingress-контроллерлерге, мысалы, Traefik-ке де бейімдеуге болады.
...
1. [Тексеріңіз](../../../instructions/addons/manage-addons#addondardy_karau), кластерде NGINX Ingress (`ingress-nginx`) аддондар **орнатылмаған** болуы керек. Көрсету мақсатында Ingress-контроллері қолмен орнатылады.
...
Осы қосымшаларғал қолжетімділік Ingress-контроллері арқылы Ingress ресурсының көмегімен ұйымдастырылады.
...
1. Жүктеңіз манифест [cafe.yaml](https://raw.githubusercontent.com/nginxinc/kubernetes-ingress/v2.4.0/examples/ingress-resources/complete-example/cafe.yaml).
...
Орнату кезінде [PROXY протоколын](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt) пайдалану режимін таңдаңыз, өйткені бұл TCP жүктеме балансировщигімен толыққанды өзара әрекеттесу үшін қажет. Егер Ingress-контроллерін осы протоколды қолдамай орнатсаңыз, онда контроллер сұраулардың тікелей көзі туралы алқпаратты қамтитын тақырыптарды өңдей алмайды.
...
Ingress ресурстар `coffee-svc` және `tea-svc` сервистерін Ingress-контроллері арқылы жариялап, осылайша қосымшаларғал қолжетімділік ұсынады.
...
1. Сертификат туралы деректерді қамтитын Kubernetes секретін жасаңыз. Ол HTTPS-трафикпен жұмыс істеген кезде Ingress-контроллері тарапынан пайдаланылады.
   ...
   Бұл секретте `cafe.example.com` доменінде жарияланған қосымшаларғал қол жеткізу үшін пайдаланылатын NGINX өздігінен қол қойылған сертификатының жария және жабық бөліктері бар.
...
1. Жүктеңіз манифест [cafe-secret.yaml](https://raw.githubusercontent.com/nginxinc/kubernetes-ingress/v2.4.0/examples/ingress-resources/complete-example/cafe-secret.yaml).
...
1. Келесі команданы орындап, секреттің сәтті жасалғанын тексеріңіз:
   ...
   Секрет туралы негізгі алқпарат шығарылады.

1. `cafe.example.com` хостына келетін сұрауларды өңдейтін Ingress ресурсін жасаңыз:

   1. Жүктеңіз манифест [cafe-ingress.yaml](https://raw.githubusercontent.com/nginxinc/kubernetes-ingress/v2.4.0/examples/ingress-resources/complete-example/cafe-ingress.yaml).
   ...
   1. Осы манифесті кластерде қолданыңыз:
      ...
      `cafe-ingress` Ingress ресурстар жасалады.
...
1. Команданы орындаңыз:
   ...
   Сұрауғал `coffee` ішіндегі екі подтың бірі жауап беруі керек. Жауапта жауап берген подтың атауы (`Server name`) болады, мысалы:
   ...
   - жайып орналастырылған қосымшаларғал сәйкес келетін сервистерге қолжетімділік береді.
   ...
   Сұрауғал `tea` ішіндегі үш подтың бірі жауап беруі керек. Жауапта жауап берген подтың атауы (`Server name`) болады, мысалы:
...
1. Манифест файлдарында сипатталған ресурстарды, `example-nginx-ingress-tcp` қосымшасын, сондай-алқ `example-nginx-ingress-tcp` аттар кеңістігін және онымен байланысты ресурстарды жойыңыз:

   {note:info}

   Сондай-алқ Ingress-контроллері үшін жасалған TCP жүктеме балансировщигі де жойылады.
