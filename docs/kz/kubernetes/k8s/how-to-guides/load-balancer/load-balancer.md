...
`spec.type: LoadBalancer` түріндегі барлық Kubernetes сервистері үшін VK Cloud платформасы берілген мінез-құлыққал сәйкес келетін TCP жүктеме теңгергішін автоматты түрде жасай алады.
...
- Интернеттен қолжетімді жария IP мекенжайына немесе интернеттен қолжетімсіз жеке IP мекенжайына ие бола алады. IP мекенжайын теңгергішке қолмен немесе автоматты түрде тағайындауғал болады.
...
1. `kubectl` көмегімен кластерге қосыла алатыныңызғал көз жеткізіңіз.

## 1. Қолданба жасаңыз

Бұл қолданбағал сұрауларды жүктеме теңгергіші өңдейді. Теңгергіштің жұмысын көрсету үшін қолданба екі репликадан тұратын StatefulSet түрінде жайылады. Бұл жағдайда қолданбаның барлық подтары нөмірленеді, сондықтан теңгергіш сұрауды қай репликағал бағыттайтынын оңай анықтауғал болады.
...
1. Теңгергішке тағайындау керек жария IP мекенжайын таңдаңыз немесе жаңасын жасаңыз. Мұны [VK Cloud жеке кабинетінде](../../../../networks/vnet/instructions/ip/floating-ip#add) жасауғал болады.
...
1. `lb-session-affinity.yaml` манифест файлын жасаңыз.
   ...
   `spec.sessionAffinity` параметрінде `ClientIP` мәнін көрсетіңіз. Ол пайдаланушы сессиясын нақты бір репликағал бекітуге жауап береді.
...
1. Қолданбағал қолжеткізуге рұқсат етілуі тиіс хост үшін жария IP мекенжайын анықтаңыз.
...
1. `lb-restrict-access-by-ip.yaml` манифест файлын жасаңыз.
   ...
   `spec.loadBalancerSourceRanges` параметрінде рұқсат етілген қолжеткізу үшін `IP-мекенжай/префикс` форматындағы IP мекенжайларының тізімін көрсетіңіз. Басқал IP мекенжайларынан қолжеткізуге тыйым салынады.
...
1. `lb-private-ip.yaml` манифест файлын жасаңыз.
   ...
   Сервис метадеректерінде `service.beta.kubernetes.io/openstack-internal-load-balancer: "true"` аннотациясын көрсетіңіз. Бұл аннотация жеке IP мекенжайы бар жүктеме теңгергішін жасауғал жауап береді.
...
1. Қолданбағал жүктеме теңгергіші арқылы бірнеше рет сұрау орындаңыз:
   ```console
...
1. Қолданбағал жүктеме теңгергіші арқылы бірнеше рет сұрау орындаңыз:
   ...
   Барлық сұрауларғал таңдалған бір под жауап береді: `coffee-0` немесе `coffee-1`. Мұндай мінез-құлық теңгергіштің нақты IP мекенжайынан келетін сұрауларды қолданбаның бір ғана репликасына жіберетінін білдіреді.
...
1. Қолданбағал жүктеме теңгергіші арқылы бірнеше рет сұрау орындаңыз (қолжеткізуге рұқсат етілген IP мекенжайынан):
...
1. Басқал IP мекенжайы бар хосттан осындай сұрау орындап көріңіз. Сұрау сәтсіз аяқталады.
...
1. Жасалған сервистер мен StatefulSet-ті жойыңыз:
   ...
   Сервистермен бірге оларғал сәйкес келетін теңгергіштер де жойылады. Бұл процесс ұзақ уақыт алуы мүмкін.
