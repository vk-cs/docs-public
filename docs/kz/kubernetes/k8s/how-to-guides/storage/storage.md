# {heading(Тұрақты томдармен (PV) жұмыс істеу)[id=k8s-storage]}

{include(/kz/_includes/_translated_by_ai.md)}

{linkto(../../reference/pvs-and-pvcs#k8s-pvs-and-pvcs)[text=Тұрақты томдарды]} (Persistent Volumes, PV) қарапайым демо-қосымшаларға әртүрлі тәсілдермен қосуға болады. Әрі қарай қосу үшін Persistent Volume Claims (PVC) пайдаланылады. Қосымшалардың және оларға қосылған PV-лардың жұмысқа қабілеттілігін тексеру үшін Ingress ресурсы жасалады.

## {heading(Дайындық қадамдары)[id=k8s-storage-prepare]}
...
1. Кластерде әдепкі параметрлермен NGINX Ingress (`ingress-nginx`) аддоны {linkto(../../instructions/addons/advanced-installation/install-advanced-ingress#k8s-install-advanced-ingress)[text=орнатылғанына]} {linkto(../../instructions/addons/manage-addons#k8s-manage-addons-view)[text=көз жеткізіңіз]}. Ол демо-қосымшаларға қолжетімділікті қамтамасыз ету үшін қажет болады.

    {note:warn}

    Аддонды орнату кезінде ол үшін {linkto(../../../../networks/balancing/concepts/load-balancer#balancing-load-balancer-types)[text=стандартты жүктеме теңгергіші]} жасалады.

    Теңгергішті пайдалану {linkto(../../../../networks/vnet/tariffication#vnet-tariffication)[text=тарифтеледі]}.
...
## {heading(1. Демо-қосымшаларды жасаңыз және оларға PV қосыңыз)[id=k8s-storage-create-demo]}
...
### {heading(Блоктық сақтау қоймаларын қосу)[id=k8s-storage-connect-block-storage]}

Блоктық сақтау қоймалары кластерге {linkto(../../concepts/storage#k8s-storage)[text=Cinder CSI көмегімен]} қосылады.
...
{note:info}
Бұл сценарий тек {linkto(../../concepts/cluster-generations#k8s-cluster-generations)[text=бірінші буын]} кластерлері үшін қолжетімді.
{/note}
...
1. {linkto(../../../../computing/iaas/instructions/volumes#iaas-volumes)[text=Желілік HDD дискін жасаңыз]}.
...
### {heading(Файлдық сақтау қоймаларын қосу)[id=k8s-storage-connect-file-storage]}
...
1. {linkto(../../../../computing/iaas/instructions/fs-manage/fs-create#iaas-fs-create)[text=Файлдық сақтау қоймасын жасаңыз]}.
...
1. Жасалған файлдық сақтау қоймасы туралы {linkto(../../../../computing/iaas/instructions/fs-manage/fs-operations#iaas-fs-operations)[text=ақпаратты қараңыз]}.
...
## {heading(2. Демо-қосымшалардың және PV-лардың жұмысқа қабілеттілігін тексеріңіз)[id=k8s-storage-check]}
...
1. Ingress контроллерінің жария IP-мекенжайын {linkto(../../instructions/addons/advanced-installation/install-advanced-ingress#k8s-install-advanced-ingress-get-ip)[text=анықтаңыз]}.
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=k8s-storage-delete]}

Жұмыс істеп тұрған кластер тарифтеледі және есептеу ресурстарын тұтынады. Егер тұрақты томдардың жұмысын тексеру үшін жасалған Kubernetes ресурстары сізге енді қажет болмаса, оларды жойыңыз:

1. Манифест файлдарында сипатталған ресурстарды жойыңыз: 
...
1. Пайдаланылмайтын сақтау қоймаларын жойыңыз:
   ...
   - `tea` қосымшасы пайдаланған диск; 
   - `milkshake` қосымшасы пайдаланған NFS сақтау қоймасы.
