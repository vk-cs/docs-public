# {heading(GPU Operator пайдалану)[id=k8s-gpu-operator]}

{include(/kz/_includes/_translated_by_ai.md)}

Cloud Containers машиналық оқытуды орындау немесе үлкен деректерді өңдеу үшін {linkto(../../concepts/flavors#k8s-flavors-gpu)[text=GPU (графикалық процессорлары) бар кластерлерді]} жасауға мүмкіндік береді.
...
- GPU ресурстары бірнеше pod арасында {linkto(../../concepts/flavors#k8s-flavors-gpu-sharing)[text=MPS]} стратегиясы бойынша бөлінеді. Әрбір процесс GPU ресурстарының бөлінген жиынтығын алады және олар бір-бірімен процестер арасындағы өзара әрекеттесу (IPC) арқылы әрекеттеседі.
- GPU ресурстары бірнеше pod арасында {linkto(../../concepts/flavors#k8s-flavors-gpu-sharing)[text=MIG]} стратегиясы бойынша бөлінеді. Бұл ретте әрбір бөлік өз ресурстарына (жадына, есептеу блоктарына) ие болады және басқаларынан оқшау жұмыс істей алады.
...
## {heading(Дайындық қадамдары)[id=k8s-gpu-operator-prepare]}

1. {linkto(../../../../computing/gpu/connect#gpu-connect)[text=Cloud GPU сервисін қосыңыз]}, егер ол әлі қосылмаған болса.
1. {linkto(../../instructions/create-cluster/create-webui#k8s-create-webui)[text=GPU бар кластер жасаңыз]}, егер ол әлі жасалмаған болса. Келесі параметрлерді көрсетіңіз:
...
1. {linkto(../../connect/kubectl#k8s-kubectl)[text=Кластерге kubectl арқылы қосылыңыз]}.

## {heading(1. GPU-operator аддонын орнатыңыз)[id=k8s-gpu-operator-install]}

{linkto(../../instructions/addons/advanced-installation/install-advanced-gpu-operator#k8s-install-advanced-gpu-operator)[text=Орнату жөніндегі нұсқаулықты]} пайдаланыңыз.

## {heading(2. Кластердің жұмысын тексеріңіз)[id=k8s-gpu-operator-check]}
...
## {heading(3. GPU-ді MPS технологиясы бойынша бөліңіз)[id=k8s-gpu-operator-mps]}

1. [NVIDIA device plugin](https://github.com/NVIDIA/k8s-device-plugin/tree/v0.17.0?tab=readme-ov-file#with-cuda-mps) конфигурациясын GPU Operator аддонының кодының ішіне {linkto(../../instructions/addons/manage-addons#k8s-manage-addons-edit-code)[text=қосыңыз]}:
...
1. Аддон конфигурациясына қосылған `mps-on` режимін қосатын GPU бар түйін үшін метканы (label) қосыңыз:
   ...
   Топтағы барлық түйіндер үшін метканы жеке кабинет арқылы {linkto(../../instructions/manage-node-group#k8s-manage-node-group-labels-taints)[text=қоса аласыз]}.
...
## {heading(4. GPU-ді MIG технологиясы бойынша бөліңіз)[id=k8s-gpu-operator-mig]}
