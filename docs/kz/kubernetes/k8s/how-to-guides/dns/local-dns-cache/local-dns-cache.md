# {heading(Жергілікті DNS-кэшті баптау)[id=k8s-local-dns-cache]}
...
## {heading(Кластердегі DNS архитектурасына шолу)[id=k8s-local-dns-cache-architecture]}

Белгілі бір {linkto(../../../reference/pods#k8s-pods)[text=под]} DNS сұрауын бастайды делік.
...
## {heading(Дайындық қадамдары)[id=k8s-local-dns-cache-prepare]}
...
## {heading(1. Әрбір торапта кэштейтін DNS серверін жазыңыз)[id=k8s-local-dns-cache-deploy]}
...
- Сервер тыңдайтын IP мекенжайлар:
  ...
  Мұндай конфигурация Managed Containers кластерлерінде `kube-proxy` {linkto(../../../concepts/addons-and-settings/settings#k8s-settings-kube-proxy-mode)[text=жұмыс істейтін]} `iptables` режиміне байланысты пайдаланылады.
...
## {heading(2. Кэштейтін DNS серверінің жұмысын тексеріңіз)[id=k8s-local-dns-cache-check]}
...
## {heading(Пайдаланылмайтын ресурстарды жойыңыз)[id=k8s-local-dns-cache-delete]}
