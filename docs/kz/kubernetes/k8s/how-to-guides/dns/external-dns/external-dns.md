...
1. Сыртқы IP мекенжайы бар және интернеттен қолжетімді ең өзекті нұсқадағы Cloud Containers кластерін [жасаңыз](../../../instructions/create-cluster).
...
1. Кластерге арналған kubeconfig файлын көрсететін орта айнымалысын орнатыңыз. Бұл ExternalDNS орнату кезінде `kubectl` және `helm` утилиталарымен жұмысты жеңілдетеді.
   ...
   Сіздің kubeconfig файлдарңызғал жол төмендегі мысалдан өзгеше болуы мүмкін.
...
1. [таңдаңыз](/kz/tools-for-using-services/account/instructions/project-settings/access-manage#zhoba_katysushylaryn_karau) бұрыннан бар пайдаланушыны немесе [шақырыңыз](/kz/tools-for-using-services/account/instructions/project-settings/access-manage#zhobaga_zhana_katysushyny_shakyru) жаңа пайдаланушыны.

   Пайдаланушығал қойылатын талаптар:

   - [белсендірілген](/kz/tools-for-using-services/api/rest-api/enable-api) API бойынша қолжетімділік болуы тиіс.
   - ExternalDNS DNS-аймағы шеңберінде ресурстық жазбалармен жұмыс істей алуы үшін келесі рөлдердің бірі [тағайындалған](/kz/tools-for-using-services/account/instructions/project-settings/access-manage#katysushynyn_rolin_ozgertu) болуы тиіс:
...
1. API VK Cloud-қал қол жеткізу үшін қажетті деректемелерді алыңыз:

   1. [өтіңіз](https://kz.cloud.vk.com/app/) в жеке кабинет VK Cloud, используя реквизиты пользователя, выделенного үшін ExternalDNS.
   ...
   1. **API арқылы қолжеткізу** қойындысына өтіп, келесі параметрлердің мәндерін жазып алыңыз:
      ...
      - Auth URL (аутентификацияғал арналған URL).

1. Осы пайдаланушының құпиясөзін жазып алыңыз: ол да API-ғал қол жеткізу үшін қажет.
...
1. Helm көмегімен ExternalDNS орнатуғал қажет мәндері ([values](https://helm.sh/docs/chart_template_guide/values_files/)) бар файл жасаңыз:
   ...
   Helm-чарттың (chart) ExternalDNS жұмысына көптеген мәндер әсер етеді. Жасалған файлда ExternalDNS-пен жұмысты бастауғал жеткілікті ең аз мәндер жиыны берілген. Төменде ExternalDNS-тің DNS VK Cloud-пен жұмысына әсер ететін ең маңызды мәндер сипатталған. VK Cloud плагиніне қатысы жоқ мәндердің сипаттамалары (`sidecars[]` мәнінен басқасының бәрі) чартқал арналған [README.md](https://github.com/kubernetes-sigs/external-dns/tree/master/charts/external-dns#values) файлында келтірілген.
   ...
   - `txtPrefix`: ExternalDNS жасайтын TXT жазбаларына арналған префикс.
     ...
     ExternalDNS DNS-аймақтың қай ресурстық жазбаларын өзі басқаратынын қызметтік алқпаратты TXT жазбаларында орналастыру арқылы қадағалайды. Атап айтқанда, әдепкі баптаулармен ол қосылатын жазбамен бірдей атауы бар қызметтік TXT жазбасын жасайды: мысалы, `example.com` A-жазбасы үшін дәл сондай `example.com` атауы бар сәйкес TXT жазбасы жасалады.

     Алайда [RFC 1912](https://www.rfc-editor.org/rfc/rfc1912) бойынша CNAME-жазбалары дәл сондай атауы бар басқал жазбалармен қатар өмір сүре алмайды. Сондықтан ExternalDNS TXT-жазбаларының атына `txtPrefix` ішінде берілген префикс қосылатын етіп бапталады. Бұл CNAME-жазбаларымен жұмыс істеген кезде ықтимал коллизияларды болдырмауғал мүмкіндік береді: мысалы, `example.com` CNAME-жазбасы үшін `externaldns-example.com` атауымен сәйкес TXT-жазбасы жасалады.

     Қажет болса, `externaldns-` мәнінен өзгеше басқал префикс бере аласыз.
   ...
   - (**Міндетті баптаулар**) `OS_` префиксі бар айнымалыларғал сәйкес келетін баптаулар плагиннің API VK Cloud-пен өзара әрекеттесуі кезінде аутентификация үшін пайдаланылады.
   ...
   - DNS-аймақтарды сүзгілеу баптаулары:
     ...
     - Ресурстық жазбалар жасауғал рұқсат етілген DNS-аймақтарғал арналған сүзгілер:

       - `DOMAIN_FILTERS`: үтір арқылы бөлінген домендік атаулар тізімі бар жол. Мысалы, `example.com,contoso.com`.
       - `REGEX_DOMAIN_FILTER`: жол с регулярным выражением ([RE2 синтаксисі](https://github.com/google/re2/wiki/Syntax)). Мысалы, `.*.com$`.
     ...
     - Ресурстық жазбалар жасауғал тыйым салынған DNS-аймақтарғал арналған сүзгілер:

       - `EXCLUDE_DOMAINS`: үтір арқылы бөлінген домендік атаулар тізімі бар жол. Мысалы, `example.org,foo.bar.com`.
       - `REGEX_DOMAIN_FILTER_EXCLUSION`: жол с регулярным выражением ([RE2 синтаксисі](https://github.com/google/re2/wiki/Syntax)). Мысалы, `^stage-.*.com$`.
     ...
     - `LOG_LEVEL`: плагин жұмысы кезінде туындайтын оқжәнеғаларды логтау деңгейі.

       `error`, `warn`, `info`, `debug`, `trace` деңгейлері қолдау табады. Әдепкі мәні: `info`.

     - `DRY_RUN`: плагинді «dry run» («сынақтық іске қосу») режимінде іске қосуғал мүмкіндік беретін жалау.

       - `false` (әдепкі бойынша): «dry run» режимі **өшірілген**. Плагин жұмыс істейді және баптауларғал сәйкес DNS-аймақтағы ресурстық жазбаларды басқарады.
...
1. Kubernetes сервисіне (`Service`) арналған `tea-service.yaml` манифестін жасаңыз.
   ...
   - `external-dns.alpha.kubernetes.io/ttl` опционалды аннотациясы көрсетілген: ExternalDNS жасайтын ресурстық жазбағал арналған секундтардағы TTL.
   ...
   - `LoadBalancer` сервис түрі таңдалған. Мұндай сервис үшін [стандартты жүктеме теңгергіші](/kz/networks/balancing/concepts/load-balancer#zhukteme_tengergishterinin_turleri) жасалады. Әдепкі бойынша теңгергіш жария IP мекенжайымен жасалатындықтан, сервиспен байланысты қолданба интернеттен қолжетімді болады.
     ...
     Қолдану баланстаушының [тарифтелмейді](/kz/networks/vnet/tariffication).
...
1. ExternalDNS қажетті ресурстық жазбаларды жасағанына көз жеткізіңіз:
   ...
   1. [ресурстық жазбалар тізімін алыңыз](/kz/networks/dns/instructions/publicdns/records#resurstyk_zhazbalar_tizimin_karau) үшін зоны `example.com`.
   1. Тізімнен ExternalDNS жасаған жазбаларды табыңыз:
      ...
      - `externaldns-tea.example.com` және `externaldns-a-tea.example.com` екі TXT-жазбасын.
        ...
        Мұндай жазбаларды атауындағы `externaldns-` префиксі бойынша оңай ажыратуғал болады. Олардың мәндері `heritage=.../owner=.../resource=...` түріндегі стандартты құрылымғал ие.

        Егер [ExternalDNS орнату кезінде](#2_externaldns_ornatynyz) сіз префикстің басқал мәнін берген болсаңыз, онда қызметтік TXT жазбаларының атаулары өзгеше болады.

        {/note}

      Егер қажетті ресурстық жазбалар болмаса, тағы бірнеше минут күтіңіз. ExternalDNS сервисте IP мекенжай тағайындалғаннан кейін ресурстық жазбаларды жасай бастайды. Бұл үшін бірет уақыт қажет.
...
1. Кластерге ең өзекті нұсқадағы Ingress NGINX аддонын [орнатыңыз](/kz/kubernetes/k8s/instructions/addons/advanced-installation/install-advanced-ingress).
   ...
   **Стандартты орнатуды** орындаңыз. Ешқандай параметрлерді өзгертпеңіз, тек аддонды баптау кодын түзетіңіз:
   ...
   1. `controller.publishService.enabled` өрісіне `true` мәнін беріңіз:
      ...
      Бұл Ingress ресурсына Ingress контроллерінің жария IP мекенжайы тағайындалуы үшін қажет. Бұл ExternalDNS-ке Ingress үшін дұрыс ресурстық жазбаларды жасауғал мүмкіндік береді.
...
1. Ingress ресурсына арналған `cafe-ingress.yaml` манифестін жасаңыз.
   ...
   Бұл Ingress ресурстар жазылған қолданбаны жариялау үшін пайдаланылады. Қолданба `http://cafe.example.com/coffee` URL мекенжайы бойынша `cafe.example.com` доменінде қолжетімді болады.
   ...
   Егер ресурстық жазбаларғал стандартты емес TTL беру қажет болса, `external-dns.alpha.kubernetes.io/ttl` аннотациясын көрсетіңіз (әдепкі бойынша: 86400 секунд, 24 сағат).
...
1. ExternalDNS қажетті ресурстық жазбаларды жасағанына көз жеткізіңіз:
   ...
   1. Тізімнен ExternalDNS жасаған жазбаларды табыңыз:
      ...
      Егер қажетті ресурстық жазбалар болмаса, тағы бірнеше минут күтіңіз. ExternalDNS ресурстық жазбаларды Ingress ресурсына IP мекенжайы тағайындалғаннан кейін жасай бастайды. Бұл үшін бірет уақыт қажет.
...
1. Егер ExternalDNS жұмысқал қабілеттілігін тексеру үшін жасалған Kubernetes ресурстары сізге енді қажет болмаса, оларды жойыңыз:
...
1. Егер ExternalDNS сізге енді қажет болмаса, оны жойыңыз:
   ...
   1. `external-dns` атаулар кеңістігін жойыңыз.
      ...
      Сондай-алқ API VK Cloud-қал қол жеткізу деректемелерін қамтитын `vkcs-auth` құпиясы да жойылады.
