...
Managed Containers кластерлері үшін әртүрлі аддондар (қосымша сервистер) қолжетімді. Оларды кез келген комбинацияда таңдап, не {linkto(/kz/kubernetes/mk8s/instructions/create-cluster/create-terraform#mk8s-create-terraform)[text=Terraform көмегімен кластерді құру кезінде]}, не бұрыннан бар кластерге {linkto(/kz/kubernetes/mk8s/instructions/addons/manage-addons#mk8s-manage-addons-install)[text=кейінірек]} орнатуға болады. Орнату процесі автоматтандырылған және пайдаланушының ең аз араласуын талап етеді.
...
- Аддондар кластердің worker-тораптарына орнатылады және олардың есептеу ресурстарын тұтынады.
  ...
  Кейбір аддондар кластердің барлық тораптарына (соның ішінде master-тораптарға) орнатылуы мүмкін. Толығырақ {linkto(../../../instructions/addons/advanced-installation#mk8s-advanced-installation)[text=%text]} бөлімінен оқыңыз.
...
- Аддондарды орнату үш нұсқада мүмкін:
  ...
  Орнату процесі {linkto(../../../instructions/addons/advanced-installation#mk8s-advanced-installation)[text=%text]} бөлімінде сипатталған.
...
- **CPU**: 512m;
- **RAM**: 1024Mi;
- **Стандартты жүктеме теңгергіші**: бір дана (кластерге сыртқы қолжетімділікті баптау кезінде, мысалы [Ingress Controller](#mk8s-addons-ingress-controller-nginx) арқылы).
...
<!--Container Storage Interface (CSI) S3 үшін-->

{tabs}

{tab(Сипаттама)}

[Container Storage Interface (CSI) S3 үшін](https://github.com/yandex-cloud/k8s-csi-s3) — S3 үйлесімді объектілік сақтау жүйелерін ({linkto(/kz/storage/s3/concepts/about#s3-concepts-about)[text=VK Object Storage]}, AWS S3, MinIO және басқаларын) {linkto(/kz/kubernetes/mk8s/reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=тұрақты томдар]} (Persistent Volumes, PV) ретінде ілуге мүмкіндік беретін Kubernetes-ке арналған Container Storage Interface драйвері.

S3-CSI FUSE негізінде жұмыс істейтін жоғары өнімділікті [GeeseFS](https://github.com/yandex-cloud/geesefs) файлолық жүйесіне негізделген. Аддон [POSIX үйлесімділігіне](https://github.com/yandex-cloud/geesefs#posix-compatibility-matrix) дерлік толық сәйкес келеді және объектілермен келесі әрекеттерді қолдайды:

- оқу және жазу;
- бұғаттау;
- қайта атау және жою.

S3-CSI объектілік сақтау жүйесінің {linkto(/kz/storage/s3/concepts/about#s3-concepts-about-bucket)[text=бакеттері]} деңгейінде жұмыс істейді және екі режимде қызмет ете алады:

- Бакеттерді динамикалық жасау (dynamic provisioning). Көрсетілген {linkto(/kz/kubernetes/mk8s/concepts/storage#mk8s-storage-storage-classes)[text=сақтау класы]} (Storage Class) бар PVC (Persistent Volume Claim) жасалғанда, аддон мақсатты S3 үйлесімді сақтау жүйесінде жаңа бакетті автоматты түрде жасайды. PVC жасалғаннан кейін бұл бакет PV ретінде пайдалануға дайын болады.
- Бар бакеттерді пайдалану (static provisioning). Сіз кез келген S3 үйлесімді сақтау жүйесінде алдын ала жасалған бакетті пайдалана аласыз. Ол үшін адлон бар бакеттің атын көрсетіп PV жасайды, содан кейін PVC осы PV-мен байланыстырылады.

S3-CSI сіз {linkto(/kz/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-s3-csi#mk8s-install-advanced-s3-csi-install)[text=орнатқанда]} көрсеткен кез келген S3 үйлесімді сақтау жүйесінде бакеттер жасай алады.
...
Толығырақ {linkto(../../../connect/docker-registry#mk8s-docker-registry)[text=%text]} бөлімінен қараңыз.
...
[Dynamic Resource Allocation (DRA)](https://kubernetes.io/docs/concepts/scheduling-eviction/dynamic-resource-allocation/) — подтар арасында графикалық процессорлар (GPU) сияқты арнайы құрылғыларды икемді сұрау, баптау және бөлісу үшін Kubernetes механизмі. GPU-ны бөлінбейтін ресурс ретінде бөлетін стандартты [Device Plugin](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/) механизмінен айырмашылығы, DRA жеке жүктеме үшін GPU жадының немесе есептеу ресурстарының тек бір бөлігін сұрауға мүмкіндік береді.

DRA Driver for NVIDIA GPUs аддонының артықшылықтары:

- Бір GPU картасында бірнеше жүктемені іске қосуға мүмкіндік береді, олардың әрқайсысы үшін жад пен есептеу ресурстарын тұтынуды шектей отырып.
- GPU бар worker-тораптарды пайдалануды арттырады және GPU инфрақұрылымын иеленудің жиынтық құнын төмендетеді.
- Инференс, RAG, кескіндерді өңдеу сияқты тапсырмалар үшін бүтін GPU картасы қажет етпейтін машиналық оқыту және жасанды интеллект командалары үшін кіру шегін төмендетеді.
- Кластерде GPU-ны бірлесіп пайдалануды басқарудың Helm-чарттар, қолжетімділік саясаттары және жүктемені орналастыру ережелері арқылы дербес әзірлеудің орнына қолдау көрсетілетін және стандартталған тәсілін қамтамасыз етеді.

Аддон құрамы:

- worker-тораптарда GPU-ны подтарға дайындау және қосу үшін [DRA Kubelet Plugin](https://pkg.go.dev/k8s.io/dynamic-resource-allocation/kubeletplugin).
- Кластер тораптарында қолжетімді функцияларды анықтау және тіркеу үшін [Node Feature Discovery](https://kubernetes-sigs.github.io/node-feature-discovery). Компонент келесі қызметтерден тұрады:
  - NFD-Master Kubernetes API серверіне қосылу және торап объектілерін жаңарту үшін жауап береді.
  - NFD-Worker аппараттық функцияларды хабарлау үшін NFD-Master қызметіне қосылады.
  - NFD Garbage-Collector Node Feature Discovery объектілерінің барлығының сәйкес тораптары бар екеніне кепілдік береді және жоқ тораптар үшін ескірген объектілерді жояды.

{note:warn}
DRA Driver for NVIDIA GPUs аддоны тек GPU бар worker-тораптары бар кластерлер үшін жарамды. Аддонның жұмыс істеуі үшін {linkto(../../versions/version-support#mk8s-version-support)[text=1.34 нұсқасынан]} жоғары Kubernetes қажет.
{/note}
...
Аддон компоненттерінің жеке талаптары:

- DRA Kubelet Plugin (GPU бар әрбір worker-торапта):
...
- NFD-Worker (GPU бар әрбір worker-торапта):

  - **CPU**: 205m — 2000m;
  - **RAM**: 192Mi — 2Gi.
  
Аддон бірнеше worker-тораптарға орнатылса, NFD-Worker және DRA Kubelet Plugin осы тораптардың әрқайсысына орнатылады және әр торап үшін көрсетілген RAM мөлшерін талап етеді. Қалған компоненттер тек бір торапқа орнатылады.
...
Қосымша бөлімде VK Cloud құпия менеджерімен қалай пайдалану керектігі туралы толығырақ {linkto(../../../how-to-guides/external-secrets-operator#mk8s-external-secrets-operator)[text=%text]}.
...
Логтардың көздері ретінде кластер тораптарында орналасқан [kubelet қызметтері](https://kubernetes.io/docs/concepts/overview/components/#kubelet) мен {linkto(../../../reference/pods#mk8s-pods)[text=подтар]} (pods) қолданылады.

{/tab}

{tab(Жүйелік талаптар)}

Аддонның өз жүйелік талаптары жоқ. Аддон подтары әдепкі {linkto(../settings#mk8s-settings-requests-and-limits)[text=лимит баптауларын]} пайдаланады.
...
<!--Cloud Logging үшін Fluent Bit (logaas-integration) бірінші буын үшін-->
...
<!--Cloud Logging үшін Fluent Bit (logaas-integration) екінші буын үшін-->
...
Логтардың көздері ретінде кластер тораптарында орналасқан [kubelet қызметтері](https://kubernetes.io/docs/concepts/overview/components/#kubelet) мен {linkto(../../../reference/pods#mk8s-pods)[text=подтар]} (pods) қолданылады. Аддонның қалай жұмыс істейтіні туралы толығырақ оның {linkto(/kz/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-logaas-integration-mk8s#mk8s-install-advanced-logaas-integration-gen-2)[text=орнату]} бөлімінен оқыңыз.
...
GPU Operator {linkto(../../flavors#mk8s-flavors-gpu)[text=кластер тораптарындағы GPU-ді]} машиналық оқыту немесе үлкен деректерді өңдеу міндеттерін орындау үшін басқаруға мүмкіндік береді.

Кластерде GPU пайдаланудың келесі нұсқалары қолжетімді:

- Бір под бір немесе бірнеше GPU пайдаланады.
- Аддон бір GPU-ді бірнеше под арасында {linkto(../../flavors#mk8s-flavors-gpu-sharing)[text=MIG]} стратегиясы бойынша бөледі.
- Аддон бір GPU-ді бірнеше под арасында {linkto(../../flavors#mk8s-flavors-gpu-sharing)[text=MPS]} стратегиясы бойынша бөледі.
...
[HAMi](https://project-hami.io/) (Heterogeneous AI Computing Virtualization Middleware) — Kubernetes үшін гетерогенді ЖИ-үдеткіштерін басқаруға арналған құралдар жинағы.

Managed Containers қызметіндегі HAMi аддоны GPU ресурстарын (жады, есептеу қуаты немесе виртуалды карталар) әртүрлі процестер арасында бөлуге мүмкіндік береді, мысалы, бар жабдықта көбірек ML-модельдерін іске қосу үшін. Осылайша, бір физикалық карта бірден бірнеше тапсырманы орындайды, әрі бұл үшін қолданбаларыңыздың кодына өзгеріс енгізу қажет емес.

HAMi аддонының артықшылықтары:

- GPU-ді подтар арасында тиімді бөледі, әрбір под үшін ресурстардың нақты бөлігін бөлу мүмкіндігімен.
- GPU жады мен есептеу ресурстарын жеке под деңгейінде оқшаулайды.
- GPU бар worker-түйіндердің утилизациясын арттырады және GPU инфрақұрылымын иеленудің жиынтық құнын төмендетеді.
- Инференс, RAG, кескінді өңдеу сияқты тапсырмалар үшін толық GPU картасы қажет емес машиналық оқыту және жасанды интеллект командалары үшін кіру шегін төмендетеді.

Аддон және оның архитектурасы туралы толығырақ [HAMi ресми құжаттамасында](https://project-hami.io/docs).

{note:warn}
HAMi аддоны тек {linkto(../../flavors#mk8s-flavors-gpu)[text=GPU бар worker-түйіндері]} бар кластерлерге жарайды. Аддонның жұмыс істеуі үшін Kubernetes {linkto(../../versions/version-support#mk8s-version-support)[text=1.33 нұсқасы]} немесе одан жоғары нұсқасы қажет.
{/note}
...
Ingress-контроллер VK Cloud платформасымен тығыз интеграцияланады. Толығырақ {linkto(../../network#mk8s-network)[text=%text]} бөлімінен қараңыз.
...
- **RAM**: 238Mi — 660Mi.
- **Стандартты жүктеме теңгергіші**: бір дана.
- **Floating IP-адрес**: бір дана ({linkto(../../../instructions/addons/advanced-installation/install-advanced-ingress#mk8s-install-advanced-ingress)[text=әдепкі баптаулармен]} орнатылғанда).
...
- Worker-тораптар саны таңдалған Elasticsearch репликаларының санынан кем болмауы тиіс.
  ...
  Репликалар санын таңдау туралы толығырақ {linkto(../../../instructions/addons/advanced-installation/install-advanced-jaeger#mk8s-install-advanced-jaeger)[text=Jaeger орнату бөлімі]} нен оқыңыз.
...
Аддонмен жұмыс істеу туралы толығырақ {linkto(../../../how-to-guides/kgateway#mk8s-kgateway)[text=Kgateway пайдалану]} бөлімінде.
...
[Kiali](https://kiali.io/) — {linkto(#mk8s-addons-istio)[text=Istio]}-мен жұмыс істеуге арналған веб-интерфейс. Ол service mesh-ті басқаруға, оның күйін бақылауға және оны визуализациялауға мүмкіндік береді.
...
Kube Prometheus Stack — [Prometheus](https://prometheus.io/) және [Grafana](https://grafana.com/) визуализациялау құралы негізінде іске асырылған кластер күйін және оның ішінде жайылған сервистерді мониторингтеу жүйесі. Толығырақ {linkto(../../../monitoring#mk8s-monitoring-connect-grafana)[text=%text]} бөлімінен қараңыз.

{var(cloud)} {linkto(../../versions/components#mk8s-components-addons)[text=Kube Prometheus Stack аддонының екі нұсқасын]} ұсынады: базалық және VK Tech компаниясының жетілдірулері бар нұсқа (`vk` жұрнағы бар нұсқа нөмірімен). Жетілдірулері бар аддонда қосымша кіріктірілген функциялар бар, атап айтқанда {linkto(../../../monitoring#mk8s-monitoring-forecast-consumption)[text=кластер ресурстарын тұтынуды болжау]}.
...
[Velero](https://velero.io/docs/main/) — Kubernetes кластерлерін резервтік көшіру және апаттан қалпына келтіруге арналған ашық бастапқы кодты құрал.

Velero аддонының көмегімен жұмыс жүктемелерін, атаулар кеңістіктерін және Kubernetes ресурстарын резервтік көшіруді оңай баптауға және автоматтандыруға, кластерлерді көшіруге, сондай-ақ ақаулардан, баптау қателерінен, дұрыс емес релиздерден және деректер оқиғаларынан кейін қалпына келтіруге болады. Аддонмен жұмыс кезінде жасалатын резервтік көшірмелер {linkto(../../../../../storage/s3/concepts/about#s3-concepts-about)[text={var(s3)}]} сервисіндегі `Hotbox` сақтау класы бар бакеттерде сақталады.

Velero аддоны {linkto(../../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=тұрақты томдарды]} (Persistent Volumes, PV) резервтік көшіруге қолдау көрсетпейді.

Аддонмен жұмыс туралы толығырақ {linkto(/kz/kubernetes/k8s/how-to-guides/velero/velero-backup#k8s-velero-backup)[text=%text]} бөлімінде.
...
1. Жаңа және бұрыннан іске қосылған подтардың {linkto(../../../reference/resource-limiting#mk8s-resource-limiting)[text=ресурс сұрауларына]} арналған ұсынымдарды ұсыну және, егер бапталған болса, оларды автоматты түрде қолдану.
...
- Updater: ұсынымдарды алады, VPA объектілерін жаңартады және ұсынылған ресурстардың жаңа және бұрыннан бар подтар үшін қолжетімді болуын қадағалайды. Жұмыс режиміне байланысты Updater компоненті Admission Controller компонентіне бар подты жасауға немесе жаңартуға сұрау жібереді. Updater компонентінің жұмыс режимдері:
  ...
  - `Recreate` — толық көлемде жұмыс істейді. `Recreate` режимінде Updater компоненті, егер осы под сұрайтын ресурстар Recommender есептеген мәндерге сәйкес келмесе, подты жояды. Содан кейін ол Admission Controller компонентіне ұсынылған мәндермен осы подты қайта жасауға сұрау жібереді. Жаңа подтар бірден ұсынылған мәндермен жасалады. `Recreate` режиміндегі жұмыс туралы толығырақ {linkto(/kz/kubernetes/k8s/how-to-guides/vertical-pod-autoscaler#k8s-vertical-pod-autoscaler)[text=%text]} бөлімінен қараңыз.
