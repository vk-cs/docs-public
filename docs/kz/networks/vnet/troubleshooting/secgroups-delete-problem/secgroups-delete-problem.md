...
1. Қосымша `neutron` [орнатылғанына](/kz/tools-for-using-services/cli/openstack-cli#2_opcionalno_ustanovite_dopolnitelnye_pakety).
1. Қауіпсіздік тобымен байланысты порт идентификаторларының тізімін алыңыз:
   ...
   ```
   Мұнда `<ID_ГРУППЫ_БЕЗОПАСНОСТИ>` — [жою мүмкін болмаған қауіпсіздік тобының идентификаторы](/kz/networks/vnet/instructions/secgroups#view_secgroups), которую не получилось удалить.
