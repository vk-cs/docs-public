...
1. Жобаны таңдаңыз.
1. Бөлімге өтіңіз **Виртуалды желілер** → **Direct Connect**.
...
1. [API арқылы қатынауды белсендіріңіз](/kz/tools-for-using-services/api/rest-api/enable-api) доступ по API.
1. Егер әлі орнатылмаған болса [cURL](https://curl.se) және [jq](https://jqlang.org/), утилиталарын орнатыңыз.
1. [`X-Auth-Token` қатынау токенін алыңыз](/kz/tools-for-using-services/api/rest-api/case-keystone-token) токен доступа `X-Auth-Token`.
