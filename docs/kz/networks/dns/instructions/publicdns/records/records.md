# {heading(Ресурстық жазбалар)[id=dns-records]}

{include(/kz/_includes/_translated_by_ai.md)}

DNS {var(cloud)} сервисі өзіңіздің DNS-серверлеріңізді жаймай-ақ, аймақтарыңыз бен жазбаларыңызды DNS-те жариялауға мүмкіндік береді. Қоғамдық DNS интернетте көрінетін DNS-аймақтарды басқарады.

## {heading(Рөлдік модельді баптау)[id=dns-records-role-model]}

Қоғамдық DNS-пен жұмыс істеу үшін {var(cloud)} [жеке кабинетінің](https://kz.cloud.vk.kz/app/) пайдаланушыларына арналған {linkto(../../../../../access/iam/concepts/rolesandpermissions#iam-concepts-rolesandpermissions)[text=рөлдерді]} баптаңыз:
...
## {heading(Квоталар мен лимиттер)[id=dns-records-quotas-limits]}

- Жоба ішіндегі DNS-аймақтардың ең көп саны — 100.
- Бір DNS-аймақ үшін әр түрдегі ресурстық жазбалардың ең көп саны — 500.

## {heading(DNS-аймақтар тізімін қарау)[id=dns-records-zone-list]}
...
## {heading(DNS-аймақ құру)[id=dns-records-zone-add]}
...
1. DNS-аймақ параметрлерін орнатыңыз:
    ...
    - **Time to expire**: бастапқы NS-сервер жауап бермесе, екіншілік NS-сервер осы аймақ үшін сұрауларға жауап беруді тоқтататын уақыт (секундпен). Мәні **Time to refresh** және **Time to retry** өрістеріндегі мәндердің қосындысынан үлкен болуы тиіс.
    ...
    - **Time to retry**: бастапқы NS-сервер жауап бермесе, екіншілік NS-сервер бастапқы серверден SOA-жазбаны қайта сұрайтын уақыт (секундпен). Мәні **Time to refresh** өрісінде көрсетілген мәннен кіші болуы тиіс.
    - **Time to live (TTL)**: аймақтағы сұрауға теріс жауап берілген кезде кэштің өмір сүру уақыты.

1. **Аймақ қосу** батырмасын басыңыз.
1. Аймақты {var(cloud)} DNS-серверлеріне делегациялау үшін [көрсетілген](https://kz.cloud.vk.kz/app/services/dns/list) домен иесіне хабарласыңыз.
...
{note:info}
Көптеген провайдерлерде аймақты басқаруды дербес делегациялауға болады. Мұны қалай жасау керектігі туралы сұрақтарыңыз болса, [көрсетілген](https://kz.cloud.vk.kz/app/services/dns/list) домен иесінен көмек сұраңыз.
{/note}

## {heading(DNS-аймақты қарау)[id=dns-records-zone-view]}
...
1. Жобаны таңдаңыз.
1. **DNS** → **DNS-аймақтар** бөліміне өтіңіз.
1. Қажетті аймақ үшін ![ ](../../../../../assets/more-icon.svg "inline") басып, **Өңдеу** тармағын таңдаңыз.
...
## {heading(DNS-аймақты өңдеу)[id=dns-records-zone-edit]}
...
## {heading(DNS ішкіаймағын құру)[id=dns-records-subzone-add]}

Ішкіаймақ — ағымдағы аймақтан бір деңгей төмен орналасқан DNS-аймақ. Мысалы, `example.com` домені үшін `subzone.example.com` ішкіаймақ болады.
...
{var(cloud)} жобасында ішкіаймақ құру үшін {var(cloud)} DNS-серверлеріне ішкіаймақты қайта делегациялайтын ішкіаймақ атауымен екі ресурстық NS-жазбаны {linkto(#dns-records-zone-add)[text=құрыңыз]}.
...
## {heading(DNS-аймақты жою)[id=dns-records-zone-delete]}
...
1. Жобаны таңдаңыз.
1. **DNS** → **DNS-аймақтар** бөліміне өтіңіз.
1. Қажетті аймақ үшін ![ ](../../../../../assets/more-icon.svg "inline") басып, **Жою** тармағын таңдаңыз.
1. Әрекетті растаңыз.
...
## {heading(Ресурстық жазбаларды қосу)[id=dns-records-add]}
...
{var(cloud)} ресурстық жазбалардың келесі түрлерін қолдайды:

- `A` — домендік атауды IPv4 мекенжайымен сәйкестендіретін DNS-жазба.
- `AAAA` — домендік атауды IPv6 мекенжайымен сәйкестендіретін DNS-жазба.
- `NS` — осы аймаққа немесе ішкіаймаққа қызмет көрсететін атаулар серверінің мекенжайын қамтитын DNS-жазба. Әдепкі бойынша аймақта екі `NS` жазбасы орнатылады. Бұл жазбалар доменді басқару құқықтарын {var(cloud)} атаулар серверіне беру үшін домен атаулары иесінің тарапында орнатылады.
...
## {heading(Ресурстық жазбаларды қосу)[id=dns-records-record-add]}
...
1. **Жазба қосу** батырмасын басыңыз.
1. **Жазба түрі** мәніне байланысты өрістерді толтырыңыз:
   ...
   {cut(CNAME)}
    ...
    - **Атауы** (қосылатын бүркеншік атау):
        ...
        - `subzone` немесе `subzone.example.com` — `subzone` ішкіаймағын көрсетеді;
        - `\*.dns.zone` — доменге сұраудағы кез келген атауды көрсетеді.

    - **Time to live (TTL)**: кэштің өмір сүру уақыты секундпен.
    - **Мәні**: тағайындалған FQDN-мекенжайы (қосылатын бүркеншік атау нені көрсетеді). Ең көп ұзындығы — 255 таңба. Екі немесе одан көп ішкідоменнен тұруы тиіс. Ішкідоменнің ең көп ұзындығы — 63 таңба. Тек сандар, латын әріптері, `.` және `-` арнайы таңбаларына рұқсат етіледі.

   Мысал: `example.com` доменін көрсететін `www.example.com` CNAME-жазбасын құру қажет. Ол үшін **Атауы** өрісінде `www`, ал **Мәні** өрісінде `example.com` көрсетіңіз.
   ...
   {cut(MX)}
    ...
    - **Time to live (TTL)**: кэштің өмір сүру уақыты секундпен.
    - **Мәні**: пошта серверінің FQDN-мекенжайы. Ең көп ұзындығы — 255 таңба. Екі немесе одан көп ішкідоменнен тұруы тиіс. Ішкідоменнің ең көп ұзындығы — 63 таңба. Тек сандар, латын әріптері, `.` және `-` арнайы таңбаларына рұқсат етіледі.
   ...
   {cut(SRV)}
    ...
    - **Time to live (TTL)**: кэштің өмір сүру уақыты секундпен.
    - **Хост**: қызмет орналастырылған хосттың FQDN-і. Ең көп ұзындығы — 255 таңба. Екі немесе одан көп ішкідоменнен тұруы тиіс. Ішкідоменнің ең көп ұзындығы — 63 таңба. Тек сандар, латын әріптері, `.` және `-` арнайы таңбаларына рұқсат етіледі.
...
API спецификациясынан {linkto(../../../../../tools-for-using-services/api/api-spec/network-api/api-dns#api-spec-dns)[text=әдістерімен]} пайдаланыңыз.
...
## {heading(Ресурстық жазбалар тізімін қарау)[id=dns-records-list]}
...
## {heading(Ресурстық жазбалардың параметрлерін қарау)[id=dns-records-view]}
...
1. {var(cloud)} жеке кабинетіне [өтіңіз](https://kz.cloud.vk.kz/app/).
1. Жобаны таңдаңыз.
1. **DNS** → **DNS-зоны** бөліміне өтіңіз.
1. Ресурстық жазбаны қарау қажет аймақ атауын басыңыз.
1. Қажетті жазба үшін ![ ](../../../../../assets/more-icon.svg "inline") батырмасын басып, **Редактировать** тармағын таңдаңыз.

{/tab}

{tab(API)}

API спецификациясындағы {linkto(../../../../../tools-for-using-services/api/api-spec/network-api/api-dns#api-spec-dns)[text=әдістерді]} пайдаланыңыз.
...
## {heading(Ресурстық жазбаларды өңдеу)[id=dns-records-edit]}
...
1. Қажетті жазба үшін ![ ](../../../../../assets/more-icon.svg "inline") батырмасын басып, **Редактировать** тармағын таңдаңыз.
1. Өзгерістерді енгізіп, **Сохранить изменения** батырмасын басыңыз.
...
## {heading(Ресурстық жазбаларды жою)[id=dns-records-delete]}
...
1. Қажетті жазба үшін ![ ](../../../../../assets/more-icon.svg "inline") батырмасын басып, **Удалить** тармағын таңдаңыз.
1. Әрекетті растаңыз.
