...
1. **SSL-сертификат** параметрінде SSL-сертификатты пайдалануға жауап беретін опцияны таңдаңыз:
    ...
    - **(По умолчанию) Let's Encrypt** — тегін [Let's Encrypt](https://letsencrypt.org) сертификаты пайдаланылады. Сертификат CDN-ресурсы жасалғаннан кейін, бастапқы серверлер қолжетімді болып, DNS-та дербес домендер үшін CNAME-жазбаларына қатысты өзгерістер таралған кезде жасалады. Әдетте бұл 30 минутқа дейін созылады. Бұл опцияны таңдағанда {linkto(../../../../../networks/cdn/instructions/manage-cdn/enable-cdn#cdn-enable-cdn)[text=Доступ к контенту конечным пользователям]} опциясын да қосу қажет.
...
- Тегін [Let's Encrypt](https://letsencrypt.org) сертификатын пайдалану үшін `"sslEnabled": true` параметрін көрсетіп, сертификатты жаңарту параметрлерін орнатыңыз:
