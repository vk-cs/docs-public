...
- AWS CLI конфигурациясындағы өңір {ifdef(public)}аккаунттың {linkto(../../../../../tools-for-using-services/account/concepts/regions#tools-account-concepts-regions)[text=өңіріне]} {/ifdef}{ifdef(s3,s3-pdf)}{var(s3)} инсталляцияңыздың өңіріне{/ifdef} сәйкес келмейді;
...
1. Бакет атауы дұрыс көрсетілгеніне көз жеткізіңіз.
   {ifdef(public)}
1. Команданың `--endpoint-url` параметрінде сервис домені дұрыс көрсетілгеніне және {var(s3)} аккаунтының {linkto(../../../../../tools-for-using-services/account/concepts/regions#tools-account-concepts-regions)[text=өңіріне]} сәйкес келетініне көз жеткізіңіз:
...
1. AWS CLI конфигурациясын тексеріңіз:
   ...
   ```
   {ifdef(public)}
   Жауаптағы `region` өрісінде {var(s3)} аккаунтының {linkto(../../../../../tools-for-using-services/account/concepts/regions#tools-account-concepts-regions)[text=өңіріне]} сәйкес келетін мәндердің бірі болуы керек:
