...
1. Консольде команданы орындаңыз:
   ...
   - `<ИМЯ_БАКЕТА>` — имя целевого бакеттің.
...
## {heading(Изменение уровня қолжетімділіка к объекту)[id=manage_access]}
...
- арқылы CLI, API, SDK и файлдардыңые менеджеры, егер есть [қолжетімділік кілттері к бакету или аккаунту](../../../concepts/access/account-and-keys).

Вы можете өзгерту уровень қолжетімділіка объектінің, чтобы сделать его қолжетімділікным сторонним пользователям. Уровни қолжетімділіка соответствуют [стандартным ACL](../../../concepts/access/s3-acl#standard_acl). По умолчанию болады орнату уровни:

- `private` — полные права при наличии [ключей қолжетімділіка к бакету или аккаунту](../../../concepts/access/account-and-keys), всем остальным объект неқолжетімділікен;
...
Чтобы өзгерту уровень қолжетімділіка к объекту:
...
1. Көшіріңіз сілтеме и закройте окно баптаулар қолжетімділіка.
...
1. В консоли выполните команду үшін изменения уровня қолжетімділіка:
   ...
   Время модификации объектілердің обновится. Учитывайте это при управлении жизненным циклом (lifecycle) объектілердің и использовании условных запросов (`if-modified-since`).
...
## {heading(Блокировка удаления объектілердің)[id=lock_object]}
...
1. Консольді ашып, бұғаттауға қатысты қажетті әрекетті орындаңыз.
   ...
   {tab(Установить үшін нового объектінің)}
   ...
   Чтобы узнать статус бессрочной блокировки объектінің, выполните команду:
...
1. Консольді ашып, қажетті әрекетті орындаңыз.
   ...
   Чтобы снять с объектінің управляемую временную блокировку, выполните команду:
   ...
   Чтобы сократить үшін объектінің срок управляемой временной блокировки, выполните команду:
...
## {heading(Копирование объектілердің)[id=copy_object]}
...
1. Консольде команданы орындаңыз:
   ...
   - `--recursive` — команда бакетте немесе көрсетілген директорияда орналасқан барлық объектілер үшін орындалатын параметр.
   - `<БАКЕТ_ИСТОЧНИК>` — бакет атауы, из которого копируется объект.
   - `<КЛЮЧ_ОБЪЕКТА_ИСТОЧНИКА>` — полное имя копируемого объектінің или директории, из которой копируются объектілер, включая путь до нее.
...
1. Қажетті бөлік үшін мына әрекеттердің бірін орындаңыз:
   ...
   - Жойылуы тиіс бөлікті таңдап, оң жақтағы ![Удалить](assets/delete-icon.svg "inline") белгішесін басыңыз.
1. Жоюды растаңыз.
