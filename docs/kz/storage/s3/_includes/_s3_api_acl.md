...
- `<Owner>` — бакет иесінің канондық идентификаторын көрсетеді. Оны {linkto(#api-spec-s3-get-bucket-acl)[text=%text]} әдісі арқылы немесе AWS CLI арқылы, {ifdef(public)}{linkto(/kz/storage/s3/instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=сұратып]}{/ifdef}{ifdef(s3,s3-pdf)}{linkto(../../instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=сұратып]}{/ifdef} бакеттің ACL-і туралы ақпаратты алу арқылы біліңіз.
...
{var(cloud)} пайдаланушысының канондық идентификаторын білу үшін, {ifdef(public)}{linkto(/kz/storage/s3/instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=ACL-ді алыңыз]}{/ifdef}{ifdef(s3,s3-pdf)}{linkto(../../instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=ACL-ді алыңыз]}{/ifdef} — пайдаланушының қол жеткізу құқықтары бар бакет немесе объект үшін.
...
Жобаға қол жеткізуді беру үшін, сұрауда {ifdef(public)}{linkto(/kz/tools-for-using-services/account/instructions/project-settings/manage#project-pid-view)[text=жоба идентификаторын]}{/ifdef}{ifdef(s3,s3-pdf)}жоба идентификаторын{/ifdef} (PID) көрсетіңіз.
