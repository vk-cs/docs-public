...
- `<Owner>` — бакет немесе объект орналасқан {linkto(#api-spec-s3-acl-project)[text=жобаның]} идентификаторын (`ID`) және атауын (`DisplayName`) қамтиды. `ID` және `DisplayName` мәндерін алу үшін бакеттің ACL-і туралы ақпаратты {ifdef(public)}{linkto(/kz/storage/s3/instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=сұратыңыз]}{/ifdef}{ifdef(s3,s3-pdf)}{linkto(../../instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=сұратыңыз]}{/ifdef}.
- `<Grant>` — қай алушыға қандай рұқсаттар берілетінін анықтайды. `<Grant>` ішінде `<Grantee>` және `<Permission>` элементтері болады.
  ...
  - `<Grantee>` — қол жеткізу құқықтарын алушыны атрибуттар арқылы көрсетеді:
    ...
    - `Type` — құқықтарды алушының түрі: `CanonicalUser` немесе `Group`.
...
- `x-amz-acl` — бакетке немесе объектіге алдын ала бапталған {ifdef(public)}{linkto(/kz/storage/s3/concepts/access/s3-acl#s3-concepts-acl-pre-set)[text=стандартты ACL]}{/ifdef}{ifdef(s3,s3-pdf)}{linkto(../../concepts/access/s3-acl#s3-concepts-acl-pre-set)[text=стандартты ACL]}{/ifdef} тағайындайды.
...
Жоба — {var(s3)} сервисіндегі түрі `CanonicalUser` болатын жалғыз қол жеткізу құқықтарын алушы. Ол барлық бакеттер мен объектілерге `FULL_CONTROL` құқығымен автоматты түрде тағайындалады және иесімен толығымен сәйкес келеді. Бұл алушыны жою немесе өзгерту мүмкін емес. {var(cloud)} жобасына берілген құқықтар жоба ішіндегі барлық {var(s3)} есептік жазбаларына қолданылады.

### {heading(Метатоп)[id=api-spec-s3-acl-meta]}

Алдын ала анықталған метатоп — {var(s3)} сервисіндегі түрі `Group` болатын қол жеткізу құқықтарын алушы, ол {var(cloud)} жобасынан тыс екі пайдаланушы санатына жеке құқықтар беруге мүмкіндік береді.

- `Authenticated Users` (авторизацияланған пайдаланушылар) — {var(cloud)} жүйесіндегі барлық жобалардағы {var(s3)} сервисінің бар есептік жазбалары.

  {note:warn}
  `Authenticated Users` пайдаланушылар тобына қол жеткізу берілген кезде, кез келген авторизацияланған {var(s3)} пайдаланушысы ресурсқа қол жеткізе алады. Қолтаңбаланбаған (анонимді) сұраулар қабылданбайды.
  {/note}
...
Белгісіз адамдар тобына қол жеткізуді ашқанда, ресурстарыңызбен операцияларды кім орындайтынын бақылаудан айырыласыз. Бұл ретте {var(s3)} қызметтері үшін төлем бакет тиесілі жобаның шотынан алынады.
{/note}
