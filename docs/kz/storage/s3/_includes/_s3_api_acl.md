...
- `<Grant>` — қай алушыға қандай рұқсаттар берілетінін анықтайды. `<Grant>` ішінде `<Grantee>` және `<Permission>` элементтері болады.
  ...
  - `<Permission>` — берілетін {linkto(#api-spec-s3-acl-permissons)[text=рұқсаттың]} түрін көрсетеді.
...
## {heading(Рұқсаттар)[id=api-spec-s3-acl-permissons]}
