# {heading(Работа с экземпляром сервиса)[id=jupyterhub_management]}

{include(../../../_includes/_standalone.md)[tags=difference_management]}

## {heading(Общая информация)[id=jupyterhub_info]}

### {heading(Просмотр общей информации)[id=jupyterhub_view_info]}
...
### {heading(Редактирование общей информации)[id=jupyterhub_edit_info]}
...
## {heading(Изменение IP и портов)[id=jupyterhub_change-ip]}
...
## {heading(Просмотр статуса компонентов)[id=jupyterhub_view_status]}
...
## {heading(Горизонтальное масштабирование)[id=jupyterhub_horizontal_scaling]}
...
### {heading(Добавление учетной записи)[id=jupyterhub_admin_create]}
...
1. Укажите логин и пароль учетной записи.
   ...
   {note:info}
   Подробнее об особенностях ввода логина и пароля — в разделе {linkto(../create#jupyterhub_create_user)[text=Шаг 5. Учетные данные]}
   {/note}
...
## {heading(Подключения)[id=jupyterhub_connect]}

### {heading(Просмотр подключений)[id=jupyterhub_view_connect]}
...
### {heading(Добавление подключений)[id=jupyterhub_create_connect]}
...
## {heading(Обслуживание экземпляра)[id=jupyterhub_service]}
...
## {heading(Просмотр истории изменений)[id=jupyterhub_changes_history]}
...
## {heading(Удаление экземпляра сервиса)[id=jupyterhub_delete]}
{tabs}
