...
{note:warn}
Экземпляр Cloud Kafka [тарифицируется](../tariffication) и потребляет вычислительные ресурсы. После прохождения быстрого старта [удалите экземпляр сервиса](#kafka-quick-start-delete), если он вам больше не нужен.
{/note}
...
1. На шаге **Конфигурация**:
   ...
   1. Выберите конфигурацию **Одиночный узел**.
...
1. На шаге **Параметры**:
   ...
   1. Выберите **Sprut** в качестве SDN.
   1. **Адрес подсети**: укажите `10.0.1.0/24`.
   1. **Зона доступности**: `Москва (PA2)`.
   1. Нажмите кнопку **Следующий шаг**.
...
## {heading(Удалите неиспользуемые ресурсы)[id=kafka-quick-start-delete]}
