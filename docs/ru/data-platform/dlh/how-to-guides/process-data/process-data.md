# {heading(Работа с данными)[id=dlh_process-data]}
...
## {heading(Подготовительные шаги)[id=dlh_process-data_preparations]}
...
1. {linkto(../../quick-start#dlh_quick-start)[text=Разверните]} базовую архитектуру сервиса Data Lakehouse.

    В качестве примера в процессе развертывания при {linkto(../../quick-start#dlh_quick-start_create_trino_instance)[text=настройке Cloud Trino]} созданы:

    - подключение `s3db1` к хранилищу {var(s3)};
...
## {heading(1. Создайте схемы данных в хранилище S3)[id=dlh_process-data_create_data-scheme]}
...
## {heading(2. Загрузите данные из внешних баз данных в хранилище S3)[id=dlh_process-data_load_data_from_db]}
...
## {heading(3. Загрузите данные из таблиц виртуальной БД в хранилище S3)[id=dlh_process-data_load_data_from_tables]}
...
## {heading(4. Выполните аналитический запрос к хранилищу S3)[id=dlh_process-data_make_request_to_s3]}
...
## {heading(5. Выгрузите данные из хранилища S3 во внешние базы данных)[id=dlh_process-data_upload]}
...
## {heading(6. Выполните аналитический запрос одновременно к нескольким источникам данных)[id=dlh_process-data_make_request_to_several_sources]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=dlh_process-data_delete_unused_resources]}
