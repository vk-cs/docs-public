# {heading(Подключение Cloud Spark в качестве инструмента обработки данных)[id=dlh_connect-spark]}

Включение в {linkto(../../quick-start#dlh_quick-start)[text=базовую структуру]} Data Lakehouse (DLH) сервиса {linkto(../../../spark#spark)[text=Cloud Spark]} позволяет выполнять сложную трансформацию данных перед попаданием их в S3-объектное хранилище, в отличие от [Cloud Trino](../../../trino), который может реализовывать относительно простые сценарии.

Для хранения оперативной информации может дополнительно использоваться аналитическая БД, например [Cloud ClickHouse](../../../clickhouse), хотя такая архитектура усложняет и увеличивает стоимость решения.

## {heading(Подготовительные шаги)[id=dlh_connect-spark_preparations]}

{linkto(../../quick-start#dlh_quick-start)[text=Разверните]} базовую архитектуру сервиса Data Lakehouse.

## {heading(1. Разверните экземпляр сервиса Cloud Spark)[id=dlh_connect-spark_create_instance]}
...
1. На шаге **Подключения**:
    ...
    1. Добавьте подключние к PostgreSQL, созданной при {linkto(../../quick-start#dlh_quick-start_create_postgresql_instance)[text=развертывании]} базовой конфигурации Data Lakehouse:
   ...
   1. Добавьте подключение к S3-объектному хранилищу, созданный при {linkto(../../quick-start#dlh_quick-start_create_trino_instance)[text=развертывании]} базовой конфигурации Data Lakehouse:
        ...
        1. Укажите параметры подключения:
            ...
            - **Бакет**: имя бакета, заданное при создании {var(s3)}.
            - **Путь в бакете S3**: адрес размещения файлов в S3-хранилище. Параметр доступен на странице экземпляра {var(s3)}.
