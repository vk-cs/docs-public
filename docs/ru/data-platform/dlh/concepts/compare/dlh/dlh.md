...
Data Lakehouse (DLH) — современная гибридная архитектура, которая объединяет преимущества {linkto(../dwh#dlh_dwh)[text=%text]} (DWH) и {linkto(../data-lake#dlh_data-lake)[text=%text]}, устраняя при этом их ключевые недостатки. 
...
## {heading(Модель хранения данных в Data Lakehouse)[id=dlh_dlh_models]}
...
## {heading(Преимущества архитектуры Data Lakehouse)[id=dlh_dlh_advantages]}

- Бесконечная масштабируемость (нет ограничений на объем).
- Снижение затрат (оплата по принципу «pay as you go» только за реально используемое место).
...
- Управление метаданными, которое позволяет избежать превращения озера данных в «болото данных» (data swamp).
