...
* [VK Object Storage](../concepts/components/s3) — S3-хранилище, которое используется в качестве недорогого бесконечно расширяемого хранилища неструктурированных данных, получаемых из различных БД, например PostgreSQL.
* [Cloud Iceberg Metastore](../concepts/components/iceberg) — используется для организации каталога метаданных на базе Apache Iceberg. Обеспечивает согласованность операций между различными компонентами сервиса DLH.
* [Cloud Trino](../concepts/components/trino) — в связке с Cloud Iceberg Metastore обеспечивает клиентам привычный SQL-интерфейс для выполнения следующих операций:
...
1. На шаге **Параметры**:
   ...
   1. Выберите [зону доступности](/ru/start/concepts/architecture#az), например: `Москва (ME1)`.
...
1. На шаге **Учетные данные**:
   ...
   1. В поле **Пароль** нажмите кнопку **Сгенерировать**, чтобы сгенерировать пароль администратора. Сохраните пароль, он понадобится вам позже.
      ...
      Утерянный пароль невозможно восстановить, но можно [поменять](/ru/data-platform/trino/instructions/management#change_password) на новый.
...
1. [Подключите](/ru/data-platform/dlh/how-to-guides/connect-adb) аналитическую БД на базе Cloud ClickHouse, чтобы выполнять быструю обработку части данных до их попадания в S3-хранилище.
1. [Подключите](/ru/data-platform/dlh/how-to-guides/connect-spark) сервис Spark для работы со сложными ETL- и ELT-задачами.
