# {heading(Быстрый старт)[id=dlh_quick-start]}

Быстрый старт включает базовый сценарий развертывания сервиса Data Lakehouse (DLH) на основе трех компонентов:

- {linkto(../concepts/components/s3#dlh_s3)[text=VK Object Storage]} — S3-хранилище, которое используется в качестве недорогого бесконечно расширяемого хранилища неструктурированных данных, получаемых из различных БД, например PostgreSQL.
- {linkto(../concepts/components/iceberg#dlh_iceberg)[text=Cloud Iceberg Metastore]} — используется для организации каталога метаданных на базе Apache Iceberg. Обеспечивает согласованность операций между различными компонентами сервиса DLH.
- {linkto(../concepts/components/trino#dlh_trino)[text=Cloud Trino]}— в связке с Cloud Iceberg Metastore обеспечивает клиентам привычный SQL-интерфейс для выполнения следующих операций:
...
## {heading(1. Создайте экземпляр сервиса Cloud Iceberg Metastore)[id=dlh_quick-start_create_iceberg_instance]}
...
1. На шаге **Параметры**:
   ...
   1. Выберите [зону доступности](/ru/start/concepts/architecture#architecture-az), например: `Москва (ME1)`.
...
1. На шаге **Учетные данные**:
   ...
   1. В поле **Пароль** нажмите кнопку **Сгенерировать**, чтобы создать пароль администратора. Сохраните пароль, он понадобится вам позже.
...
## {heading(2. Создайте инстанс БД PostgreSQL)[id=dlh_quick-start_create_postgresql_instance]}
...
1. На шаге **Создание инстанса** задайте:
   ...
   - **Сеть**: выберите сеть, которую указывали при [создании экземпляра сервиса Cloud Iceberg Metastore](#dlh_quick-start_create_iceberg_instance) на этапе развертывания базовой конфигурации.
...
## {heading(3. Создайте экземпляр сервиса Cloud Trino)[id=dlh_quick-start_create_trino_instance]}
...
1. На шаге **Параметры**:
   ...
   1. Включите опцию **Назначить внешний IP**.
   1. Выберите кластер, который указывали при [создании экземпляра сервиса Cloud Iceberg Metastore](#dlh_quick-start_create_iceberg_instance) на этапе развертывания базовой конфигурации.
   1. Выберите вариант настройки `Базовая`, шаблон кластера `Малый`.
   1. Нажмите кнопку **Следующий шаг**.
...
1. На шаге **Подключения**:
   ...
   1. Добавьте к экземпляру сервиса Cloud Trino источник данных PostgreSQL:
      ...
      1. Выберите тип подключение `PostgreSQL` и укажите:
         ...
         - **hostname:port**: адрес подключения к созданному инстансу PostgreSQL в формате: `<IP-АДРЕС>:<ПОРТ>`. Например `10.0.1.2:5432`. Адрес подключения к БД можно [посмотреть](/ru/dbs/dbaas/instructions/manage-instance/postgresql#dbaas-postgresql-get-info) в карточке инстанса.
   ...
   1. Добавьте к экземпляру сервиса Cloud Trino источник данных Apache Iceberg с {var(s3)}:
      ...
      1. Укажите параметры подключения:
         ...
         - **Логин сервиса Iceberg Metastore**: логин администратора, указанный при [создании экземпляра сервиса Iceberg Metastore](#dlh_quick-start_create_iceberg_instance) на этапе развертывания базовой конфигурации. В этом примере: `vkdata`.
         ...
         - **Bucket**: задайте имя бакета для создания S3-объектного хранилища, следуя [правилам](/ru/storage/s3/concepts/about#s3-concepts-about-bucket-naming) именования бакетов.
...
1. На шаге **Учетные данные**:
   ...
   1. В поле **Пароль** нажмите кнопку **Сгенерировать**, чтобы сгенерировать пароль администратора. Сохраните пароль, он понадобится вам позже.
      ...
      {note:info}
      Утерянный пароль невозможно восстановить, но можно {linkto(../../trino/instructions/management#trino_change_password)[text=поменять]} на новый.
      {/note}
...
## {heading(Что дальше?)[id=dlh_quick-start_next_steps]}

1. {linkto(../how-to-guides/connect-adb#dlh_connect-adb)[text=Подключите]} аналитическую БД на базе Cloud ClickHouse, чтобы выполнять быструю обработку части данных до их попадания в S3-хранилище.
1. {linkto(../how-to-guides/connect-spark#dlh_connect-spark)[text=Подключите]} сервис Spark для работы со сложными ETL- и ELT-задачами.
1. {linkto(../how-to-guides/process-data#dlh_process-data)[text=Изучите]} процесс загрузки данных в сервис и их аналитической обработки.
