...
Задать эти параметры можно через [личный кабинет VK Cloud](https://msk.cloud.vk.ru/app/) или через Terraform.
...
|`S3 VK Cloud`
|`s3_int`
|{linkto(#parameters_s3_int)[text=Подключение]} к облачному хранилищу {var(s3)} в том же проекте
...
|`name`
|Имя подключения к {var(s3)}, отображаемое в личном кабинете
...
|`s3_bucket`
|Уникальное имя бакета {var(s3)}. Этот бакет используется только для хранения внутренних данных приложений
