# {heading(О сервисе)[id=gpu-about]}

Cloud GPU — это облачный сервис, который предоставляет доступ к графическим процессорам (GPU) в формате готовых {linkto(#gpu-about-flavors)[text=шаблонов конфигураций]} с заданными параметрами производительности. GPU оптимизированы для параллельной обработки данных, что делает их эффективными для задач, требующих высокой вычислительной мощности: высокопроизводительных вычислений (HPC), машинного обучения, моделирования, обработки изображений и видео.

## {heading(Преимущества сервиса)[id=gpu-about-advantages]}

{ifdef(private-pdf,private-pg-pdf)}Преимущества сервиса приведены в {linkto(#tab-gpu-advantages)[text=таблице %number]}.

{caption(Таблица {counter(table)[id=numb-tab-gpu-advantages]} — Преимущества сервиса)[align=right;position=above;id=tab-gpu-advantages;number={const(numb-tab-gpu-advantages)}]}{/ifdef}
...
## {heading(Для каких задач подходит сервис)[id=gpu-about-task]}
...
## {heading(Шаблоны конфигураций)[id=gpu-about-flavors]}
...
Чтобы использовать шаблоны конфигурации ВМ с GPU или vGPU, обратитесь к администратору {var(cloud)} с просьбой добавить их в проект.
{/ifdef}

{ifdef(public)}
В зависимости от бюджета и сложности задач, доступны разные варианты {linkto(../../../../computing/iaas/concepts/vm/flavor#iaas-flavor)[text=шаблонов конфигураций]} (flavors): с {linkto(#gpu-about-flavors)[text=выделенными GPU]} или на базе {linkto(#gpu-about-vgpu-flavors)[text=технологии vGPU]}. Подробнее об отличиях — в {linkto(../../../../computing/gpu/concepts/vgpu#gpu-vgpu)[text=разделе о vGPU]}.

### {heading(Выделенные GPU)[id=gpu-about-flavors-model]}
...
Суффикс `SXM4` в {linkto(../../../../computing/iaas/concepts/vm/flavor#iaas-flavor-vm-names)[text=названии шаблона конфигурации]} отображает формфактор и интерфейс подключения GPU NVIDIA.
...
### {heading(vGPU)[id=gpu-about-vgpu-flavors]}

Доступно подключение не более одного GPU. Ресурс графического ускорителя распределяется с помощью {linkto(../../../../computing/gpu/concepts/vgpu#gpu-vgpu)[text=технологии vGPU]}.
...
Для vGPU доступны только следующие конфигурации на основе графического ускорителя Tesla L4:
