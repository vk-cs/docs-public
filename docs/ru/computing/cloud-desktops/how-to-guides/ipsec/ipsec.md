# {heading(IPsec для VDI)[id=desktops-ipsec]}

Для организации VPN-туннеля на базе {var(cloud)} нужно указать одну или несколько существующих подсетей, находящихся за маршрутизатором.
...
Чтобы показать настройку защищенного подключения с виртуальными рабочими столами:

- Будет создан VPN-туннель между LDAP-окружением и VDI-окружением в {var(cloud)}.
- На стороне LDAP в качестве VPN-решения будет использован встроенный VPN-сервис {var(cloud)}. В реальном сценарии вы можете использовать любое оборудование или ПО с возможностью создания VPN-подключения по технологии IPsec Site-2-Site.
- На стороне VDI в качестве VPN-решения будет использована ВМ на базе ОС Debian 11 и дополнительными пакетами strongSwan.
- Будет создана транзитная сеть, которая свяжет VPN-шлюз на базе Debian 11 с маршрутизатором {var(cloud)}, за которым будут сформированы сети VDI.
- Для имитации VDI будет добавлена сеть с ВМ. ВМ будет выполнять пинг LDAP-сервера для проверки сетевой связности.

## {heading(Подготовительные шаги)[id=desktops-ipsec-preparatory-steps]}
...
1. Создайте LDAP-окружение:
   ...
   1. Выберите или {linkto(../../../../networks/vnet/instructions/net#vnet-net-add)[text=создайте]} в {var(cloud)} виртуальную сеть с доступом к интернету. Используйте существующий маршрутизатор с подключением к внешней сети или {linkto(../../../../networks/vnet/instructions/router#vnet-router-add)[text=создайте]} новый.
   ...
   1. Создайте в выбранной сети {linkto(../../../../computing/iaas/concepts/vm#iaas-concepts-vm)[text=виртуальную машину]} c ОС Windows Server 2019.

      Запишите имя и IP-адрес ВМ.

1. Создайте VDI-окружение:

   1. Выберите или {linkto(../../../../networks/vnet/instructions/net#vnet-net-add)[text=создайте]} в {var(cloud)} виртуальную сеть с доступом к интернету, которая будет выполнять функцию транзитной сети. Используйте существующий маршрутизатор с подключением к внешней сети или {linkto(../../../../networks/vnet/instructions/router#vnet-router-add)[text=создайте]} новый.
...
1. {linkto(../../../../computing/iaas/concepts/vm#iaas-concepts-vm)[text=Создайте виртуальную машину]} с подключением к внешней сети `ext-net`. Это позволит не использовать Floating IP-адрес при дальнейшей настройке.
...
1. Обновите OC на ВМ Debian:
   ...
   1. {linkto(../../../../computing/iaas/instructions/vm/vm-connect/vm-connect-nix#iaas-vm-connect-nix)[text=Подключитесь]} к виртуальной машине `Debian-IPsec-Gate` по SSH и получите права root-пользователя (команда `sudo bash`).
...
## {heading(1. Настройте VPN-туннель на стороне AD/LDAP)[id=desktops-ipsec-vpn-setting]}

В личном кабинете {linkto(../../../../networks/vnet/instructions/vpn#vnet-vpn)[text=создайте VPN]} со следующими параметрами:
...
## {heading(2. Подключите дополнительный сетевой интерфейс к ВМ Debian)[id=desktops-ipsec-net-interface-connect]}
...
## {heading(3. Настройте дополнительный сетевой интерфейс в ВМ Debian)[id=desktops-ipsec-net-interface-settings]}
...
## {heading(4. Отключите Port Security на порте в сторону транзитной сети)[id=desktops-ipsec-port-security-off]}

Отключите IP-адрес Source Guard для порта VPN-шлюза, чтобы он мог пересылать любой трафик:
...
## {heading(5. Включите пересылку пакетов в ВМ)[id=desktops-ipsec-enable-packet-forwarding]}
...
## {heading(6. Установите пакеты для настройки VPN-туннеля на стороне ВМ Debian)[id=desktops-ipsec-vpn-packages-install]}
...
## {heading(7. Добавьте конфигурацию IPsec и автоматизируйте ее запуск)[id=desktops-ipsec-config-add]}
...
## {heading(8. Запустите IPsec и проверьте состояние VPN-туннеля)[id=desktops-ipsec-run]}
...
## {heading(9. Настройте правила iptables)[id=desktops-ipsec-iptables-settings]}
...
## {heading(10. Настройте статистический маршрут из VDI-окружения в AD/LDAP)[id=desktops-ipsec-route-settings]}
...
## {heading(11. Проверьте работоспособность)[id=desktops-ipsec-functionality-check]}

1. {linkto(../../../../networks/vnet/instructions/net#vnet-net-add)[text=Создайте]} виртуальную сеть с CIDR `10.55.4.0/22` и маршрутизатором `vdi-router`. Эта сеть имитирует сеть VDI и нужна для проверки сетевой связности. При развертывании VDI такая сеть создается автоматически.
1. Создайте тестовую {linkto(../../../../computing/iaas/concepts/vm#iaas-concepts-vm)[text=виртуальную машину]} в сети `10.55.4.0/22` и подключитесь к ней по SSH.
...
## {heading(Удалите неиспользуемые ресурсы)[id=desktops-ipsec-delete]}

Если созданные ресурсы вам больше не нужны, удалите их:

1. {linkto(../../../../computing/iaas/instructions/vm/vm-manage#iaas-vm-manage-delete)[text=Удалите]} виртуальные машины.
1. {linkto(../../../../networks/vnet/instructions/vpn#vnet-vpn-delete)[text=Удалите]} VPN.
1. {linkto(../../../../networks/vnet/instructions/router#vnet-router-delete)[text=Удалите]} маршрутизаторы.
1. Удалите {linkto(../../../../networks/vnet/instructions/net#vnet-net-subnet-delete)[text=подсеть]} и {linkto(../../../../networks/vnet/instructions/net#vnet-net-delete)[text=сеть]}, в которой была размещена ВМ.
