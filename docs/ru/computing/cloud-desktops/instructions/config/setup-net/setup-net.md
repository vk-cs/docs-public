...
1. На вкладке **Настройка сети** задайте параметры:
   ...
   - **Режим доступа к рабочим столам**: выберите вариант из списка.
      ...
      Выбрать режим `Внутренний доступ` не получится, если используется SSL-сертификат, подписанный публичным центром сертификации. Чтобы перейти на внутренний доступ, [используйте базовый SSL-сертификат](/ru/computing/cloud-desktops/instructions/config/ssl-certificates#vkcloud_certificate) от платформы VK Cloud или [подключите собственный](/ru/computing/cloud-desktops/instructions/config/ssl-certificates#own_certificate).
