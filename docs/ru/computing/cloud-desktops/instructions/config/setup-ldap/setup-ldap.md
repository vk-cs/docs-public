# {heading(Настройка подключения к каталогу пользователей)[id=desktops-setup-ldap]}
...
## {heading(1. Настройте подключение к каталогу пользователей)[id=desktops-setup-ldap-step1]}
...
1. На вкладке **Настройка каталога** задайте параметры:
   ...
   - **DN пользователя для подключения к каталогу LDAP**: укажите полный путь к учетной записи в базе OpenLDAP. Эта учетная запись будет использоваться для синхронизации с каталогом учетных записей.

     Для Active Directory такая учетная запись должна состоять в группе **Domain Admins**, а для ALD Pro — в группе **Computers Administrators**.

   - **Пароль**: задайте пароль от учетной записи, указанной в **DN пользователя для подключения к каталогу LDAP**.
...
{note:info}
Если изменить настройки подключения к каталогу пользователей при работе c Cloud Desktop, все существующие пулы переключатся на новые настройки подключения автоматически.
{/note}

## {heading(2. Проверьте настройки подключения)[id=desktops-setup-ldap-step2]}

Чтобы проверить настройки подключения к каталогу пользователей, необязательно сохранять изменения в настройках.
