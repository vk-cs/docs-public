# {heading(Настройка групповых политик AD)[id=desktops-group-policies]}
...
## {heading(Подготовительные шаги)[id=desktops-ad-preparatory-steps]}

Запустите оснастку Group Policy Management на контроллере домена Active Directory.

## {heading(1. Добавьте правило для Windows Defender Firewall)[id=desktops-ad-windows-role-add]}
...
## {heading(2. Настройте механизм согласования безопасности для RDP-сеанса подключения к виртуальным рабочим столам)[id=desktops-ad-windows-rdp]}
...
## {heading(3. Разрешите конечным пользователям подключение к виртуальным рабочим столам)[id=desktops-ad-windows-connect]}
...
1. Добавьте группу `<ИМЯ_ВАШЕГО_ДОМЕНА>\<НАЗВАНИЕ_ГРУППЫ_ПОЛЬЗОВАТЕЛЕЙ_AD>` в члены стандартной группы `BUILTIN\Remote Desktop Users`.
