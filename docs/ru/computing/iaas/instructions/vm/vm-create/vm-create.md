...
1. Настройте подключение ВМ к сети:
   ...
   - **Настройки Firewall**: выберите нужные группы безопасности.
      ...
      Чтобы подключиться по SSH, добавьте группу `ssh` или `ssh+www`. Подробнее про настройку правил сетевого доступа в разделе [Управление правилами файервола](/ru/networks/vnet/instructions/secgroups).
...
1. Если необходимо, включите опцию **Использовать резервное копирование** и [задайте](/ru/storage/backups/instructions/create-backup-plan#create_vm_backup_plan) нужные настройки.
