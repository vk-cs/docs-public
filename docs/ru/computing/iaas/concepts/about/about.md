...
VK Cloud распределяет по [поддерживаемым зонам доступности](/ru/intro/start/concepts/architecture) следующие виртуальные ресурсы:
...
Сеть, созданную в зоне доступности QAZ, невозможно объединить стандартными способами с сетями из других зон доступности, так как зона QAZ находится в другом [регионе](../../../../tools-for-using-services/account/concepts/regions). В этом случае сетевую связность можно обеспечить средствами [VPN](/ru/networks/vnet/how-to-guides/onpremise-connect/vpn-tunnel).
...
## {heading(Файловое хранилище)[id=file_share]}
