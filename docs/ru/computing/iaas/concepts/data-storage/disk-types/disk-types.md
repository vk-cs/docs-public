...
## {heading(Загрузочный и root-диски)[id=root_boot_disk]}

Root-диск — основной диск виртуальной машины, с которого запускается операционная система при старте ВМ. Такой диск создается и монтируется к виртуальной машине автоматически при ее создании, но его можно [заменить](/ru/computing/iaas/instructions/vm/vm-root-replace).
