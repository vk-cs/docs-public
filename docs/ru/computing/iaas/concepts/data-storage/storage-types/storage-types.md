# {heading(Типы хранилищ данных)[id=iaas-concepts-storage-types]}

{var(cloud)} поддерживает следующие типы хранилищ:

- _Блочное (block-based)_. Хранит данные в виде логических блоков, аналогично жестким дискам. Блочное хранилище подходит для хранения данных ВМ и контейнеров, а также снимков дисков. В {var(cloud)} тип представлен подключаемыми к ВМ дисками и {linkto(../../../../../kubernetes/k8s/concepts/storage#k8s-storage)[text=дисками в кластерах Kubernetes]}.

  Блочные диски в {var(cloud)} могут быть сетевыми и локальными, различаться надежностью хранения, количеством копий реплицируемых данных и параметрами {linkto(../../../../../computing/iaas/concepts/data-storage/volume-sla#iaas-volume-sla)[text=производительности]}. Подробнее — в разделе о {linkto(../../../../../computing/iaas/concepts/data-storage/disk-types#iaas-disk-types)[text=дисках блочного хранения]}.

- _Объектное (object-based)_. Хранит данные в виде независимых объектов с метаданными. Подходит для хранения данных большого объема (резервные копии, BigData, большие медиафайлы и т.п.), к которым не нужно часто обращаться. В {var(cloud)} тип представлен хранилищем {linkto(../../../../../storage/s3#s3-main)[text=VK Object Storage]} с поддержкой S3 API.
- _Файловое (file-based)_. Виртуальная файловая система, которую можно подключить к нескольким виртуальным машинам. Реализует централизованный обмен файлами между клиентами, находящимися внутри приватной сети облака. Подходит для регулярных задач, требующих минимальной задержки при обращении к данным. Подробнее — в разделе о {linkto(../../../../../computing/iaas/concepts/data-storage/file-share#iaas-file-share)[text=файловом хранилище]}.
...
|Блочное
|Предоставляет интерфейс для чтения и записи блоков данных.
...
|Объектное
|Предоставляет интерфейс для доступа и управления объектами.
