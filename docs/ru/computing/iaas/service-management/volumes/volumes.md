...
Создание диска для конкретной ВМ описано в разделе [Управление ВМ](/ru/computing/iaas/service-management/vm/vm-manage#mount_disk).
...
1. [Увеличьте](#uvelichenie_razmera_diska_s_perezagruzkoy_vm) размер виртуального диска в личном кабинете VK Cloud или через Openstack CLI, но не перезагружайте ВМ. Это изменит размер диска, но не изменит размер разделов диска в ОС.
