# {heading(Политика привязки групп серверов)[id=iaas-managing-node]}
...
{var(cloud)} поддерживает {linkto(../../../../computing/iaas/concepts/server-group#iaas-server-group)[text=четыре политики распределения групп серверов]}, для `soft`-политик необходима поддержка версии `compute-api` выше 2.15.
