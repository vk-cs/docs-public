# {heading(Создание образа ВМ с ОС Windows)[id=iaas-win-image]}

{note:info}
Список ОС Windows, для которых поддерживается миграция в {var(cloud)}, приведен в разделе об {linkto(../../../../computing/iaas/concepts/oper-system#iaas-oper-system)[text=операционных системах]}.
{/note}

Создание и загрузка образа ВМ в {var(cloud)} рассмотрены на примере ОС Windows Server 2016 редакции Standard. Для подготовки образа используется компьютер с ОС Windows 11 Pro. Все команды выполняются в консоли Windows PowerShell.
...
## {heading(Подготовительные шаги)[id=iaas-win-image-preparatory-steps]}

1. Скачайте установочный ISO-образ ОС Windows, для которого планируется миграция в {var(cloud)}. Рекомендуется использовать en-US версию образа.
...
## {heading(1. Подготовьте инструментарий)[id=iaas-win-image-toolkit-prepare]}
...
## {heading(2. Создайте внешний виртуальный коммутатор в Hyper-V)[id=iaas-win-image-switch-create]}
...
## {heading(3. Подготовьте конфигурацию Windows Imaging Tools)[id=iaas-win-image-config-prepare]}
...
## {heading(4. Подготовьте установочный WIM-файл ОС)[id=iaas-win-image-file-prepare]}
...
## {heading(5. Соберите образ локально)[id=iaas-win-image-locally-assemble]}
...
1. Откройте созданный файл и отредактируйте параметры:
   ...
   {note:info}
   {var(cloud)} поддерживает загрузочные диски, размеченные для загрузки ОС с помощью BIOS. Разметка для загрузки через UEFI не поддерживается.
   {/note}
...
## {heading(6. Импортируйте образ в {var(cloud)})[id=iaas-win-image-import]}

{linkto(../../../../computing/iaas/instructions/images/images-manage#iaas-images-manage-import)[text=Воспользуйтесь OpenStack CLI]} для импорта образа:
...
openstack image create \
    ...
    <ИМЯ_ОБРАЗА>
```
Замените <ИМЯ_ОБРАЗА> на актуальное. Аргументы вида `--property <КЛЮЧ>=<ЗНАЧЕНИЕ>` используются для присвоения образу {linkto(../../../../computing/iaas/instructions/images/image-metadata#iaas-image-metadata)[text=метатегов]}.

Дождитесь завершения операции. После загрузки образа появится возможность {linkto(../../../../computing/iaas/instructions/vm/vm-create#iaas-vm-create)[text=создавать ВМ]} стандартными средствами {var(cloud)}.

## {heading(Удалите неиспользуемые ресурсы)[id=iaas-win-image-delete]}

Если импортированный образ вам больше не нужен, {linkto(../../../../computing/iaas/instructions/images/images-manage#iaas-images-manage-delete)[text=удалите его]}.