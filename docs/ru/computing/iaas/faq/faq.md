...
Освободите ресурсы на проекте или [увеличьте](/ru/tools-for-using-services/account/instructions/project-settings/manage#increase-quota) квоты.
...
{cut(Как мигрировать ВМ из VK Cloud на локальный гипервизор?)}

1. [Создайте](/ru/computing/iaas/instructions/images/images-manage#sozdanie_obraza) образ ВМ.
1. [Эспортируйте](/ru/computing/iaas/instructions/images/images-manage#eksport_obraza) образ.
...
Для рендеринга и стриминга 3D-графики подходят [ВМ с GPU](/ru/computing/gpu/concepts/about). Чтобы подборать оптимальную конфигурацию, обратитесь в [отдел продаж](https://cloud.vk.com/contacts/).
...
Подробнее про ISO-образы — в разделе [Создание образа ВМ с ОС Windows](/ru/computing/iaas/how-to-guides/win-image).
{/cut}

{cut(Как сделать копию ВМ с новым IP-адресом?)}

1. [Создайте](/ru/computing/iaas/instructions/volumes/volumes-snapshots) снимок диска ВМ.
1. [Создайте](/ru/computing/iaas/instructions/volumes/volumes-snapshots#use_snapshots) новую виртуальную машину из снимка.
...
Да, [импортируйте](/ru/computing/iaas/instructions/images/images-manage#import_obraza) образ с нужной ОС.

{/cut}

{cut(Почему мне отказали в увеличении квот?)}

Платформа имеет строгие правила использования. Если возникли подозрения, что ваша деятельность связана с маскированием трафика, проксированием или нарушением законодательства, это может привести к блокировке квот.
...
Полная информация доступна в статье [Управление дисками](../instructions/volumes/volumes-manage#change_disk_size).
...
Воспользуйтесь [инструкцией](../instructions/volumes/volumes-transfer).
...
Замена root-диска возможна, только если в проекте VK Cloud уже был создан другой диск. Это может быть как пустой диск, так и загрузочный, содержащий операционную систему. Инструкция приведена в статье о [замене root-диска](../instructions/vm/vm-root-replace).

{/cut}

{cut(Можно ли создать снимок диска?)}

Создание снимка диска доступно [в личном кабинете или через OpenStack CLI](../instructions/volumes/volumes-snapshots). Созданный снимок будет храниться до момента удаления самого диска.

{/cut}

{cut(Как восстановить диск из снимка?)}

Воспользуйтесь [инструкцией](../instructions/volumes/volumes-snapshots).
...
[Отключите диск от ВМ](../instructions/volumes/volumes-connect#dismount_disk), и кнопка станет доступна в контекстном меню диска и на странице самого диска.

{/cut}

{cut(Как загрузить диск из облака?)}

[Создайте](/ru/computing/iaas/instructions/images/images-manage#sozdanie_obraza) и [экспортируйте](/ru/computing/iaas/instructions/images/images-manage#eksport_obraza) образ.
...
Подробнее о создании снимков в статье [Работа со снимками](../instructions/fs-manage/fs-snapshots#creating_a_snapshot).
...
[Зона доступности](/ru/start/concepts/architecture#az) хранилища зависит от [региона](/ru/tools-for-using-services/account/concepts/regions) проекта:
...
Да, можно. О подключении файлового хранилища к ВМ в статье [Подключение и отключение файлового хранилища](../instructions/fs-manage/fs-connect).
...
Да, можно. Для этого [воспользуйтесь инструкцией](/ru/computing/iaas/instructions/vm/vm-lic-manage).
