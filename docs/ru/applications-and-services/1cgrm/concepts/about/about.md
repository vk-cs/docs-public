# {heading(О сервисе)[id=1cgrm-about]}

{var(cloud)} совместно с Фирмой 1С предлагает готовое инфраструктурное решение для вашего бизнеса — сервис 1С:Готовое рабочее место (1С:ГРМ). 

Сервис предоставляет удобный интерфейс для запуска инфраструктуры для продуктов 1С, развернутых в облачной инфраструктуре {var(cloud)}. Благодаря этому вы сможете использовать только нужные вам приложения 1С:ГРМ без необходимости самостоятельно разворачивать, обновлять и поддерживать 1C:ГРМ. {var(cloud)} обеспечивает отказоустойчивость, стабильную работу и безопасное хранение данных.

Работать с приложениями 1С:ГРМ в {var(cloud)} можно через браузер с любого устройства с подключением к интернету. Для тестирования функциональности приложений предоставляется бесплатный {linkto(../../../applications-and-services/1cgrm/tariffication#1cgrm-tariffication-demo-period)[text=пробный период]} на 30 дней, после чего оплата производится ежемесячно только за фактически использованные ресурсы. Количество лицензий можно увеличить по мере необходимости.

1С:ГРМ в {var(cloud)} сопровождается квалифицированной {linkto(#1cgrm-support)[text=технической поддержкой]}.

## {heading(Доступные приложения)[id=1cgrm-description]}

В 1С:ГРМ в {var(cloud)} вы можете добавить приложения:
...
## {heading(Режимы работы с приложениями)[id=1cgrm-modes]}
...
## {heading(Ограничения)[id=1cgrm-limitations]}
...
## {heading(Резервное копирование)[id=1cgrm-backup]}
...
## {heading(Поддержка)[id=1cgrm-support]}
...
{note:warn}
Не передавайте в техническую поддержку логин и пароль от портала 1С:ИТС и личного кабинета {var(cloud)}. Это может привести к потере данных. Однако поддержка может запросить логин и пароль пользователя приложения 1С с правами администратора.
{/note}
