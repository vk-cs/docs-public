# {heading(Установка и настройка сервиса Кибер Бэкап Облачный)[id=marketplace-cyber-backup-quick-start]}

Вы можете настроить резервное копирование и восстановление ваших виртуальных машин с помощью сервиса [Кибер Бэкап Облачный](https://msk.cloud.vk.ru/app/services/marketplace/v2/apps/service/630008e7-a649-4111-a21f-6d8461cd5f7a/latest/info).
...
1. Подключите в личном кабинете {var(cloud)} сервис Кибер Бэкап Облачный и создадите аккаунт в сервисе.
1. Зарегистрируете в сервисе ВМ, созданную в {var(cloud)}.
...
Используя сервис, вы соглашаетесь с [лицензионным соглашением](../../../../start/legal/offer/private-special-conditions/policy-marketplace) сервиса Marketplace и с [пользовательским соглашением](https://cyberprotect.ru/static/pdf/Cyberprotect_EULA_RUS.pdf) вендора ООО «Киберпротект».
...
1. Убедитесь, что ВМ, которая будет зарегистрирована в сервисе:
   ...
   - Доступна для подключения по SSH:
     ...
     - Ей назначена {linkto(../../../../networks/vnet/concepts/traffic-limiting#vnet-traffic-limiting-secgroups)[text=группа безопасности]} `ssh`.

       Если группа не назначена, {linkto(../../../../networks/vnet/instructions/secgroups#vnet-secgroups-instance-add-sg)[text=назначьте]} ее.

     - У вас есть приватный SSH-ключ из {linkto(../../../../computing/iaas/instructions/vm/vm-connect/vm-connect-nix#iaas-vm-connect-nix-key-pared)[text=ключевой пары]}, которая используется для доступа к ВМ. Имя ключевой пары отображается на странице ВМ в разделе **Облачные вычисления**.
...
## {heading(1. Подключите сервис Кибер Бэкап Облачный)[id=marketplace-cyber-backup-quick-start-connect-service]}
...
1. Дождитесь завершения установки.
   ...
   На почту, привязанную к аккаунту {var(cloud)}, придет письмо для активации аккаунта в сервисе Кибер Бэкап Облачный.

1. Перейдите по ссылке из письма. В окне регистрации задайте пароль для вашего аккаунта.

   Ваш логин будет совпадать с {linkto(../../../../tools-for-using-services/account/instructions/project-settings/manage#project-pid-view)[text=идентификатором (PID)]} проекта в {var(cloud)}, в котором вы подключили сервис.
...
## {heading(2. Установите на ВМ агент сервиса Кибер Бэкап Облачный)[id=marketplace-cyber-backup-quick-start-install-agent]}
...
1. Установите агент на ВМ `Ubuntu-CBC`.
   ...
   1. {linkto(../../../../computing/iaas/instructions/vm/vm-connect/vm-connect-nix#iaas-vm-connect-nix)[text=Подключитесь]} к ВМ, загрузите на нее дистрибутив агента сервиса для Linux.
   ...
   1. Запустите на ВМ дистрибутив, выполнив команду запуска со следующими параметрами:
      ...
      ```console
      sudo ./<ПУТЬ_К_ДИСТРИБУТИВУ_LINUX> -i BackupAndRecoveryAgent --rain=https://cloud.acronis-infoprotect.ru --token=<МАРКЕР_РЕГИСТРАЦИИ>
      ```

      Замените переменные:

      - `<ПУТЬ_К_ДИСТРИБУТИВУ_LINUX>` — полный путь к дистрибутиву, включая имя файла и расширение, например: `ubuntu/Backup_AgentForLinux_x86_64.bin`.
      - `<МАРКЕР_РЕГИСТРАЦИИ>` — маркер, который вы сгенерировали в консоли Кибер Бэкап Облачный.
...
## {heading(3. Создайте план защиты для ВМ и первую резервную копию)[id=marketplace-cyber-backup-quick-start-create-plan]}
...
## {heading(4. Удалите данные с ВМ)[id=marketplace-cyber-backup-quick-start-delete-data]}

{linkto(../../../../computing/iaas/instructions/vm/vm-connect/vm-connect-nix#iaas-vm-connect-nix)[text=Подключитесь]} к ВМ `Ubuntu-CBC` и удалите из вашей домашней директории файл `cp_help.txt`.

## {heading(5. Восстановите ВМ из резервной копии)[id=marketplace-cyber-backup-quick-start-restore-vm]}
...
Данные на исходной ВМ в облаке {var(cloud)} будут восстановлены, ВМ будет перезагружена. Все изменения, которые происходили на ВМ с момента последнего применения плана защиты, будут потеряны. Имя ВМ в {var(cloud)} после восстановления не изменится.

## {heading(6. Проверьте работоспособность восстановленной ВМ)[id=marketplace-cyber-backup-quick-start-check-vm]}

1. {linkto(../../../../computing/iaas/instructions/vm/vm-connect/vm-connect-nix#iaas-vm-connect-nix)[text=Подключитесь]} к ВМ `Ubuntu-CBC`.
1. Проверьте наличие файла `cp_help.txt` в домашней директории.
1. {linkto(../../../../computing/iaas/instructions/vm/vm-manage#iaas-vm-manage)[text=Выполните]} произвольные операции с ВМ.
