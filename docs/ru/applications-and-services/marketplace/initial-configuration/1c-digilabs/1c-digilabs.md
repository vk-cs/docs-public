...
Для проверки работоспособности системы 1С в облачный кластер 1С «из коробки» будет установлена демонстрационная ограниченная лицензия.

После проверки установите собственную лицензию или приобретите ее у партнера [Дигилабс](https://digilabs.ru).
...
1. Проверьте, что предоставленных в проекте квот на вычислительные ресурсы хватает для развертывания нужного тарифного плана с учетом количества потенциальных пользователей:
...
1. (Опционально) [Увеличьте](/ru/tools-for-using-services/account/instructions/project-settings/manage#increase-quota) квоты на вычислительные ресурсы, необходимые для подключения тарифа.
1. Проверьте доступность нужных конфигураций виртуальных машин.
...
1. Выберите [тариф](#preparatory_steps).
1. Задайте параметры на странице настройки сервиса:
   ...
   - **Название базы 1С**: укажите название информационной базы 1С, которая будет создана на облачной инфраструктуре. В названии можно использовать только строчные латинские буквы и цифры. Название не должно начинаться с цифры.
   ...
   Некоторые конфигурации ВМ могут быть недоступны вам. Список таких конфигураций появится в сообщении об ошибке. Чтобы получить доступ к ним, обратитесь в [техническую поддержку](/ru/contacts).
...
1. Пройдите в письме по одноразовой ссылке и скопируйте данные для импорта информационной базы 1С в удаленное хранилище VK Object Storage:
   ...
   - `s3_access_key` — идентификатор ключа доступа к VK Object Storage через [AWS CLI](/ru/tools-for-using-services/cli/aws-cli).
   - `s3_name` — имя бакета в хранилище VK Object Storage.
   - `s3_secret_key` — секретный ключ доступа к VK Object Storage через [AWS CLI](/ru/tools-for-using-services/cli/aws-cli).
...
1. Разверните на локальных машинах подготовленный для вас образ виртуальной машины:
...
1. Настройте VPN-соединение на локальных машинах:
...
1. Установите и настройте приложение AWS CLI.
   ...
   1. Введите данные, полученные в письме:
      ...
      Команда создаст файл `C:\Users\<ИМЯ_ПОЛЬЗОВАТЕЛЯ>\.aws\credentials` и сохранит в нем ключи доступа, чтобы их не приходилось вводить заново при каждом обращении к AWS.

      {/note}

1. Загрузите файл резервной копии базы 1С в удаленное хранилище VK Object Storage, выполнив команду:
    ...
    - `<ИМЯ_БАКЕТА>` — имя бакета в хранилище VK Object Storage, полученное в письме.
...
1. Запустите развертывание базы 1С из резервной копии в хранилище VK Object Storage.
...
1. Загрузите файл резервной копии базы 1С в удаленное хранилище VK Object Storage, выполнив команду:
      ...
      У файла резервной копии базы 1С в хранилище VK Object Storage будет то же имя, что и на вашем компьютере.

1. Запустите развертывание базы 1С из резервной копии в удаленном хранилище VK Object Storage.
   ...
   1. Создайте файл скрипта `start_import_linux.sh` со следующим содержимым:
      ...
      #!/bin/bash
      read -p "Введите имя файла бэкапа, загруженного в VK Object Storage: " s3_backup_path
   ...
   1. Введите запрашиваемые данные:
      ...
      - **Введите имя файла бэкапа, загруженного в VK Object Storage**: имя резервной копии базы (путь указывать не нужно).
...
## 4. Подключите облачную базу 1С к клиентскому приложению системы 1С:Предприятие
...
1. Установите на локальные машины клиентское приложение системы 1С:Предприятие.
