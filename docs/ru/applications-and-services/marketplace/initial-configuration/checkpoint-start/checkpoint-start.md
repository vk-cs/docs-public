# {heading(Защита инфраструктуры с помощью Check Point CloudGuard Network)[id=marketplace-checkpoint-start]}
...
Используя сервис Check Point CloudGuard Network, вы соглашаетесь с лицензионным соглашением [магазина приложений](../../../../start/legal/offer/private-special-conditions/policy-marketplace), а также с [лицензионным соглашением Check Point](https://www.checkpoint.com/support-services/software-license-agreement-limited-hardware-warranty).
...
## {heading(Подготовительные шаги)[id=marketplace-checkpoint-start-preparatory-steps]}
...
1. (Опционально) {linkto(../../../../networks/vnet/instructions/net#vnet-net-add)[text=Создайте]} сеть с доступом в интернет, если она не была создана ранее.

## {heading(1. Разверните сервис Check Point CloudGuard Network)[id=marketplace-checkpoint-start-install-service]}
...
1. Разверните шлюз **Security Gateway BYOL**:
   ...
   1. Задайте параметры шлюза на странице настройки сервиса:
      ...
      - **Сеть**: выберите ранее созданную сеть с доступом в интернет или создайте новую, указав ее {linkto(../../../../networks/vnet/concepts/sdn#vnet-sdn)[text=тип]} и адрес подсети.
...
1. Разверните сервер управления **Security Management BYOL**:
   ...
   1. Задайте параметры сервера управления на странице настройки сервиса:
      ...
      - Остальные параметры выберите в соответствии с {linkto(#marketplace-checkpoint-start-preparatory-steps)[text=рекомендациями]} или оставьте значения по умолчанию.
...
## {heading(2. Настройте виртуальный шлюз)[id=marketplace-checkpoint-start-setup-gateway]}
...
## {heading(3. Настройте сервер управления)[id=marketplace-checkpoint-start-setup-management]}
...
## {heading(4. Увеличьте размеры логических томов ВМ шлюза и севера управления)[id=marketplace-checkpoint-start-resize-volumes]}
...
## {heading(5. Настройте сетевой доступ к шлюзу)[id=marketplace-checkpoint-start-configure-net-access-gateway]}
...
1. Подключитесь к виртуальному шлюзу через веб-интерфейс GAiA Portal и завершите настройку шлюза с помощью мастера настройки First Time Configuration Wizard:
   ...
   1. Перейдите по адресу `https://<ВНЕШНИЙ_IP-АДРЕС>:443`, где `<ВНЕШНИЙ_IP-АДРЕС>` — внешний IP-адрес шлюза, указанный в инструкции по настройке шлюза.
   1. Введите логин, полученный в инструкции по настройке шлюза, и пароль, установленный при {linkto(#marketplace-checkpoint-start-setup-gateway)[text=настройке]}.
...
Теперь вы можете подключаться к ВМ шлюза с помощью веб-интерфейса GAiA Portal по адресу `https://<ВНЕШНИЙ_IP-АДРЕС>` (или по SSH `<ВНЕШНИЙ_IP-АДРЕС>:22`).

## {heading(6. Настройте сетевой доступ к серверу управления)[id=marketplace-checkpoint-start-configure-net-access-management]}
...
1. Подключитесь к серверу управления через веб-интерфейс GAiA Portal и завершите настройку с помощью мастера настройки First Time Configuration Wizard:
   ...
   1. Перейдите по адресу `https://<ВНЕШНИЙ_IP-АДРЕС>:443`, где `<ВНЕШНИЙ_IP-АДРЕС>` — внешний IP-адрес севера управления, указанный в инструкции по настройке сервера управления.
   1. Введите логин, полученный в инструкции по настройке сервера управления, и пароль, установленный при {linkto(#marketplace-checkpoint-start-setup-management)[text=настройке]}.
...
Теперь вы можете подключаться к ВМ сервера управления с помощью веб-интерфейса GAiA Portal по адресу `https://<ВНЕШНИЙ_IP-АДРЕС>` (или по SSH `<ВНЕШНИЙ_IP-АДРЕС>:22`), где `<ВНЕШНИЙ_IP-АДРЕС>` — внешний IP-адрес севера управления, указанный в инструкции по настройке сервера управления.

## {heading(7. Добавьте сетевые интерфейсы)[id=marketplace-checkpoint-start-add-interface]}
...
1. {linkto(../../../../computing/iaas/instructions/vm/vm-add-net#iaas-vm-add-net-connect)[text=Подключите]} нужную сеть к ВМ шлюза и {linkto(../../../../networks/vnet/instructions/secgroups#vnet-secgroups-add-rule)[text=добавьте]} правила в группу безопасности для этой сети.
...
## {heading(8. Настройте политику безопасности)[id=marketplace-checkpoint-start-settings-policy]}
...
1. [Подключите](https://sc1.checkpoint.com/documents/R81.20/WebAdminGuides/EN/CP_R81.20_SecurityManagement_AdminGuide/Content/Topics-SECMG/Creating-New-Security-Gateway.htm?tocpath=Managing%20Gateways%7C_____1) шлюз к серверу управления, указав:
   ...
   - **Name**: имя ВМ шлюза, заданное в параметре `hostname` во время {linkto(#marketplace-checkpoint-start-setup-gateway)[text=настройки шлюза]}.
   - **IPv4 Address**: внутренний IP-адрес шлюза, заданный в параметре `<ВНУТРЕННИЙ_IP>` во время {linkto(#marketplace-checkpoint-start-setup-gateway)[text=настройки шлюза]}.

   На этапе подключения введите SIC-пароль, который вы указали во время {linkto(#marketplace-checkpoint-start-configure-net-access-gateway)[text=настройки сетевого доступа к шлюзу]}.
...
## {heading(9. Предусмотрите резервное копирование)[id=marketplace-checkpoint-start-backup]}

{note:warn}
Резервное копирование и восстановление инстансов сервиса Check Point CloudGuard Network средствами {var(cloud)} не поддерживается.
{/note}
...
## {heading(Удалите неиспользуемые ресурсы)[id=marketplace-checkpoint-start-delete]}

Работающая инфраструктура сервиса потребляет вычислительные ресурсы. Если она вам больше не нужна:

- {linkto(../../../../applications-and-services/marketplace/instructions/pr-instance-manage#marketplace-pr-instance-manage-delete)[text=Удалите]} инстанс сервиса Check Point CloudGuard Network.
- {linkto(../../../../computing/iaas/instructions/vm/vm-manage#iaas-vm-manage-delete)[text=Удалите]} ВМ, созданные в качестве шлюза и сервера управления.
