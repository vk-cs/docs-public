# {heading(Установка и настройка сервиса WordPress)[id=marketplace-wordpress-start]}
...
1. Подключите в личном кабинете {var(cloud)} сервис WordPress.
...
{note:info}
{var(cloud)} предоставляет доменное имя для вашего WordPress-сайта. Чтобы перенести сайт в новый домен, воспользуйтесь {linkto(../../../../applications-and-services/marketplace/initial-configuration/wordpress-domain#marketplace-wordpress-domain)[text=инструкцией]}.
{/note}

## {heading(1. Подключите сервис WordPress)[id=marketplace-wordpress-start-connect]}
...
1. В [личном кабинете {var(cloud)}](https://msk.cloud.vk.com/app) перейдите в раздел **Магазин приложений**.
...
1. Введите данные на странице **Параметры сервера**.
   ...
   1. Включите мониторинг, чтобы автоматически отправлять в сервис {linkto(../../../../monitoring-services/monitoring#monitoring)[text=Cloud Monitoring]} метрики виртуальной машины, на которой развернут WordPress. Эти метрики будут доступны в разделе **Мониторинг** личного кабинета.
   ...
   1. Включите резервное копирование на S3, чтобы ежедневно создавать резервные копии данных WordPress в объектном хранилище {var(cloud)}. Хранилище создается автоматически. Сохраняются последние 7 копий.
   1. Нажмите кнопку **Следующий шаг**.
...
## {heading(2. Настройте ваш веб-сайт и загрузите на него первый контент)[id=marketplace-wordpress-start-settings]}
...
## {heading(3. (Опционально) Узнайте о дополнительных возможностях WordPress)[id=marketplace-wordpress-start-opportunities]}

{cut(Модульный подход к редактированию контента)}
...
{cut(Добавление на сайт категорий и тегов)}
...
{cut(Управление ролями и правами доступа пользователей сайта)}
...
{cut(Добавление на сайт виджетов)}
...
{cut(Создание мультимедийных библиотек и галерей)}
...
{cut(Настройка резервного копирования сайта)}
...
## {heading(Удаление неиспользуемых ресурсов)[id=marketplace-wordpress-start-delete]}

Для сервиса WordPress создается инфраструктура, которая потребляет вычислительные ресурсы. Если сервис вам больше не нужен, {linkto(../../../../applications-and-services/marketplace/instructions/pr-instance-manage#marketplace-pr-instance-manage-delete)[text=удалите]} инстанс сервиса. Его инфраструктура также будет удалена.
