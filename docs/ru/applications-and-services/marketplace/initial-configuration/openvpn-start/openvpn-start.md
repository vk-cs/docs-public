...
Используя сервис OpenVPN, вы соглашаетесь с лицензионными соглашениями сервисов [Marketplace](../../../../start/legal/offer/private-special-conditions/policy-marketplace) и [OpenVPN](https://openvpn.net/legal).
...
1. {linkto(../../../../applications-and-services/marketplace/instructions/pr-instance-add#marketplace-pr-instance-add)[text=Разверните]} сервис OpenVPN:
...
1. Подключитесь к ВМ сервиса по SSH. Для этого выполните команду в терминале:
   ...
   ```console
   ssh -i <КЛЮЧ_ДОСТУПА> ubuntu@<IP-АДРЕС_ВМ>
   ```

   Здесь:

   - `<КЛЮЧ_ДОСТУПА>` — ключ доступа, полученный по одноразовой ссылке;
   - `<IP-АДРЕС_ВМ>` — внешний IP-адрес ВМ, полученный по одноразовой ссылке.

1. Получите права root-пользователя (команда `sudo bash`).
1. Создайте нового пользователя. Выполните команду:

   ```console
   /home/ubuntu/create_client.sh <ИМЯ_ПОЛЬЗОВАТЕЛЯ>
   ```
...
1. (Опционально) Заблокируйте пользователя. Выполните команду:
   ...
   ```console
   /home/ubuntu/revoke_client.sh <ИМЯ_ПОЛЬЗОВАТЕЛЯ>
   ```

   После успешного завершения скрипта обновится сертификат `crl.pem`, а сервис OpenVPN перезапустится. Доступ для пользователя `<ИМЯ_ПОЛЬЗОВАТЕЛЯ>` будет отозван.
