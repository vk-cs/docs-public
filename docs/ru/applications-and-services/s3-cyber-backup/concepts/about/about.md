# {heading(О сервисе)[id=s3-cyber-backup-about]}

Шлюз S3 позволяет сервису {linkto(../../../../applications-and-services/marketplace/initial-configuration/cyber-backup-quick-start#marketplace-cyber-backup-quick-start)[text=Кибер Бэкап Облачный]} использовать публичный облачный сервис {linkto(../../../../storage/s3#s3-main)[text=VK Object Storage]} для хранения резервных копий. Это обеспечивает возможность доступа к резервным копиям через интернет.
...
- Виртуальные диски.
- Бакет VK Object Storage для хранения резервных копий.
...
Резервные копии представляют собой холодные данные со специфической схемой доступа: к этим данным обращаются редко, но они должны быть немедленно доступны при обращении. Для хранения резервных копий в {linkto(../../../../storage/s3#s3-main)[text=VK Object Storage]} используются бакеты с классом хранения Hotbox и Icebox.
...
В процессе развертывания шлюза в вашем проекте VK Cloud будут созданы необходимые облачные ресурсы, а также настроены учетные записи, тенанты и {linkto(../../../../tools-for-using-services/account/concepts/quotasandlimits#tools-account-concepts-quotasandlimits)[text=квоты]}.
