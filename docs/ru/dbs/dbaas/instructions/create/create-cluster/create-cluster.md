...
1. На шаге **Параметры**:
   ...
   1. Задайте общие параметры кластера:
      ...
      - **Сеть**: сеть, в которой будет размещаться кластер. Если нужной сети нет в списке, [создайте ее](/ru/networks/vnet/instructions/net#sozdanie_seti).
      - **Ключ для доступа по SSH**: выберите существующий ключ или создайте новый.

        Ключ используется для [подключения к хостам кластера по SSH](/ru/computing/iaas/instructions/vm/vm-connect/vm-connect-nix).

   1. Настройте резервное копирование. При необходимости параметры резервного копирования можно [задать](/ru/storage/backups/instructions/create-backup-plan#create_db_backup_plan) или [изменить](/ru/storage/backups/instructions/manage-backup-plan#edit_backup_plan) после создания кластера.
      ...
      - При необходимости включите резервное копирование по [стратегии GFS](/ru/storage/backups/concepts/gfs-backup) и настройте параметры хранения.
...
1. На шаге **Ноды**:
   ...
   1. В блоке **Dashboards** выберите опцию **Включить Dashboards**, если необходим доступ к веб-интерфейсу [OpenSearch Dashboards](https://opensearch.org/docs/latest/dashboards/quickstart-dashboards/).
      ...
      - **Зона доступности**: [зона доступности](/ru/start/concepts/architecture#az) для кластера.

      - **Тип диска**: [тип диска](/ru/computing/iaas/concepts/data-storage/disk-types#disk_types) для кластера.
...
1. На шаге **Конфигурация**:
   ...
   1. Выберите конфигурацию:
      ...
      - **Мультизональный кластер** — для создания кластера с [межзональной репликацией](../../../concepts/work-configs#multi-az).
...
1. На шаге **Создание инстанса**:
   ...
   1. Задайте общие параметры кластера:
      ...
      - **Настройки Firewall**: список групп безопасности для кластера.
        ...
        Добавьте в список группу безопасности `ssh`, чтобы получить возможность [подключаться к хостам кластера по SSH](/ru/computing/iaas/instructions/vm/vm-connect/vm-connect-nix).
      ...
      - **Зона доступности Master**: [зона доступности](/ru/start/concepts/architecture#az) master-узла кластера.
      ...
      - **Количество синхронных реплик** и **Количество асинхронных реплик**: выберите количество синхронных и асинхронных реплик, которые будут автоматически распределены по другим зонам.
          ...
          Это необходимо для корректной работы алгоритма согласования данных между узлами [мультизонального кластера](/ru/dbs/dbaas/concepts/work-configs#multi-az).
...
1. На шаге **Шарды** задайте настройки шардов.
   ...
   1. Для каждого шарда задайте:
      ...
      - **Зона доступности**: [зона доступности](/ru/start/concepts/architecture#az) для хостов шарда.
      ...
      - **Тип диска**: [тип диска](/ru/computing/iaas/concepts/data-storage/disk-types#disk_types) для хостов шарда.
