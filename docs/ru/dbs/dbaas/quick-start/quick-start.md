# {heading(Быстрый старт)[id=dbaas-quick-start]}
...
1. Научитесь подключаться к инстансу, как для {linkto(#instance_logs)[text=просмотра логов]}, так и для {linkto(#db_connect)[text=выполнения SQL-запросов]}.
