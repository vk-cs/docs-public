...
Допустим, у нас в наличии кластер ClickHouse, например с двумя шардами — в каждом по две реплики. Имя кластера ClickHouse в VK Cloud всегда `cluster`. Выполним запрос:
