# {heading(Использование расширения PostGIS в PostgreSQL)[id=dbaas-using-postgis]}

### {heading(Как построить пространственный запрос?)[id=dbaas-using-postgis-query]}
...
### {heading(Как вставить объект ГИС в базу данных?)[id=dbaas-using-postgis-put-object]}
