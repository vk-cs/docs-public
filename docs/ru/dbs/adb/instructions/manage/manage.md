# {heading(Управление кластером)[id=adb-manage]}

## {heading(Масштабирование кластера)[id=adb-manage-scaling]}
...
1. Откройте страницу масштабирования кластера одним из способов:
   ...
   - Нажмите ![](../../../../assets/more-icon.svg "inline") для нужного инстанса АБД и выберите пункт **Масштабировать кластер**.
...
{note:info}
Поддерживается автоматическое масштабирование дисков. Чтобы включить его, при {linkto(../../quick-start/create-adb#adb-create-adb)[text=создании]} кластера активируйте опцию **Автомасштабирование дисков**.
{/note}

## {heading(Удаление кластера)[id=adb-manage-delete]}
...
1. [Перейдите](https://msk.cloud.vk.com/app/) в личный кабинет {var(cloud)}.
1. Перейдите в раздел **Аналитические БД** → **Инстансы АБД**.
1. Нажмите ![](../../../../assets/more-icon.svg "inline") для нужного кластера и выберите пункт **Удалить кластер**.
