# {heading(Запуск, подключение и загрузка данных)[id=adb-launch-adb]}

## {heading(Подключение к инстансу)[id=adb-launch-adb-connect]}

Для подключения к инстансу ADB, воспользуйтесь {linkto(../../../dbaas/connect#dbaas-connect)[text=инструкцией по подключению к инстансу базы данных]}.

## {heading(Загрузка данных)[id=adb-launch-adb-data-download]}
