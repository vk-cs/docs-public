# {heading(Медленная загрузка больших файлов через AWS CLI)[id=s3-slow-load-problem]}
...
При передаче файлов по одному TCP-соединению проблема может возникать из-за высокой задержки сети (latency). Чтобы максимально использовать пропускную способность интернет-канала, загружайте файл одновременно в несколько потоков. Для этого используйте {linkto(../../concepts/features#s3-concepts-features-object-uploading)[text=составную загрузку]} и выберите наиболее подходящие для вашей ситуации количество одновременно загружаемых частей файла и их размер:
...
### {heading(Решение)[id=s3-slow-load-problem-resolve]}
