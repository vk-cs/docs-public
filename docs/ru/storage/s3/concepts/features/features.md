...
- набор инструментов для [управления доступом](../access) к бакетам и объектам;
...
- Переименовывать объекты.
  ...
  Прямое переименование объектов в бакете не поддерживается. Обойти это ограничение можно, создав копию объекта с новым именем и затем удалив исходный объект. В [файловых менеджерах](../../connect/s3-file-managers) с графическим интерфейсом эти действия выполняются автоматически.
