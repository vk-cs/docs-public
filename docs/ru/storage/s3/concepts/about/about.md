...
В проекте можно создать [несколько бакетов](/ru/tools-for-using-services/account/concepts/quotasandlimits#cloud_storage_limits), но нельзя вложить один бакет в другой. Внутри каждого бакета можно построить иерархию директорий. Для этого к именам объектов нужно добавить [префиксы](#object_key), которые Cloud Storage интерпретирует как имена директорий.
