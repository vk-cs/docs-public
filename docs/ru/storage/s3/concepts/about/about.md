Object Storage — это [объектное хранилище данных](/ru/computing/iaas/concepts/data-storage/storage-types) на платформе VK Cloud, основанное на протоколе S3. Оно подходит для хранения данных большого объема, к которым нечасто обращаются. Работать с Object Storage можно в личном кабинете VK Cloud, с помощью интерфейса командной строки AWS CLI, а также ряда [других инструментов](#instruments).

Object Storage отличает:
...
Данные в Object Storage хранятся в [бакетах](#bucket) в виде независимых [объектов](#object) и их [метаданных](#metadata).
...
В проекте можно создать [несколько бакетов](/ru/tools-for-using-services/account/concepts/quotasandlimits#object_storage_limits), но нельзя вложить один бакет в другой. Внутри каждого бакета можно построить иерархию директорий. Для этого к именам объектов нужно добавить [префиксы](#object_key), которые Object Storage интерпретирует как имена директорий.
...
Объекты в Object Storage хранятся в [бакетах](#bucket). В зависимости от размера файла используется [стандартная или составная загрузка](../features#object_uploading) объекта в бакет.
...
- предоставление доступа к объекту по [подписанной ссылке](../../instructions/access-management/signed-url);
...
Если в ключе указаны префиксы к имени объекта, Object Storage будет использовать их как имена директорий в бакете. При выборе имени объекта и префиксов рекомендуется следовать приведенным ниже правилам.
...
На основе префиксов `Development/` и `Finance/` Object Storage создаст директории в бакете. Открыв директорию `Development`, вы увидите в ней объект `Projects.xlsx`, в директории `Finance` найдете объекты `statement1.pdf` и `taxdocument.pdf`. Имя `s3-dg.pdf` не имеет префикса, поэтому этот объект будет размещаться на корневом уровне бакета.

{/cut}

## {heading(Метаданные объекта)[id=metadata]}

У каждого объекта Object Storage есть *метаданные*. Это пары ключ/значение, описывающие свойства объекта. Различают метаданные, присваиваемые системой (например: класс хранения, дата создания), и пользовательские метаданные. Некоторые системные метаданные доступны пользователю для управления, в частности класс хранения объекта.
...
Кроме личного кабинета VK Cloud для работы с объектным хранилищем Object Storage доступны следующие инструменты:
...
Классы хранения отличаются назначением и [стоимостью хранения](https://cloud.vk.com/pricelist) данных и доступны для бакетов, объектов и составных загрузок. В VK Cloud поддерживается несколько классов хранения:
