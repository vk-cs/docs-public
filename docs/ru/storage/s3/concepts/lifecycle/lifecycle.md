...
- Каждое правило жизненного цикла включает в себя:
  ...
  - Время жизни объекта (параметр `Expiration`). Определяет количество дней, через которое объект будет удален.
...
- в блоке `Expiration` из элементов `Days`, `ExpiredObjectDeleteMarker` оставить только один и удалить остальные;
...
- Текущая версия маркер удаления — ничего не происходит. Однако если нет других версий и вместо `Days` используется `ExpiredObjectDeleteMarker`, маркер удаления будет удален.
...
Задается с помощью элементов конфигурации `Days` или `ExpiredObjectDeleteMarker`.
...
Несовместимо с элементом конфигурации `Rules[i]` → `Expiration` → `ExpiredObjectDeleteMarker`.
...
Несовместимо с элементом конфигурации `Rules[i]` → `Expiration` → `Days`.
