...
В режиме `GOVERNANCE` пользователь, обладающий {linkto(../access/s3-acl#s3-concepts-acl-permissons)[text=правами на запись]} (`WRITE`), может обойти блокировку:
...
Бессрочная блокировка не имеет срока истечения и действует, пока не будет снята явно. Такую блокировку может в любой момент применить к объекту или снять с объекта пользователь, обладающий {linkto(../access/s3-acl#s3-concepts-acl-permissons)[text=правами на запись]} (`WRITE`).
