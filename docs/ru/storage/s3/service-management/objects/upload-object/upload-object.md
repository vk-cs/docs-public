...
При загрузке объекта ему назначается [ключ](#object_key_rules): комбинация имени объекта и имен директорий, в которых находится объект (при наличии).

<err>

Если ключ загружаемого файла совпадает с ключом объекта в бакете, Cloud Storage заменит существующий объект на новый.

</err>

## {heading(Рекомендации по созданию ключа объекта)[id=object_key_rules]}

Ключ объекта — это последовательность символов Юникод, длина которой в кодировке UTF-8 не более 1024 байт.

При создании ключа вы можете указать префиксы к имени объекта, которые Cloud Storage будет использовать как имена директорий в бакете. Префиксы отделяются друг от друга и от имени объекта разделителем `/`.
...
- В ключе объекта рекомендуется использовать латинские буквы, цифры и спецсимволы `/`, `!`, `-`, `_`, `.`, `*`, `'`, `(`, `)`.
...
Префиксы имени объекта (`Development/` и `Finance/`) будут использованы как имена директорий в бакете. Открыв директорию `Development`, вы увидите в ней объект `Projects.xlsx`, в директории `Finance` найдете объекты `statement1.pdf` и `taxdocument.pdf`. Имя `s3-dg.pdf` не имеет префикса, поэтому этот объект будет размещаться на корневом уровне бакета.
...
1. (Опционально) Добавьте папку для хранения объекта:
   ...
   1. Введите имя папки. Следуйте [рекомендациям](#object_key_rules) при выборе имени папки, так как оно станет частью ключа объекта.
...
1. Откройте консоль и выполните команду:
   ...
   Здесь:
      ...
      - `<КЛЮЧ_ОБЪЕКТА>` — имя объекта и имена директорий, в которых он будет находиться. Следуйте [рекомендациям](#object_key_rules) при выборе имен.
      ...
      - (Опционально) `<НАСТРОЙКА_ACL>` — задает [настройки ACL](../../access-management/s3-acl#fiksirovannyy_acl). Доступные значения:
...
1. Инициируйте составную загрузку. Откройте консоль и выполните команду:
   ...
   - `<КЛЮЧ_ОБЪЕКТА>` — имя объекта, для создания которого инициируется составная загрузка, и имена директорий, в которых он будет находиться. Следуйте [рекомендациям](#object_key_rules) при выборе имен.
