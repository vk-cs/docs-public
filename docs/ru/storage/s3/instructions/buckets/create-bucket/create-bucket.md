Чтобы загружать объекты в хранилище, предварительно нужно создать [бакет](../../../concepts/about#bucket) для их размещения.
...
1. Выберите [класс хранения](../../../concepts/about#storage_class). Вы сможете [изменить его](../../change-storage-class) после создания бакета.
1. (Опционально) Включите [версионирование объектов](/ru/storage/s3/concepts/versioning) в бакете.
1. (Опционально) Настройте параметры [блокировки объектов](../../../concepts/objects-lock) от удаления и перезаписи:

   1. Включите опцию **Блокировка объектов**, в этом случае версионирование объектов будет подключено автоматически.

      {note:warn}
      Опцию **Блокировка объектов** можно подключить только в момент создания бакета, позднее это сделать невозможно. Отключить эту опцию после создания бакета нельзя.
      {/note}
   
   1. (Опционально) Настройте параметры автоматической блокировки для загружаемых в бакет новых версий объектов:

      1. Включите опцию **Блокировать объекты по умолчанию**.
      1. Выберите режим защиты:

         - `Governance`: [управляемый режим блокировки](../../../concepts/objects-lock#governance-lock) с возможностью ее обхода.
         - `Compliance`: [строгий режим блокировки](../../../concepts/objects-lock#compliance-lock) без возможности ее снятия до истечения установленного срока.

      1. Выберите срок хранения версий объектов в днях.

   1. Подтвердите включение блокировки.
...
1. Откройте консоль и выполните одну из команд в зависимости от того, нужно ли включить для бакета возможность устанавливать [блокировку объектов](/ru/storage/s3/concepts/objects-lock):
   ...
   - `<URL_СЕРВИСА>` — домен сервиса VK Object Storage, должен соответствовать [региону](../../../../../tools-for-using-services/account/concepts/regions) аккаунта:
