Для работы с VK Object Storage через AWS CLI, API и файловые менеджеры нужны ключи доступа. Чтобы получить ключи, создайте аккаунт VK Object Storage, к которому они будут привязаны. Для каждого аккаунта доступно добавление двух одновременно действующих ключей.
