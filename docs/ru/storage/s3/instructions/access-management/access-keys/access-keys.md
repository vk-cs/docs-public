# {heading(Аккаунт {var(s3)} и его ключи доступа)[id=s3-instructions-access-keys]}

Для работы с {var(s3)} через AWS CLI, API и файловые менеджеры нужен ключ доступа. Чтобы получить ключ, создайте {linkto(#s3-instructions-access-keys-new-account-access-key)[text=аккаунт {var(s3)}]}, к которому он будет привязаны.

{note:warn}
Ключи, привязанные к аккаунту, дают доступ ко всем бакетам аккаунта. Если нужно предоставить доступ только к одному бакету или части объектов в нем, {linkto(../bucket-keys#s3-instructions-bucket-keys-add)[text=добавьте]} в этот бакет отдельные ключи доступа.
{/note}

## {heading(Создание аккаунта и ключа доступа)[id=s3-instructions-access-keys-new-account-access-key]}
...
## {heading(Добавление ключа доступа в существующий аккаунт)[id=s3-instructions-access-keys-existing-account-access-key]}

Если для аккаунта создано два ключа, добавление новых недоступно. {linkto(#s3-instructions-access-keys-delete-access-key)[text=Удалите]} один из ключей, чтобы добавить новый.
...
1. Добавьте ключ одним из способов:
   ...
   Откроется окно, которое содержит идентификатор ключа (**Access Key ID**) и секретный ключ (**Secret Key**).

1. Скопируйте значение поля **Secret Key** и сохраните его на своем устройстве. После закрытия окна восстановить секретный ключ будет невозможно.

## {heading(Деактивация ключа доступа)[id=s3-instructions-access-keys-deactivate-access-key]}
...
## {heading(Удаление ключа доступа)[id=s3-instructions-access-keys-delete-access-key]}
...
## {heading(Удаление аккаунта)[id=s3-instructions-access-keys-delete-account]}
