...
ACL для бакета не распространяются на ACL объекта.
...
1. В консоли выполните команду:
   ...
   Пример файла конфигурации ACL в формате JSON для выдачи прав на чтение (`READ`) метагруппе `AllUsers`:
...
1. В консоли выполните команду:
   ...
   Пример файла конфигурации ACL в формате JSON для выдачи прав на чтение (`READ`) всем авторизированным пользователям {var(s3)} {ifdef(public)}во всех проектах {var(cloud)}{/ifdef}{ifdef(s3,s3-pdf)}инсталляции {var(s3)}{/ifdef}:
