...
Если ключ загружаемого файла совпадает с ключом объекта в бакете и этот объект не [защищен от перезаписи](../../../concepts/features#object_lock), Cloud Storage заменит существующий объект на новый.
...
1. Откройте консоль и выполните команду:
   ...
   Здесь:
      ...
      - `<КЛЮЧ_ОБЪЕКТА>` — имя объекта и имена директорий, в которых он будет находиться. Следуйте [рекомендациям](../../../concepts/about#object_key_rules) при выборе имен.
...
1. Инициируйте составную загрузку. Откройте консоль и выполните команду:
   ...
   - `<КЛЮЧ_ОБЪЕКТА>` — имя объекта, для создания которого инициируется составная загрузка, и имена директорий, в которых он будет находиться. Следуйте [рекомендациям](../../../concepts/about#object_key_rules) при выборе имен.
