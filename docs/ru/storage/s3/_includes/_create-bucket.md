...
1. Убедитесь, что {linkto(../../../tools-for-using-services/cli/aws-cli#tools-cli-aws)[text=AWS CLI]} установлен.
...
1. Нажмите кнопку **Создать**.
{includetag(create-bucket)}
1. Введите имя бакета, соответствующее {linkto(../../../../storage/s3/concepts/about#s3-concepts-about-bucket-naming)[text=рекомендуемым правилам]}.
{/includetag}
...
{includetag(create-bucket)}
1. Выберите {linkto(../../../../storage/s3/concepts/about#s3-concepts-about-storage-class)[text=класс хранения]}. Вы сможете {linkto(../../../../storage/s3/instructions/change-storage-class#s3-instructions-change-storage-class-bucket)[text=изменить его]} позже.
{/includetag}
{includetag(quick-start)}
1. Выберите {linkto(../../../storage/s3/concepts/about#s3-concepts-about-storage-class)[text=класс хранения]}. Вы сможете {linkto(../../../storage/s3/instructions/change-storage-class#s3-instructions-change-storage-class-bucket)[text=изменить его]} позже.
{/includetag}
...
1. Добавьте объект:
   ...
   1. [Перейдите](https://msk.cloud.vk.ru/app) в личный кабинет VK Cloud.
   1. Выберите проект, в котором находится созданный бакет.
   1. Перейдите в раздел **Object Storage** → **Бакеты**.
...
1. Предоставьте доступ к загруженному объекту по временной {linkto(../../../storage/s3/concepts/access/signed-url#s3-concepts-signed-url)[text=подписанной ссылке]}:
   ...
   1. Сгенерируйте временную ссылку для доступа к объекту с приватными настройками ACL, выполнив команду:
      ...
      - `<КЛЮЧ_ОБЪЕКТА>` — {linkto(../../../storage/s3/concepts/about#s3-concepts-about-object-key)[text=ключ объекта]}. В этом сценарии ключ совпадает с именем загруженного объекта.
...
{/includetag}
{includetag(create-bucket)}
   1. Нажмите ![ ](../../../../assets/more-icon.svg "inline") для загруженного ранее объекта и выберите пункт **Доступ к файлу**.
{/includetag}
{includetag(quick-start)}
   1. Нажмите ![ ](../../../assets/more-icon.svg "inline") для загруженного ранее объекта и выберите пункт **Доступ к файлу**.
{/includetag}
...
{includetag(not_bucket_onboarding)}
1. Перейдите в браузере по сгенерированной временной ссылке. Должен загрузиться объект, {linkto(#create_private_object)[text=добавленный ранее с приватными настройками ACL]}.
1. {/includetag}Перейдите в браузере по сгенерированной прямой ссылке. Должен загрузиться объект, {linkto(#create_public_object)[text=добавленный ранее с публичными настройками ACL]}.

## {heading(Удалите неиспользуемые ресурсы)[id=delete_unused]}

{includetag(create-bucket)}
Бакет и загруженные в него объекты {linkto(../../../../storage/s3/tariffication#s3-tariffication)[text=тарифицируются]}. Если они вам больше не нужны:

1. {linkto(../../../../storage/s3/instructions/objects/manage-object#s3-instructions-manage-object-delete)[text=Удалите объекты]}.
1. {linkto(../../../../storage/s3/instructions/buckets/manage-bucket#s3-instructions-manage-bucket-delete)[text=Удалите бакет]}.
{/includetag}
{includetag(quick-start)}
Бакет и загруженные в него объекты {linkto(../../../storage/s3/tariffication#s3-tariffication)[text=тарифицируются]}. Если они вам больше не нужны:
