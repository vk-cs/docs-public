...
1. [Создайте аккаунт и ключ доступа](../../instructions/access-management/access-keys) к VK Object Storage в личном кабинете VK Cloud. Сохраните **Secret Key**.
...
{tab(AWS CLI)}
  ...
  1. Укажите идентификатор ключа доступа к VK Object Storage.
...
- `<домен>` — регион аккаунта VK Object Storage.  По умолчанию AWS CLI настроен на работу с серверами Amazon, поэтому обязательно указывайте `--endpoint-url` при выполнении любой команды. Возможные значения:
