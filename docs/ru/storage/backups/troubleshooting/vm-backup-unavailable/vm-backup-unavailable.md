# {heading(Не работает резервное копирование ВМ)[id=backup-vm-backup-unavailable]}
...
- На ВМ нет метаданных, необходимых для резервного копирования. Пример: ВМ создана из загруженного образа или смигрирована с помощью Hystax.
- Возникает высокая нагрузка на дисковую подсистему в момент снятия резервной копии.

### {heading(Решение)[id=backup-vm-backup-unavailable-solution]}
