...
Нельзя удалить резервные копии с установленной блокировкой от удаления, отмеченные значком ![ ](/ru/assets/protected-icon.svg "inline") рядом с именем.
...
## Снятие блокировки от удаления с резервной копии ВМ

[Временная блокировка](../../concepts/blocked-backup#temporary_lock) снимается автоматически по истечении ее срока. Чтобы снять [бессрочную блокировку](../../concepts/blocked-backup#legal-hold-lock) или изменить параметры временной:

1. [Откройте](#view_backups_list) список резервных копий нужной ВМ.
1. Узнайте тип блокировки, наведя курсор мыши на значок ![ ](/ru/assets/protected-icon.svg "inline") рядом с именем нужной резервной копии.
1. Если тип блокировки `Период защиты - С возможностью обхода` или `Бессрочная защита`, для ее снятия или изменения параметров блокировки [обратитесь в техническую поддержку](/ru/contacts).

   Блокировку `Период защиты - Абсолютная защита` нельзя снять или изменить.
