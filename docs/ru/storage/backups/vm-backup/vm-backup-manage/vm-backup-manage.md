...
7. В появившемся окне:
   ...
   - **Тип диска**: выберите одно из значений — HDD, SSD или High-IOPS SSD. Подробнее в [обзоре сервиса Cloud Servers](/ru/base/iaas/concepts/about#diski).
...
Если старый диск больше не нужен, для экономии ресурсов [удалите диск](/ru/base/iaas/service-management/volumes#udalenie_diska) вручную.
...
6. Измените нужные параметры.
   ...
   - [Подход к хранению](/storage/backups/retention-policy) полных резервных копий: **Включить стратегию хранения полных бэкапов (GFS)** или **Макс. количество полных бэкапов**.
      - Для [стратегии GFS](/storage/backups/retention-policy/gfs-backup): сроки хранения полных резервных копий.
      - Для опции **Макс. количество полных бэкапов**: [лимит количества](/storage/backups/retention-policy/forward-incremental) полных резервных копий. При достижении лимита самые старые резервные копии будут удаляться автоматически.
