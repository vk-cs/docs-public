# {heading(Резервное копирование по стратегии GFS)[id=backup-gfs]}

В {var(cloud)} для хранения и удаления резервных копий может быть использована GFS-стратегия («дед-отец-сын», Grandfather-Father-Son).
...
## {heading(Принцип работы)[id=backup-gfs-principle]}
