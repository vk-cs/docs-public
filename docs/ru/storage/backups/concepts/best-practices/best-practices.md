...
Также вы можете [дополнительно снизить затраты на резервное копирование](#cost-manage), оптимизировав настройки сервиса Cloud Backup.
...
[Создайте](/ru/storage/backups/instructions/create-backup-plan#create_db_backup_plan) для СУБД план с инкрементальным копированием и хранением по [стратегии GFS](/ru/storage/backups/concepts/gfs-backup). [Создайте план](/ru/storage/backups/instructions/point-in-time-recovery-pitr) PITR для PostgreSQL, чтобы минимизировать потери и ускорить восстановление.
...
- Используйте стратегию [ограничения количества резервных копий](/ru/storage/backups/concepts/forward-incremental) — при превышении лимита старые копии будут удаляться автоматически. Оптимально использовать не менее 3 полных копий единовременно.
...
## {heading(Как снизить затраты на использование сервиса)[id=cost-manage]}

- Следите за актуальностью [ручных](/ru/storage/backups/instructions/create-backup-copy) резервных копий и удаляйте ненужные.
- [Настраивая](/ru/storage/backups/instructions/create-backup-plan) план резервного копирования, уменьшите количество дней, в которое оно запускается.
- Используйте инкрементальные копии для [некритичных](#noncritical_data) данных.
- Выбирайте более оптимальную стратегию хранения резервных копий:

  - Удаление копий спустя определенный срок с помощью [стратегии GFS](ru/storage/backups/concepts/gfs-backup).
  - Фиксированный [лимит](/ru/storage/backups/concepts/forward-incremental) на количество резервных копий, при превышении которого старые копии будут удаляться автоматически.

- Используйте [защиту от удаления](/ru/storage/backups/concepts/blocked-backup) только при необходимости. Чем дольше работает защита от удаления, тем больше период хранения копий и объем данных, а значит, и стоимость их хранения.