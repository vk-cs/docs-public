...
1. Задайте конфигурацию пула:
    ...
    - **Тип пула**: выберите один из вариантов:
        ...
        - **Персонализированный** — в таком пуле администратор назначает рабочие столы конкретным пользователям. Подходит для развертывания рабочих столов с индивидуальной конфигурацией под отдельных пользователей.
    ...
    - **Горячий резерв** — доступен только для сессионных пулов. Столы из горячего резерва становятся доступными в только момент подключения пользователя и при завершении пользовательской сессии возвращаются в резерв.
    ...
    - **Таймаут неактивности пользователя**: время, через которое пользователь будет отсоединен от рабочего стола при отсутствии активности в рабочем столе. Таймаут работает для протоколов RDP и RX.

      При значении `0` пользовательская сессия не будет прерываться по таймауту.

    - **Действия при завершении сессии**: выберите, что произойдет с рабочим столом после завершения пользовательской сессии:

        - **Удалить РС** — удалить рабочий стол.
        - **Вернуть в горячий резерв** — вернуть рабочий стол в резерв пула. Доступно только для сессионных пулов.
        - **Выключить** — выключить ВМ рабочего стола. Опция по умолчанию.

        Если параметр не задан, ВМ рабочего стола по завершении сессии останется включенной.
