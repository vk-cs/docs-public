## Как устроен Kubernetes as a Service на платформе VK Cloud

Kubernetes aaS в VK Cloud включает:
...
7.  Автоматизированное развертывание федеративных кластеров Kubernetes на базе AWS и VK Cloud (о чем [мы писали тут](https://habr.com/ru/company/mailru/news/t/509684/)). Решение протестировано обоими провайдерами.
...
13.  Создание и масштабирование кластеров Kubernetes с помощью UI или API VK Cloud, управление сущностями через Kubernetes dashboard и kubectl.
...
VK Cloud входит в CNCF (Cloud Native Computing Foundation). Дистрибутив Kubernetes от VK Cloud получил сертификат Certified Kubernetes — Hosted. Его проверили на надежность и соответствие стандартам, он отвечает всем функциональным требованиям сообщества и совместим со стандартным Kubernetes API. VK Cloud — пока единственный в России облачный провайдер, получивший такую сертификацию.
...
Kubernetes от VK Cloud поддерживает деплой в формате Multi Master, при этом каждая пользовательская группа нод уже находится в конкретной зоне доступности. Геораспределение гарантируется созданием виртуальных машин по принципу Soft Anti Affinity, когда машины по возможности запускаются на разных гипервизорах.
...
_Несколько Storage-классов в VK Cloud_
...
В VK Cloud есть Cluster Autoscaler. Это не просто автоскейлинг подов внутри кластера, а автоскейлинг самого кластера по необходимости: новые ноды добавляются, когда нагрузка выросла, и удаляются, если нагрузка упала. Масштабирование происходит автоматически — до 100 узлов и обратно за несколько минут.
...
Kubernetes по сути можно использовать для любых микросервисных приложений, работающих с данными. Чем Kubernetes на платформе VK Cloud интересен для data scientist’ов:
...
<table><tbody><tr><td><p><strong>Еще о сервисах VK Cloud</strong></p><p><br></p><ol><li><a href="https://habr.com/ru/company/mailru/blog/497570/">Базы данных в IIoT-платформе: как VK Cloud работают с петабайтами данных от множества устройств</a></li><li><a href="https://habr.com/ru/company/mailru/blog/474180/">Как реализуется отказоустойчивая веб-архитектура в платформе VK Cloud</a></li><li><a href="https://habr.com/ru/company/mailru/blog/472694/">Больше чем Ceph: блочное хранилище облака VK Cloud</a></li><li><a href="https://habr.com/ru/company/mailru/blog/452846/">Как мы построили надежный кластер PostgreSQL на Patroni</a></li></ol></td></tr></tbody></table>
