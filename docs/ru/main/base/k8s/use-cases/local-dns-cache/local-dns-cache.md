...
- IP-адреса, которые слушает сервер:
  ...
  Такая конфигурация используется, потому что `kube-proxy` в кластерах Kubernetes VK Cloud [работает](../../concepts/preconfigured-features/settings#rezhim-raboty-kube-proxy) в режиме `iptables`.
