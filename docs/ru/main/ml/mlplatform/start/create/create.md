...
| Размер диска | Задает размер диска ВМ в GB. В настоящий момент увеличение или уменьшение размера диска инстанса JupyterHub не поддерживается. Рекомендуем сохранять большие объемы данных во внешнем хранилище, например в [объектном хранилище S3](https://mcs.mail.ru/docs/ru/base/s3).|
