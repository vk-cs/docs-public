...
Для того, чтобы настройки вступили в силу необходимо перезагрузить сервер JupyterHub. Это можно сделать из JupyterHub в Hub Control Panel, либо перезагрузить VM с JupyterHub в разделе [Облачные вычисления -> Виртуальные машины](https://mcs.mail.ru/docs/base/iaas/vm-start/manage-vm/vm-state).
