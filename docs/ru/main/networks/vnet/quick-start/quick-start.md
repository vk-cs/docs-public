...
Создание виртуальных машин может занять некоторое время, после чего они появятся в списке. На ваш компьютер загрузятся файлы с ключами от виртуальной машины.

## 3. Создайте группу правил
...
1. Выполните пинг до внешнего ресурса в интернете с помощью команды:
    ...
    Обмена пакетами не произойдет из-за отсутствия соединения с интернетом.
...
1. Перейдите в раздел **Облачные вычисления** → **Виртуальные машины**.
1. Откройте контекстное меню для `test-vm4` и выберите опцию **Перезагрузить**. Дождитесь завершения перезагрузки ВМ.
1. Откройте страницу ВМ `test-vm4`, нажав на ее имя в списке.
1. Перейдите на вкладку **Консоль**.
1. Выполните вход на ВМ.
1. Выполните пинг до внешнего ресурса в интернете с помощью команды:
    ...
    Если идет обмен пакетами, то соединение с интернетом работает корректно.
...
## 8. Проконтролируйте использование ресурсов

Работающие ВМ потребляют вычислительные ресурсы. Если они вам больше не нужны:

- [удалите](/ru/base/iaas/vm-start/manage-vm/vm-delete) созданные ВМ;<!-- todo изменить ссылку после переделки IaaS-->
