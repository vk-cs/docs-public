...
- При работе с [сервисом облачных вычислений](/ru/main/base/iaas) для распределения входящего трафика между виртуальными серверами.
- При работе с [сервисом контейнеров](/ru/main/base/k8s/), например при использовании [Ingress-контроллеров](/ru/base/k8s/use-cases/ingress) (в том числе [предустановленного](/ru/base/k8s/concepts/preconfigured-features/addons#ingress-controller--nginx-)) или сервисов типа [Load Balancer](/ru/base/k8s/use-cases/load-balancer).
...
- В [сервисе контейнеров](/ru/main/base/k8s/) для каждого кластера Kubernetes, чтобы обеспечить доступ к Kubernetes API.
- В [сервисе баз данных](/ru/main/dbs/dbaas/) для инстансов БД в кластерной конфигурации для балансировки нагрузки между узлами инстансов. Это применимо для инстансов:
  - MySQL;
  - PostgreSQL;
