...
HTTP API расположено [здесь](/additionals/iot/api/public-api/api-devices/http-api).
...
Данное описание может быть неполным/неточным, для просмотра актуальной спецификации API перейдите по [ссылке](/additionals/iot/api/public-api).
