...
**Платформа «Mail.Ru Cloud Solutions\*\***» (Платформа)\*\* — доступная Заказчику через Сайт и оперируемая Компанией программа для ЭВМ, которая предоставляет Заказчику доступ к выбранным Сервисам.
...
6.7 В случае недоступности Сервиса, Компания обязуется перечислить Заказчику Компенсацию (зачислить на Лицевой баланс (счет)), размер которой определяется на основании нижеприведенной таблицы (далее — Таблица №1):
