# {heading(Обновление сертификата федерации удостоверений)[id=vk-cloud-account-update-fim-cert]}

Срок действия сертификатов ADFS ограничен, после его истечения пользователи не смогут войти в систему. Для корректной работы аутентификации обновите сертификат федерации. Вы можете сделать это {linkto(#vk-cloud-account-update-fim-cert-manual)[text=вручную]} или настроить {linkto(#vk-cloud-account-update-fim-cert-auto)[text=автоматическое]} обновление.

## {heading(Ручное обновление сертификата)[id=vk-cloud-account-update-fim-cert-manual]}
...
## {heading(Автоматическое обновление сертификата)[id=vk-cloud-account-update-fim-cert-auto]}
...
1. В поле **Relying party's federation metadata URL** вставьте URL для обновления метаданных: 
   ...
   Здесь `<FEDERATION_ID>` — значение параметра **ID федерации** раздела **Федерация удостоверений** в аккаунте {var(cloud)}.
