# {heading(Настройка федерации удостоверений через MS Entra)[id=iam-ms-entra-fim]}
...
{note:info}
Операции по созданию федерации удостоверений VK Cloud доступны только пользователям с [ролью](/ru/access/iam/concepts/roles-reference) владельца проекта.
{/note}
...
1. Измените существующие утверждения и добавьте новые:
    ...
    1. Для утверждения `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname` нажмите ![](/ru/assets/more-icon.svg "inline") и выберите **Удалить**.
...
## {heading(4. Настройте связь групп и ролей в VK Cloud)[id=iam-ms-entra-fim-groups-roles]}
...
1. В том же разделе [аккаунта VK Cloud](https://cloud.vk.ru/account) перейдите на вкладку **Группы**.
...
1. Проверьте, что автоматически назначенная роль пользователя VK Cloud соответствует выбранной при [добавлении группы](#iam-ms-entra-fim-groups-roles).
