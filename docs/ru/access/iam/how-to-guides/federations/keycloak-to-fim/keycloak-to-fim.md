# {heading(Настройка федерации удостоверений через Keycloak)[id=iam-keycloak-fim]}
...
Далее будет показана настройка федерации удостоверений для входа в VK Cloud c [Keycloak](https://www.keycloak.org) в качестве поставщика удостоверений.
...
Операции по настройке связи групп и ролей в VK Cloud доступны только следующим [ролям](/ru/access/iam/concepts/roles-reference) личного кабинета: владельцу, суперадминистратору и администратору пользователей (IAM).
