# {heading(Настройка федерации удостоверений через Active Directory)[id=iam-ad-fim]}
...
Далее будет показана настройка федерации удостоверений для входа в VK Cloud на примере [службы федерации Active Directory](https://learn.microsoft.com/ru-ru/windows-server/identity/ad-fs/ad-fs-overview) (AD FS).

{note:info}
Чтобы настроить вход на портал технической поддержки VK Cloud с помощью федерации удостоверений, ознакомьтесь с [инструкцией](/ru/access/iam/instructions/support-fim).
{/note}

## {heading(Подготовительные шаги)[id=iam-ad-fim-prepare]}
...
## {heading({counter(ad-fim)}. Создайте федерацию удостоверений в {var(cloud)})[id=iam-ad-fim-create]}
...
## {heading({counter(ad-fim)}. Настройте AD FS)[id=iam-ad-fim-set]}
...
## {heading({counter(ad-fim)}. Настройте связь групп и ролей в {var(cloud)})[id=iam-ad-fim-connect]}
...
1. Добавьте группы, которые используете в Active Directory:
    ...
    1. Настройте группу:
        ...
        - **Роли группы**: выберите те [роли](/ru/access/iam/concepts/roles-reference) или [разрешения](/ru/access/iam/concepts/permissions-reference) VK Cloud, которые соответствуют вашей матрице доступа для создаваемой группы.

    1. Нажмите кнопку **Добавить**.

## {heading({counter(ad-fim)}. Проверьте возможность входа через федерацию)[id=iam-ad-fim-check]}
...
1. Проверьте, что автоматически назначенная роль пользователя VK Cloud соответствует выбранной при [добавлении группы](#iam-ad-fim-connect).

{note:warn}

Active Directory автоматически не синхронизируется с VK Cloud. После блокировки пользователя в Active Directory [удалите](../../../instructions/access-manage#iam-access-manage-delete-user) его из проектов в личном кабинете VK Cloud.
