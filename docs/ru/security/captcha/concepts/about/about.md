...
Сервис отсеивает ботов и верифицирует легитимных пользователей при минимальном участии с их стороны. Инфраструктура сервиса включает SDK для веб-приложений, iOS и Android, а также API для бэкенда. Проверку может инициировать как бэкенд, так и фронтенд приложения.

## {heading(Принцип работы сервиса)[id=captcha-about-how-it-works]}

Сервис поддерживает два сценария подключения: {linkto(#captcha-about-backend-first)[text=проверка по запросу бэкенда]} и {linkto(#captcha-about-frontend-first)[text=проверка по запросу фронтенда]}.
...
![Принцип работы сервиса: капча по запросу бэкенда](./assets/captcha-about-backend-first.png){params[align=center; noBorder=true]}
...
![Принцип работы сервиса: капча по запросу фронтенда](./assets/captcha-about-frontend-first.png){params[align=center; width=70%; noBorder=true]}
...
Невидимая капча доступна только в веб-приложении и работает в сценарии {linkto(#captcha-about-frontend-first)[text=капчи по запросу фронтенда]}. Проверка в этом случае происходит в фоновом режиме. Если сервис оценивает пользователя как подозрительного, невидимая капча будет автоматически заменена на видимую.

Видимая капча работает как в сценарии {linkto(#captcha-about-frontend-first)[text=капчи по запросу фронтенда]}, так и в сценарии {linkto(#captcha-about-backend-first)[text=капчи по запросу бэкенда]}. Доступно три типа видимой капчи:
