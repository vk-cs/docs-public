...
Менеджер секретов — сервис для централизованного хранения {linkto(#sm-about-secrets)[text=секретов]} в безопасной и изолированной среде {var(cloud)}.
...
При каждом обновлении секрета создается новая версия, которая помечается как текущая. Прежняя версия остается в менеджере секретов. Изменить предыдущие версии нельзя, но можно ознакомиться с их содержанием через личный кабинет {var(cloud)} или {linkto(../../../../tools-for-using-services/api/api-spec/sm-api#api-spec-sm)[text=API]}.
