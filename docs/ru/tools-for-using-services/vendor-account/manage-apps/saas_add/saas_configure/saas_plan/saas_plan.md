...
Подробнее типы тарификации описаны в разделе {linkto(/ru/tools-for-using-services/vendor-account/manage-apps/concepts/about#xaas_billing)[text=%text]}.
...
* Параметр `cost` — определяет стоимость плана за отчетный период `<COST>` без учета платных тарифных опций. Задается в валюте страны, где развернут магазин. Если план бесплатный, указывается `0`. Поддерживается только предоплатная тарификация.
* (Опционально) Параметр `billing_cycle_flat` — определяет длительность отчетного периода для тарификации. Можно указать только для предоплатного тарифного плана. По умолчанию — `1 mons 0 days`.

  Задается в формате: `<КОЛИЧЕСТВО_МЕСЯЦЕВ> mons <КОЛИЧЕСТВО_ДНЕЙ> days`, например: `1 mons 15 days`, `30 days`. Количество дней в месяце `mons` рассчитывается на основе календарного значения, поэтому периоды `1 mons 0 days` и `0 mons 31 days` не равны между собой.

* (Опционально) Параметр `billing_cycle_step` — определяет длительность периода, с которым магазин опрашивает брокера на наличие необработанных отчетов (подробнее — в разделе [Тарификация](../../../concepts/about#xaas_billing)). Можно указать только для тарифного плана с постоплатными тарифными опциями. По умолчанию — `1 mons 0 days`.

  Задается в формате: `<КОЛИЧЕСТВО_МЕСЯЦЕВ> mons <КОЛИЧЕСТВО_ДНЕЙ> days`, например: `1 mons 15 days`, `30 days`. Количество дней в месяце `mons` рассчитывается на основе календарного значения, поэтому периоды `1 mons 0 days` и `0 mons 31 days` не равны между собой.

* (Опционально) Параметр `refundable` — определяет, возвращать или нет на бонусный счет проекта денежные средства за оставшиеся дни расчетного периода, если пользователь изменил тарифный план или удалил инстанс сервиса. Можно указать только для предоплатного тарифного плана. По умолчанию — `true`.
