...
1. Создайте root-диск. Используйте ресурс `vkcs_blockstorage_volume`.
   ...
   ```
   {/caption}
1. Создайте отдельный том для хранения данных сервиса. Используйте ресурс `vkcs_blockstorage_volume`. В нем укажите метаданные диска так же, как для root-диска.

   {note:info}

   Для хранения данных создавайте отдельный том, чтобы не потерять их при переустановке сервиса.
...
1. Создайте ВМ из загрузочного образа сервиса и установите агент. Используйте ресурсы:
   ...
   * `vkcs_compute_instance`. Укажите метаданные ВМ так же, как для root-диска. Это позволит в ЛК облачной платформы на странице **Виртуальные машины** отображать ВМ инстанса сервиса в отдельном блоке для ВМ магазина.
   * `ivkcs_user_data`.
...
1. Присоедините отдельный том данных к ВМ. Используйте ресурс `vkcs_compute_volume_attach`.
