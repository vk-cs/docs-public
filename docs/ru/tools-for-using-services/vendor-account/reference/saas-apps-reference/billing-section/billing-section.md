...
* (Опционально) `<ПЕРИОДИЧНОСТЬ_ПРОВЕРКИ>` — длительность периода, с которым Marketplace проверяет, есть ли в брокере необработанные отчеты. Подробнее — в разделе [Тарификация](/ru/tools-for-using-services/vendor-account/manage-apps/concepts/about#xaas_billing). Можно задать только для тарифного плана с постоплатными тарифными опциями.
