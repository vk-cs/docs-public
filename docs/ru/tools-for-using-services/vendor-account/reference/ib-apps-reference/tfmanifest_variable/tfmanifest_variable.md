...
При описании входных переменных можно использовать блоки `locals`. Они позволяют вычислять значения переменных при их объявлении, в том числе через значения других объявленных переменных. Пример: [указание имени хоста с помощью локальной переменной](../../../manage-apps/ivkcs/ivkcs_resources/ivkcs_agent_init#if-hosts-names-examples).
