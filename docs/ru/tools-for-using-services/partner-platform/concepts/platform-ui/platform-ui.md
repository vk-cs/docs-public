# {heading(Личный кабинет партнера)[id=partner-platform-ui]}

Личный кабинет партнера {var(cloud)} — это интерфейс, с помощью которого вы можете управлять проектами своих клиентов на платформе {var(cloud)}. Кабинет партнера является частью партнерской инфраструктуры VK Tech.
