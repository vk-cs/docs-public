# {heading(Создание и настройка балансировщиков нагрузки)[id=terraform-lb]}
...
## {heading(Создание сети)[id=terraform-lb-net]}
...
## {heading(Создание балансировщика нагрузки)[id=terraform-lb-create]}
...
- Источники данных (data source):
  ...
  - `vkcs_compute_flavor`: флейвор (CPU, RAM, Disk) ВМ. Можно посмотреть в мастере создания ВМ в личном кабинете {var(cloud)}.
...
- Ресурсы (resource):
  ...
  - `vkcs_lb_loadbalance`: управляет ресурсом балансировщика нагрузки в {var(cloud)}. Включает в себя следующие ресурсы:
    ...
    - `availability_zone`: [зона доступности](../../../../../start/concepts/architecture#architecture-az), в которой будет размещен балансировщик. Для оптимизации и ускорения подключения ВМ к балансировщику размещайте их в одной зоне доступности. Список зон доступности можно посмотреть в личном кабинете или через {linkto(../../../../cli/openstack-cli#tools-cli-openstack)[text=OpenStack CLI]}.

    - `tags`: список простых строк, назначенных балансировщику нагрузки.

  - `vkcs_lb_listener`: управляет ресурсом прослушивателя в {var(cloud)}. Включает в себя следующие ресурсы:
  ...
  - `vkcs_lb_pool`: управляет ресурсом пула в {var(cloud)}. Включает в себя следующие ресурсы:
  ...
  - `vkcs_lb_member`: управляет ресурсом участника в {var(cloud)}. Включает в себя следующие ресурсы:
...
## {heading(Применение изменений)[id=terraform-lb-apply]}
