...
- Для маршрутизатора публичной сети настроен доступ к внешней сети, поэтому объектам в этой сети можно назначать плавающие IP-адреса.
...
1. Создайте файл конфигурации Terraform `variables.tf` с переменными:
   ...
   Из документа, посвященного [зонам доступности](/ru/additionals/start/architecture#az).
