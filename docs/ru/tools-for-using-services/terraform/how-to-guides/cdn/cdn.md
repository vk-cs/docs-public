# {heading(Создание кластера Spark)[id=terraform-cdn]}
...
{include(../../../../_includes/_cdn_tf.md)}

{/cut}

## {heading(Подготовительные шаги)[id=terraform-cdn-prepare]}

1. Ознакомьтесь с {linkto(../../../../networks/cdn/concepts#cdn-concepts)[text=документаций {var(cloud)} CDN]}, чтобы иметь представление о принципах работы сервиса.
...
## {heading(1. Создайте файл с описанием группы источников)[id=terraform-cdn-file]}
...
{include(../../../../_includes/_cdn_tf.md)[tags=origin]}
...
- `backup` — признак резервного {linkto(../../../../networks/cdn/concepts/origin-groups#cdn-origin-groups)[text=типа источника]} (задается на уровне источника):

  - `backup = true` — источник является резервным;
  - `backup = false` или параметр не указан — источник является активным.

- `use_next` — использование {linkto(../../../../networks/cdn/concepts/origin-groups#cdn-origin-groups)[text=следующего источника из списка]} (задается на уровне всей группы):
...
## {heading(2. (Опционально) Добавьте шилдинг в проект)[id=terraform-cdn-shielding]}
...
Если вы не планируете использовать шилдинг, перейдите к {linkto(#terraform-cdn-ssl)[text=добавлению SSL-сертификата]}.
...
1. Выберите точку присутствия прекеш-сервера и добавьте ее данные в файл конфигурации CDN-ресурса. Для этого откройте файл `cdn.tf` и добавьте в него содержимое:
   ...
   {include(../../../../_includes/_cdn_tf.md)[tags=shielding]}

## {heading(3. (Опционально) Добавьте SSL-сертификат в проект)[id=terraform-cdn-ssl]}
...
Если вы планируете использовать сертификат Let's Encrypt или не использовать SSL-сертификат вообще, перейдите к {linkto(#terraform-cdn-cdn)[text=добавлению CDN-ресурса]}.
...
1. Откройте файл `cdn.tf` и добавьте в него содержимое:
   ...
   {include(../../../../_includes/_cdn_tf.md)[tags=ssl]}
...
## {heading(4. Добавьте описание CDN-ресурса)[id=terraform-cdn-cdn]}

Откройте файл `cdn.tf` и добавьте в него содержимое:

{include(../../../../_includes/_cdn_tf.md)[tags=cdn]}
...
- `origin_group` — идентификатор группы источников для CDN-ресурса. Идентификатор можно указать в конфигурационном файле, получить из источника данных или ресурса. Обязательный параметр.
  ...
  {cut(Примеры)}
    ...
    - `origin_group = "266524"`: указан идентификатор, полученный из {linkto(../../../../networks/cdn/instructions/manage-origin-groups#cdn-manage-origin-groups-list)[text=списка групп источников]} в личном кабинете {var(cloud)}.
...
## {heading(5. Создайте необходимые ресурсы с помощью Terraform)[id=terraform-cdn-resource]}
...
## {heading(6. Проверьте применение конфигурации)[id=terraform-cdn-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=terraform-cdn-delete]}
