# {heading(Управление файловыми хранилищами)[id=api-spec-manila]}
...
1. Найдите эндпоинт **Manila** в блоке **Сервис OpenStack**.
1. {linkto(../../../rest-api/case-keystone-token#rest-api-keystone-token)[text=Получите]} токен доступа `X-Auth-Token`.

{/cut}

{note:info}
Исходная спецификация в формате JSON доступна по [ссылке](./assets/manila-api.json "download").
{/note}
