...
Квоты на наиболее часто используемые ресурсы отображаются на главной странице {ifdef(public)}[личного кабинета](https://msk.cloud.vk.ru/app/){/ifdef}{ifndef(public)}{linkto(../../lk-entry#tools-account-lk-entry)[text=личного кабинета]}{/ifndef}.
...
1. Если для проекта разрешено быстрое или автоматическое увеличение квот, будет доступен выбор типа запроса. Выберите один из них:

   - **Быстрое увеличение квот** — увеличение квот в один клик, без согласования с менеджером.

     {cut(Условия для быстрого увеличения квот)}

     {include(/ru/_includes/_quotas.md)[tags=fastquota-conditions]}

     {/cut}

   - **Автоматическое увеличение квот** — квоты будут расширены автоматически, без согласования с менеджером. Значение каждой квоты можно увеличить не более, чем на 30% от текущего значения.
   ...
   - **Дополнительные ресурсы** — запрос согласовывается с менеджером. Используйте, если автоматического увеличения квот недостаточно.

   Если выбор типа запроса отсутствует, запрос на увеличение квот будет согласован с менеджером.
...
1. Подтвердите выбор одним из доступных способов:
   ...
   - Нажмите кнопку **Отправить**. Сформируется запрос менеджеру.
   - Нажмите кнопку **Увеличить**. Квоты будут увеличены автоматически. Доступно только для проектов с разрешенным быстрым или автоматическим увеличением квот.
...
## {heading(Просмотр цен на услуги проекта)[id=project-prices-view]}

{include(/ru/_includes/_project_prices.md)[tags=prices]}
