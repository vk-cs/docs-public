...
У проекта может быть только один владелец. После назначения владельцем другого пользователя вы поменяетесь с ним [ролями](/ru/access/iam/concepts/rolesandpermissions).
...
1. Выполните подготовительные шаги:
    ...
    1. Убедитесь, что пользователь, которого вы хотите сделать владельцем:
        ...
        - Добавлен в [список участников](/ru/access/iam/instructions/access-manage) и принял приглашение в проект.
...
1. (Опционально) Проверьте смену ролей в [списке участников](/ru/access/iam/instructions/access-manage) проекта.
...
## {heading(Увеличение квот проекта)[id=increase-quota]}

Увеличение квот выполняется через специальную форму от имени пользователя с [ролью](/ru/access/iam/concepts/rolesandpermissions) владельца проекта:
...
1. Выберите ресурс, для которого необходимо увеличить квоты. Для увеличения квот на несколько типов ресурсов нажмите кнопку **Добавить еще**.
    ...
    Чтобы удалить ресурс из запроса увеличения квот, нажмите на значок ![delete-icon](/ru/assets/delete-icon.svg "inline").
...
Информация об увеличении квот для участников программы GeekBrains приведена в [Вопросах и ответах](../../../faq).
...
На вкладке отображаются SDN по умолчанию и дополнительная [SDN](/ru/networks/vnet/concepts/sdn) проекта, если подключена. Если в проекте используется только SDN Neutron, обратитесь в [техническую поддержку](/ru/contacts), чтобы подключить SDN Sprut.
