# {heading(Работа с журналом событий)[id=tools-account-actionslogs]}

<!--- #todo Привести к единому источнику с /ru/monitoring-services/event-log/instructions/view-event-log, на этой странице информация уже устарела -->

В журнале событий хранится история операций, выполненных компонентами {var(cloud)}. Сохраняются действия следующих компонентов (указаны в формате для запросов к API):
...
## {heading(Скачивание журнала событий)[id=account-actionslogs-download]}
...
## {heading(Просмотр записей журнала)[id=account-actionslogs-view]}
...
1. Выполните запрос:
   ...
   Подробнее о формулировании запроса в разделе [Примеры API-запросов при работе с журналом](#account-actionslogs-example).
   ...
   | `source`
   | string
   | Компонент-источник операции

   | `marker`
   | string
   | Токен для запроса следующей страницы, ранее возвращенный API. TTL маркеров — 1 час

   | `limit`
   | integer
   | Количество возвращаемых записей. Если не указан, возвращает 100 записей
   |===
...
В каждой записи журнала событий представлена информация:
...
## {heading(Примеры API-запросов при работе с журналом)[id=account-actionslogs-example]}
...
1. Получите значение параметра `marker` из файла `nova_part1.log`:
...
1. Запросите вывод в файл `nova_part2.log` 10 более ранних по времени записей журнала, используя значение параметра `marker`:
...
1. Повторяйте предыдущий запрос, меняя в нем только имя файла (например: `nova_part3.log`, `nova_part4.log`, …), пока не получите все записи журнала за нужный период.
