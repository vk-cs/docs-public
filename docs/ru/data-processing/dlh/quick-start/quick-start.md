...
## 1. Разверните хранилище Cloud Storage

Для использования S3-хранилища в базовой конфигурации DLH достаточно создать один бакет. Оптимальное количество бакетов определяется объемом данных, которые планируется загружать в DLH из БД-источников.
...
1. В открывшемся окне укажите параметры:
   ...
   * **Название бакета**: укажите имя нового бакета, соблюдая [правила](/ru/storage/s3/service-management/buckets/create-bucket#bucket_naming) именования.
   * **S3 Bucket URL**: оставьте параметр без изменений, он формируется автоматически.
   * **Класс хранения**: выберите `Hotbox`.
1. Нажмите кнопку **Создать бакет**.

## 2. Создайте экземпляр сервиса Iceberg Metastore
...
## 3. Создайте инстанс БД PostgreSQL
...
## 4. Создайте экземпляр сервиса Cloud Trino
...
1. На шаге **Подключения**:
   ...
   1. Добавьте к экземпляру сервиса Cloud Trino источник данных Apache Iceberg с S3 VK Cloud:
      ...
      1. Укажите параметры подключения:
         ...
         * **Bucket**: укажите имя бакета, созданного при развертывании сервиса Cloud Storage.
...
1. [Подключите]() аналитическую БД на базе ClickHouse, чтобы выполнять быструю обработку части данных до их попадания в S3-хранилище.
1. [Подключите]() сервис Spark для работы со сложными ETL и ELT задачами.
