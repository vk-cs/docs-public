...
{ifndef(private-cert)}
{var(system)} поставляется в двух редакциях:

* Light Cloud — редакция {var(system)} с сокращенным набором компонентов и меньшими требованиями к оборудованию.
* Enterprise Cloud — редакция {var(system)} с полным набором компонентов, подходящая для всех сценариев использования.

В каждой редакции доступны конфигурации с разным компонентным составом.

Документ содержит описание максимально полного состава {var(sys2)}. Фактический состав конкретной инсталляции зависит от выбранной конфигурации и может не включать все описанные в документе компоненты и функции.
{/ifndef}
