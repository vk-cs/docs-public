# {heading(Cloud Backup: настройте резервное копирование)[id=onboarding-backup]}
...
- Воспользуйтесь [инструкцией](#onboarding-backup-guide), чтобы протестировать восстановление удаленной ВМ из резервной копии.
- Посмотрите {linkto(#onboarding-backup-video)[text=видео]} о работе с сервисом Cloud Backup.

{note:info}Облачные ресурсы, создаваемые при резервном копировании, тарифицируются.{/note}

## {heading(Настройте резервное копирование)[id=onboarding-backup-guide]}

Пройдя все шаги этой инструкции, вы:

1. [Создадите](#onboarding-backup-backup-create) резервную копию виртуальной машины.
1. [Удалите](#onboarding-backup-vm-delete) ВМ.
1. [Восстановите](#onboarding-backup-vm-restore) ВМ из резервной копии.

### {heading(Подготовительные шаги)[id=onboarding-backup-prepare]}

1. Для тестирования {linkto(../../../../intro/onboarding/quick-start/create-vm#onboarding-create-vm)[text=создайте]} новую ВМ на базе Linux с именем `test-vm`.
...
### {heading(1. Создайте резервную копию вручную)[id=onboarding-backup-backup-create]}
...
### {heading(2. Удалите ВМ)[id=onboarding-backup-vm-delete]}

1. В списке виртуальных машин нажмите ![ ](../../../../assets/more-icon.svg "inline") для тестовой ВМ, которую необходимо удалить.
1. Выберите пункт **Удалить** и подтвердите действие.

### {heading(3. Восстановите ВМ из резервной копии)[id=onboarding-backup-vm-restore]}
...
1. Нажмите ![ ](../../../../assets/more-icon.svg "inline") для резервной копии и выберите пункт **Восстановить из бэкапа**.
...
### {heading(Удалите неиспользуемые ресурсы)[id=onboarding-backup-delete]}

Восстановленная виртуальная машина `test-vm-restored` {linkto(../../../../computing/iaas/tariffication#iaas-tariffication)[text=тарифицируется]}, а резервная копия занимает дисковое пространство в платном хранилище сервиса {var(s3)}. Если они вам больше не нужны:

1. Удалите виртуальную машину `test-vm-restored`:

   1. Перейдите в раздел **Облачные вычисления → Виртуальные машины**.
   1. В списке виртуальных машин нажмите ![ ](../../../../assets/more-icon.svg "inline") для ВМ ``test-vm-restored``.
   1. Выберите пункт **Удалить** и подтвердите действие.

1. Удалите резервную копию:

   1. Перейдите в раздел **Cloud Backup → Резервное копирование** на вкладку **Ручное**.
   1. Нажмите ![ ](../../../../assets/more-icon.svg "inline") для ВМ `test-vm`, выберите **Удалить** и подтвердите действие.

## {heading(Видеоинструкции)[id=onboarding-backup-video]}
...
## {heading(Вопросы и ответы)[id=onboarding-backup-faq]}
...
В сервисе {var(s3)} (объектное хранилище S3). Бакеты для резервных копий создаются автоматически, их нельзя самостоятельно создать или удалить.
{/cut}
...
Узнать актуальные тарифы можно в [прайс-листе](https://cloud.vk.ru/pricelist/).
{/cut}