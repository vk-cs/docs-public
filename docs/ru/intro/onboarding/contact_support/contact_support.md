# {heading(Обратитесь в техническую поддержку)[id=onboarding-contact-support]}
...
## {heading(Обращение через портал технической поддержки)[id=onboarding-contact-support-portal]}

1. [Перейдите](https://support.mcs.mail.ru/) в портал технической поддержки VK Tech.
1. Пройдите аутентификацию с помощью учетной записи {var(cloud)}, для этого нажмите кнопку **Войти с помощью {var(cloud)}**.
