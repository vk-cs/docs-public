# {heading(Возврат денежных средств)[id=billing-refund]}

## {heading(Условия возврата)[id=billing-refund-return-terms]}

- Заявка на возврат должна быть оформлена владельцем проекта.
- Вернуть деньги можно только с {linkto(../../concepts/balance#billing-balance)[text=баланса лицевого счета]} проекта. Сумма возврата не может превышать сумму неизрасходованных средств на балансе лицевого счета.
...
1. Создайте заявку со следующими параметрами: **{var(cloud)}: Биллинг: Оплата и возврат: Возврат денежных средств**.
1. Укажите в заявке:
...
1. Отправьте письмо в отдел документооборота {var(cloud)} по адресу [docs_vktech@vk.company](mailto:docs_vktech@vk.company).
