{includetag(openstack-intro)}
Интерфейс командной строки OpenStack (OpenStack CLI) позволяет работать с сервисами {var(cloud)} через консоль.
{/includetag}

{includetag(openstack-prepare)}
{tabs}
...
{ifdef(private,private-pdf,private-pg,private-pg-pdf)}
Установите Openstack CLI:

{note:warn}
Для подключения рекомендуется использование пакета  `python3-openstackclient-6.6.0`.
{/note}
...
{ifdef(private-cert)}
Для подключения необходимо использование пакета `python3-openstackclient` из дистрибутива {var(cloud)}.
...
{includetag(openstack-install-package)}
{ifdef(public)}
...
{ifdef(private,private-pdf,private-pg,private-pg-pdf)}
Установите пакеты для работы с отдельными сервисами OpenStack:

```console
pip install python-cinderclient
```

Рекомендуемые для использования пакеты:
...
{ifdef(private-cert)}
Установите клиент компонента, с которым необходимо работать через OpenStack CLI, например:

```console
$ pip install python-cinderclient
```

Доступные для установки клиенты из дистрибутива {var(cloud)}:
...
1. Убедитесь, что {linkto(../../access/iam/instructions/manage-2fa#iam-manage-2fa-on)[text=включена]} двухфакторная аутентификация и {linkto(../../api/rest-api/enable-api#rest-api-enable-activate)[text=активирован]} доступ по API.
   ...
   Двухфакторная аутентификация должна быть включена не только на время первоначальной настройки OpenStack CLI, но и во время всего периода его использования.
   {/ifdef}
1. Выберите проект.
1. Нажмите на имя пользователя в шапке страницы и выберите пункт **Настройки проекта**.
1. Перейдите на вкладку **Доступ по API**.
1. Нажмите кнопку **Скачать openrc версии 3**. Будет загружен файл с именем `<НАЗВАНИЕ_ПРОЕКТА>-openrc.sh`.
   {ifdef(public)}
1. Укажите в переменных среды учетные данные для аутентификации.
   ...
   {tab(Windows (cmd))}
    ...
    1. Укажите пароль, выполнив команду:
...
{/ifdef}
{ifndef(public)}
Выполните команду, которая использует клиент OpenStack, например:
...
+--------------------------------------+----------------+-------+------+-----------+-------+-----------+
| 2e275f1f-c032-4425-b0fb-dbf8583f62d3 | Standard-12-24 | 24576 |    0 |         0 |    12 | True      |
...
+--------------------------------------+----------------+-------+------+-----------+-------+-----------+
```

В случае получения ошибки, обратитесь к администратору {var(cloud)}.
{/ifndef}
{/includetag}

{includetag(openstack-example)}
...
- Вывести информацию об отдельном образе:
  ...
  {/cut}
  {ifdef(public)}
- Вывести список {linkto(../../../start/concepts/architecture#architecture-az)[text=зон доступности]}:
