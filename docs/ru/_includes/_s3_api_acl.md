...
- {linkto(#api-spec-s3-get-bucket-acl)[text=%text]} — Получить ACL бакета.
- {linkto(#api-spec-s3-put-bucket-acl)[text=%text]} — Установить ACL бакета.
- {linkto(#api-spec-s3-get-object-acl)[text=%text]} — Получить ACL объекта.
- {linkto(#api-spec-s3-put-object-acl)[text=%text]} — Установить ACL объекта.

Устанавливать ACL можно как при создании бакета или загрузке объекта, так и после. Задать конфигурацию ACL в методах `PutBucketACL` и `PutObjectACL` можно двумя способами:

- В теле запроса — в виде {linkto(#api-spec-s3-acl-request-body)[text=XML-структуры конфигурации ACL]}. Способ используется только для существующих бакетов или объектов.
- В {linkto(#api-spec-s3-acl-request-headers)[text=заголовках запроса]}. Способ используется как при создании бакета или загрузке объекта, так и для существующих бакетов или объектов.

В одном запросе можно использовать только один из этих способов.

## {heading(XML-структура конфигурации ACL)[id=api-spec-s3-acl-request-body]}

Конфигурация ACL в формате XML содержит следующие элементы:

- `<Owner>` — указывает канонический идентификатор владельца бакета. Узнайте его с помощью метода {linkto(#api-spec-s3-get-bucket-acl)[text=%text]} или через AWS CLI, {ifdef(public)}{linkto(../../../../../storage/s3/instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=запросив]}{/ifdef}{ifdef(s3,s3-pdf)}{linkto(../../instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=запросив]}{/ifdef} информацию об ACL бакета.
...
## {heading(Заголовки запроса)[id=api-spec-s3-acl-request-headers]}

Для указания конфигурации ACL в запросах `PUT` используются следующие заголовки:

- `x-amz-acl` — назначает бакету или объекту преднастроенный {ifdef(public)}{linkto(../../../../../storage/s3/concepts/access/s3-acl#s3-concepts-acl-pre-set)[text=стандартный ACL]}{/ifdef}{ifdef(s3,s3-pdf)}{linkto(../../concepts/access/s3-acl#s3-concepts-acl-pre-set)[text=стандартный ACL]}{/ifdef}.
...
## {heading(Получатели доступа)[id=api-spec-s3-acl-permittees]}

Права доступа к бакетам и объектам могут быть даны следующим категориям получателей:

- {linkto(#api-spec-s3-acl-user)[text=Пользователь]} {var(cloud)}.
- {linkto(#api-spec-s3-acl-project)[text=Проект]} {var(cloud)}.
- Предварительно определенная {linkto(#api-spec-s3-acl-meta)[text=метагруппа]}.

### {heading(Пользователь)[id=api-spec-s3-acl-user]}

Доступ пользователю {var(cloud)} предоставляется по его каноническому идентификатору (Canonical ID). Это уникальный идентификатор пользователя {var(s3)}, представляющий собой буквенно-цифровую последовательность. Пример: `fcd68908-XXXX-82ae2a5a251d`.

Чтобы узнать канонический идентификатор пользователя {var(cloud)}, {ifdef(public)}{linkto(../../../../../storage/s3/instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=получите ACL]}{/ifdef}{ifdef(s3,s3-pdf)}{linkto(../../instructions/access-management/acl#s3-instructions-bucket-acl-view)[text=получите ACL]}{/ifdef} бакета или объекта, к которому пользователь уже имеет права доступа.
...
### {heading(Проект)[id=api-spec-s3-acl-project]}

Чтобы предоставить доступ проекту, укажите в запросе {ifdef(public)}{linkto(../../../../../tools-for-using-services/account/instructions/project-settings/manage#project-pid-view)[text=идентификатор проекта]}{/ifdef}{ifdef(s3,s3-pdf)}идентификатор проекта{/ifdef} (PID).
...
{note:info}
Владелец учетной записи {var(cloud)} может делегировать определенные разрешения другим пользователям или группам. Пользователи, которым были делегированы разрешения, получают доступ к ресурсам и могут выполнять действия от имени делегировавшей учетной записи.
{/note}

### {heading(Метагруппа)[id=api-spec-s3-acl-meta]}

В сервисе {var(s3)} есть набор предопределенных метагрупп. Чтобы предоставить доступ группе пользователей, укажите в запросе URI группы.
...
- `Authenticated Users` (авторизованные пользователи). Содержит все существующие учетные записи {var(cloud)}.

  {note:warn}
  При предоставлении доступа группе пользователей `Authenticated Users` любой авторизованный пользователь {var(cloud)} может получить доступ к ресурсу. Все запросы должны быть подписаны (авторизованы).
  {/note}
...
- `All Users` (все пользователи).
  ...
  Анонимные неавторизированные пользователи, если им предоставлен доступ, совершают действия под каноническим ID пользователя `65a011a29cdf8ec533ec3d1ccaae921c`, так как у них нет учетной записи в {var(cloud)}.
...
## {heading(Рекомендации по обновлению ACL)[id=api-spec-s3-acl-recommendations]}

Политика назначения прав доступа такова, что при обновлении ACL существующего бакета или объекта в некоторых случаях могут быть потеряны ранее назначенные права доступа. Например, при запросе на предоставление доступа любой метагруппе будут потеряны все права доступа, установленные для отдельных проектов {var(cloud)}. Чтобы этого избежать, рекомендуется устанавливать права доступа всем нужным категориям получателей одновременно.
