...
1. Подключитесь к ВМ по протоколу SSH:
   ...
   {includetag(not_vm_onboarding)}
## {heading({counter(vm)}. Подключитесь к ВМ)[id=includes-connect-to-vm]}
...
К виртуальной машине на базе Linux, созданной из образа Ubuntu, можно подключиться по протоколу SSH.
   ...
   1. Выполните команду в терминале:
      ...
      - `<IP-АДРЕС>` — внешний IP-адрес ВМ. Его можно посмотреть в [личном кабинете](https://msk.cloud.vk.ru/app/) VK Cloud в разделе **Облачные вычисления → Виртуальные машины**.
