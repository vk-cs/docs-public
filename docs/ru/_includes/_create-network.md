## {counter(network)}. Создайте сети и подсети
...
## {counter(network)}. Создайте несколько виртуальных машин
...
## {counter(network)}. Создайте группу безопасности
...
## {counter(network)}. Назначьте группу безопасности
...
## {counter(network)}. Проверьте сетевую связность между ВМ
...
## {counter(network)}. Проверьте доступ в интернет
...
## {counter(network)}. Подключитесь к ВМ по SSH
