## {heading({counter(network)}. Создайте сети и подсети)[id=includes-create-network-network-create]}
...
1. Создайте сеть с доступом в интернет:
   ...
   1. Задайте название сети: `test-network`.
   1. Выберите [SDN](/ru/networks/vnet/concepts/sdn): `Sprut` (по умолчанию). Настройка недоступна, если для проекта подключена только одна SDN.
   1. Включите опцию **Доступ в интернет**.
   1. Из списка **Маршрутизатор** выберите `Создать новый`.
   1. Нажмите кнопку **Добавить сеть**.

1. Создайте сеть без доступа в интернет:

   1. Нажмите кнопку **Создать**.
   1. Задайте название сети: `test-network-2`.
   1. Выберите [SDN](/ru/networks/vnet/concepts/sdn): `Sprut` (по умолчанию). Настройка недоступна, если для проекта подключена только одна SDN.
   1. Выключите опцию **Доступ в интернет**.
   1. Из списка **Маршрутизатор** выберите `Создать новый`.
   1. Нажмите кнопку **Добавить сеть**.

## {heading({counter(network)}. Создайте несколько виртуальных машин)[id=includes-create-network-vms-create]}
...
1. В блоке **Конфигурация** задайте соответствующие вашим требованиям параметры ВМ.
1. Заполните блок **Общая информация**:

   - **Имя виртуальной машины**: `test-vm1`.
   - Остальные параметры оставьте без изменений.

1. В блоке **Размер и скорость диска** выберите подходящий тип диска и укажите его размер.
1. В блоке **Сеть и настройки firewall** задайте следующие параметры:
...
1. В блоке **Резервное копирование** отключите резервное копирование.
1. Нажмите кнопку **Создать**.
...
1. Заполните блок **Общая информация**:
   ...
   - **Имя виртуальной машины**: `test-vm2`.
   - Остальные параметры оставьте без изменений.
...
1. Заполните блок **Общая информация**:
   ...
   - **Имя виртуальной машины**: `test-vm3`.
   - Остальные параметры оставьте без изменений.
...
1. В блоке **Сеть и настройки firewall** задайте следующие параметры:
   ...
   - **Ключ виртуальной машины**: `Создать новый ключ`.
   - **Настройки Firewall**: добавьте группы безопасности `default`, `ssh`.
   - **Назначить внешний IP**: отключите опцию.
...
## {heading({counter(network)}. Создайте группу безопасности)[id=includes-create-network-sg-create]}
...
1. В блоке **Входящий трафик** создайте правила для управления трафиком:
   ...
   1. Выберите тип: `ICMP`.
   1. Выберите удаленный адрес: `Все IP-адреса`.
...
## {heading({counter(network)}. Назначьте группу безопасности)[id=includes-create-network-sg-assign]}
...
## {heading({counter(network)}. Проверьте сетевую связность между ВМ)[id=includes-create-network-connection-check]}
...
## {heading({counter(network)}. Проверьте доступ в интернет)[id=includes-create-network-internet-check]}
...
## {heading({counter(network)}. Подключитесь к ВМ по SSH)[id=includes-create-network-ssh-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=includes-create-network-delete]}

Работающие ВМ потребляют вычислительные ресурсы. Если они вам больше не нужны:

- {linkto(/ru/computing/iaas/instructions/vm/vm-manage#iaas-vm-manage-delete)[text=удалите]} созданные ВМ;
- {linkto(/ru/networks/vnet/instructions/net#vnet-net-delete)[text=удалите]} сети `test-network` и `test-network-2`.