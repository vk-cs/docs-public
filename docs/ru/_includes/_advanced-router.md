{linkto(/ru/networks/vnet/concepts/router#vnet-router-advanced)[text=Продвинутый маршрутизатор]} позволяет связать сеть, которая находится в вашей локальной инфраструктуре, с виртуальными сетями и подсетями {var(cloud)} без использования выделенной виртуальной машины в качестве маршрутизатора. Это упрощает обмен данными за счет возможности динамической маршрутизации по протоколу [BGP](https://datatracker.ietf.org/doc/html/rfc1163).
...
## {heading(Подготовительные шаги)[id=includes-advanced-router-prep]}
...
1. Выберите или {linkto(/ru/networks/vnet/instructions/net#vnet-net-add)[text=создайте]} виртуальную сеть с доступом в интернет в {var(cloud)}. Используйте уже существующий маршрутизатор с подключением к внешней сети или {linkto(/ru/networks/vnet/instructions/router#vnet-router-add)[text=создайте]} новый.
...
1. {linkto(/ru/computing/iaas/instructions/vm/vm-create#iaas-vm-create)[text=Создайте виртуальную машину]} в выбранной сети. Добавьте доступ по SSH и отключите резервное копирование.

   Запишите IP-адрес ВМ.

1. Обратитесь в [техническую поддержку](/ru/contacts), чтобы подключить в ваш проект сеть для организации выделенного канала подключения (сеть Cloud Direct Connect).
...
## {heading(1. Добавьте продвинутый маршрутизатор)[id=includes-advanced-router-add-router]}
...
## {heading(2. Настройте сетевые интерфейсы продвинутого маршрутизатора)[id=includes-advanced-router-network-configure]}
...
- Направленный в сетевой стык — `x-net`. Этот интерфейс поможет организовать связность между {var(cloud)} и локальной сетью заказчика.
...
## {heading(3. Настройте сетевые интерфейсы BGP-маршрутизатора клиентской сети)[id=includes-advanced-router-network-client]}
...
## {heading(4. Настройте BGP-соседство для продвинутого маршрутизатора)[id=includes-advanced-router-bgp-router]}
...
## {heading(5. Настройте BGP-соседство для маршрутизатора клиентской сети)[id=includes-advanced-router-bgp-client]}
...
## {heading(6. Настройте статические маршруты между сетями)[id=includes-advanced-router-static-routs]}
...
## {heading(7. Проверьте работоспособность)[id=includes-advanced-router-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=includes-advanced-router-delete]}
