...
- Для приложений Spark, у которых нет зависимостей, достаточно передать код приложения в манифесте задания. Этот подход показан [на примере вычисления числа π](../calculate-pi).
...
1. Установите библиотеку Cloud ML Platform для Python. По приведенной ссылке доступна актуальная версия библиотеки.
...
1. [Создайте токен доступа](../../instructions/tokens#sozdanie_tokena_dostupa) с ролью `Администратор`. Этот токен нужен для работы с библиотекой.
   ...
   При работе в production-среде не размещайте токен в открытом виде непосредственно в скриптах. Используйте переменные среды окружения, хранилища секретов или другие инструменты, предназначенные для безопасной работы с чувствительными данными.
...
1. [Создайте инстанс ClickHouse](/ru/dbs/dbaas/instructions/create/create-single-replica).

   Укажите значения для следующих параметров инстанса:

   - Версия: самая новая из доступных версий.
   - Конфигурация: single.
   - Опция **Назначить внешний IP** должна быть выбрана.
   - Тип создания: новая база данных.
   - Имя базы данных для создания: `db_spark`.
   - Имя пользователя: `user_spark`.

   Остальные параметры выберите на свое усмотрение.

   Запишите пароль пользователя, он потребуется позднее.

1. [Получите информацию](/ru/dbs/dbaas/instructions/manage-instance/clickhouse#poluchenie_informacii_ob_instanse_bd_i_ego_hostah) о созданном инстансе ClickHouse.

   Запишите IP-адрес, указанный в параметре **Внешний IP-адрес** на вкладке **Информация**.
...
1. [Загрузите](/ru/base/s3/instructions/objects/upload-object#standartnaya_zagruzka) файлы в директорию `spark-files` этого бакета:
   ...
   - [clickhouse-jdbc-0.5.0-shaded.jar](https://repo1.maven.org/maven2/com/clickhouse/clickhouse-jdbc/0.5.0/clickhouse-jdbc-0.5.0-shaded.jar) с JDBC-драйвером для ClickHouse.
     ...
     При необходимости вы можете загрузить другую версию драйвера из [репозитория Maven](https://repo1.maven.org/maven2/com/clickhouse/clickhouse-jdbc/). В этом случае скорректируйте приведенный ниже скрипт, заменив версию драйвера на нужную.
...
При отправке задания Spark будут указаны пути к файлам, загруженным в бакет. Кластер Cloud Spark уже имеет доступ к объектам из этого бакета, дополнительных настроек не требуется.
...
1. Отправьте задание на кластер, выполнив скрипт:
   ...
   Ранее в бакет, который используется кластером, были загружены файлы, необходимые для работы приложения Spark. Манифест для задания Spark по умолчанию не содержит в себе сведений о том, где расположены необходимые файлы. Поэтому в манифест была добавлена недостающая информация:
   ...
   Также в манифест были добавлены значения [необходимых переменных среды окружения](#1_sozdayte_fayl_s_kodom_prilozheniya_spark). Значения чувствительных переменных среды окружения `CH_USER` и `CH_PASSWORD` были взяты из секрета `clickhouse-secret`, созданного из файла `clickhouse-secret.yaml`.

## 4. Отслеживайте состояние задания Spark

1. Убедитесь, что в логах задания появился результат выполнения SQL-запроса к ClickHouse. Если результат не появился, запустите скрипт для получения логов еще раз: могут быть выведены промежуточные логи работы на момент времени, когда задание еще не завершилось.
...
1. (Опционально) Получите информацию о событиях в кластере. Она нужна, чтобы узнать текущее состояние кластера и заданий, например, в ходе поиска проблем.
