...
В зависимости от того, какие методы библиотеки планируется использовать, может потребоваться токен доступа с определенной ролью. В [справочнике методов библиотеки](../library-reference) указывается, какая роль должна быть у токена, чтобы вызвать конкретный метод.
...
## Получение информации о текущем токене доступа
...
1. Выполните скрипт Python:
   ...
   Будет выведена информация об используемом токене доступа. Пример вывода в случае успеха:
