# {heading(Базовая работа с заданиями Spark)[id=mlspark-htg-basic]}
...
- Для приложений Spark, которые требуют дополнительные артефакты для своей работы, необходимо вручную добавить нужные артефакты в бакет [VK Object Storage](/ru/storage/s3) и отредактировать манифест задания. Этот подход показан {linkto(../submit-advanced-job-clickhouse#mlspark-htg-advanced)[text=на примере работы с ClickHouse]}.

Для примера будет использоваться приложение, которое вычисляет приближенное значение числа π.

## {heading(Подготовительные шаги)[id=mlspark-htg-basic-prerequirements]}

1. {linkto(../../ml-platform-library/install#mlspark-library-install)[text=Установите]} библиотеку Cloud ML Platform.

1. {linkto(../../ml-platform-library/authz#mlspark-library-authz)[text=Создайте токен доступа]}. Этот токен нужен для работы с библиотекой.
...
1. {linkto(../../instructions/create#mlspark-instructions-create)[text=Создайте]} кластер Spark.
...
## {heading(1. Создайте файл с кодом приложения Spark)[id=mlspark-htg-basic-create-spark-file]}
...
## {heading(2. Отправьте задание Spark на кластер)[id=mlspark-htg-basic-send-to-cluster]}
...
Для работы приложения, которое вычисляет число π, требуется [только один исполняемый файл](#mlspark-htg-basic-create-spark-file), никаких дополнительных артефактов не используется.

В таком простом случае не нужно размещать исполняемый файл приложения Spark в бакете {var(s3)}, а затем править манифест по умолчанию, чтобы добавить в манифест необходимые сведения.
...
## {heading(3. Отслеживайте состояние задания Spark)[id=mlspark-htg-basic-check-jobs]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mlspark-htg-basic-delete-unused-resources]}
