...
- Для приложений Spark, которые требуют дополнительные артефакты для своей работы, необходимо вручную добавить нужные артефакты в бакет [Object Storage](/ru/storage/s3) и отредактировать манифест задания. Этот подход будет показан далее.
...
1. Определите имя бакета Object Storage, который используется кластером:
   ...
   1. Найдите необходимую информацию:
      ...
      - имя бакета Object Storage (содержится в поле `s3_bucket_name`).
