# {heading(Режимы MLflow)[id=mlplatform-modes]}
...
## {heading(Связка с JupyterHub)[id=mlplatform-with_jh]}
...
## {heading(Режим Standalone)[id=mlplatform-standalone]}
...
## {heading(Развертывание модели в режиме Standalone)[id=mlplatform-standalone-deploying]}
