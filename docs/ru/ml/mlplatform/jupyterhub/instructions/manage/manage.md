...
Опция позволяет добавить к инстансу бакет S3 для хранения в нем данных. Бакет будет доступен внутри инстанса как директория, а хранимые в нем данные будут доступны в [Object Storage](/ru/storage/s3) даже после удаления инстанса.

Чтобы подключить бакет Object Storage к инстансу:

1. [Перейдите](https://msk.cloud.vk.com/app/) в личный кабинет VK Cloud.
1. Перейдите в раздел **ML Platform** → **Инстансы**.
1. Нажмите ![ ](/ru/assets/more-icon.svg "inline") для нужного инстанса и выберите пункт **Подключить S3 бакет как диск**.
1. В открывшемся окне выберите бакет, который нужно подключить к инстансу. Если подходящих бакетов нет, выберите **Создать новый бакет** — будет создан новый бакет Object Storage и подключен к инстансу.  
1. Нажмите кнопку **Сохранить**.
