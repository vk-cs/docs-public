Управление инстансом JupyterHub доступно через личный кабинет VK Cloud, веб-интерфейс инстанса и [библиотеку Cloud ML Platform](../../../mlplatform-lib/lib-reference).
...
## {heading(Подключение бакета VK Object Storage)[id=add-s3]}

Опция позволяет добавить к инстансу бакет VK Object Storage для хранения в нем данных. Бакет будет доступен внутри инстанса как директория, а хранимые в нем данные будут доступны в [VK Object Storage](/ru/storage/s3) даже после удаления инстанса.

Чтобы подключить бакет VK Object Storage к инстансу:

1. [Перейдите](https://msk.cloud.vk.com/app/) в личный кабинет VK Cloud.
1. Перейдите в раздел **ML Platform** → **Инстансы**.
1. Нажмите ![ ](/ru/assets/more-icon.svg "inline") для нужного инстанса и выберите пункт **Подключить S3 бакет как диск**.
1. В открывшемся окне выберите бакет, который нужно подключить к инстансу. Если подходящих бакетов нет, выберите **Создать новый бакет** — будет создан новый бакет VK Object Storage и подключен к инстансу.  
1. Нажмите кнопку **Сохранить**.
