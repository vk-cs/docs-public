...
1. {linkto(../../instructions/create-cluster/create-webui#k8s-create-webui)[text=Создайте]} кластер Kubernetes, к которому нужно подключиться, или выберите нужный кластер из списка в [личном кабинете](https://msk.cloud.vk.ru/app/) на странице **Кластеры Kubernetes → Кластеры Kubernetes**.
