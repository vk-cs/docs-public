...
1. Задайте:
   ...
   - Настройки master-узлов:
     ...
     - **Зона доступности:** [зона доступности](/ru/start/concepts/architecture#az) для узлов. Для регионального кластера автоматически выбраны все три зоны доступности, изменить выбор нельзя.
