Далее описывается создание кластера первого поколения с помощью Terraform. Также вы можете создать кластер первого поколения [в личном кабинете VK Cloud](../create-webui).
