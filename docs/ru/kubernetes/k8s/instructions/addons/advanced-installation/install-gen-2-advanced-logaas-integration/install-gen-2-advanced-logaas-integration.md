# {heading(Fluent Bit для Cloud Logging (logaas-integration) в кластерах второго поколения)[id=k8s-install-gen-2-advanced-logaas-integration]}

## {heading(Подготовительные шаги)[id=k8s-install-gen-2-advanced-logaas-integration-preparation]}
...
1. {linkto(../../../../../../tools-for-using-services/account/instructions/project-settings/service-account-manage#service-account-create)[text=Cоздайте]} сервисную учетную запись (СУЗ) с правами на запись логов в сервис Cloud Logging. Роли, для которых доступны эти права, перечислены в столбце **Создание сервисных пользователей и названий сервисов** в разделе {linkto(../../../../../../tools-for-using-services/account/concepts/rolesandpermissions#rolesandpermissions-logging)[text=Права в сервисе Cloud Logging]}. Рекомендуемая роль для этой СУЗ — `Аудитор Kubernetes`.
...
1. Установите аддон:
   ...
   1. При необходимости {linkto(#k8s-install-gen-2-advanced-logaas-integration-edit-code)[text=внесите]} дополнительные изменения в код настройки аддона.
...
### {heading(Тонкая настройка поведения аддона при работе с разными уровнями критичности)[id=k8s-install-gen-2-advanced-logaas-integration-settings]}
