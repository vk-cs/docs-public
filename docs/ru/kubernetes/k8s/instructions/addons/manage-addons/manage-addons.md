...
{note:warn}
Постоянные тома (PV), созданные для аддона, не удаляются автоматически, так как на них могут оставаться нужные данные. При необходимости удалите их самостоятельно с помощью {linkto(../../../concepts/storage#k8s-storage-reclaim-policies)[text=политики освобождения томов]}.
{/note}
