...
{note:warn}
Постоянные тома (PV), созданные для аддона, не удаляются автоматически, так как на них могут оставаться нужные данные. При необходимости удалите их самостоятельно одним из способов:

- {linkto(../../manage-pvs#k8s-manage-pvs)[text=через личный кабинет]} (только для кластеров {linkto(../../../concepts/cluster-generations#k8s-cluster-generations)[text=второго поколения]});
- с помощью {linkto(../../../concepts/storage#k8s-storage-reclaim-policies)[text=политики освобождения томов]}.
{/note}
...
{ifndef(public)}
{note:warn}
Постоянные тома (PV), созданные для аддона, не удаляются автоматически, так как на них могут оставаться нужные данные. При необходимости удалите их самостоятельно.
{/note}
{/ifndef}