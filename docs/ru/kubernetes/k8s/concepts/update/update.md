# {heading(Обновление версии кластера)[id=k8s-update]}

Для кластеров Kubernetes в {var(cloud)} доступна операция обновления версии Kubernetes на одну из {linkto(../versions/version-support#k8s-version-support)[text=поддерживаемых версий]}. Понижать версии нельзя.

При обновлении кластера также обновляется часть {linkto(../versions/components#k8s-components)[text=компонентов]} кластера. Причем если обновляемый вместе с кластером компонент был удален, он восстановится при обновлении кластера. Перечень обновляемых компонентов:
...
Аддоны не обновляются вместе с кластером. При необходимости {linkto(../../instructions/addons/manage-addons#k8s-manage-addons-update)[text=обновите аддоны самостоятельно]} после завершения обновления кластера.
...
## {heading(Обновление master- и worker-узлов)[id=k8s-update-process]}
...
1. Убедитесь, что у вас в запасе есть достаточное количество узлов, чтобы Managed Containers мог перенести на них нагрузку с обновляемых узлов. Если worker-узлов недостаточно, {linkto(../../instructions/manage-node-group#k8s-manage-node-group-add-group)[text=добавьте]} их. 
