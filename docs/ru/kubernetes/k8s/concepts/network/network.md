...
## {heading(Работа с сетевыми подсистемами (CNI))[id=supported-cni]}
...
- [Cilium](https://docs.cilium.io/en/stable/index.html) использует eBPF (Linux eXpress Data Path) для реализации сетевых политик и маршрутизации непосредственно в ядре ОС, минуя iptables. Cilium поддерживает фильтрацию трафика на уровнях L3, L4 и L7 (например, по HTTP-заголовкам), а также предоставляет расширенные возможности мониторинга (например, через встроенный инструмент Hubble). Cilium оптимально подходит для очень больших кластеров с высокой нагрузкой и микросервисных архитектур.
   ...
   {note:info}
   Cilium доступен только для кластеров [второго поколения](/ru/kubernetes/k8s/concepts/cluster-generations). 
   {/note}

Обе CNI взаимодействует с платформой VK Cloud с помощью программно-определяемой сети собственной разработки [SDN Sprut](/ru/networks/vnet/concepts/sdn#sprut). Чтобы подключить SDN Sprut к вашему проекту, обратитесь в [техническую поддержку](/ru/contacts).
...
- [Отдельный HTTP(S)-балансировщик с дополнительными настройками](../../how-to-guides/ingress/ingress-http):
