# {heading(Доступные вычислительные ресурсы)[id=k8s-flavors]}

## {heading(Шаблоны конфигурации)[id=k8s-flavors-vm-flavor]}

{ifdef(public)}
При создании кластеров Kubernetes можно выбрать различные {linkto(../../../../computing/iaas/concepts/vm/flavor#iaas-flavor)[text=шаблоны конфигураций]} виртуальных машин, которые определяют вычислительные ресурсы, доступные узлам кластера. Часть шаблонов доступна сразу, а часть — по запросу в [службу поддержки](/ru/contacts).
...
{ifndef(public)}
Шаблон конфигурации ВМ (flavor) Master при создании кластера Kubernetes определяет количество ресурсов ВМ: количество CPU и объем RAM.

При установке {var(cloud)} для кластера создаются шаблоны конфигурации ВМ Standard со следующими характеристиками:

Для master-узлов:
* vCPU: от 2 до 16.
* RAM: от 6 ГБ до 64 ГБ.

Для worker-узлов:
* vCPU: от 1 до 16.
* RAM: от 2 ГБ до 64 ГБ.

Полный перечень доступных шаблонов конфигурации ВМ можно увидеть, развернув список **Тип виртуальной машины Master** при {linkto(../../instructions/create-cluster#k8s-create-cluster)[text=создании кластера]}.
...
## {heading(Расчет подходящей конфигурации кластера)[id=k8s-flavors-configuration-calculation]}
...
1. Создайте кластер со следующими параметрами:
   ...
   {/ifdef}
   {ifndef(public)}
   - **Количество vCPU**: на 5-10% больше, чем расчетное количество для пиковой нагрузки.
   {/ifndef}
   {ifdef(public)}
   - **Количество RAM**: на 20% больше, чем расчетный объем для планируемой нагрузки.
   {/ifdef}
   {ifndef(public)}
   - **Количество RAM**: на 20% больше, чем расчетный объем для пиковой нагрузки.
   {/ifndef}
   ...
   {/ifdef}
   {ifndef(public)}
   - **Количество worker-узлов**: на 10% больше, чем расчетное количество для пиковой нагрузки.
   {/ifndef}

{ifdef(public)}
## {heading(Расчет доступных вычислительных ресурсов CPU и RAM)[id=k8s-flavors-cpu-ram-calculation]}
...
## {heading(Доступные вычислительные ресурсы на узле)[id=k8s-flavors-available-node-resources]}
...
## {heading(Шаблоны конфигурации с GPU)[id=k8s-flavors-gpu]}

{linkto(../../../../computing/gpu/concepts/about#gpu-about)[text=GPU-ускоренные серверы]} позволяют проводить сложные и ресурсозатратные вычисления в области машинного обучения и обработки больших данных. 

Чтобы использовать шаблоны конфигурации с GPU на узлах кластера, [оставьте заявку](https://cloud.vk.ru/cloud-gpu/) на подключение сервиса Cloud GPU.

### {heading(Особенности создания кластера с GPU)[id=k8s-flavors-gpu-features]}

Вы можете использовать шаблон конфигурации с GPU только для worker-узлов кластера. Шаблон конфигурации с GPU должен находиться в той же {linkto(../../../../start/concepts/architecture#architecture-az)[text=зоне доступности]}, что и узел, который его использует.
...
При работе через личный кабинет {var(cloud)} в поле **Тип Node-узлов** будут доступны только шаблоны с GPU, которые находятся в одной зоне доступности с этим узлом.

{note:warn}
Нельзя использовать шаблоны конфигурации с GPU для master-узлов кластера.
{/note}

### {heading(Технологии совместного использования GPU)[id=k8s-flavors-gpu-sharing]}
...
На платформе {var(cloud)} в кластерах Kubernetes доступны две технологии такого рода — MIG и MPS.
