...
При создании кластеров Kubernetes можно выбрать различные [шаблоны конфигураций](/ru/computing/iaas/concepts/vm/flavor) виртуальных машин, которые определяют вычислительные ресурсы, доступные узлам кластера. Часть шаблонов доступна сразу, а часть — по запросу в [службу поддержки](/ru/contacts).
...
[GPU-ускоренные серверы](/ru/computing/gpu/concepts/about) позволяют проводить сложные и ресурсозатратные вычисления в области машинного обучения и обработки больших данных. 
...
Вы можете использовать шаблон конфигурации с GPU только для worker-узлов кластера. Шаблон конфигурации с GPU должен находиться в той же [зоне доступности](/ru/intro/start/concepts/architecture#az), что и узел, который его использует.
...
При работе через личный кабинет VK Cloud в поле **Тип Node-узлов** будут доступны только шаблоны с GPU, которые находятся в одной зоне доступности с этим узлом.
