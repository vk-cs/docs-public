# {heading(Аддоны)[id=k8s-addons]}

Для кластеров Cloud Containers доступны различные аддоны (дополнительные сервисы). Их можно выбрать в любой комбинации и установить либо при {linkto(../../../instructions/create-cluster/create-terraform#k8s-create-terraform)[text=создании кластера с помощью Terraform]}, либо {linkto(../../../instructions/addons/manage-addons#k8s-manage-addons-install)[text=позднее]} в уже существующий кластер. Процесс установки автоматизирован и требует минимального вмешательства пользователя.

## {heading(Особенности установки аддонов)[id=k8s-addons-install-features]}
...
- Аддоны устанавливаются на worker-узлы кластера и потребляют их вычислительные ресурсы.
  ...
  {note:info}
  Некоторые аддоны могут устанавливаться на все узлы кластера (включая master-узлы). Подробнее в разделе {linkto(../../../instructions/addons/advanced-installation#k8s-advanced-installation)[text=%text]}.
  {/note}
...
- Установка аддонов возможна в трех вариантах:
  ...
  Процесс установки описан в разделе {linkto(../../../instructions/addons/advanced-installation#k8s-advanced-installation)[text=%text]}.

## {heading(Доступные аддоны)[id=k8s-addons-available]}
...
- **CPU**: 512m;
- **RAM**: 1024Mi;
- **Стандартный балансировщик нагрузки**: одна штука (при настройке внешнего доступа к кластеру, например через [Ingress Controller](#k8s-addons-ingress-controller-nginx)).
...
{note:info}
Этот аддон доступен только для кластеров {linkto(../../cluster-generations#k8s-cluster-generations)[text=первого поколения]}.
{/note}
...
[Container Storage Interface для S3 (S3-CSI)](https://github.com/yandex-cloud/k8s-csi-s3) — драйвер [Container Storage Interface](https://kubernetes-csi.github.io/docs/) для Kubernetes, который позволяет монтировать S3-совместимые объектные хранилища ({linkto(/ru/storage/s3/concepts/about#s3-concepts-about)[text={var(s3)}]}, AWS S3, MinIO и другие) как {linkto(/ru/kubernetes/k8s/reference/pvs-and-pvcs#k8s-pvs-and-pvcs)[text=постоянные тома]} (Persistent Volumes, PV). 
...
Подробнее в разделе {linkto(../../../connect/docker-registry#k8s-docker-registry)[text=%text]}.
...
Подробнее о том, как использовать аддон с менеджером секретов {var(cloud)}, в разделе {linkto(../../../how-to-guides/external-secrets-operator#k8s-external-secrets-operator)[text=%text]}.
...
Источниками логов выступают [службы kubelet](https://kubernetes.io/docs/concepts/overview/components/#kubelet) и {linkto(../../../reference/pods#k8s-pods)[text=поды]} (pods), расположенные на узлах кластера.
...
### {heading(Fluent Bit для Cloud Logging (logaas-integration))[id=k8s-addons-fluent-bit-logaas]}
...
Источниками логов выступают [службы kubelet](https://kubernetes.io/docs/concepts/overview/components/#kubelet) и {linkto(../../../reference/pods#k8s-pods)[text=поды]} (pods), расположенные на узлах кластера. Подробнее о том, как работает аддон, читайте в разделах про его установку для кластеров {linkto(../../../instructions/addons/advanced-installation/install-advanced-logaas-integration/install-advanced-logaas-integration-magnum#k8s-install-advanced-logaas-integration)[text=первого]} и {linkto(../../../instructions/addons/advanced-installation/install-advanced-logaas-integration/install-advanced-logaas-integration-mk8s#k8s-install-gen-2-advanced-logaas-integration)[text=второго]} поколений.
...
GPU Operator позволяет управлять {linkto(../../flavors#k8s-flavors-gpu)[text=GPU на узлах кластера]} для выполнения машинного обучения или обработки больших данных.

Доступны следующие варианты использования GPU в кластере:

- Один под использует один или несколько GPU.
- Аддон распределяет один GPU между несколькими подами по стратегии {linkto(../../flavors#k8s-flavors-gpu-sharing)[text=MIG]}.
- Аддон распределяет один GPU между несколькими подами по стратегии {linkto(../../flavors#k8s-flavors-gpu-sharing)[text=MPS]}.
...
Ingress-контроллер тесно интегрируется с платформой VK Cloud. Подробнее в разделе {linkto(../../network#k8s-network)[text=%text]}.
...
- **RAM**: 238Mi — 660Mi.
- **Стандартный балансировщик нагрузки**: одна штука.
- **Floating IP-адрес**: одна штука (при установке с {linkto(../../../instructions/addons/advanced-installation/install-advanced-ingress#k8s-install-advanced-ingress)[text=настройками по умолчанию]}).
...
{include(/ru/_includes/_istio_descr.md)}

Установка аддона отличается для кластеров {linkto(../../../instructions/addons/advanced-installation/install-advanced-istio/install-advanced-istio-magnum#k8s-install-advanced-istio-magnum)[text=первого]} и {linkto(../../../instructions/addons/advanced-installation/install-advanced-istio/install-advanced-istio-mk8s#k8s-install-advanced-istio-mk8s)[text=второго]} поколений.
...
- Количество worker-узлов должно быть не менее выбранного количества реплик Elasticsearch.
  ...
  Подробнее про выбор количества реплик читайте в {linkto(../../../instructions/addons/advanced-installation/install-advanced-jaeger#k8s-install-advanced-jaeger)[text=разделе про установку Jaeger]}.
...
Подробнее о работе с аддоном в разделе {linkto(../../../how-to-guides/kgateway#k8s-kgateway)[text=Использование Kgateway]}.
...
[Kiali](https://kiali.io/) — веб-интерфейс для работы с {linkto(#k8s-addons-istio)[text=Istio]}. Он позволяет управлять сервисной сеткой (service mesh), отслеживать ее состояние и визуализировать ее.
...
Kube Prometheus Stack — система мониторинга состояния кластера и развернутых в нем сервисов, реализованная на базе [Prometheus](https://prometheus.io/) и инструмента визуализации [Grafana](https://grafana.com/). Подробнее в разделе {linkto(../../../monitoring#k8s-monitoring-connect-grafana)[text=%text]}.

{var(cloud)} предоставляет две {linkto(../../versions/components#k8s-components-addons)[text=версии аддона Kube Prometheus Stack]}: базовую и с доработками от компании VK Tech (версия с суффиксом `vk` в номере). Аддон с доработками содержит дополнительные встроенные функции, в частности, {linkto(../../../monitoring#k8s-monitoring-forecast-consumption)[text=прогнозирование потребления ресурсов кластера]}.
...
1. Предоставить рекомендации и, если настроено, автоматически применить их к {linkto(../../../reference/resource-limiting#k8s-resource-limiting)[text=запросам ресурсов]} новых и уже запущенных подов.
