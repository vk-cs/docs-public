...
## {heading(Интеграция с платформой VK Cloud)[id=integration]}
...
- [Container Network Interface](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/) (CNI) — интеграция с сетевыми подсистемами.
  ...
  В кластерах Kubernetes, которые вы создаете в сервисе Cloud Containers, реализованы плагины, поддерживающие этот интерфейс: [Calico](https://projectcalico.docs.tigera.io/about/about-calico) и [Cilium](https://docs.cilium.io/en/stable/index.html) (доступен только для кластеров второго поколения). Они обеспечивают:
  ...
  Calico и Cilium интегрируются с платформой VK Cloud с помощью SDN Sprut. Подробнее в разделе [Сеть в кластере](../network).
