...
## {heading(Типы развертывания worker-узлов)[id=k8s-architecture-deployment-types]}

В Cloud Containers доступны два типа worker-узлов для развертывания:
...
- `Bare Metal` — узлы будут развернуты на арендованном сервере {linkto(../../../../../computing/bare-metal/concepts/about#bare-metal-about)[text=Bare Metal]}, без виртуализации и с доступом к полному вычислительному ресурсу физического сервера. При таком типе развертывания на производительность не влияет активность других виртуальных машин. Рекомендуется для ресурсоемких приложений.
   ...
   Группу узлов с типом развертывания `Bare Metal` можно добавить только в уже существующий кластер {linkto(../../concepts/cluster-generations#k8s-cluster-generations)[text=второго поколения]} с {linkto(../../concepts/versions/version-support#k8s-version-support)[text=версией]} не ниже 1.34.
...
- Для worker-узлов оно происходит с помощью {linkto(../cluster-autoscaler#k8s-cluster-autoscaler)[text=Cluster Autoscaler]} и включается вручную при определении {linkto(../../instructions/manage-node-group#k8s-manage-node-group)[text=настроек для каждой группы узлов]}.