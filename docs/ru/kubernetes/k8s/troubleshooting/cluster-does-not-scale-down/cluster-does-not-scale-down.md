# {heading(Недозагруженные узлы не удаляются при автоматическом масштабировании)[id=k8s-cluster-does-not-scale-down]}

В кластере есть недозагруженные узлы, но они не удаляются автоматически, несмотря на работу {linkto(../../concepts/cluster-autoscaler#k8s-cluster-autoscaler)[text=агента автоматического масштабирования (Cluster Autoscaler)]}. По умолчанию агент автоматического масштабирования должен удалять узлы, на которых нет запущенных подов или все поды которых можно безопасно вытеснить. 
...
1. Проверьте, есть ли в {linkto(../../how-to-guides/autoscaler-logs#k8s-autoscaler-logs)[text=логах агента автоматического масштабирования]} предупреждения, содержащие `pdb blocking`. Если есть, то сейчас выполняется операция или набор операций, которые требуют прерывания (disruption) подов, и эти прерывания превышают допустимое количество, указанное в Pod Disruption Budget (PDB). 
...
1. {linkto(../../connect/kubectl#k8s-kubectl)[text=Подключитесь к кластеру]} с помощью `kubectl`.
1. Проверьте, есть ли в {linkto(../../how-to-guides/autoscaler-logs#k8s-autoscaler-logs)[text=логах агента автоматического масштабирования]} сообщения, содержащие строчку:
