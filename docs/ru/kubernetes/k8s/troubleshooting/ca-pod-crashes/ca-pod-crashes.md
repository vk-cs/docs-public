# {heading(Агент автоматического масштабирования не может пройти аутентификацию в OpenStack или API Kubernetes)[id=k8s-ca-pod-crashes]}

При попытке подключения к OpenStack или API Kubernetes под {linkto(../../concepts/cluster-autoscaler#k8s-cluster-autoscaler)[text=агента автоматического масштабирования (Cluster Autoscaler)]} находится в статусе `CrashLoopBackOff`, из-за чего не проходит аутентификация.
...
### {heading(Решение)[id=k8s-ca-pod-crashes-solution]}
