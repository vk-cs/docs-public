# {heading(Быстрый старт)[id=k8s-quick-start]}
...
1. Познакомитесь с Kubernetes и {linkto(../concepts/addons-and-settings/addons#k8s-addons)[text=аддонами для него]}:
...
После прохождения быстрого старта {linkto(#k8s-quick-start-delete-resources)[text=остановите или удалите кластер]}, если он вам больше не нужен.
{/note}
...
1. Получите kubeconfig для кластера в [личном кабинете {var(cloud)}](https://msk.cloud.vk.com/app/):
...
## {heading(Удалите неиспользуемые ресурсы)[id=k8s-quick-start-delete-resources]}
...
## {heading(Что дальше?)[id=k8s-quick-start-what-next]}
...
- {linkto(../connect#k8s-connect)[text=Познакомьтесь с подробными инструкциями]} по подключению к кластеру.