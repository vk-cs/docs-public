...
- IP-адреса, которые слушает сервер:
  ...
  Такая конфигурация используется, потому что `kube-proxy` в кластерах Cloud Containers {linkto(../../../concepts/addons-and-settings/settings#k8s-settings-kube-proxy-mode)[text=работает]} в режиме `iptables`.
