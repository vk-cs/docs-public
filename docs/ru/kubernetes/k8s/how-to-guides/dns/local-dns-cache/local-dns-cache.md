# {heading(Настройка локального DNS-кеша)[id=k8s-local-dns-cache]}
...
## {heading(Обзор архитектуры DNS в кластере)[id=k8s-local-dns-cache-architecture]}

Пусть некоторый {linkto(../../../reference/pods#k8s-pods)[text=под]} инициирует DNS-запрос.
...
## {heading(Подготовительные шаги)[id=k8s-local-dns-cache-prepare]}
...
## {heading(1. Разверните кеширующий DNS-сервер на каждом узле)[id=k8s-local-dns-cache-deploy]}
...
- IP-адреса, которые слушает сервер:
  ...
  Такая конфигурация используется, потому что `kube-proxy` в кластерах Kubernetes в сервисе Managed Containers {linkto(../../../concepts/addons-and-settings/settings#k8s-settings-kube-proxy-mode)[text=работает]} в режиме `iptables`.
...
## {heading(2. Проверьте работу кеширующего DNS-сервера)[id=k8s-local-dns-cache-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=k8s-local-dns-cache-delete]}

Работающий кластер тарифицируется и потребляет вычислительные ресурсы. Если ресурсы Kubernetes, созданные для проверки работы локального DNS-кеша, вам больше не нужны, удалите их:

1. Удалите созданный под и ресурсы, описанные в манифесте `nodelocaldns.yaml`:
