# {heading(Настройка обязательных меток)[id=k8s-labels]}
...
## {heading(Подготовительные шаги)[id=k8s-labels-prepare]}
...
## {heading(Создайте ограничение, проверяющее метки)[id=k8s-labels-create-constraint]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=k8s-labels-delete]}
