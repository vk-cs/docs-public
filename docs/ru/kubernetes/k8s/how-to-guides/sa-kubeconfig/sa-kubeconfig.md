...
1. (Опционально) Познакомьтесь со структурой kubeconfig:
   ...
   - `clusters`: перечень кластеров и данных для подключения к ним.
     ...
     Kubeconfig для кластера Kubernetes содержит запись о единственном кластере.

   - `users`: перечень пользователей и данных для их аутентификации в кластере.

     Kubeconfig для кластера Kubernetes содержит запись о единственном пользователе, который аутентифицируется с помощью `keystone-auth`.
   ...
   - `contexts`: контекст, в рамках которого работает `kubectl`. В самом простом случае контекст — это комбинация имени кластера и имени пользователя.
     ...
     Kubeconfig для кластера Kubernetes содержит запись о единственном контексте. Этот контекст использует запись о кластере и пользователе, которые уже определены в kubeconfig.
...
Если при выполнении команд пароль не был запрошен, то полученный kubeconfig можно использовать в комбинации с автоматизированными инструментами для доступа к кластеру Kubernetes в сервисе Managed Containers.
