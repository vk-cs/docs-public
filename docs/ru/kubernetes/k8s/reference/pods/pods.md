...
- Более редкий случай: под может состоять из нескольких контейнеров. Эти контейнеры могут взаимодействовать друг с другом и разделяют между собой ресурсы, назначенные поду (например, хранилище).
  ...
  {ifdef(public)}
  - Для стабильной работы кластера рекомендуется {linkto(../resource-limiting#k8s-resource-limiting)[text=ограничивать потребление вычислительных ресурсов]} контейнером пода.
  {/ifdef}
  {ifndef(public)}
  - Для стабильной работы кластера рекомендуется ограничивать потребление вычислительных ресурсов контейнером пода.
  {/ifndef}
  {/note}
