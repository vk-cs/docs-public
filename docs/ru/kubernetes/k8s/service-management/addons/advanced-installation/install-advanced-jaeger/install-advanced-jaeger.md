...
Реплики Elasticsearch размещаются на worker-узлах кластера и используют [постоянные тома](../../../../reference/pvs-and-pvcs) в качестве хранилища. По умолчанию эти постоянные тома размещаются в той же [зоне доступности](/ru/additionals/start/architecture#zony_dostupnosti_d9f6db93), в которой находятся master-узлы кластера. Если worker-узлы кластера и постоянные тома будут находиться в разных зонах доступности, то реплики на этих узлах не смогут работать с томами.
