...
1. Задайте:
   ...
   - Настройки сети:
     ...
     - **Сеть:** выберите сеть и подсеть, в которых будут размещаться master- и worker-узлы кластера. Если нужных сети и подсети нет в списке, [создайте](/ru/networks/vnet/networks/create-net) их.
     - **Использовать сеть Load Balancer**: включите эту опцию, чтобы для балансировщиков нагрузки, создаваемых кластером, использовалась отдельная подсеть в выбранной сети. Если нужной подсети нет в списке, [создайте](/ru/networks/vnet/networks/create-net#sozdanie_podseti) ее.
