...
1. Создайте файл, который содержит в себе значения ([values](https://helm.sh/docs/chart_template_guide/values_files/)), нужные для установки ExternalDNS с помощью Helm:
   ...
   - Настройки фильтрации DNS-зон:
     ...
     - `SERVER_READ_TIMEOUT`: таймаут на чтение при открытом соединении к серверу (в секундах). Значение по умолчанию: `30`.
     - `SERVER_WRITE_TIMEOUT`: таймаут на запись при открытом соединении к серверу (в секундах). Значение по умолчанию: `30`.
