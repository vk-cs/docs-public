...
[HAMi](https://project-hami.io/) (Heterogeneous AI Computing Virtualization Middleware) — набор инструментов для управления гетерогенными ИИ-ускорителями для Kubernetes. 

Аддон HAMi в Managed Containers дает возможность дробить ресурсы GPU (память, вычислительную мощность или виртуальные карты) между разными процессами, например, чтобы запускать больше ML-моделей на существующем оборудовании. Так одна физическая карта будет обслуживать несколько задач сразу, и для этого не нужно вносить изменения в код ваших приложений.

Преимущества аддона HAMi:

- Эффективно разделяет GPU между подами с возможностью выделить определенную часть ресурсов для каждого пода.
- Изолирует память и вычислительные ресурсы GPU на уровне отдельного пода.
- Повышает утилизацию worker-узлов на GPU и снижает совокупную стоимость владения GPU-инфраструктурой.
- Снижает порог входа для команд, занимающихся машинным обучением и искусственным интеллектом, которым не требуется целая GPU-карта для таких задач, как инференс, RAG, обработка изображений.

Подробнее об аддоне и его архитектуре в [официальной документации HAMi](https://project-hami.io/docs).

{note:warn}
Аддон HAMi подходит только для кластеров с {linkto(../../flavors#mk8s-flavors-gpu)[text=worker-узлами на GPU]}. Для работы аддона необходим Kubernetes {linkto(../../versions/version-support#mk8s-version-support)[text=версии 1.33]} или выше.
{/note}
