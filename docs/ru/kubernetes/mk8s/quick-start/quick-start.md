# {heading(Быстрый старт)[id=mk8s-quick-start]}
...
1. Познакомитесь с Kubernetes и {linkto(../concepts/addons-and-settings/addons#mk8s-addons)[text=аддонами для него]}:
   1. Подключите инструменты для управления и мониторинга.
   1. Загрузите Docker-образы в реестр Docker.
   1. Развернете простые приложения на основе загруженных образов, с возможностью использовать хранилище VK Object Storage.
...
После прохождения быстрого старта {linkto(#mk8s-quick-start-delete-resources)[text=остановите или удалите кластер]}, если он вам больше не нужен.
{/note}

## {counter(kuber_level_1)[id=create_cluster]}. Создайте и настройте кластер
...
1. Нажмите кнопку **Настроить** в блоке **Сеть и настройки** и задайте:

   1. **Сеть:** `Создать новую сеть`.
   1. **Назначить внешний IP:** убедитесь, что эта опция выбрана.
   1. Другие настройки оставьте без изменений.

1. Нажмите кнопку **Настроить** в блоке **Группы узлов** и задайте:
...
1. [Установите аддон](/ru/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-registry) `docker-registry`.

   Запишите данные для доступа к реестру Docker.

1. [Установите аддон](/ru/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-monitoring) `kube-prometheus-stack`.

   Запишите пароль для доступа к веб-интерфейсу Grafana.

1. [Установите аддон](/ru/kubernetes/mk8s/instructions/addons/advanced-installation/install-advanced-ingress) `ingress-nginx` с параметрами по умолчанию.
...
- Браузер.
- Утилиту `kubectl`. Подробнее в разделе [Подключение к кластеру с помощью kubectl](/ru/kubernetes/mk8s/connect/kubectl#mk8s-kubectl-before-work) и [официальной документации](https://kubernetes.io/docs/tasks/tools/#kubectl).
...
- Утилиту `client-keystone-auth`. Подробнее в разделе [Подключение к кластеру с помощью kubectl](/ru/kubernetes/mk8s/connect/kubectl).
...
## {counter(kuber_level_1)}. Подключитесь к кластеру
...
1. [Активируйте доступ по API](/ru/tools-for-using-services/api/rest-api/enable-api#rest-api-enable-activate) для этого пользователя.

1. Получите kubeconfig для кластера в [личном кабинете {var(cloud)}](https://msk.cloud.vk.ru/app/):
...
В кластере был установлен аддон со [средствами мониторинга](/ru/kubernetes/mk8s/monitoring) на базе Prometheus и Grafana. 
...
## {heading({counter(kuber_level_1)}. Загрузите нужные образы в реестр Docker)[id=mk8s-quick-start-upload-to-registry]}

В кластере был установлен аддон [реестра Docker](/ru/kubernetes/mk8s/connect/docker-registry), в котором будут храниться Docker-образы.
...
1. Можно монтировать хранилище {var(cloud)} к подам с помощью Persistent Volume Claim.
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-quick-start-delete-resources]}

Работающий кластер потребляет вычислительные ресурсы. Если они вам больше не нужны:

- [Остановите](/ru/kubernetes/mk8s/instructions/manage-cluster#mk8s-manage-cluster-start-stop) кластер, чтобы воспользоваться им позже, или [удалите](/ru/kubernetes/mk8s/instructions/manage-cluster#mk8s-manage-cluster-delete) его навсегда.
