# {heading(Недозагруженные узлы не удаляются при автоматическом масштабировании)[id=mk8s-cluster-does-not-scale-down]}

В кластере есть недозагруженные узлы, но они не удаляются автоматически, несмотря на работу {linkto(../../concepts/cluster-autoscaler#mk8s-cluster-autoscaler)[text=агента автоматического масштабирования (Cluster Autoscaler)]}. По умолчанию агент автоматического масштабирования должен удалять узлы, на которых нет запущенных подов или все поды которых можно безопасно вытеснить. 
...
1. Проверьте, есть ли в {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=логах агента автоматического масштабирования]} предупреждения, содержащие `pdb blocking`. Если есть, то сейчас выполняется операция или набор операций, которые требуют прерывания (disruption) подов, и эти прерывания превышают допустимое количество, указанное в Pod Disruption Budget (PDB). 
...
1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=Подключитесь к кластеру]} с помощью `kubectl`.
1. Проверьте, есть ли в {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=логах агента автоматического масштабирования]} сообщения, содержащие строчку:
   ...
   В Managed Containers время простоя составляет 5 минут. Если узел пуст в течение 5 минут (все поды с него вытеснены), и за это время ни один под не появляется, агент автоматического масштабирования инициирует удаление этого узла.
