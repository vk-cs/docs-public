# {heading(Не добавляются новые узлы при автоматическом масштабировании)[id=mk8s-cluster-does-not-scale-up]}

Новые узлы не добавляются, несмотря на работу {linkto(../../concepts/cluster-autoscaler#mk8s-cluster-autoscaler)[text=агента автоматического масштабирования (Cluster Autoscaler)]}, и поды находятся в состоянии `Pending`. 
...
1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=Подключитесь к кластеру]} с помощью `kubectl`.
...
Подробнее о ресурсах в разделе {linkto(../../reference/resource-limiting#mk8s-resource-limiting)[text=Ограничение использования ресурсов]} и [официальной документации Kubernetes](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/). 
...
1. Просмотрите количество узлов в группе узлов. 
   ...
   {tab(kubectl)}
   1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=Подключитесь к кластеру]} с помощью `kubectl`.
   1. Проверьте, есть ли в {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=логах агента автоматического масштабирования]} предупреждения, содержащие `max size reached`. Если есть, то в группе достигнуто максимальное количество узлов.
   {/tab}
   {/tabs}

1. {linkto(../../instructions/scale#mk8s-instructions-scale-horizontal-autoscaling-worker-nodes)[text=Увеличьте]} максимальное количество узлов до нужного значения.

Подробнее в разделе {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=Управление группой worker-узлов]}. 
...
1. Просмотрите квоты на vCPU, RAM и виртуальные машины.
   ...
   1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=Подключитесь к кластеру]} с помощью `kubectl`.
   1. Проверьте, есть ли в {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=логах агента автоматического масштабирования]} предупреждения, содержащие `quota exceeded`. Если есть, то в проекте закончились квоты на соответствующие ресурсы.
...
1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=Подключитесь к кластеру]} с помощью `kubectl`.
1. Проверьте, есть ли в {linkto(../../how-to-guides/autoscaler-logs#mk8s-autoscaler-logs)[text=логах агента автоматического масштабирования]} предупреждения, содержащие `predicate failed`. Если есть, то проблема в правилах планирования.
1. Выведите манифест пода, в котором наблюдается проблема, с помощью команды:
...
1. В выводе манифеста просмотрите правила планирования. Они могут быть заданы с помощью селектора узлов (`nodeSelector`), правил размещения узлов (`affinity`) или исключений (`tolerations`). Если существующие или потенциальные новые узлы не удовлетворяют ни одним из существующих правил планирования, выберите один из вариантов:
   ...
   - {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-labels-taints)[text=Cогласуйте]} правила пода с указанными правилами и требованиями.
   - {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=Добавьте]} новую группу узлов, соответствующую указанным требованиям. 
   
Подробнее о метках, ограничениях и исключениях в разделе {linkto(../../reference/labels-and-taints#mk8s-labels-and-taints)[text=Метки и ограничения]}.
...
Подробнее о томах в разделе {linkto(../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=Постоянные тома и PVC]}.
...
1. Сравните ресурсы, указанные в блоке `resources.requests`, с характеристиками виртуальных машин в соответствующих {linkto(../../instructions/helpers/node-group-settings#mk8s-node-group-settings)[text=группах узлов]}. 

1. Если ресурсов недостаточно, {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-add-group)[text=создайте]} новую группу узлов с {linkto(../../concepts/flavors#mk8s-flavors-vm-flavor)[text=шаблоном виртуальной машины]}, подходящим по количеству ресурсов.
