# {heading(Не работает режим RWX)[id=mk8s-pv-rwx]}

Режим доступа к тому ReadWriteMany (RWX) не работает: у узлов кластера нет доступа к диску.

Это может быть связано с теми, что в Managed Containers доступ к PVC в режиме RWX не реализован.

### {heading(Решение)[id=mk8s-pv-rwx-solution]}
