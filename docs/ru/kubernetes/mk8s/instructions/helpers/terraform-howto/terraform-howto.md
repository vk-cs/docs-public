# {heading(Использование Terraform)[id=mk8s-terraform-howto]}

## {heading(Как начать пользоваться Terraform)[id=mk8s-terraform-howto-begin]}
...
1. {linkto(../../create-cluster/create-terraform#mk8s-create-terraform)[text=Создайте]} новый кластер с одной или несколькими группами worker-узлов.
...
## {heading(Особенности использования Terraform для управления сервисом Managed Containers)[id=mk8s-terraform-howto-features]}

- Некоторые операции с кластером выполняются только в {linkto(../../../../../tools-for-using-services/account#tools-account)[text=личном кабинете]}. Для каждой операции в {linkto(../../../instructions#mk8s-instructions)[text=пошаговых инструкциях]} указано, можно ли ее выполнить также с помощью Terraform.
