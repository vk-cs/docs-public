# {heading(Создание кластера с помощью Terraform)[id=mk8s-create-terraform]}

Далее описывается создание кластера с помощью Terraform. Также вы можете создать кластер {linkto(../create-webui-gen-2#mk8s-create-webui-gen-2)[text=в личном кабинете {var(cloud)}]}.
...
## {heading(1. Подготовьте необходимые источники данных)[id=mk8s-create-terraform-prepare-data-sources]}
...
1. Добавьте в конфигурационный файл источники данных:
   ...
   1. {linkto(../../../concepts/flavors#mk8s-flavors-vm-flavor)[text=Шаблон виртуальной машины]} для master-узлов. Пример:
...
## {heading(2. Опишите конфигурацию кластера)[id=mk8s-create-terraform-describe-conf]}
...
- `master_count` — количество master-узлов. Должно быть нечетным числом. Для стандартного кластера количество master-узлов должно быть `1`, `3` или `5`. Для регионального — `3` или `5`. Подробнее в разделе {linkto(../../../concepts/architecture#mk8s-architecture)[text=Архитектура сервиса]}.
- `cluster_node_volume_type` — тип диска для {linkto(../../../concepts/storage#mk8s-storage-supported-storage-types)[text=хранения данных]}, который будет использоваться узлами. Выбранный тип диска влияет на производительность кластера. Доступные значения: `ceph-ssd` (по умолчанию) и `high-iops`.
...
Чтобы установить аддоны в кластер с помощью Terraform, {linkto(../../addons/manage-addons#mk8s-manage-addons-available)[text=получите список доступных аддонов]} и {linkto(../../addons/advanced-installation#mk8s-advanced-installation)[text=установите нужные]}.

## {heading(3. Опишите конфигурацию одной или нескольких групп worker-узлов)[id=mk8s-create-terraform-describe-workers]}
...
Эта операция подробно описана в разделе {linkto(../../manage-node-group#mk8s-manage-node-group)[text=Управление группой worker-узлов]}.

## {heading(4. Запустите процедуру создания кластера)[id=mk8s-create-terraform-apply]}
...
## {heading(Что дальше?)[id=mk8s-create-terraform-what-next]}
