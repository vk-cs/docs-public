...
## {heading(Получить реквизиты для подключения к кластеру)[id=mk8s-manage-cluster-get-requisites]}

Доступные реквизиты:

- файл конфигурации `kubectl`, чтобы иметь возможность управлять кластером Kubernetes из командной строки. 
- секрет для Headlamp (его можно получить только когда кластер запущен).

Эти процессы подробно описаны в разделе {linkto(../../connect#mk8s-connect)[text=Подключение к кластеру]}.
