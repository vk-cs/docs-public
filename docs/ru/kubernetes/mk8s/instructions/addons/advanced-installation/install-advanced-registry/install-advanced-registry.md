# {heading(Docker Registry)[id=mk8s-install-advanced-registry]}

## {heading(Подготовительные шаги)[id=mk8s-install-advanced-registry-prepare]}
...
## {heading(Установка аддона)[id=mk8s-install-advanced-registry-install]}
...
Для аддона {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-docker-registry)[text=Docker Registry]} доступно {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-install-features)[text=несколько вариантов установки]}:
...
1. Установите аддон:
   ...
   1. Нажмите кнопку **Установить** на карточке аддона `docker-registry` и нажмите кнопку **Установить аддон**.
   1. (Опционально) Отредактируйте:

      - название приложения;
      - название пространства имен, куда будет установлен аддон.

   1. Отредактируйте {linkto(#mk8s-install-advanced-registry-edit-code)[text=код настройки аддона]}.
...
1. {linkto(#mk8s-install-advanced-registry-connect)[text=Получите данные для доступа к реестру]}.
...
## {heading(Редактирование кода настройки аддона при установке)[id=mk8s-install-advanced-registry-edit-code]}

{note:info}
- При редактировании кода настройки аддона воспользуйтесь сведениями, {linkto(#mk8s-install-advanced-registry-prepare)[text=полученными ранее]}.
...
После редактирования кода {linkto(#mk8s-install-advanced-registry-install)[text=продолжите установку аддона]}.

## {heading(Подключение к реестру)[id=mk8s-install-advanced-registry-connect]}
...
1. {linkto(../../../../connect/docker-registry#mk8s-docker-registry)[text=Подключитесь]} к реестру Docker.