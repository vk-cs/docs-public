...
## {heading(Подготовительные шаги)[id=mk8s-install-advanced-argo-cd-prep]}

{include(/ru/_includes/_addon-prep.md)}

## {heading(Установка аддона)[id=mk8s-install-advanced-argo-cd-install]}

Для аддона {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-argo-cd)[text=Argo CD]} доступно {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-install-features)[text=несколько вариантов установки]}.
...
1. Установите аддон:
   ...
   {tab(Личный кабинет)}
    ...
    1. Нажмите кнопку **Установить** на карточке аддона `argocd` и нажмите кнопку **Установить аддон**.
    1. (Опционально) Отредактируйте:

        - выбранную версию;
        - название приложения;
        - название пространства имен, куда будет установлен аддон;
        - {linkto(#mk8s-install-advanced-argo-cd-edit-code)[text=код настройки аддона]}.
...
## {heading(Редактирование кода настройки аддона при установке)[id=mk8s-install-advanced-argo-cd-edit-code]}
