# {heading(Fluent Bit для Cloud Logging (logaas-integration))[id=mk8s-install-gen-2-advanced-logaas-integration]}

## {heading(Подготовительные шаги)[id=mk8s-install-gen-2-advanced-logaas-integration-preparation]}
...
1. {linkto(../../../../../../../access/iam/instructions/service-account-manage#service-account-create)[text=Cоздайте]} сервисную учетную запись (СУЗ) с правами на запись логов в сервис Cloud Logging. Роли, для которых доступны эти права, перечислены в  {linkto(../../../../../../../access/iam/concepts/roles-reference#iam-roles-reference)[text=справочнике ролей]}. Рекомендуемая роль для этой СУЗ — `Аудитор Kubernetes`.
...
1. Установите аддон:
   ...
   1. При необходимости {linkto(#mk8s-install-gen-2-advanced-logaas-integration-edit-code)[text=внесите]} дополнительные изменения в код настройки аддона.
...
## {heading(Редактирование кода настройки аддона при установке)[id=mk8s-install-gen-2-advanced-logaas-integration-edit-code]}
...
### {heading(Тонкая настройка поведения аддона при работе с разными уровнями критичности)[id=mk8s-install-gen-2-advanced-logaas-integration-settings]}
...
1. Добавляет в логи дополнительные метаданные, что облегчает работу с логами кластеров Kubernetes сервиса Managed Containers (например, поиск нужных логов в Cloud Logging). Для этого применяются специальные фильтры Fluent Bit (filters), [написанные на языке Lua](https://docs.fluentbit.io/manual/pipeline/filters/lua). Эти метаданные содержат, в том числе, уровень критичности логируемых событий.
   ...
   - Идентификатор ресурса (`stream-id`) в одном из форматов:
     ...
     - Для {linkto(../../../../../reference/pods#mk8s-pods)[text=пода]} (pod): `<имя пространства имен (namespace)>.<имя>`.
...
- Единственное правило действует на уровне пространства имен `kube-system`.
...
- Единственное правило действует на уровне подов, которые находятся в пространстве имен `default`, и чьи имена начинаются с префикса `test-pod`.
...
- Несколько правил действуют на уровне пространства имен `my-namespace` и на уровне подов с префиксом `example-pod`, которые находятся в этом пространстве имен.
