...
## {heading(Подготовительные шаги)[id=mk8s-install-advanced-eso-prep]}

{include(/ru/_includes/_addon-prep.md)}

## {heading(Установка аддона)[id=mk8s-install-advanced-eso-install]}

Для аддона {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-eso)[text=External Secrets Operator]} доступно {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-install-features)[text=несколько вариантов установки]}.
...
1. Установите аддон:
   ...
   {tab(Личный кабинет)}
    ...
    1. Нажмите кнопку **Установить** на карточке аддона `external-secrets-operator` и нажмите кнопку **Установить аддон**.
    1. (Опционально) Отредактируйте:

        - выбранную версию;
        - название приложения;
        - название пространства имен, куда будет установлен аддон;
        - {linkto(#mk8s-install-advanced-eso-edit-code)[text=код настройки аддона]}.
...
1. (Опционально) {linkto(../../../../how-to-guides/external-secrets-operator#mk8s-external-secrets-operator)[text=Познакомьтесь с практическим руководством по работе с External Secrets Operator]}.
...
## {heading(Редактирование кода настройки аддона при установке)[id=mk8s-install-advanced-eso-edit-code]}
