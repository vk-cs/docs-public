# {heading(Kgateway)[id=mk8s-install-advanced-kgateway]}

## {heading(Подготовительные шаги)[id=mk8s-install-advanced-kgateway-prep]}

{include(/ru/_includes/_addon-prep.md)}

## {heading(Установка аддона)[id=mk8s-install-advanced-kgateway-install]}

Для аддона {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-kgateway)[text=Kgateway]} доступно {linkto(../../../../concepts/addons-and-settings/addons#mk8s-addons-install-features)[text=несколько вариантов установки]}.
...
1. Установите аддон:
   ...
   {tab(Личный кабинет)}
    ...
    1. Нажмите кнопку **Установить** на карточке аддона `kgateway` и нажмите кнопку **Установить аддон**.
    1. (Опционально) Отредактируйте:

        - выбранную версию;
        - название приложения;
        - название пространства имен, куда будет установлен аддон;
        - {linkto(#mk8s-install-advanced-kgateway-edit-code)[text=код настройки аддона]}.
...
1. (Опционально) {linkto(../../../../how-to-guides/kgateway#mk8s-kgateway)[text=Познакомьтесь с практическим руководством по работе с Kgateway]}.
...
## {heading(Редактирование кода настройки аддона при установке)[id=mk8s-install-advanced-kgateway-edit-code]}
