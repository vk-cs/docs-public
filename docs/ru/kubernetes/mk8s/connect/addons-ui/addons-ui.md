# {heading(Подключение к веб-интерфейсам аддонов)[id=mk8s-addons-ui]}

У некоторых {linkto(../../concepts/addons-and-settings/addons#mk8s-addons)[text=аддонов]}, которые можно {linkto(../../instructions/addons/manage-addons#mk8s-manage-addons)[text=установить]} в кластер, есть веб-интерфейс. Способ подключения к веб-интерфейсу зависит от IP-адреса кластера:
...
## {heading(Подготовительные шаги)[id=mk8s-addons-ui-before-work]}

1. На хосте, с которого планируется подключаться к веб-интерфейсу аддона, {linkto(../kubectl#mk8s-kubectl-check-connection)[text=убедитесь]}, что вы можете подключаться к кластеру с помощью `kubectl`.
...
1. {linkto(../../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=Убедитесь]}, что аддон `jaeger` {linkto(../../instructions/addons/advanced-installation/install-advanced-jaeger#mk8s-install-advanced-jaeger)[text=установлен]} в кластере.
...
1. {linkto(../../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=Убедитесь]}, что аддон `kube-prometheus-stack` {linkto(../../instructions/addons/advanced-installation/install-advanced-monitoring#mk8s-install-advanced-monitoring)[text=установлен]} в кластере.
1. На хосте в отдельной сессии терминала выполните команду:
