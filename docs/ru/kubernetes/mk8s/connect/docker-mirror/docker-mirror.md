# {heading(Настройка зеркала Docker)[id=mk8s-docker-mirror]}
...
{tab(Приватный реестр с аддоном Docker Registry)}
  ...
  1. {linkto(../../../k8s/instructions/addons/advanced-installation/install-advanced-registry#mk8s-install-advanced-registry)[text=Установите аддон]} Docker Registry, если этого еще не сделано.
  ...
  1. {linkto(../../../mk8s/quick-start#mk8s-quick-start-upload-to-registry)[text=Загрузите]} скачанный образ в приватный репозиторий.

  {/tab}

{tab(Daemonset для Cloud Containers)}

  Используйте зеркало в конфигурации [CRI-O](https://cri-o.io/) на рабочих узлах кластера. Для этого примените следующий манифест в кластере {linkto(../kubectl#mk8s-kubectl)[text=с помощью kubectl]} или в {linkto(../../instructions/manage-resources#mk8s-manage-resources)[text=личном кабинете]}:
