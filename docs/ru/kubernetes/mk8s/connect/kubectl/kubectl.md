# {heading(Подключение к кластеру с помощью kubectl)[id=mk8s-kubectl]}
...
## {heading(Подготовительные шаги)[id=mk8s-kubectl-before-work]}
...
1. Подготовьте все необходимое для подключения с использованием {linkto(../../concepts/access-management#mk8s-access-management)[text=Single Sign-On (SSO)]}.
...
## {heading(Подключение к кластеру)[id=mk8s-kubectl-connect]}

{include(/ru/_includes/_kubeconfig.md)}


## {heading(Проверка подключения к кластеру)[id=mk8s-kubectl-check-connection]}
...
1. Введите пароль пользователя от личного кабинета {var(cloud)}.
   ...
   Это необходимо для {linkto(../../concepts/access-management#mk8s-access-management)[text=аутентификации]} при подключении к кластеру.
