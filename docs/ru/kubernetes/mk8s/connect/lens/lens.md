# {heading(Подключение к кластеру с помощью Lens)[id=mk8s-lens]}
...
## {heading(Подготовительные шаги)[id=mk8s-lens-before-work]}

1. {linkto(../../instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=Создайте]} кластер Kubernetes, к которому нужно подключиться, или выберите нужный кластер из списка в [личном кабинете](https://msk.cloud.vk.ru/app/) на странице **Кластеры Kubernetes → Кластеры Kubernetes**.
1. На хосте, с которого планируется подключаться к кластеру, [установите Lens](https://docs.lenshq.io/k8slens/getting-started/install-lens/), если этого еще не сделано.
...
## {heading(Подключение к кластеру)[id=mk8s-lens-connect]}
