# {heading(Настройка Canary Deployment Ingress)[id=mk8s-k8s-canary]}
...
## {heading(Подготовительные шаги)[id=mk8s-k8s-canary-prepare]}
...
## {heading(1. Создайте ресурс Ingress)[id=mk8s-k8s-canary-create-ingress]}
...
## {heading(2. Создайте копию развернутого приложения)[id=mk8s-k8s-canary-create-copy-app]}
...
## {heading(3. Проверьте работоспособность распределения трафика)[id=mk8s-k8s-canary-check-traffic]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-k8s-canary-delete]}
