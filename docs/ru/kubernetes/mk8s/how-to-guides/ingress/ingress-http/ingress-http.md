# {heading(Развертывание Ingress-контроллера с HTTP-балансировщиком)[id=mk8s-ingress-http]}
...
Ingress-контроллер можно развернуть  {linkto(../../../concepts/network#mk8s-network)[text=в связке с HTTP-балансировщиком нагрузки]} платформы {var(cloud)}. Далее для примера будут созданы простые демоприложения и ресурс Ingress для проверки работоспособности контроллера.
...
## {heading(1. Подготовительные шаги)[id=mk8s-ingress-http-prepare]}
...
1. {linkto(../../../instructions/addons/manage-addons#mk8s-manage-addons-view)[text=Убедитесь]}, что аддон NGINX Ingress (`ingress-nginx`) **не установлен** в кластере. В демонстрационных целях Ingress-контроллер будет установлен вручную.

1. {linkto(../../../connect/kubectl#mk8s-kubectl)[text=Убедитесь]}, что вы можете подключиться к кластеру с помощью `kubectl`.

1. {linkto(../../../install-tools/helm#mk8s-helm)[text=Установите]} Helm, если утилита еще не установлена.

1. Установите [curl](https://curl.se/docs/), если утилита еще не установлена.

## {heading(2. Разверните демоприложения)[id=mk8s-ingress-http-deploy-demo]}
...
## {heading(3. Установите Ingress-контроллер)[id=mk8s-ingress-http-install-ingress]}
...
## {heading(4. Создайте HTTP-балансировщик нагрузки)[id=mk8s-ingress-http-create-http-balancer]}
...
## {heading(5. Создайте ресурс Ingress)[id=mk8s-ingress-http-create-ingress]}
...
## {heading(6. Проверьте доступность приложений)[id=mk8s-ingress-http-check-apps]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-ingress-http-delete]}
