# {heading(Поддержка GitOps: маркировка объектов автоматизации и ограничение ручных изменений с помощью Argo CD и Gatekeeper)[id=mk8s-ag]}

## {heading(Подготовительные шаги)[id=mk8s-ag-prepare]}
...
1. {linkto(../../instructions/addons/advanced-installation/install-advanced-argo-cd#mk8s-install-advanced-argo-cd)[text=Установите]} аддон Argo CD, если это еще не сделано.
   
   {linkto(../../../../access/iam/concepts/service-accounts#iam-concepts-service-accounts)[text=Сервисные учетные записи]} по умолчанию: `argocd-application-controller` и `argocd-server`.

## {heading({counter(ag)}. Установите Gatekeeper (Helm))[id=mk8s-ag-gatekeeper]}
...
1. Убедитесь, что поды работают:
   ...
   У чарта есть хук `gatekeeper-update-namespace-label` (навешивает на пространство имен {linkto(../../reference/labels-and-taints#mk8s-labels-and-taints)[text=метку]} (label) исключения). На платформе VK Cloud этот `job` часто завершается ошибкой (`BackoffLimitExceeded`), и `helm install` завершается статусом `INSTALLATION FAILED`. Само ядро Gatekeeper при этом устанавливается и работает (CRD, вебхук и поды создаются). Проверьте это, используя команды:
...
## {heading({counter(ag)}. Ознакомьтесь с архитектурой решения)[id=mk8s-ag-arch]}
...
## {heading({counter(ag)}. Включите явную маркировку объектов в Argo CD)[id=mk8s-ag-mark]}
...
1. Задайте метод трекинга в конфигурации Argo CD. В аддоне это делается через {linkto(../../instructions/addons/manage-addons#mk8s-manage-addons-edit-code)[text=код настройки аддона]} в поле `configs.cm`:
...
## {heading({counter(ag)}. Проверьте маркировку)[id=mk8s-ag-mark-check]}
...
## {heading({counter(ag)}. Разрешите Gatekeeper перехватывать операции DELETE)[id=mk8s-ag-allow]}
...
## {heading({counter(ag)}. Исключите системные пространства имен)[id=mk8s-ag-exclude]}
...
## {heading({counter(ag)}. Создайте правило запрета ручных изменений)[id=mk8s-ag-template]}
...
## {heading({counter(ag)}. Создайте область действия (Constraint) и allowlist)[id=mk8s-ag-constraint]}
...
1. Проверьте, что ограничение загрузилось на все поды, иначе при `failurePolicy: Ignore` защита не работает на части webhook‑подов:
   ...
   Если у какого-то пода строки нет, или значение параметра `enforced` не `true`, почти всегда это [превышение лимита в 63 символа](#mk8s-ag-template) в имени status‑объекта. В таком случае сократите значение параметра `kind` и имени ограничения. Чтобы проверите наличие status‑объектов, выполните команду:
...
## {heading({counter(ag)}. (Опционально) Защитите маркер от снятия)[id=mk8s-ag-protect]}
...
## {heading({counter(ag)}. Проверьте политику)[id=mk8s-ag-protect]}
...
## {heading({counter(ag)}. Последовательно внедрите режимы enforcementAction)[id=mk8s-ag-stages]}
...
## {heading({counter(ag)}. Настройте дополнительную защиту на стороне Argo CD)[id=mk8s-ag-additional]}
...
## {heading({counter(ag)}. Проведите диагностику)[id=mk8s-ag-diagnostics]}
...
| Объект не помечен `argocd.argoproj.io/tracking-id` (см. [шаг 3](#mk8s-ag-mark)), либо параметр `enforcementAction` не установлен в режим `deny`

| Не блокируется `DELETE`
| Не добавлена операция `DELETE` в webhook (см. [шаг 5](#mk8s-ag-allow)). Проверьте `validatingwebhookconfiguration`
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-ag-remove]}
