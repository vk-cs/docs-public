# {heading(Балансировка нагрузки по алгоритмам LEAST_REQUEST и MAGLEV с помощью аддона Istio)[id=mk8s-istio-balancing]}

В этом практическом руководстве показана внутренняя балансировка между подами одного Kubernetes Service с помощью аддона {linkto(../../concepts/addons-and-settings/addons#mk8s-addons-istio)[text=Istio]}:
...
## {heading(Подготовительные шаги)[id=mk8s-im-prepare]}
...
1. {linkto(../../instructions/addons/advanced-installation/install-advanced-istio-mk8s#mk8s-install-advanced-istio-mk8s)[text=Установите]} аддон Istio, если это еще не сделано. Если аддон установлен, переустанавливать его не нужно.

## {heading({counter(im)}. Проверьте Istio)[id=mk8s-im-check]}
...
## {heading({counter(im)}. Создайте пространство имен с инъекцией sidecar)[id=mk8s-im-sidecar]}
...
## {heading({counter(im)}. Разверните бэкэнд)[id=mk8s-im-backend]}
...
## {heading({counter(im)}. Разверните клиент внутри mesh)[id=mk8s-im-mesh]}
...
## {heading({counter(im)}. Проверьте исходное распределение)[id=mk8s-im-distrib]}
...
## {heading({counter(im)}. Настройте LEAST_REQUEST)[id=mk8s-im-least]}
...
## {heading({counter(im)}. Настройте MAGLEV)[id=mk8s-im-maglev]}
...
## {heading({counter(im)}. Проверьте MAGLEV)[id=mk8s-im-maglev-check]}
...
## {heading({counter(im)}. Проверьте поведение при удалении пода)[id=mk8s-im-pod-delete]}
...
## {heading({counter(im)}. Выполните проверку из Windows PowerShell)[id=mk8s-im-win]}
...
## {heading({counter(im)}. Проведите диагностику)[id=mk8s-im-diagnostics]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-im-delete]}
...
## {heading(Итог)[id=mk8s-im-summary]}
...
## {heading(Ограничения)[id=mk8s-im-restrictions]}
