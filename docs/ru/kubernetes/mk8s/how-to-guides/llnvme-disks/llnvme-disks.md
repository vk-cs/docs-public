# {heading(Подключение дисков LL NVMe к worker-узлам)[id=mk8s-llnvme-disks]}
...
## {heading(Подготовительные шаги)[id=mk8s-llnvme-disks-prepare]}
...
1. В личном кабинете {var(cloud)} {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-add-group)[text=добавьте]} группу узлов с ВМ на основе конфигурации для LL NVMe:
...
1. {linkto(../../connect/kubectl#mk8s-kubectl-connect)[text=Подключитесь]} к кластеру при помощи `kubectl`.

## {heading(1. Добавьте класс хранения (StorageClass) для дисков LL NVMe)[id=mk8s-llnvme-disks-nvme-storageclass]}
...
1. Создайте файл манифеста для `StorageClass`, например `csi-ef-nvme.yaml`, и добавьте в него следующее содержимое:
   ...
   - `volumeBindingMode: WaitForFirstConsumer` позволяет отложить создание и привязку {linkto(../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=постоянного тома]} (persistent volume) до момента, когда будет создан под, использующий [Persistent Volume Claim](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#introduction). Это нужно, чтобы Kubernetes создал диск после того, как под будет запланирован на узел.
...
## {heading(2. Создайте приложение)[id=mk8s-llnvme-disks-nvme-app]}

Чтобы изучить процесс подключения дисков LL NVMe к worker-узлам, запустите тестовое приложение `coffee`.

1. [Перейдите](https://msk.cloud.vk.ru/app/) в личный кабинет {var(cloud)}.
1. {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-labels-taints)[text=Добавьте]} метку `disktype: ef-nvme` для ранее созданной группы узлов, чтобы обеспечить размещение пода на узлах с дисками LL NVMe:
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-llnvme-disks-delete]}
...
1. {linkto(../../concepts/storage#mk8s-storage-reclaim-policies)[text=Удалите]} созданный постоянный том.
