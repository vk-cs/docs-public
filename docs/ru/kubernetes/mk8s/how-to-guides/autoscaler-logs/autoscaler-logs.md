# {heading(Просмотр логов агента автоматического масштабирования)[id=mk8s-autoscaler-logs]}

{linkto(../../concepts/cluster-autoscaler#mk8s-cluster-autoscaler)[text=Агент автоматического масштабирования]} кластера Kubernetes (Cluster Autoscaler) отслеживает нагрузку на worker-узлы кластера, при необходимости уменьшая или увеличивая их количество. Вы можете просматривать логи {linkto(../../concepts/scale#mk8s-scale-autoscaling)[text=автоматического масштабирования]} для диагностики проблем.
...
## {heading(Подготовительные шаги)[id=mk8s-autoscaler-logs-prepare]}
...
## {heading(1. Определите имя пода агента автоматического масштабирования)[id=mk8s-autoscaler-logs-review]}
...
{linkto(../../instructions/manage-resources#mk8s-manage-resources-view-resources)[text=Просмотрите]} список подов кластера в личном кабинете {var(cloud)}.
...
## {heading(2. Просмотрите логи пода агента автоматического масштабирования)[id=mk8s-autoscaler-logs-autoscaling-agent]}
...
## {heading(3. Просмотрите события кластера)[id=mk8s-autoscaler-logs-cluster-events]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-autoscaler-logs-delete]}
