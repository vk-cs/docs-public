# {heading(Использование GPU Operator)[id=mk8s-gpu-operator]}

Managed Containers позволяет создавать {linkto(../../concepts/flavors#mk8s-flavors-gpu)[text=кластеры с GPU (графическими процессорами)]} для выполнения машинного обучения или обработки больших данных.
...
- Ресурсы GPU распределены несколькими подами по стратегии {linkto(../../concepts/flavors#mk8s-flavors-gpu-sharing)[text=MPS]}. Каждый процесс получает выделенный набор ресурсов GPU, и они взаимодействуют друг с другом через межпроцессное взаимодействие (IPC).
- Ресурсы GPU распределены несколькими подами по стратегии {linkto(../../concepts/flavors#mk8s-flavors-gpu-sharing)[text=MIG]}. При этом каждая часть имеет собственные ресурсы (память, вычислительные блоки) и может работать изолированно от других.
...
## {heading(Подготовительные шаги)[id=mk8s-gpu-operator-prepare]}

1. {linkto(../../../../computing/gpu/connect#gpu-connect)[text=Подключите]} сервис Cloud GPU, если он еще не подключен.
1. {linkto(../../instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=Создайте]} кластер с GPU, если это еще не сделано. Укажите следующие параметры:
...
1. {linkto(../../connect/kubectl#mk8s-kubectl)[text=Подключитесь]} к кластеру через kubectl.

## {heading(1. Установите аддон GPU-operator)[id=mk8s-gpu-operator-install]}

Воспользуйтесь {linkto(../../instructions/addons/advanced-installation/install-advanced-gpu-operator#mk8s-install-advanced-gpu-operator)[text=инструкцией по установке]}.

## {heading(2. Проверьте работу кластера)[id=mk8s-gpu-operator-check]}
...
## {heading(3. Разделите GPU по технологии MPS)[id=mk8s-gpu-operator-mps]}

1. {linkto(../../instructions/addons/manage-addons#mk8s-manage-addons-edit-code)[text=Добавьте]} конфигурацию [NVIDIA device plugin](https://github.com/NVIDIA/k8s-device-plugin/tree/v0.17.0?tab=readme-ov-file#with-cuda-mps) в код аддона GPU Operator:
...
1. Добавьте метку (label) для узла с GPU, которая включит режим `mps-on`, добавленный в конфигурации аддона:
   ...
   {note:info}
   Вы можете {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-labels-taints)[text=добавить метку]} для всех узлов в группе через личный кабинет.
   {/note}
...
## {heading(4. Разделите GPU по технологии MIG)[id=mk8s-gpu-operator-mig]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-gpu-operator-delete]}
