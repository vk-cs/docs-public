...
- Использовать разные алгоритмы балансировки соединений между экземплярами приложения:
  ...
  - Случайный выбор реплики (по умолчанию).
    ...
    Балансировщик ведет себя так, потому что `kube-proxy` в кластерах Cloud Containers {linkto(../../concepts/addons-and-settings/settings#mk8s-settings-kube-proxy-mode)[text=работает]} в режиме `iptables`.
