# {heading(Использование External Secrets Operator)[id=mk8s-external-secrets-operator]}

Используйте аддон {linkto(../../concepts/addons-and-settings/addons#mk8s-addons-eso)[text=External Secrets Operator]}, чтобы настроить синхронизацию с секретами Kubernetes, которые хранятся в {linkto(../../../../security/secret-manager/concepts/about#sm-about)[text=менеджере секретов {var(cloud)}]}. 

## {heading(Подготовительные шаги)[id=mk8s-eso-prepare]}
...
1. {linkto(../../instructions/addons/advanced-installation/install-advanced-eso#mk8s-install-advanced-eso)[text=Установите аддон External Secrets Operator]}, если это еще не сделано.
...
## {heading({counter(eso)}. Создайте секрет в менеджере секретов)[id=mk8s-eso-kms]}
...
## {heading({counter(eso)}. Создайте секрет Kubernetes)[id=mk8s-eso-create]}
...
## {heading({counter(eso)}. Создайте SecretStore)[id=mk8s-eso-store]}
...
## {heading({counter(eso)}. Создайте ExternalSecret)[id=mk8s-eso-external]}
...
1. Создайте манифест `externalsecret.yaml` — объект типа `ExternalSecret`, который описывает, из какого `SecretStore` загружать секрет и как его обновлять:
   ...
   - `<ID_СЕКРЕТА>` — идентификатор секрета, который вы [создали](#mk8s-eso-kms) ранее в менеджере секретов {var(cloud)}. Обязательно передавать в двойных кавычках: `""`.
...
## {heading({counter(eso)}. Создайте ExternalSecret для синхронизации всех ключей секрета)[id=mk8s-eso-secrets-all]}
...
## {heading({counter(eso)}. Проверьте работу аддона)[id=mk8s-eso-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-eso-delete]}
