# {heading(Настройка локального DNS-кеша)[id=mk8s-local-dns-cache]}
...
## {heading(Обзор архитектуры DNS в кластере)[id=mk8s-local-dns-cache-architecture]}

Пусть некоторый {linkto(../../../reference/pods#mk8s-pods)[text=под]} инициирует DNS-запрос.
...
## {heading(Подготовительные шаги)[id=mk8s-local-dns-cache-prepare]}
...
## {heading(1. Разверните кеширующий DNS-сервер на каждом узле)[id=mk8s-local-dns-cache-deploy]}
...
## {heading(2. Проверьте работу кеширующего DNS-сервера)[id=mk8s-local-dns-cache-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-local-dns-cache-delete]}
