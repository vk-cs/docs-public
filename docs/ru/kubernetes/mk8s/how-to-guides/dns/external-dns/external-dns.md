# {heading(Установка и использование ExternalDNS)[id=mk8s-external-dns]}
...
## {heading(Подготовительные шаги)[id=mk8s-external-dns-prepare]}
...
1. {linkto(../../../instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=Создайте]} кластер Kubernetes актуальной версии, который имеет внешний IP-адрес и доступен из интернета.
...
## {heading(1. Подготовьте пользователя для ExternalDNS)[id=mk8s-external-dns-prepare-user]}
...
1. {linkto(../../../../../access/iam/instructions/access-manage#iam-access-manage-view-user)[text=Выберите]} существующего пользователя или {linkto(../../../../../access/iam/instructions/access-manage#iam-access-manage-invite-user)[text=пригласите в проект]} нового пользователя.

   Требования к пользователю:

   - Должен быть {linkto(../../../../../tools-for-using-services/api/rest-api/enable-api#rest-api-enable)[text=активирован]} доступ по API.
   - Должна быть {linkto(../../../../../access/iam/instructions/access-manage#iam-access-manage-user-role-edit)[text=назначена]} одна из следующих ролей, чтобы ExternalDNS мог оперировать ресурсными записями в рамках DNS-зоны:

     - Администратор сети (минимально необходимая {linkto(../../../../../access/iam/concepts/roles-reference#iam-roles-reference)[text=роль]}).
...
1. Получите реквизиты, нужные для доступа к API {var(cloud)}:
   ...
   1. [Перейдите](https://msk.cloud.vk.ru/app/) в личный кабинет {var(cloud)}, используя реквизиты пользователя, выделенного для ExternalDNS.
...
## {heading(2. Установите ExternalDNS)[id=mk8s-external-dns-install]}
...
1. Создайте в этом пространстве имен секрет, который содержит в себе реквизиты для доступа к API {var(cloud)}, {linkto(#mk8s-external-dns-prepare-user)[text=полученные при подготовке пользователя]}:
...
## {heading(3. Проверьте работу External DNS)[id=mk8s-external-dns-check]}
...
### {heading(3.1. Опубликуйте приложение, используя сервис типа LoadBalancer)[id=mk8s-external-dns-publish-loadbalancer]}
...
1. Убедитесь, что ExternalDNS создал необходимые ресурсные записи:
   ...
   1. Найдите в списке записи, созданные ExternalDNS:
      ...
      - Две TXT-записи `externaldns-tea.example.com` и `externaldns-a-tea.example.com`.
        ...
        Если {linkto(#mk8s-external-dns-install)[text=при установке ExternalDNS]} вы задали другое значение префикса, то имена служебных TXT-записей будут отличаться.
        {/note}
...
### {heading(3.2. Опубликуйте приложение, используя Ingress)[id=mk8s-external-dns-publish-ingress]}

1. {linkto(../../../instructions/addons/advanced-installation/install-advanced-ingress#mk8s-install-advanced-ingress)[text=Установите]} в кластер аддон Ingress NGINX самой актуальной версии.
...
1. Создайте манифест для сервиса Kubernetes, который будет использоваться приложением.
   ...
   Здесь для сервиса задан тип [ClusterIP](https://kubernetes.io/docs/concepts/services-networking/service/#type-clusterip), его достаточно, поскольку приложение будет опубликовано с помощью Ingress. Такой сервис доступен только изнутри кластера и не имеет выделенного балансировщика нагрузки в отличие от сервиса, {linkto(#mk8s-external-dns-publish-loadbalancer)[text=созданного ранее]}.
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-external-dns-delete]}
...
1. {linkto(../../../instructions/addons/manage-addons#mk8s-manage-addons-delete)[text=Удалите аддон Ingress NGINX]}.
...
1. {linkto(../../../../../networks/dns/instructions/publicdns/dns-zone#dns-dns-zone-delete)[text=Удалите ресурсные записи]}, созданные ExternalDNS.
   ...
   Это необходимо сделать, если вы не изменяли файл `external-dns-vkcs-values.yaml` при {linkto(#mk8s-external-dns-install)[text=установке ExternalDNS]}: тогда ExternalDNS использует политику `upsert-only` и не удаляет ресурсные записи из DNS-зоны при удалении ресурсов Kubernetes. Если вы изменили этот файл и выбрали политику `sync`, то эти записи будут удалены автоматически.
