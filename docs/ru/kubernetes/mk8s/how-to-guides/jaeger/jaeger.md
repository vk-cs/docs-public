# {heading(Использование Jaeger)[id=mk8s-jaeger]}
...
## {heading(Подготовительные шаги)[id=mk8s-jaeger-prepare]}
...
1. {linkto(../../instructions/addons/advanced-installation/install-advanced-jaeger#mk8s-install-advanced-jaeger)[text=Установите аддон Jaeger]}, если это еще не сделано.

## {heading({counter(jaeger)}. Создайте приложение для HotROD)[id=mk8s-jaeger-app]}
...
## {heading({counter(jaeger)}. Сгенерируйте нагрузку и получите трассировки запросов)[id=mk8s-jaeger-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-jaeger-delete]}
