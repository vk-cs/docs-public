# {heading(Резервное копирование по расписанию)[id=mk8s-backup-schedule-managed]}

Используйте аддон {linkto(../../../concepts/addons-and-settings/addons#mk8s-addons-velero)[text=Velero]}, чтобы настраивать резервное копирование кластеров по заданному расписанию в формате [cron](https://crontab.guru/every-1-minute) и восстанавливать кластер на основании этого расписания.

## {heading(Подготовительные шаги)[id=mk8s-backup-schedule-managed-prepare]}
...
## {heading({counter(managed-velero-schedule)}. Создайте приложение)[id=mk8s-velero-backup-schedule-managed-create-app]}

{include(/ru/_includes/_velero-magnum-how-to.md)[tags=create-app]}

## {heading({counter(managed-velero-schedule)}. Создайте расписание резервного копирования)[id=mk8s-backup-schedule-managed-create-schedule]}
...
## {heading({counter(managed-velero-schedule)}. Просмотрите созданное расписание)[id=mk8s-backup-schedule-managed-view-schedule]}
...
## {heading({counter(managed-velero-schedule)}. Выполните восстановление данных из копии, созданной по расписанию)[id=mk8s-backup-schedule-managed-perform-backup]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-backup-schedule-managed-delete]}
