# {heading(Резервное копирование вручную)[id=mk8s-velero-backup-managed]}

Используйте аддон {linkto(../../../concepts/addons-and-settings/addons#mk8s-addons-velero)[text=Velero]}, чтобы вручную создавать резервные копии данных кластера и восстанавливать их.

## {heading(Подготовительные шаги)[id=mk8s-velero-backup-managed-prepare]}
...
1. {linkto(../../../../../tools-for-using-services/cli/openstack-cli#tools-cli-openstack)[text=Установите OpenStack CLI]}, если это еще не сделано. Убедитесь, что вы можете авторизоваться в облаке с его помощью.
1. {linkto(../../../instructions/addons/advanced-installation/install-advanced-velero#mk8s-install-advanced-velero)[text=Установите аддон Velero]}, если это еще не сделано.

## {heading({counter(managed-velero)}. Создайте приложение)[id=mk8s-velero-backup-managed-create-app]}

{include(/ru/_includes/_velero-magnum-how-to.md)[tags=create-app]}

## {heading({counter(managed-velero)}. Создайте резервную копию приложения)[id=mk8s-velero-backup-managed-create]}
...
1. (Опционально) {linkto(../../../how-to-guides/managed-velero/managed-backup-schedule#mk8s-backup-schedule-managed)[text=Настройте]} расписание для автоматического создания резервных копий.

## {heading({counter(managed-velero)}. Восстановите приложение из резервной копии)[id=mk8s-velero-backup-managed-restore]}
...
1. Примените манифест в кластере:
   ...
   Этот манифест восстанавливает ресурсы из резервной копии, которая была создана ранее. Данные восстанавливаются в том же кластере, в котором было выполнено резервное копирование. Если нужно восстановить данные в новый кластер:
    ...
    1. {linkto(../../../instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=Создайте]} кластер.
    1. {linkto(../../../instructions/addons/advanced-installation/install-advanced-velero#mk8s-install-advanced-velero)[text=Установите аддон Velero]} в кластер.
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-velero-backup-managed-delete]}
