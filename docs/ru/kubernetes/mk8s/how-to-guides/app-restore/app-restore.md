# {heading(Автоматическое восстановление приложений при недоступности внешнего хранилища)[id=mk8s-app]}
...
В Managed Containers эту проблему можно решить стандартными механизмами Kubernetes: `readinessProbe`, `livenessProbe` и `startupProbe`. Они позволяют временно исключить неисправный экземпляр приложения из балансировки, а при необходимости — автоматически перезапустить контейнер.

## {heading(Как должна работать схема восстановления)[id=mk8s-app-schema]}
...
## {heading({counter(app)}. Создайте скрипт для проверки файлового хранилища)[id=mk8s-app-script]}
...
## {heading({counter(app)}. Настройте Deployment)[id=mk8s-app-deployment]}
...
{linkto(../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs)[text=PVC]} (`PersistentVolumeClaim`) позволяет приложению использовать постоянное хранилище, не зная деталей его реализации в конкретной облачной инфраструктуре.

## {heading({counter(app)}. Создайте PVC)[id=mk8s-app-pvc]}
...
1. Поверьте доступные в кластере {linkto(../../concepts/storage#mk8s-storage-storage-classes)[text=классы хранения]}:
...
1. Проверьте состояние PVC:
   ...
   После успешного {linkto(../../reference/pvs-and-pvcs#mk8s-pvs-and-pvcs-life-cycle)[text=динамического выделения]} тома статус должен стать `Bound`:
...
## {heading({counter(app)}. Примените манифесты в правильном порядке)[id=mk8s-app-manifests]}
...
## {heading({counter(app)}. Проверьте сценарий восстановления)[id=mk8s-app-script-check]}
...
## {heading(Почему проверки настроены именно так)[id=mk8s-app-probes]}

### {heading(Startup probe)[id=mk8s-app-startup]}
...
### {heading(Readiness probe)[id=mk8s-app-readiness]}
...
### {heading(Liveness probe)[id=mk8s-app-liveness]}
...
## {heading(Практические рекомендации)[id=mk8s-app-recs]}
...
## {heading(Мониторинг)[id=mk8s-app-monitoring]}
...
Такая конфигурация применяется в Managed Containers без установки дополнительного оператора и помогает избежать как обслуживания запросов неисправными экземплярами, так и бесконечных перезапусков во время продолжительного сбоя внешнего хранилища.
