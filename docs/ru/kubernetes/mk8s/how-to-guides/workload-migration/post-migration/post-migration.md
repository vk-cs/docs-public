# {heading(Перенос рабочей нагрузки после миграции ресурсов)[id=mk8s-post-migration]}

Это руководство поможет вам завершить перенос своей рабочей нагрузки (workload) после того, как сотрудники {var(cloud)} завершат {linkto(../../../concepts/az-migration-magnum#mk8s-az-migration)[text=работы по миграции]} ресурсов кластера между {linkto(../../../../../start/concepts/architecture#architecture-az)[text=зонами доступности]}. Вы выполните следующие шаги:
...
## {heading(Подготовительные шаги)[id=mk8s-post-migration-prepare]}

1. Перейдите в кластер Kubernetes, ресурсы которого были перенесены на другую зону доступности.
{include(/ru/_includes/_az-migration-magnum.md)[tags=prep]}

## {heading({counter(az)}. Определите зависимость рабочей нагрузки от зоны доступности)[id=mk8s-post-migration-check]}
...
## {heading({counter(az)}. Обновите конфигурации смигрированных групп узлов)[id=mk8s-post-migration-update]}

1. Дождитесь окончания работ по миграции ресурсов со стороны {var(cloud)}.

   Узлы смигрированных групп worker-узлов (то есть тех, которые были созданы до миграции) будут иметь следующие {linkto(../../../reference/labels-and-taints#mk8s-labels-and-taints)[text=метки]}, указывающие на старую зону доступности:
...
## {heading({counter(az)}. Перенесите stateful-нагрузку на новый PVC)[id=mk8s-post-migration-stateful]}

Так как работы по {linkto(../../../concepts/az-migration-magnum#mk8s-az-migration)[text=миграции ресурсов]} со стороны {var(cloud)} уже завершены, диски, на которых расположены PV, уже физически изменили свою зону. Теперь вам нужно актуализировать их метаданные в кластере. 

Чтобы перенести stateful-нагрузку на PVC в новой зоне доступности,

1. [Подготовьте PVC](#mk8s-post-migration-pvc-check).
1. [Перенесите нагрузку одним из предложенных способов](#mk8s-post-migration-pvc).

### {heading(3.1. Подготовьте PVC к переносу)[id=mk8s-post-migration-pvc-check]}

{include(/ru/_includes/_az-migration-magnum.md)[tags=pvc-prep]}

### {heading(3.2. Выберите способ переноса)[id=mk8s-post-migration-pvc]}
...
## {heading({counter(az)}. Обновите Helm-чарты и настройки аддонов)[id=mk8s-post-migration-helm-addons]}

{include(/ru/_includes/_az-migration-magnum.md)[tags=helm-addons]}

## {heading({counter(az)}. (Опционально) Синхронизируйте Terraform State)[id=mk8s-post-migration-tf]}

{include(/ru/_includes/_az-migration-magnum.md)[tags=tf-managed]}

## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-post-migration-delete]}
