# {heading(Настройка разрешенных репозиториев)[id=mk8s-repositories]}
...
{note:info}
В кластерах Kubernetes {var(cloud)} версий 1.23 и выше можно {linkto(../../../instructions/manage-security#mk8s-manage-security-add-policy)[text=создать]} нужную {linkto(../../../concepts/security-policies#mk8s-security-policies-allowed-repos)[text=политику безопасности]} через личный кабинет.

В этом случае работать с ресурсами Gatekeeper напрямую не обязательно.
{/note}

## {heading(Подготовительные шаги)[id=mk8s-repositories-prepare]}
...
## {heading(Создайте ограничение, проверяющее репозитории)[id=mk8s-repositories-create-constraint]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-repositories-delete]}
