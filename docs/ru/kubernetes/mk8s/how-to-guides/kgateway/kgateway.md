# {heading(Использование Kgateway)[id=mk8s-kgateway]}

Используйте аддон {linkto(../../concepts/addons-and-settings/addons#mk8s-addons-kgateway)[text=Kgateway]}, чтобы настроить маршрутизацию трафика в кластере Kubernetes через [Gateway API](https://gateway-api.sigs.k8s.io/) и направить запросы к тестовому приложению.

## {heading(Подготовительные шаги)[id=mk8s-kgateway-prepare]}
...
1. {linkto(../../instructions/addons/advanced-installation/install-advanced-kgateway#mk8s-install-advanced-kgateway)[text=Установите аддон Kgateway]}, если это еще не сделано.

## {heading({counter(kgateway)}. Создайте тестовое приложение)[id=mk8s-kgateway-app]}
...
## {heading({counter(kgateway)}. Создайте объект Gateway)[id=mk8s-kgateway-gateway]}
...
## {heading({counter(kgateway)}. Создайте объект HTTPRoute)[id=mk8s-kgateway-route]}
...
## {heading({counter(kgateway)}. Проверьте работу Kgateway)[id=mk8s-kgateway-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-kgateway-delete]}
