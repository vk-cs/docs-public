# {heading(Горизонтальное масштабирование приложений)[id=mk8s-hpa]}
...
## {heading(Подготовительные шаги)[id=mk8s-hpa-prepare]}
...
1. {linkto(../../instructions/addons/advanced-installation/install-advanced-monitoring#mk8s-install-advanced-monitoring)[text=Установите]} аддон Kube Prometheus Stack версии 68.3.3, если это еще не сделано.
   
   {note:warn}
   Аддоны потребляют ресурсы worker-узлов. {linkto(../../concepts/addons-and-settings/addons#mk8s-addons-kube-prometheus-stack)[text=Kube Prometheus Stack]} требует ориентировочно CPU 850m–2500m CPU и 968Mi–3804Mi RAM, а также диски HDD 2 ГБ и SSD 10 ГБ под данные Prometheus. Убедитесь, что в группе worker-узлов достаточно ресурсов, и {linkto(../../instructions/scale#mk8s-instructions-scale-horizontal-worker-nodes)[text=включите]} для нее автомасштабирование — иначе под Prometheus и масштабируемые по HPA поды могут зависнуть в состоянии `Pending`. Рекомендуется устанавливать ресурсоемкие аддоны на выделенную автомасштабируемую группу узлов.
   {/note}
...
## {heading({counter(hpa)}. Проверьте доступ к кластеру)[id=mk8s-hpa-access]}
...
## {heading({counter(hpa)}. Проверьте Metrics Server)[id=mk8s-hpa-metrics-server]}
...
## {heading({counter(hpa)}. Разверните тестовое приложение)[id=mk8s-hpa-test-app]}
...
### {heading(Почему обязательно указывать requests)[id=mk8s-hpa-test-app]}
...
## {heading({counter(hpa)}. Настройте HPA по CPU)[id=mk8s-hpa-cpu]}
...
## {heading({counter(hpa)}. Проверьте масштабирование по CPU)[id=mk8s-hpa-cpu-check]}
...
## {heading({counter(hpa)}. Добавьте масштабирование по памяти)[id=mk8s-hpa-ram]}
...
### {heading(Процент или абсолютное значение RAM)[id=mk8s-hpa-percent]}
...
## {heading({counter(hpa)}. Ознакомьтесь с архитектурой масштабирования по пользовательским метрикам)[id=mk8s-hpa-custom]}
...
## {heading({counter(hpa)}. Установите и проверьте аддон Kube Prometheus Stack)[id=mk8s-hpa-kube]}

В VK Cloud Prometheus разворачивается аддоном Kube Prometheus Stack. 

1.{linkto(../../instructions/addons/advanced-installation/install-advanced-monitoring#mk8s-install-advanced-monitoring)[text=Установите]} аддон при создании кластера или в существующий кластер — через Terraform или веб-интерфейс. Пример установки через Terraform:
...
Под Prometheus запрашивает заметные CPU/RAM и монтирует PVC. На кластере из одного небольшого worker-узла (или при уже запущенных по HPA репликах) он может остаться в состоянии `Pending` (`Insufficient cpu` или  `unbound PersistentVolumeClaims`). {linkto(../../instructions/scale#mk8s-instructions-scale-horizontal-autoscaling-worker-nodes)[text=Включите]} автомасштабирование группы worker-узлов — {linkto(../../concepts/cluster-autoscaler#mk8s-cluster-autoscaler)[text=Cluster Autoscaler]} добавит узел автоматически.

## {heading({counter(hpa)}. Подключите метрики приложения к Prometheus)[id=mk8s-hpa-enable]}
...
1. Примените манифест:
   ...
   В аддоне Kube Prometheus Stack на платформе VK Cloud Prometheus сконфигурирован с пустыми `serviceMonitorSelector: {}` и `serviceMonitorNamespaceSelector: {}` — то есть подхватываются все `ServiceMonitor` во всех пространствах имен, дополнительная {linkto(../../reference/labels-and-taints#mk8s-labels-and-taints)[text=метка]} (label) (например `release: kube-prometheus-stack`) не требуется. В классических Helm-установках `kube-prometheus-stack` это не так — там `ServiceMonitor` обычно должен иметь метку `release`, совпадающую с именем релиза. Чтобы проверить фактический селектор, используйте команду:
...
## {heading({counter(hpa)}. Установите Prometheus Adapter)[id=mk8s-hpa-adapter]}
...
## {heading({counter(hpa)}. Проверьте Custom Metrics API)[id=mk8s-hpa-api]}
...
## {heading({counter(hpa)}. Создайте HPA по пользовательской метрике)[id=mk8s-hpa-custom-create]}
...
## {heading({counter(hpa)}. Объедините CPU, RAM и пользовательскую метрику)[id=mk8s-hpa-all]}
...
## {heading({counter(hpa)}. Проведите нагрузочное тестирование)[id=mk8s-hpa-test]}
...
1. Наблюдайте:
   ...
   Как и в [шаге 5](#mk8s-hpa-cpu-check), команда `kubectl get hpa,pods -n hpa-demo -w` выдаст `error: you may only specify a single resource type`. Для одновременного наблюдения используйте команду:
...
## {heading({counter(hpa)}. Проведите диагностику HPA)[id=mk8s-hpa-diagnostics]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=mk8s-hpa-remove]}
...
## {heading(Итог)[id=mk8s-hpa-summary]}
...
## {heading(Практические рекомендации)[id=mk8s-hpa-recs]}
