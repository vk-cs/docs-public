...
## {heading(Принцип работы Gatekeeper)[id=mk8s-gatekeeper-about]}
...
## {heading(Устройство ресурсов для Gatekeeper)[id=mk8s-gatekeeper-resources]}

### {heading(Структура ограничения)[id=mk8s-gatekeeper-constraints]}
...
### {heading(Структура шаблона ограничения)[id=mk8s-gatekeeper-template]}
...
## {heading(Как использовать Gatekeeper)[id=mk8s-gatekeeper-how-to-use]}
...
Примеры использования ограничений Gatekeeper приведены в {linkto(../../how-to-guides/gatekeeper#mk8s-gatekeeper)[text=сценариях использования]}.