# {heading(Ограничение использования ресурсов)[id=mk8s-resource-limiting]}
...
Хотя кластеры Kubernetes сервиса Managed Containers уже содержат {linkto(../../concepts/addons-and-settings/settings#mk8s-settings-requests-and-limits)[text=преднастроенные лимиты]}, хорошей практикой является ограничение ресурсов вручную. Например, предустановленные ограничения могут не подходить для ресурсоемкой рабочей нагрузки. Вы также можете использовать аддон {linkto(../../concepts/addons-and-settings/addons#mk8s-addons-vpa)[text=Vertical Pod Autoscaler]} для автоматической настройки ресурсов (CPU и RAM). Он поможет определить требования к вычислительным ресурсам и лимитам пода в зависимости от требующейся нагрузки. 
