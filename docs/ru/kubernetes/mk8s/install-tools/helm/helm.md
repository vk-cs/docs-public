# {heading(Установка Helm)[id=mk8s-helm]}
...
## {heading(Установка)[id=mk8s-helm-install]}

1. {linkto(../../connect/kubectl#mk8s-kubectl-check-connection)[text=Убедитесь]}, что вы можете подключаться к кластеру с помощью `kubectl` с хоста, на который планируется установить клиент Helm.
...
## {heading(Проверка работоспособности)[id=mk8s-helm-check]}
...
## {heading(Удаление)[id=mk8s-helm-delete]}
