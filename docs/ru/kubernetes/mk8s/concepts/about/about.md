# {heading(О сервисе)[id=mk8s-about]}
...
## {heading(Возможности сервиса)[id=mk8s-about-features]}

- Управление {linkto(../../instructions/manage-cluster#mk8s-manage-cluster)[text=кластером]} и {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=группами узлов]} с помощью личного кабинета {var(cloud)} и собственного Terraform-провайдера {var(cloud)}.

- Управление объектами и ресурсами Kubernetes после {linkto(../../connect#mk8s-connect)[text=подключения к кластеру]} с помощью `kubectl` или Headlamp.

- Автоматическое и ручное {linkto(../../instructions/scale#mk8s-instructions-scale)[text=масштабирование кластера]} (cluster autoscaling).
...
- Обеспечение безопасности на всех этапах работы с кластером:
  ...
  - Можно применять {linkto(../network#mk8s-network-cni)[text=сетевые политики]} Calico и Cilium. Подробнее о сетевых политиках в [официальной документации Kubernetes](https://kubernetes.io/docs/concepts/services-networking/network-policies/).
  - Можно применять {linkto(../architecture#mk8s-architecture-opa-gatekeeper)[text=ограничительные политики]} Gatekeeper. Подробнее в [официальной документации Gatekeeper](https://open-policy-agent.github.io/gatekeeper/website/docs/howto).
  - Доступна интеграция [ролевой модели безопасности Kubernetes](https://kubernetes.io/docs/reference/access-authn-authz/rbac/) с ролями платформы {var(cloud)}. Подробнее читайте в разделе {linkto(../iam-access#mk8s-concepts-iam)[text=Управление доступом]}.
...
- Мониторинг состояния кластера с помощью Prometheus. Посмотреть данные мониторинга можно {linkto(../../monitoring#mk8s-monitoring)[text=несколькими способами]}.

- {linkto(../../monitoring#mk8s-monitoring-forecast-consumption)[text=Прогнозирование потребления]} ресурсов кластера на основе данных мониторинга. Аддон мониторинга `kube-prometheus-stack` с доработками от VK Tech предоставляет дополнительные возможности, преднастроенные в {linkto(../../monitoring#mk8s-monitoring-connect-grafana)[text=Grafana]}:
...
## {heading(Что дальше)[id=mk8s-about-next-steps]}

- {linkto(../architecture#mk8s-architecture)[text=Познакомьтесь с архитектурой сервиса]}.
- {linkto(../network#mk8s-network)[text=Познакомьтесь с устройством сети в кластере]}.
- {linkto(../storage#mk8s-storage)[text=Познакомьтесь с устройством хранилища в кластере]}.