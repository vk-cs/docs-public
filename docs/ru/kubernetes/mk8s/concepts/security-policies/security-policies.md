# {heading(Политики безопасности)[id=mk8s-security-policies]}
...
Чтобы снизить эти риски, в сервисе Cloud Containers кроме поддержки {linkto(../access-management#mk8s-access-management)[text=разграничения прав доступа]} к кластеру реализованы политики безопасности.

Возможности кластеров Kubernetes по работе с политиками безопасности:

- В кластеры встроен {linkto(../../reference/gatekeeper#mk8s-gatekeeper)[text=Gatekeeper]}, который позволяет реализовывать политики безопасности в кластере с помощью шаблонов ограничений (constraint template) и ограничений (constraint). Также поддержаны {linkto(#mk8s-security-policies-default)[text=преднастроенные политики безопасности по умолчанию]}.
- Предусмотрена возможность {linkto(#mk8s-security-policies-management-console)[text=работы с политиками безопасности из личного кабинета {var(cloud)}]}.

## {heading(Политики безопасности по умолчанию)[id=mk8s-security-policies-default]}
...
|-------------------------------------------------| ---------------------------------- |
| {linkto(#mk8s-security-policies-host-filesystem)[text=Ограничение host-filesystem]} | Ограничение и шаблон ограничения: `k8spsphostfilesystem` |
| {linkto(#mk8s-security-policies-host-namespaces)[text=Ограничение host-namespaces]} | Ограничение и шаблон ограничения: `k8spsphostnamespace` |

Эти политики реализуются с помощью {linkto(../../concepts/addons-and-settings/settings#mk8s-settings-templates-and-limitations)[text=преднастроенных шаблонов и ограничений Gatekeeper]}.
...
## {heading(Работа с политиками безопасности через личный кабинет)[id=mk8s-security-policies-management-console]}
...
Примеры работы со сторонними политиками безопасности смотрите в разделе {linkto(../../how-to-guides/gatekeeper#mk8s-gatekeeper)[text=Использование Gatekeeper]}.

При {linkto(../../instructions/manage-security#mk8s-manage-security)[text=работе с политиками]} через личный кабинет не рекомендуется изменять отдельные ограничения или шаблоны, соответствующие любым политикам, которые настроены в личном кабинете: некорректная работа или отсутствие политик может снизить безопасность кластера.

{note:warn}
{var(cloud)} не несет ответственность за проблемы с кластером, напрямую или косвенно возникшие из-за указанных действий.
{/note}

## {heading(Доступные политики безопасности и их настройки)[id=mk8s-security-policies-available-policies]}

Часть политик не имеет настроек, для них можно только выбрать пространство имен.

### {heading(Блокировать NodePort)[id=mk8s-security-policies-block-nodeport]}
...
### {heading(Блокировать Wildcard Ingress)[id=mk8s-security-policies-block-wildcard-ingress]}
...
### {heading(Блокировать обновление Service Account)[id=mk8s-security-policies-block-service-account-upd]}
...
### {heading(Запросы ресурсов контейнеров)[id=mk8s-security-policies-resources-request]}
...
### {heading(Лимиты ресурсов контейнеров)[id=mk8s-security-policies-resources-limits]}
...
### {heading(Запуск образов с SHA-хешем)[id=mk8s-security-policies-sha-hash-images]}
...
### {heading(Разрешенные репозитории)[id=mk8s-security-policies-allowed-repos]}
...
### {heading(Ограничение host-filesystem)[id=mk8s-security-policies-host-filesystem]}
...
### {heading(Ограничение host-namespaces)[id=mk8s-security-policies-host-namespaces]}
...
### {heading(Ограничение количества реплик)[id=mk8s-security-policies-min-max-replicas]}
