# {heading(Архитектура сервиса)[id=mk8s-architecture]}
...
## {heading(Топологии кластера)[id=mk8s-architecture-topology]}
...
- _Master-узлы_ хранят информацию о состоянии всего кластера и управляют распределением рабочей нагрузки по worker-узлам. Управление master-узлами пользователям не доступно, оно происходит на стороне платформы {var(cloud)}.
   ...
   Когда вы {linkto(../../instructions/create-cluster/create-webui-gen-2#mk8s-create-webui-gen-2)[text=создаете]} кластер Kubernetes, Managed Containers выбирает минимально подходящий {linkto(../../../../computing/iaas/concepts/vm/flavor#iaas-flavor)[text=шаблон конфигурации]} для его master-узлов. По умолчанию это ВМ с процессором Intel Cascade Lake, 2 CPU и 6 ГБ оперативной памяти. {linkto(../storage#mk8s-storage-supported-storage-types)[text=Тип диска]} master-узлов — High-IOPS SSD на 20 ГБ.

   На master-узлах по умолчанию включено {linkto(../scale#mk8s-scale-types)[text=автоматическое масштабирование]}, поэтому при изменении нагрузки на кластер количество его вычислительных ресурсов будет изменяться автоматически.
...
## {heading(Типы развертывания worker-узлов)[id=mk8s-architecture-deployment-types]}

В Managed Containers доступны два типа worker-узлов для развертывания:

- `Виртуальная машина` — узлы будут развернуты на ВМ. Этот тип развертывания предполагает гибкость и автоматическую масштабируемость и лучше всего подходит для приложений и сервисов, работающих постоянно.
- `Bare Metal` — узлы будут развернуты на арендованном сервере {linkto(../../../../../computing/bare-metal/concepts/about#bare-metal-about)[text=Bare Metal]}, без виртуализации и с доступом к полному вычислительному ресурсу физического сервера. При таком типе развертывания на производительность не влияет активность других виртуальных машин. Рекомендуется для ресурсоемких приложений.

  Группу узлов с типом развертывания `Bare Metal` можно добавить только в уже существующий кластер с {linkto(../../concepts/versions/version-support#mk8s-version-support)[text=версией]} не ниже 1.34.

## {heading(Окружение кластера)[id=mk8s-architecture-cluster-environment]}
...
Подробнее в разделе {linkto(../versions#mk8s-versions)[text=Доступные версии Kubernetes и политика поддержки версий]}.

## {heading(Интеграция с Kubernetes API)[id=mk8s-architecture-kubernetes-api-integration]}
...
API-эндпоинт кластеров Kubernetes размещен за {linkto(../network#mk8s-network)[text=отдельным балансировщиком нагрузки]}, поэтому доступ к API кластера можно получить по одному и тому же IP-адресу вне зависимости от количества master-узлов.

## {heading(Интеграция с платформой {var(cloud)})[id=mk8s-architecture-platform-integration]}
...
- [Container Storage Interface](https://kubernetes-csi.github.io/docs/) (CSI) — интеграция с сервисами хранения данных.
  ...
  Интеграция достигается с помощью OpenStack Cinder API. Подробнее в разделе {linkto(../storage#mk8s-storage)[text=Хранилище в кластере]}.
...
- [Container Network Interface](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/) (CNI) — интеграция с сетевыми подсистемами.
  ...
  В кластерах Kubernetes, которые вы создаете в сервисе Managed Containers, реализованы плагины, поддерживающие CNI: [Calico](https://projectcalico.docs.tigera.io/about/about-calico) и [Cilium](https://docs.cilium.io/en/stable/index.html). Они обеспечивают:

  - сетевую связность между контейнерами, {linkto(../../reference/pods#mk8s-pods)[text=подами]} и узлами кластера;
  ...
  Calico и Cilium интегрируются с платформой {var(cloud)} с помощью SDN Sprut. Подробнее в разделе {linkto(../network#mk8s-network)[text=Сеть в кластере]}.

## {heading(Встроенная поддержка Open Policy Agent)[id=mk8s-architecture-opa-gatekeeper]}

В кластеры Kubernetes в сервисе Managed Containers встроен {linkto(../../reference/gatekeeper#mk8s-gatekeeper)[text=Open Policy Agent Gatekeeper]}. Он позволяет применять {linkto(../security-policies#mk8s-security-policies)[text=политики безопасности]} для ресурсов Kubernetes. Также в таких кластерах действуют {linkto(../security-policies#mk8s-security-policies-default)[text=политики безопасности по умолчанию]}.

## {heading(Возможности масштабирования кластера)[id=mk8s-architecture-scaling-features]}

Кластеры Kubernetes в сервисе Managed Containers имеют встроенные {linkto(../scale#mk8s-scale)[text=возможности масштабирования master-узлов и worker-узлов]}.
...
- Для worker-узлов оно происходит с помощью {linkto(../cluster-autoscaler#mk8s-cluster-autoscaler)[text=Cluster Autoscaler]} и включается вручную при определении {linkto(../../instructions/manage-node-group#mk8s-manage-node-group)[text=настроек для каждой группы узлов]}.
