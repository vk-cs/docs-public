...
- Для worker-узлов оно происходит с помощью {linkto(../cluster-autoscaler#mk8s-cluster-autoscaler)[text=Cluster Autoscaler]} и включается вручную при определении {linkto(../../instructions/helpers/node-group-settings#mk8s-node-group-settings)[text=настроек для каждой группы узлов]}.