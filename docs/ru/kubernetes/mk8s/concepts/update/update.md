# {heading(Обновление версии кластера)[id=mk8s-update]}

Для кластеров Kubernetes в {var(cloud)} доступна операция обновления версии Kubernetes на одну из {linkto(../versions/version-support#mk8s-version-support)[text=поддерживаемых версий]}. Понижать версии нельзя.

При обновлении кластера также обновляется часть {linkto(../versions/components#mk8s-components)[text=компонентов]} кластера. Причем если обновляемый вместе с кластером компонент был удален, он восстановится при обновлении кластера. Перечень обновляемых компонентов:
...
Аддоны не обновляются вместе с кластером. При необходимости {linkto(../../instructions/addons/manage-addons#mk8s-manage-addons-update)[text=обновите аддоны самостоятельно]} после завершения обновления кластера.

Процесс обновления отличается для кластеров версий 1.16 и ниже. Обновить их можно только путем переноса резервной копии данных в новый кластер нужной версии, например, с помощью [Velero](https://velero.io/docs).

## {heading(Обновление master- и worker-узлов)[id=mk8s-update-process]}
...
1. Обновляются worker-узлы в группах.
   ...
   Обновление тоже выполняется по принципу rolling update, но за шаг обновляется не один, а максимально возможное количество узлов. Это количество рассчитывается, исходя из {linkto(../../instructions/helpers/node-group-settings#mk8s-node-group-settings)[text=настройки группы узлов]} **Процент недоступных нод при обновлении кластера**. Значение этой настройки вы можете установить как в момент {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-add-group)[text=создания]} группы узлов, так и перед началом {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-configure-node-update)[text=обновления]} кластера. При расчетах используется округление до целого в большую сторону.

## {heading(Как посчитать процент недоступных узлов при обновлении кластера)[id=mk8s-update-unavailable-nodes]}

Во время обновления узлы становятся недоступными, и Managed Containers автоматически перераспределяет нагрузку с них на свободные узлы. При обновлении кластера функция автоматического масштабирования не работает, поэтому необходимо иметь достаточное количество запасных master- и worker-узлов для переноса нагрузки с обновляемых.
...
1. Убедитесь, что у вас в запасе есть достаточное количество узлов, чтобы Managed Containers мог перенести на них нагрузку с обновляемых узлов. Если worker-узлов недостаточно, {linkto(../../instructions/manage-node-group#mk8s-manage-node-group-add-group)[text=добавьте]} их. 
