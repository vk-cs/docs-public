...
Cluster Autoscaler предустановлен во всех кластерах Kubernetes, которые вы создаете в сервисе Cloud Containers, но по умолчанию не включен. Автоматическое масштабирование worker-узлов нужно включать для каждой группы worker-узлов вручную при {linkto(../../instructions/helpers/node-group-settings#mk8s-node-group-settings)[text=определении ее настроек]}.
...
1. По прошествии установленного времени простоя помечает этот узел {linkto(../../reference/labels-and-taints#mk8s-labels-and-taints)[text=ограничением (taint)]}, которое запрещает размещать на нем новые поды и рабочую нагрузку, и запускает его удаление.
...
Удаление узлов возможно только до минимального значения, которое вы задали в настройках группы. Если автоматическое масштабирование включено, но worker-узлы не удаляются, попробуйте решения из раздела {linkto(../../troubleshooting/cluster-does-not-scale-down#mk8s-cluster-does-not-scale-down)[text=Недозагруженные узлы не удаляются при автоматическом масштабировании]}.