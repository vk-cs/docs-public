...
## {heading(Структура политики аудита)[id=mk8s-cluster-audit-policy-structure]}

Ниже представлен полный файл с настройками политики аудита кластеров Kubernetes.
...
## {heading(Какие события попадают в журнал аудита)[id=mk8s-cluster-audit-policy-events]}
...
## {heading(Какие события исключаются из журнала аудита)[id=mk8s-cluster-audit-policy-excluded-events]}
