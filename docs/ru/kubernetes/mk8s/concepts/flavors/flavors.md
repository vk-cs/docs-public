# {heading(Доступные вычислительные ресурсы)[id=mk8s-flavors]}

## {heading(Шаблоны конфигурации)[id=mk8s-flavors-vm-flavor]}
...
## {heading(Расчет подходящей конфигурации кластера)[id=mk8s-flavors-configuration-calculation]}
...
## {heading(Расчет доступных вычислительных ресурсов CPU и RAM)[id=mk8s-flavors-cpu-ram-calculation]}
...
## {heading(Доступные вычислительные ресурсы на узле)[id=mk8s-flavors-available-node-resources]}
...
## {heading(Шаблоны конфигурации с GPU)[id=mk8s-flavors-gpu]}
...
### {heading(Особенности создания кластера с GPU)[id=mk8s-flavors-gpu-features]}
...
### {heading(Технологии совместного использования GPU)[id=mk8s-flavors-gpu-sharing]}
