# {heading(Создание балансировщика нагрузки с индивидуальным шаблоном конфигурации)[id=balancing-private-flavor]}

Балансировщики нагрузки в {var(cloud)} по умолчанию создаются со стандартным шаблоном конфигурации. Для работы с повышенными объемами трафика используйте балансировщик нагрузки с более производительной конфигурацией.

В {var(cloud)} можно добавлять к вашей инфраструктуре балансировщики нагрузки с созданным специально для вас шаблоном конфигурации (_flavor_). Это позволяет увеличить производительность инфраструктуры при высокой нагрузке.

Шаблон более производительной конфигурации создается специалистами {var(cloud)} после согласования технических деталей.

## {heading(Подготовительные шаги)[id=balancing-private-flavor-prep]}

1. Обратитесь к менеджеру или пресейл-архитектору {var(cloud)} с запросом на создание индивидуального шаблона, если это еще не сделано. После согласования вы получите информацию о созданном для вас шаблоне конфигурации.
1. Если вы планируете пользоваться OpenStack CLI для создания или проверки работоспособности балансировщика:

   1. Убедитесь, что клиент OpenStack с пакетами `octavia` и `neutron` {linkto(../../../../tools-for-using-services/cli/openstack-cli#openstack-install)[text=установлены]}.
   1. {linkto(../../../../tools-for-using-services/cli/openstack-cli#openstack-authorize)[text=Пройдите аутентификацию]} в проекте.

1. Установите утилиту [cURL](https://github.com/curl/curl/blob/master/docs/INSTALL.md), если она еще не установлена.
1. {linkto(../../../../tools-for-using-services/api/rest-api/endpoints#rest-api-endpoints)[text=Получите]} эндпоинт сервиса Octavia. В этом примере: `https://public.infra.mail.ru:9876`.
1. {linkto(../../../../tools-for-using-services/api/rest-api/case-keystone-token#rest-api-keystone-token)[text=Получите]} токен доступа к API `X-Auth-Token`.
1. {linkto(../../../../networks/vnet/instructions/net#vnet-net-view)[text=Узнайте]} идентификаторы сети и подсети, в которых вы хотите создать балансировщик.

## {heading(1. Получите информацию о шаблонах)[id=balancing-private-flavor-get-info]}
...
## {heading(2. Создайте балансировщик с индивидуальным шаблоном)[id=balancing-private-flavor-add]}
...
- `<ИДЕНТИФИКАТОР_ШАБЛОНА>` — идентификатор шаблона конфигурации балансировщика, полученный от {var(cloud)} или {linkto(#balancing-private-flavor-get-info)[text=самостоятельно]}.
...
- `<ИДЕНТИФИКАТОР_ПРОЕКТА>` — {linkto(../../../../tools-for-using-services/api/rest-api/endpoints#rest-api-endpoints-get-project-id)[text=идентификатор проекта]} OpenStack, в котором вы хотите создать балансировщик.
...
- `<ЗОНА_ДОСТУПНОСТИ>` — имя {linkto(../../../../computing/iaas/concepts/avail-zone#iaas-avail-zone)[text=зоны доступности]}, в которой вы хотите разместить балансировщик.
...
- `<ИМЯ_ИЛИ_ИДЕНТИФИКАТОР_ШАБЛОНА>` — имя или идентификатор шаблона конфигурации балансировщика, полученное от {var(cloud)} или {linkto(#balancing-private-flavor-get-info)[text=самостоятельно]}.
- `<ИДЕНТИФИКАТОР_СЕТИ>` — идентификатор сети, в которой вы хотите разместить балансировщик.
- `<НАЗВАНИЕ_БАЛАНСИРОВЩИКА>` — произвольное название создаваемого балансировщика.
{/tab}
{/tabs}

## {heading(3. Проверьте создание и параметры балансировщика)[id=balancing-private-flavor-check]}
...
1. Нажмите на имя нужного балансировщика.
   ...
   Откроется страница с подробной информацией о нем. На этой странице можно также {linkto(../../../../networks/balancing/instructions/manage-lb#balancing-manage-lb-edit)[text=редактировать]} параметры балансировщика.
...
1. Получите список всех балансировщиков нагрузки с идентификаторами:
   ...
   - `<ТОКЕН>` — токен доступа к API.
   - `<ИДЕНТИФИКАТОР_ПРОЕКТА>` — {linkto(../../../../tools-for-using-services/api/rest-api/endpoints#rest-api-endpoints-get-project-id)[text=идентификатор проекта]} OpenStack, список балансировщиков которого вы хотите посмотреть. Заполняется опционально.
...
После успешного создания балансировщика вы можете {linkto(../../../../networks/balancing/instructions/manage-lb#balancing-manage-lb)[text=управлять]} им.

## {heading(Удалите неиспользуемые ресурсы)[id=balancing-private-flavor-delete]}

Если созданный балансировщик вам больше не нужен, {linkto(../../../../networks/balancing/instructions/manage-lb#balancing-manage-lb-delete)[text=удалите]} его.
