# {heading(Организация VPN-туннеля с использованием SDN Sprut)[id=vnet-site-to-site-vpn]}

С помощью {linkto(../../../../../networks/vnet/how-to-guides/onpremise-connect/advanced-router#vnet-advanced-router)[text=продвинутого маршрутизатора]} вы можете настроить VPN-туннель между сетью вашей локальной инфраструктуры и виртуальными сетями {var(cloud)}.
...
- Виртуальная сеть — находится в {var(cloud)} и подключена к платформенному маршрутизатору.

Для демонстрации настройки соединения в личном кабинете {var(cloud)} будут созданы:
...
1. {linkto(../../../../../networks/vnet/instructions/advanced-router/manage-advanced-routers#vnet-manage-advanced-routers-add)[text=Создайте]} продвинутый маршрутизатор со следующими параметрами:
...
1. {linkto(../../../../../networks/vnet/instructions/advanced-router/manage-interfaces#vnet-manage-interfaces-add)[text=Добавьте]} два интерфейса созданному маршрутизатору:
...
1. {linkto(../../../../../computing/iaas/instructions/vm/vm-create#iaas-vm-create)[text=Создайте]} виртуальную машину со следующими параметрами:
...
1. {linkto(../../../../../networks/vnet/instructions/secgroups#vnet-secgroups-add)[text=Создайте]} группу безопасности `permit-remote-cidr-in`.

1. В группе безопасности `permit-remote-cidr-in` {linkto(../../../../../networks/vnet/instructions/secgroups#vnet-secgroups-add-rule)[text=добавьте]} правило для входящего трафика со следующими параметрами:
...
1. {linkto(../../../../../networks/vnet/instructions/secgroups#vnet-secgroups-instance-add-sg)[text=Примените]} группу безопасности `permit-remote-cidr-in` к ВМ `test-vm`.

## {heading(1. Настройте VPN-туннель на стороне облачной сети)[id=vnet-site-to-site-vpn-cloud-network-tunnel]}
...
## {heading(2. Настройте VPN-туннель на стороне клиентской сети)[id=vnet-site-to-site-vpn-client-network-tunnel]}
...
1. Настройте исключения:
   ...
   1. Настройте разрешение Firewall в цепочке Forward, чтобы разрешить прохождение трафика по направлению из сети {var(cloud)} в локальную сеть через IPsec VPN-туннель.
...
## {heading(3. Проверьте статус VPN-туннеля)[id=vnet-site-to-site-vpn-check]}
...
- в личном кабинете {var(cloud)} проверьте настройки NAT и групп безопасности;
...
## {heading(Удалите неиспользуемые ресурсы)[id=vnet-site-to-site-vpn-delete]}

Если созданные ресурсы вам больше не нужны, удалите их:

1. {linkto(../../../../../computing/iaas/instructions/vm/vm-manage#iaas-vm-manage-delete)[text=Удалите]} виртуальную машину.
1. {linkto(../../../../../networks/vnet/instructions/router#vnet-router-delete)[text=Удалите]} маршрутизатор.
