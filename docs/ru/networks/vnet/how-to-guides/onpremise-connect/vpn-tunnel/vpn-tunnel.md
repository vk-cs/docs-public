# {heading(Организация VPN-туннеля с использованием SDN Neutron)[id=vnet-vpn-tunnel]}

{var(cloud)} позволяет устанавливать сетевую связность между инфраструктурой on-premises и виртуальной сетью {var(cloud)} с {linkto(../../../../../networks/vnet/concepts/sdn#vnet-sdn-neutron)[text=SDN Neutron]} при помощи VPN-туннеля.

Для примера будет построен VPN-туннель на основе стандартного маршрутизатора до другой сети в {var(cloud)}. В качестве VPN-эндпоинта будет использоваться виртуальная машина. Инструкции по настройке VPN-туннеля можно адаптировать под работу с любым другим VPN-эндпоинтом, например, корпоративным файерволом или другим сетевым оборудованием.

VPN-туннель в примере будет связывать следующие сети:

- Клиентская сеть — сеть в {var(cloud)}, которая имитирует сеть в инфраструктуре on-premises.
- Виртуальная сеть — сеть в {var(cloud)}, подключенная к стандартному маршрутизатору.

В каждой сети будут созданы виртуальные машины для проверки работоспособности туннеля.

{note:warn}
Далее приведен пример настройки VPN-туннеля для сетей с SDN {linkto(../../../../../networks/vnet/concepts/sdn#vnet-sdn-neutron)[text=Neutron]}.
В сетях с SDN {linkto(../../../../../networks/vnet/concepts/sdn#vnet-sdn-sprut)[text=Sprut]} соединение виртуальной сети {var(cloud)} с клиентской сетью настраивается при помощи {linkto(../../../../../networks/vnet/how-to-guides/onpremise-connect/advanced-router#vnet-advanced-router)[text=продвинутого маршрутизатора]}.
{/note}

## {heading(Подготовительные шаги)[id=vnet-vpn-tunnel-prep]}
...
1. {linkto(../../../../../networks/vnet/instructions/net#vnet-net-add)[text=Создайте]} сети.
...
|Floating IP-адрес виртуальной машины `client_vpn_gw`
|`192.0.2.200`
...
## {heading(1. Настройте VPN-туннель на стороне облачной сети)[id=vnet-vpn-tunnel-cloud-network-tunnel]}

{linkto(../../../../../networks/vnet/instructions/vpn#vnet-vpn)[text=Создайте VPN-туннель]} со следующими параметрами:
...
## {heading(2. Настройте VPN-туннель на стороне клиентской сети)[id=vnet-vpn-tunnel-client-network-tunnel]}

1. Отключите {linkto(../../../../../networks/vnet/concepts/traffic-limiting#vnet-traffic-limiting-source-guard)[text=IP Source Guard]} для порта VPN-шлюза, чтобы он мог пересылать любой трафик:

   1. Найдите порт с приватным IP-адресом `172.16.0.5` виртуальной машины `client_vpn_gw`. {linkto(../../../../../networks/vnet/instructions/ports#vnet-ports-view)[text=Получите]} идентификатор этого порта.
...
1. Добавьте параметры VPN-соединения со стороны клиентской сети в файл `/etc/ipsec.conf`. Эти параметры — зеркальное отражение настроек туннеля, сделанных {linkto(#vnet-vpn-tunnel-cloud-network-tunnel)[text=на стороне облачной сети]}.
...
1. Укажите в файле `/etc/ipsec.secrets` ключ совместного использования (PSK). Ключ должен совпадать с ключом, указанным {linkto(#vnet-vpn-tunnel-cloud-network-tunnel)[text=на стороне облачной сети]}:
...
## {heading(3. Добавьте статические маршруты)[id=vnet-vpn-tunnel-add-static-routs]}
...
1. Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для подсети `vkcloudSubnet` и выберите пункт `Редактировать подсеть`.
...
## {heading(4. Проверьте работоспособность VPN-туннеля)[id=vnet-vpn-tunnel-check]}

1. Посмотрите состояние VPN-туннеля со стороны платформы {var(cloud)}.
...
1. {linkto(../../../../../networks/vnet/instructions/secgroups#vnet-secgroups-add)[text=Создайте]} группу безопасности `icmp`, разрешающую ICMP-трафик.
...
1. {linkto(../../../../../computing/iaas/instructions/vm/vm-create#iaas-vm-create)[text=Создайте]} две виртуальные машины:

   - `clientVM`:

     - в сети `clientNet`, подсети `clientSubnet`;
     - с Floating IP-адресом для подключения к ней по SSH;
     - с группами безопасности `default`, `ssh`, `icmp`.
...
## {heading(Удалите неиспользуемые ресурсы)[id=vnet-vpn-tunnel-delete]}

Если созданные ресурсы вам больше не нужны, удалите их:

1. {linkto(../../../../../computing/iaas/instructions/vm/vm-manage#iaas-vm-manage-delete)[text=Удалите]} виртуальные машины.
1. {linkto(../../../../../networks/vnet/instructions/vpn#vnet-vpn-delete)[text=Удалите]} VPN-туннель.
1. {linkto(../../../../../networks/vnet/instructions/router#vnet-router-static-routs-manage)[text=Удалите]} статические маршруты, прописанные на стороне клиентской сети.

   {note:info}
   Статические маршруты на стороне облачной сети будут удалены вместе с подсетью, для которой они были прописаны.
   {/note}

1. {linkto(../../../../../networks/vnet/instructions/router#vnet-router-delete)[text=Удалите]} маршрутизаторы клиентской и облачной сетей.
1. Удалите клиентскую и облачную {linkto(../../../../../networks/vnet/instructions/net#vnet-net-subnet-delete)[text=подсети]} и {linkto(../../../../../networks/vnet/instructions/net#vnet-net-delete)[text=сети]}.
1. {linkto(../../../../../networks/vnet/instructions/ip/floating-ip#vnet-floating-ip-delete)[text=Удалите]} Floating IP-адреса.
