Виртуальная сеть VK Cloud соединяется с клиентской сетью через VPN-туннель. Для примера будет построен VPN-туннель до другой сети платформы VK Cloud, в качестве VPN-эндпоинта будет использоваться виртуальная машина. Также будут созданы несколько виртуальных машин для проверки работоспособности туннеля.

Инструкции по настройке VPN-туннеля можно адаптировать под работу с любым другим VPN-эндпоинтом, например, корпоративным файерволом или другим сетевым оборудованием.
