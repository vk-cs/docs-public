В статье описана настройка сетевой связности между onpremise-инфраструктурой и виртуальной сетью VK Cloud в [SDN Neutron](../../concepts/sdn#neutron).  

Для примера будет построен VPN-туннель на основе стандартного маршрутизатора между сетями в VK Cloud:

- Клиентская сеть — сеть в VK Cloud, которая имитирует сеть в onpremise-инфраструктуре. В качестве VPN-эндпоинта будет использоваться виртуальная машина. Инструкции по настройке VPN-туннеля можно адаптировать под работу с любым другим VPN-эндпоинтом, например, корпоративным файерволом или другим сетевым оборудованием.
...
В проектах с [SDN Sprut](/ru/networks/vnet/concepts/sdn#sprut) соединение виртуальной сети VK Cloud с onpremise-инфраструктурой настраивается при помощи [продвинутого маршрутизатора](../../how-to-guides/advanced-router).
...
1. [Удалите](../../service-management/ip/floating-ip#delete) плавающие IP-адреса.
