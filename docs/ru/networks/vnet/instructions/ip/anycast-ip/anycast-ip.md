...
Управление {linkto(../../../concepts/ips-and-inet#vnet-ips-and-inet-anycast-ip)[text=Anycast IP-адресами]} доступно в личном кабинете {var(cloud)} и через {linkto(../../../../../tools-for-using-services/api/api-spec/network-api/neutron-sprut-api#api-spec-neutron-sprut)[text=API]}.
...
1. {linkto(#vnet-anycast-ip-associate)[text=Привяжите]} IP-адреса серверов или балансировщиков к Anycast IP-адресу.
1. (Опционально) {linkto(#vnet-anycast-ip-add-healthcheck)[text=Добавьте]} правило проверки работоспособности серверов или балансировщиков к Anycast IP-адресу.
...
1. Откройте окно выбора IP-адресов одним из способов:
   ...
   - Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для нужного Anycast IP-адреса и выберите пункт **Привязать IP-адрес**.
...
1. В открывшемся окне:
   ...
   1. Выберите из списка тип ресурса, из которого будут выбраны IP-адреса для привязки:
      ...
      - `Балансировщики нагрузки`: балансировщики должны размещаться в сети с доступом в интернет и не иметь привязки к {linkto(../../../concepts/ips-and-inet#vnet-ips-and-inet-floating-ip)[text=Floating IP-адресу]}.
      - `Виртуальные машины`: ВМ должны иметь публичные IP-адреса во {linkto(../../../concepts/net-types#vnet-net-types-external-net)[text=внешних сетях]}.
...
1. Удалите привязанный IP-адрес одним из способов:
   ...
   - Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для нужного IP-адреса и выберите пункт **Удалить**.
...
1. Добавьте правило одним из способов:
   ...
   - Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для нужного Anycast IP-адреса и выберите пункт **Добавить правило**.
...
1. Задайте настройки для проверки:

   - (Для TCP) **Порт** — порт для проверки работоспособности. Задается одно значение для всей группы связанных IP-адресов.
   - **Задержка между проверками** — время в секундах.
   - **Таймаут подключения** — время в секундах.
   - **Порог успешных проверок для анонса** — количество последовательно выполненных успешных проверок для возврата ресурса в анонс.
   - **Порог неудачных проверок для снятия с анонса** — количество последовательно выполненных неудачных проверок для снятия ресурса с анонса.
...
Для виртуальных машин, IP-адреса которых указаны на вкладке **Правило**, {linkto(../../secgroups#vnet-secgroups)[text=разрешите]} входящий трафик с IP-адреса `169.254.169.100/32` на заданный порт. Это технический адрес {var(cloud)} для проверки доступности сервиса. Если правило группы безопасности ограничит доступ к порту ВМ с этого адреса, сервис Anycast IP отметит ВМ как недоступную и не будет отправлять трафик на нее.
...
1. Переименуйте Anycast IP-адрес одним из способов:
   ...
   - Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для нужного Anycast IP-адреса и выберите пункт **Редактировать**.
...
1. Перейдите в раздел **Виртуальные сети** → **IP-адреса**.
1. Перейдите на вкладку **Anycast IP**.
1. Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для нужного Anycast IP-адреса и выберите пункт **Удалить**.
1. Подтвердите удаление.
