# {heading(Используемые SDN)[id=vnet-sdn]}

## {heading(Что такое SDN)[id=vnet-sdn-concept]}
...
SDN — инструмент управления overlay-сетями (виртуальная сеть поверх физической) и основа облачной инфраструктуры. SDN обеспечивает маршрутизацию, управление правилами ограничения трафика и сетевую связность между сервисами. {var(cloud)} использует собственное SDN-решение — {linkto(#vnet-sdn-sprut)[text=Sprut]}.
...
![SDN в архитектуре {var(cloud)}](./assets/img_sdn_basis.png){params[noBorder=true]}

Все сервисы и продукты в архитектуре облака связаны с SDN, поэтому для {var(cloud)} важно иметь надежное, гибкое и отказоустойчивое SDN-решение.

## {heading(Sprut)[id=vnet-sdn-sprut]}

_Sprut_ — это собственная разработка {var(cloud)}, созданная для замены {linkto(#vnet-sdn-neutron)[text=SDN Neutron]} и полностью совместимая с OpenStack Neutron API. Все новые проекты используют SDN Sprut, для старых проектов рекомендуется выполнить [миграцию](../../../../cases/sprut-migration), так как SDN Neutron выводится из эксплуатации.
...
Если вы хотите перевести ваши облачные ресурсы на SDN Sprut, воспользуйтесь [документацией по миграции](../../../../cases/sprut-migration).
{/note}
...
Долгое время в {var(cloud)} использовалась только SDN Neutron, это создавало следующие проблемы и ограничения:
...
Решением проблем с масштабируемостью и надежностью SDN Neutron стала разработка собственного {linkto(#vnet-sdn-sprut)[text=SDN Sprut]}.
