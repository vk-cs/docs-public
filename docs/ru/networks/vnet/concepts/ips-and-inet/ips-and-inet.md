...
Anycast IP-адрес работает только в SDN Sprut.
...
- В случае недоступности сервера или зоны доступности, перенаправляет трафик на другой сервер или зону доступности, обеспечивая отказоустойчивость приложения.

Отличия от Floating IP-адреса:
...
- Интерфейс [продвинутого маршрутизатора](../../concepts/router#vozmozhnosti_prodvinutogo_marshrutizatora) с публичным IP-адресом во внешней сети (должна быть подключена опция **SNAT**).

К одному Anycast IP-адресу можно привязать до 8 IP-адресов, все IP-адреса должны быть только одного типа. 
