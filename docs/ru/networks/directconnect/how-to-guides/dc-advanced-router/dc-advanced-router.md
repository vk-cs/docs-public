...
- Нельзя использовать [Floating IP-адрес](/ru/networks/vnet/concepts/ips-and-inet#floating-ip). В качестве альтернативы может быть настроена трансляция DNAT (Destination NAT).
...
В сеть Cloud Direct Connect [добавьте](/ru/networks/vnet/how-to-guides/custom-subnet) транзитную подсеть с маской `/30`. При добавлении переменных окружения укажите параметр `N_ID="<UUID_СЕТЕВОГО_СТЫКА>"`.
