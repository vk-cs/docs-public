# {heading(Подключение к {var(cloud)} через сеть Cloud Direct Connect)[id=directconnect-dc-advanced-router]}

Cloud Direct Connect позволяет организовать разные {linkto(/ru/networks/vnet/concepts/onpremise-connect#vnet-onpremise-connect)[text=варианты подключения]} сети, которая находится в вашей локальной инфраструктуре, к виртуальным сетям {var(cloud)}. В этом примере подключение будет создано с помощью выделенного канала связи Cloud Direct Connect и продвинутого маршрутизатора с динамической маршрутизацией по протоколу [BGP](https://datatracker.ietf.org/doc/html/rfc1163).
...
![Схема организации сетевой связности](/ru/networks/directconnect/how-to-guides/dc-advanced-router/assets/cdc_p1.png){params[noBorder=true]}

Ограничения конфигурации:

- Нельзя использовать {linkto(/ru/networks/vnet/concepts/ips-and-inet#vnet-ips-and-inet-floating-ip)[text=Floating IP-адрес]}. В качестве альтернативы может быть настроена трансляция DNAT (Destination NAT).
- Нельзя подключить {linkto(/ru/start/concepts/architecture#architecture)[text=PaaS-сервисы]}, так как их развертывание требует доступа в интернет через {linkto(/ru/networks/vnet/concepts/ips-and-inet#vnet-ips-and-inet-internet-address-pool)[text=публичные IP-адреса]} {var(cloud)}.
- Доступно только на проектах с SDN Sprut.

{note:info}
{var(cloud)} позволяет настроить разные {linkto(/ru/networks/vnet/concepts/onpremise-connect#vnet-onpremise-connect)[text=варианты подключения]} к удаленной инфраструктуре. Выбор варианта зависит от SDN проекта, доступа к интернету из удаленной инфраструктуры и требований к отказоустойчивости соединения.
{/note}

## {heading(Подготовительные шаги)[id=directconnect-dc-advanced-router-prep]}

1. {linkto(/ru/tools-for-using-services/api/rest-api/enable-api#rest-api-enable-activate)[text=Активируйте доступ по API]}, если этого еще не сделано.
1. Убедитесь, что клиент OpenStack {linkto(/ru/tools-for-using-services/cli/openstack-cli#openstack-install)[text=установлен]}, и {linkto(/ru/tools-for-using-services/cli/openstack-cli#openstack-authorize)[text=пройдите аутентификацию]} в проекте.
...
1. Выберите или {linkto(/ru/networks/vnet/instructions/net#vnet-net-add)[text=создайте]} виртуальную сеть в вашем проекте в {var(cloud)}. Сеть не должна быть подключена к маршрутизатору.
...
1. {linkto(/ru/computing/iaas/instructions/vm/vm-create#iaas-vm-create)[text=Создайте виртуальную машину]} в выбранной сети.

   Запишите IP-адрес ВМ.
1. {linkto(/ru/networks/directconnect/connect#directconnect-connect)[text=Подключитесь]} к сервису Cloud Direct Connect, если этого еще не сделано.
...
## {heading(1. Добавьте транзитную подсеть)[id=directconnect-dc-advanced-router-add-subnet]}

В сеть Cloud Direct Connect {linkto(/ru/networks/vnet/how-to-guides/custom-subnet#vnet-custom-subnet)[text=добавьте]} транзитную подсеть с маской `/30`. При добавлении переменных окружения укажите параметр `N_ID="<UUID_СЕТЕВОГО_СТЫКА>"`.

Запишите имя и CIDR подсети. В этом примере: `dc-subnet`, `192.168.0.0/30`.

## {heading(2. Добавьте продвинутый маршрутизатор)[id=directconnect-dc-advanced-router-add-advanced-router]}

{linkto(/ru/networks/vnet/instructions/advanced-router/manage-advanced-routers#vnet-manage-advanced-routers-add)[text=Создайте]} продвинутый маршрутизатор со следующими параметрами:

- **Название**: в этом примере `Advanced router`;
- **SNAT**: опция выключена.

## {heading(3. Настройте сетевые интерфейсы продвинутого маршрутизатора)[id=directconnect-dc-advanced-router-network-configure]}

1. {linkto(/ru/networks/vnet/instructions/advanced-router/manage-interfaces#vnet-manage-interfaces-add)[text=Добавьте]} интерфейс продвинутого маршрутизатора, направленный в виртуальную сеть:
...
## {heading(4. Настройте сетевые интерфейсы BGP-маршрутизатора клиентской сети)[id=directconnect-dc-advanced-router-client-configure]}
...
1. В [личном кабинете](https://msk.cloud.vk.ru/app/) перейдите в раздел **Виртуальные сети** → **Маршрутизаторы**.
...
## {heading(6. Настройте BGP-соседство для маршрутизатора клиентской сети)[id=directconnect-dc-advanced-router-bgp-client]}
...
## {heading(7. Настройте статические маршруты между сетями)[id=directconnect-dc-advanced-router-static-routs]}
...
1. Настройте статический маршрут от виртуальной сети `vkcloud-net` до клиентской сети `customer-net` через продвинутый маршрутизатор:
   ...
   1. В [личном кабинете](https://msk.cloud.vk.ru/app/) перейдите в раздел **Виртуальные сети** → **Сети**.
...
## {heading(8. Проверьте работоспособность)[id=directconnect-dc-advanced-router-check]}
...
## {heading(Удалите неиспользуемые ресурсы)[id=directconnect-dc-advanced-router-delete]}

Если созданные ресурсы вам больше не нужны, удалите их:

1. {linkto(/ru/computing/iaas/instructions/vm/vm-manage#iaas-vm-manage-delete)[text=Удалите]} виртуальную машину.
1. {linkto(/ru/networks/vnet/instructions/router#vnet-router-delete)[text=Удалите]} маршрутизаторы.
1. Удалите {linkto(/ru/networks/vnet/instructions/net#vnet-net-subnet-delete)[text=подсеть]} и {linkto(/ru/networks/vnet/instructions/net#vnet-net-delete)[text=сеть]}, в которой была размещена ВМ.
