# {heading(Ресурсные записи)[id=dns-records]}

Сервис DNS {var(cloud)} позволяет публиковать ваши зоны и записи в DNS, не разворачивая собственные DNS-сервера. Публичный DNS управляет DNS-зонами, которые видны в интернете.

## {heading(Настройка ролевой модели)[id=dns-records-role-model]}

Для работы с публичным DNS настройте {linkto(../../../../../access/iam/concepts/rolesandpermissions#iam-concepts-rolesandpermissions)[text=роли]} для пользователей [личного кабинета](https://msk.cloud.vk.ru/app/) {var(cloud)}:
...
## {heading(Квоты и лимиты)[id=dns-records-quotas-limits]}
...
## {heading(Просмотр списка DNS-зон)[id=dns-records-zone-list]}
...
## {heading(Создание DNS-зоны)[id=dns-records-zone-add]}
...
1. Нажмите кнопку **Добавить зону**.
1. Обратитесь к владельцу [указанного](https://msk.cloud.vk.ru/app/services/dns/list) домена, чтобы делегировать управление зоной на DNS-сервера {var(cloud)}.
...
{note:info}
У большинства провайдеров возможно самостоятельно делегировать управление зоной. Если у вас возникли вопросы, как это сделать, обратитесь за помощью к владельцу [указанного](https://msk.cloud.vk.ru/app/services/dns/list) домена.
{/note}

## {heading(Просмотр DNS-зоны)[id=dns-records-zone-view]}
...
## {heading(Редактирование DNS-зоны)[id=dns-records-zone-edit]}
...
## {heading(Создание подзоны DNS)[id=dns-records-subzone-add]}
...
Чтобы создать подзону в проекте {var(cloud)}, {linkto(#dns-records-zone-add)[text=создайте]} две ресурсные NS-записи с именем подзоны, повторно делегирующие подзону на DNS-сервера {var(cloud)}.
...
## {heading(Удаление DNS-зоны)[id=dns-records-zone-delete]}
...
## {heading(Добавление ресурсных записей)[id=dns-records-add]}
...
{var(cloud)} поддерживает типы ресурсных записей:
...
- `NS` — запись DNS, которая содержит адрес обслуживающего данную зону или подзону сервера имен. По умолчанию в зоне будут установлены две записи `NS`. Эти записи устанавливаются на стороне владельца доменных имен, чтобы передать права управления доменом серверу имен {var(cloud)}.
...
## {heading(Добавление ресурсных записей)[id=dns-records-record-add]}
...
Воспользуйтесь {linkto(../../../../../tools-for-using-services/api/api-spec/network-api/api-dns#api-spec-dns)[text=методами]} из спецификации API.
...
## {heading(Просмотр списка ресурсных записей)[id=dns-records-list]}
...
## {heading(Просмотр параметров ресурсных записей)[id=dns-records-view]}
...
1. Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для нужной записи и выберите пункт **Редактировать**.
...
## {heading(Редактирование ресурсных записей)[id=dns-records-edit]}
...
## {heading(Удаление ресурсных записей)[id=dns-records-delete]}
...
1. Нажмите ![ ](../../../../../assets/more-icon.svg "inline") для нужной записи и выберите пункт **Удалить**.
1. Подтвердите действие.
