# {heading(DNS-балансировка)[id=dns-global-balancer]}

{note:info}
Для получения доступа к сервису обратитесь в [техническую поддержку](/ru/contacts).
{/note}

## {heading(Группы балансировки)[id=dns-global-balancer-groups]}

Группы балансировки публичного DNS в {var(cloud)} позволяют создать инфраструктуру для отказоустойчивого географически распределенного приложения. Инструмент DNS-балансировки направляет трафик на бекэнд-сервера приложения клиента, которые находятся в разных {linkto(../../../../start/concepts/architecture#architecture-az)[text=зонах доступности]}, в зависимости от показателей доступности серверов и алгоритмов балансировки. Если один из дата-центров будет недоступен, DNS-балансировщик направит трафик на серверы в других дата-центрах.
...
## {heading(Алгоритм балансировки)[id=dns-global-balancer-algorithm]}
