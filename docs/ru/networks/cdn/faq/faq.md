...
Не все роли пользователей имеют доступ к CDN. Если вас пригласили в проект, проверьте доступ к сервису CDN для вашей [роли](/ru/access/iam/concepts/rolesandpermissions).
...
Обычно на это уходит до 30 минут. Если сертификат не выпустился, убедитесь, что CNAME-запись вашего домена корректно указывает на домен CDN-ресурса. При выборе этой опции также необходимо включить опцию [Доступ к контенту конечным пользователям](/ru/networks/cdn/instructions/manage-cdn/enable-cdn). Подробнее о том, как создать CNAME-запись для CDN-ресурса, — в разделе [Создание CDN-ресурса](/ru/networks/cdn/instructions/create-resource#podgotovka_cdn_resursa_k_rabote)
...
{cut(Возможно ли использовать хранилище VK Object Storage как источник, защищенный секретным ключом?)}
