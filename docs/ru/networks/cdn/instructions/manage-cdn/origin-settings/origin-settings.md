# {heading(Взаимодействие с источником)[id=cdn-origin-settings]}

## {heading(Изменение протокола взаимодействия с источником)[id=cdn-origin-settings-protocol]}
...
1. В блоке **Настройка доступа и протокола** выберите протокол взаимодействия с источником:
...
## {heading(Изменение источника данных)[id=cdn-origin-settings-data-source]}
...
1. Перейдите на вкладку **Основные настройки**.
1. Раскройте блок **Конфигурация источников и доменов**.
...
## {heading(Изменение заголовка Host)[id=cdn-origin-settings-host-header]}
...
По умолчанию значение заголовка Host совпадает с тем, которое используется при запросе к CDN.
...
1. В параметре **Изменение заголовка Host** выберите, изменять ли значение HTTP-заголовка `Host` при обращении к источникам:
    ...
    - `Не менять` —  значение заголовка совпадает с первым CNAME, то есть, с именем первого настроенного персонального домена.
        ...
        Также пусть для создания CNAME настроено два персональных домена для CDN:
    ...
    - `Кастомный` — в качестве значения заголовка будет использоваться указанное значение домена или IP-адреса. Источник должен уметь обрабатывать запросы для указанного заголовка.
...
- Чтобы в качестве заголовка Host передавался первый CNAME ресурса (по умолчанию), для параметров `forward_host_header` и  `host_header` укажите `"enabled": false`.
...
{note:warn}
После внесения изменений в опцию необходимо {linkto(../../../../../networks/cdn/instructions/manage-cdn/content-settings#cdn-content-settings-cache-clearing)[text=очистить кеш CDN-ресурса]}.
{/note}

## {heading(Включение шилдинга)[id=cdn-origin-settings-shielding]}
