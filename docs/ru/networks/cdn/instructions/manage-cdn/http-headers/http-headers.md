...
Механизм [Cross-Origin Resource Sharing (CORS)](/ru/storage/s3/concepts/access/s3-cors) использует заголовок ответа Access-Control-Allow-Origin, чтобы браузер пользователя получал разрешения на доступ к ресурсам, которые находятся на доменах, отличных от исходного.
