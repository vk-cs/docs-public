# {heading(Настройка HTTP-заголовков)[id=cdn-http-headers]}

## {heading(Добавление заголовков запросов)[id=cdn-http-headers-add-request]}
...
1. (Опционально) Нажмите кнопку ![trash-icon](../../../../../assets/trash-icon.svg "inline"), чтобы удалить заголовок запроса.
...
## {heading(Добавление заголовков ответов)[id=cdn-http-headers-add-response]}
...
1. (Опционально) Нажмите кнопку ![plus-icon](../../../../../assets/plus-icon.svg "inline") **Добавить заголовок**, чтобы добавить еще один заголовок запроса. Укажите название заголовка и значение. Вы можете установить до 50 заголовков ответа.
1. (Опционально) Нажмите кнопку ![Удалить](../../../../../assets/delete-icon.svg "inline"), чтобы удалить заголовок запроса.
1. Нажмите кнопку **Сохранить изменения**.
...
## {heading(Поддержка заголовков CORS)[id=cdn-http-headers-cors]}

Механизм {linkto(../../../../../storage/s3/concepts/access/s3-cors#s3-concepts-cors)[text=Cross-Origin Resource Sharing (CORS)]} использует заголовок ответа Access-Control-Allow-Origin, чтобы браузер пользователя получал разрешения на доступ к ресурсам, которые находятся на доменах, отличных от исходного.
...
1. Выберите вариант настройки заголовка:
   ...
   - `"$http_origin", если источник указан ниже`: разрешает отображать контент для указанных доменов:
     ...
     1. (Опционально) Нажмите кнопку ![plus-icon](../../../../../assets/plus-icon.svg "inline") **Добавить домен**, чтобы добавить еще один домен.
     1. (Опционально) Нажмите кнопку ![trash-icon](../../../../../assets/trash-icon.svg "inline"), чтобы удалить домен из списка.
...
## {heading(Настройка разрешенных HTTP-методов)[id=cdn-security-allowed-http]}

Опция **Разрешенные HTTP-методы** позволяет управлять разрешенными HTTP-методами запросов к CDN-ресурсу. По умолчанию разрешены методы: `GET`, `HEAD`. Чтобы разрешить методы `OPTIONS`, `DELETE`, `POST`, `PUT`, `PATCH`, обратитесь в [техническую поддержку](/ru/contacts).
