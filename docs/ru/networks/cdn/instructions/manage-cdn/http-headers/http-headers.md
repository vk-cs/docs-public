...
Опция **Разрешенные HTTP-методы** позволяет управлять разрешенными HTTP-методами запросов к CDN-ресурсу. По умолчанию разрешены методы: `GET`, `HEAD`, `OPTIONS`, `DELETE`. Чтобы разрешить методы `POST`, `PUT`, `PATCH`, обратитесь в [техническую поддержку](/ru/contacts).
