...
1. Настройте взаимодействие CDN-ресурса с источниками:
   ...
   - **Запрос контента:** с каких источников CDN-серверы будут запрашивать контент.
     ...
     {tab((По умолчанию) С одного источника)}
     ...
     **Примеры:**
     ...
     1. (Опционально) Измените параметры отдельного источника, нажав на значок ![Редактировать](./assets/pencil-icon.svg "inline") справа от типа источника.

     1. (Опционально) Удалите отдельный источник, нажав на значок ![Удалить](./assets/trash-icon.svg "inline") справа от типа источника.
...
1. Выберите опцию, отвечающую за SSL-сертификат. Сертификат будет использоваться при доступе к настроенным ранее персональным доменам по HTTPS.
   ...
   {tab((По умолчанию) Let's Encrypt)}
