...
- [Через интерфейс бакета](#sozdanie_cherez_interfeys_baketa) в сервисе [Object Storage](/ru/storage/s3). Используйте этот вариант, чтобы бакет выступал в качестве источника контента. Соответствующий бакету CDN-ресурс будет создан автоматически, SSL-сертификаты также будут настроены автоматически.
