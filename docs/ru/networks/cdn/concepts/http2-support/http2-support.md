# {heading(Поддержка HTTP/2)[id=cdn-http2-support]}
...
{note:info}
Для работы с CDN по HTTP/2 {linkto(../../../../networks/cdn/instructions/manage-certificates#cdn-manage-certificates)[text=настройте SSL-сертификаты]} для CDN-ресурса, так как при работе по HTTP/2 браузер выполняет запросы по HTTPS.
{/note}
