...
В примерах ниже инстансы создаются в следующей конфигурации:
...
<tabs>
<tablist>
<tab>Создание инстанса в существующей сети</tab>
<tab>Создание инстанса в новой сети</tab>
</tablist>
<tabpanel>
...
resource "vkcs_mlplatform_jupyterhub" "jupyterhub" {
  ...
  networks = [
    {
      network_id = vkcs_networking_network.add.id
    },
  ]
}
```
...
- `flavor_id`: значение берется из источника данных `vkcs_compute_flavor`. Далее будет показано, как его формировать. Вместо источника данных вы можете указать идентификатор типа ВМ, полученный через [OpenStack CLI](/ru/manage/tools-for-using-services/openstack-cli).
- `network_id`: значение будет получено после создания ресурса `vkcs_networking_network`. Далее будет показано, как его создать.

</tabpanel>
</tabs>

## 2. Создайте файл с описанием источника данных для типа ВМ
...
Указанный в файле тип ВМ будет использоваться создания инстанса JupiterHub в вашем проекте Terraform.

## 3. Создайте файл с описанием сетевой инфраструктуры для кластера
...
1. Перейдите в раздел **NL Platform** → **Инстансы**. Убедитесь, что инстанс JupiterHub создан и активен.
