...
У каждого проекта есть уникальный идентификатор (PID) вида `mscNNNNNNNNNN`, например, **msc0123456789**. Он назначается автоматически и используется при обращении в техническую поддержку.
