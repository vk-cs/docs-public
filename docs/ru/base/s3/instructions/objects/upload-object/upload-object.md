...
Файлы объемом до 1 ГБ можно загружать через личный кабинет, файловые менеджеры, CLI или API. Файлы объемом больше 1 ГБ рекомендуется загружать через CLI или API. Для файлов объемом больше 32 ГБ используйте метод [составной загрузки](../../../references#sostavnaya_zagruzka) и хранения.
...
1. Откройте консоль и введите команду:
   ...
   Здесь:
      ...
      - `<ключ_объекта>` — имя объекта и имена папок, в которых он находится.
      ...
      - (Опционально) `<класс-хранения>` — задает [класс хранения](../../../references#klass_hraneniya) объекта. Если не указано, класс хранения наследуется из бакета. Доступные значения:

         - `STANDARD` — соответствует классу хранения Hotbox;
         - `STANDARD_IA` — соответствует классу хранения Icebox.

      - (Опционально) `<настройка-ACL>` — задает [настройки ACL](../../../instructions/access-management/s3-acl#fiksirovannyy_acl). Доступные значения:
...
1. Установите и настройте [AWS CLI](../../../storage-connecting/s3-cli), если он еще не установлен. Установите выходной формат JSON или YAML, так как текстовые форматы не распознаются при выполнении команд составной загрузки.
...
1. Инициируйте составную загрузку. Откройте консоль и введите команду:
...
1. Выполните загрузку первой части файла. Откройте консоль, перейдите в директорию файла для загрузки и введите команду:
