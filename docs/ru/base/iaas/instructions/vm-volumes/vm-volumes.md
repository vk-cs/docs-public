...
## Увеличение размера диска на выключенной ВМ
...
## Увеличение размера диска на включенной ВМ

Чтобы изменить размер диска включенной ВМ без перезагрузки:

1. требуется, чтобы ВМ была включенной при изменении размера и нельзя ее перезагружать, то после изменения размера в панели VK Cloud следует произвести операции расширения в гостевой ОС ВМ.
Для Linux:
Чтобы расширить раздел vda1, нужно выполнить команды:
sudo growpart /dev/vda1
sudo resize2fs /dev/vda1
Для Windows:
В Диспетчере дисков расширить нужный раздел.
Если свободное пространство вообще не появилось — попробовать сделать принудительную перезагрузку сервера в Личном кабинете VK Cloud, после чего проверить изменения.
