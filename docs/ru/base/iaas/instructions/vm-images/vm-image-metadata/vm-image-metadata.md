...
| `os_admin_user` | Пользователь внутри гостевой ОС с правами администратора; пароль может быть установлен через [личный кабинет](../../vm/vm-manage#ustanovka_i_izmenenie_parolya) | При `os_type: windows` — `Administrator`, для всех остальных значений — `root` |
