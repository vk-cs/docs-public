...
| Виртуальные машины с GPU | ВМ с подключенной [графической картой](../../../gpu/about/) | Отображаются при заказе соответствующих типов ВМ через сервис [Cloud GPU](https://cloud.vk.com/cloud-gpu/) |
...
| Сетевой <br/>High-IOPS HA SSD | high-iops-ha | ME1 | SSD с повышенной скоростью работы. Двойная репликация на разные серверы СХД |
...
Автоматический режим поддерживает [стратегию GFS](/ru/storage/backups/retention-policy/gfs-backup) и позволяет настроить расписание резервного копирования для создания полных и инкрементальных резервных копий.
